...
vcsPath: en/yql/faq/temp.md
sourcePath: en/yql/faq/temp.md
---
# Temporary data
...
* There is no access to the `//tmp` directory of the YTsaurus cluster.
...
* new_cache: YTsaurus API file cache:
    * YQL executables.
    * Custom and system UDFs.
    * Files attached to the query.
    * Tables used in MapJoin (if transmitted not in a YTsaurus-native manner).
