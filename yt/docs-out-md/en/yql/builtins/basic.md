...
vcsPath: en/yql/builtins/basic.md
sourcePath: en/yql/builtins/basic.md
---
...
Get a list of values from the attached text file. You can use it in combination with [IN](../syntax/expressions.md#in) and attaching the file via URL (instructions on how to attach files in the web interface and in the client).

Only one file format is supported: one value per line. For something more sophisticated, you'd currently need to write a small UDF in [Python](../udf/python.md).
...
You can use EvaluateAtom to create an [atom](../types/special.md) dynamically. However, since atoms are mainly controlled from a lower s-expressions level, it is generally not recommended to use this function directly.
...
{% include [decimal args](../_includes/decimal_args-41c30463aab7.md) %}
