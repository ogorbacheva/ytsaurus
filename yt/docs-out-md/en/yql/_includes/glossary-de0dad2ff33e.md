...
[C++](../udf/cpp.md) UDFs are loaded in compiled `.so` format, with a wide range of [built-in](../udf/list/pcre.md) functions available as well. [Python](../udf/python.md) UDFs can use a script from any query string, but require additional description of their signature.

Since the YQL optimizer can't peek inside the Python interpreter, **we recommend using lambda functions and C++ UDFs** for better performance. ||
