...
vcsPath: en/yql/recipes/udaf.md
sourcePath: en/yql/recipes/udaf.md
---
...
Here are the peculiarities/benefits of this approach compared to Map/Reduce ([PROCESS](../syntax/process.md) + [REDUCE](../syntax/reduce.md) or working with YTsaurus directly without YQL):
...
If you have built or want to build an aggregate function which, in your opinion, could be useful to many users, write to us  in the [Telegram chat](https://t.me/ytsaurus). We'll consider adding it to standard functions.
...
1. To perform each aggregation step, the user provides from three to seven simple functions (Callable values, to be precise) that operate with the above abstractions. The main methods to get them is by using [lambda](../syntax/expressions.md#lambda) or [Python UDF](../udf/python.md). Main options:
