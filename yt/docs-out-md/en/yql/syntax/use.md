...
vcsPath: en/yql/syntax/use.md
sourcePath: en/yql/syntax/use.md
---
# USE
...
Usually the cluster name is specified explicitly, but you can use an expression for it. In this case, `USE` must have the notation ```USE yt:$cluster_name```, where `$cluster_name` is the [named expression](expressions.md#named-nodes) of the `String` type.
