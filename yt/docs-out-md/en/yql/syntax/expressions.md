...
vcsPath: en/yql/syntax/expressions.md
sourcePath: en/yql/syntax/expressions.md
---
# Expressions
...
Checking whether a value is in a range. Syntax: `expr [NOT] BETWEEN [ASYMMETRIC | SYMMETRIC] expr AND expr`.

* `BETWEEN` and `BETWEEN ASYMMETRIC` are equivalent, `x BETWEEN a AND b` is equivalent to `a <= x AND x <= b`.
* `BETWEEN SYMMETRIC` automatically repositions the arguments so that the range is non-empty,
`x BETWEEN SYMMETRIC a AND b` is equivalent to `(x BETWEEN a AND b) OR (x BETWEEN b AND a)`.
* `NOT` inverts the check result.
...
Unlike the same keyword in Python, in YQL `IN` **DOES NOT** involve searching for a substring in a string. You can use [String::Contains](../udf/list/string.md) or [LIKE / REGEXP](#check-match) described above to search for a substring.
...
Because there's a limit on the byte size of queries (approx. 1 MB) in YQL, to get a larger list of values you should attach them to the query via URL and use the [ParseFile](../builtins/basic.md#parsefile) function.
...
For structures and tuples, it deletes elements that are omitted in the target type. To learn more about conversions, see [here](../types/cast.md).

{% include [decimal_args](../_includes/decimal_args-41c30463aab7.md) %}

#### Examples

{% include [cast_examples](../_includes/cast_examples-9d27f148a1a1.md) %}
...
Each named expression has a scope. It starts immediately after defining the named expression and ends at the end of the closest enclosing name scope (for example, at the end of the query or at the end of a [lambda function](#lambda) body, [ACTION](action.md#define-action), [SUBQUERY](subquery.md#define-subquery), or [EVALUATE FOR](action.md#evaluate-for) cyclic path).
...
SELECT * FROM $merge_dict("Input", $dict); -- $dict here is used in scalar context.
                                           -- error, exactly one column is expected in scalar context
...
SELECT * FROM $merge_dict("Input", $dict); -- $dict is a subquery template (not a table expression)
