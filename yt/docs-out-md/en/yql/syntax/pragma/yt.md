...
vcsPath: en/yql/syntax/pragma/yt.md
sourcePath: en/yql/syntax/pragma/yt.md
---
# YT pragmas {#yt}
...
Maximum number of jobs within a single YTsaurus operation. It is used only for single-stage map, reduce, merge, and other operations. If [`yt.DataSizePerJob`](#datasizeperjob) and `yt.MaxJobCount` are both specified, job splitting is done with account for [`yt.DataSizePerJob`](#datasizeperjob), and even if the resulting value `N` exceeds `yt.MaxJobCount`, `N` jobs will be run, and `yt.MaxJobCount` will only affect whether the jobs will be split after their number reaches a particular threshold.
...
Allows management of TTL for YTsaurus file cache. Value of 0 disables use of TTL for file cache.
...
| Value type | Default value | Static/<br/>dynamic |
| --- | --- | --- |
| String, see the [documentation](https://ytsaurus.tech/docs/en/user-guide/storage/compression) | zstd_5 | Dynamic |

Compression settings for tables created through YQL.

Tables specified in [INSERT INTO](../insert_into.md) are Published. All other tables are Temporary. Also, a codec specified as Temporary is used for intermediate data in a single YTsaurus operation (e.g. unified MapReduce).

## yt.PublishedErasureCodec / yt.TemporaryErasureCodec

| Value type | Default value | Static/<br/>dynamic |
| --- | --- | --- |
| String, see the [documentation](https://ytsaurus.tech/docs/en/user-guide/storage/replication#erasure) | none | Dynamic |
...
Manages the [background table compression](https://ytsaurus.tech/docs/en/user-guide/storage/regular-system-processes#nightly_compress) process so that the tables take up less space.
...
Allows management of [TTL for tables created by the operation](https://ytsaurus.tech/docs/en/user-guide/storage/cypress#TTL).
...
Ability to select pool trees different from the standard ones.
...
Ability to "gently" spread operations across pool trees different from the standard ones.
...
Effective only when the `yt.TentativePoolTrees` pragma is present. Sets the number of jobs in a sample.
...
Effective only when the `yt.TentativePoolTrees` pragma is present. Sets the permissible job slowdown factor in an alternative pool tree. 
...
Effective only when the `yt.TentativePoolTrees` pragma is present. Sets the minimum average job duration in an alternative pool tree. 
...
If the flag is set, YTsaurus [revision is excluded from metadata during hash calculation](https://ytsaurus.tech/docs/en/user-guide/storage/cypress). Therefore, QueryCache is not invalidated when modifying input table contents.
...
Allows management of [TTL for tables created by the operation in the query cache](https://ytsaurus.tech/docs/en/user-guide/storage/cypress).
...
Management of the [same-named settingYTsaurus](https://ytsaurus.tech/docs/en/user-guide/data-processing/operations/automerge) that helps reduce the utilization quota for chunk quantity. `yt.TemporaryAutoMerge` is valid for all YT operations except for merge inside the YtPublish node.

`yt.PublishedAutoMerge` is only valid for merge inside the YtPublish node (if it's launched there). `yt.AutoMerge` sets the value for this setting simultaneously for all YTsaurus query operations.
...
Enables column renaming when executing the Ordered JOIN strategy ([rename_columns](https://ytsaurus.tech/docs/en/user-guide/data-processing/operations/operations-options#path_attributes) attribute is used). If the option is disabled, then the Ordered JOIN strategy is selected only when the left and right column names match.
...
Includes the use of columnar statistics to precisely evaluate job sizes when launching operations on top of the tables containing columnar data selections. See the [documentation](https://ytsaurus.tech/docs/en/user-guide/data-processing/operations/operations-options#common_options).
...
If the average chunk size in the resulting output table is smaller than the specified setting, launch an additional YTsaurus Merge operation to enlarges the chunks to reach the specified size. The value of 0 has a special meaning, causing merge to always launch and enlarge the chunks up to 1G.
...
If the native value is set, then the table contents are delivered to jobs via native YTsaurus mechanisms. If the file value is set, the table contents are first downloaded to the YQL server and then delivered to jobs as a regular file.
...
Maximum number of chunks in the table for it to be delivered to jobs via native YTsaurus mechanisms. If this number is exceeded, the table is delivered via a file.
...
Minimum average size of chunks in the table for it to be delivered to jobs via native YTsaurus mechanisms. If the chunk size isn't large enough, then preliminary merge is inserted.
...
Ability to specify the sequence of porto layers in order to create an environment for executing custom jobs.
...
Sets the maximum number of concurrently launched YTsaurus operations in a query.
...
Sets the job memory reservation factor. See the [documentation](https://ytsaurus.tech/docs/en/user-guide/data-processing/operations/operations-options#memory_reserve_factor).
...
Sets the `user_job_memory_digest_lower_bound` setting in the operation spec. To learn more about the setting, see the [documentation](https://ytsaurus.tech/docs/en/user-guide/data-processing/scheduler/memory-digest#nastrojki-digest).
...
Limit on the number of records that JobProxy can buffer.
...
Prohibit the YTsaurus scheduler from adaptively splitting long-running custom jobs.
...
Sets the approximate output-input ratio for the map stage in the joint MapReduce operation. See the [documentation](https://ytsaurus.tech/docs/en/user-guide/data-processing/operations/mapreduce).
...
Pause the operation if the "Account limit exceeded" error occurs in jobs. See the [documentation](https://ytsaurus.tech/docs/en/user-guide/data-processing/operations/operations-options#common_options).
...
Sets arbitrary structured information related to the operation. See the [documentation](https://ytsaurus.tech/docs/en/user-guide/data-processing/operations/operations-options).
...
Sets the map describing the client that started the operation. See the [documentation](https://ytsaurus.tech/docs/en/user-guide/data-processing/operations/operations-options).
...
Sets human-readable information displayed on the operation page in the web interface. See the [documentation](https://ytsaurus.tech/docs/en/user-guide/data-processing/operations/operations-options).
...
Sets the number of [speculatively executed jobs]({{yql.pages.syntax.pragma.speculative-job}}) in YTsaurus operations. YTsaurus cluster settings are used by default.
...
Allows management of [TTL for cached binary artifacts](https://ytsaurus.tech/docs/en/user-guide/storage/cypress#TTL). Only works together with `yt.BinaryTmpFolder`. Each use of a binary artifact in the query extends the lifetime of its TTL.
...
Allows values of composite data types to be written to tables through native support of composite types in YTsaurus.
...
Set the `@media` attribute for newly created tables. If available, assigns [mediums in YTsaurus](https://ytsaurus.tech/docs/en/user-guide/storage/media#naznachenie-mediuma), where table chunks will be stored.
...
Set the `@primary_medium` attribute for newly created tables. If available, assigns the [primary medium in YTsaurus](https://ytsaurus.tech/docs/en/user-guide/storage/media#primary), where chunks will be recorded. By default, YTsaurus sets the primary medium to `"default"`.
...
Set the medium used for intermediate data in operations (Sort, MapReduce). To learn more, see the [documentation](https://ytsaurus.tech/docs/en/user-guide/data-processing/operations/sort).
...
Sets the [primary medium in YTsaurus](https://ytsaurus.tech/docs/en/user-guide/storage/media#primary) for Published and Temporary tables and intermediate data in operations. Amounts to simultaneous setting of `yt.IntermediateDataMedium`, `yt.PublishedPrimaryMedium`, and `yt.TemporaryPrimaryMedium` pragmas.
...
Sets the use of a specified network project in jobs. See the [documentation](https://ytsaurus.tech/docs/en/user-guide/data-processing/operations/mtn).
