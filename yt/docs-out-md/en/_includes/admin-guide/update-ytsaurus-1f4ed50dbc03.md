...
We strongly advise against updating to a `dev` version compiled from the current `main` branch since those YTsaurus versions aren't stable and haven't been properly tested. Moreover, rolling back to the latest stable release will be impossible for the reasons described above.

{% endnote %}

{% note warning "Important" %}

Before updating YTsaurus, you first need to update the operator to the latest release. New stable images might not work with an outdated operator, and the operator itself might not work if it has an unstable YTsaurus version like `dev`. You can find a list of stable images on the [Releases](../../admin-guide/releases) page.
...
In the `Ytsaurus` specification, you can set one image for all server components (`coreImage`) or different images for different components (in the components' `image` field). You only need to set individual images in rare cases, and it's recommended that you discuss this with the [YTsaurus team](https://ytsaurus.tech/#contact) beforehand. If a component has its own image, it will be used. Modifying the `image` initiates a cluster update, though this only updates some of the components.
...
If unforeseen problems occur during the update process, we recommend consulting with the [YTsaurus team](https://ytsaurus.tech/#contact) before doing anything manually.
...
4. Following the cluster update, unrecognized options might remain in the master server configs (this occurs when we stop using some config fields in the `YTsaurus` version, which is common during major updates). If this situation occurs, you'll see the following alert:
