...
* `network_name` — name of the network (YTsaurus terms) used for copying. The specified network must be configured on all nodes of the remote cluster. For example: `fastbone` or `backbone`.
...
* `cluster_connection` — configuration for connecting to the source cluster. Configured by the YTsaurus system administrator when configuring the cluster.
