...
The first important point in the query execution journey is the so-called _heavy YTsaurus proxies_, which all YTsaurus users have network access to. Heavy proxies find out where instances of the clique with a given alias currently reside, then send a query to a randomly selected instance.
