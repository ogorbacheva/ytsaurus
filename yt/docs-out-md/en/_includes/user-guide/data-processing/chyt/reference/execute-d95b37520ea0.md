...
- `--format` [TabSeparated]: The [ClickHouse format](https://clickhouse.tech/docs/en/interfaces/formats/) in which to output the result.
...
- `client`: A client from the YTsaurus cluster in which the query to the clique should be run. The global client is taken by default. You can, for example, pass `yt.wrapper.YtClient("<cluster_name>")` as the client and thus specify the desired cluster. Learn more in the relevant section of the Python API documentation.
