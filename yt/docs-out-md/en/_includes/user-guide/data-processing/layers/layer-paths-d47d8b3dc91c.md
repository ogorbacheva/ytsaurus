...
- **RootFS image**: a root file system image.
...
# Using file system images to run operations in YTsaurus { #job_rootfs }

A MapReduce operation must be run with the file system image specified if its execution requires libraries or resources that aren't available in YTsaurus clusters and if there is no way to prepare a binary file with this operation.
