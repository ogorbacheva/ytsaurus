...
You can work with YTsaurus tables from [Spark SQL](https://spark.apache.org/docs/latest/sql-ref-syntax.html). This SQL dialect is used for queries in [Query Tracker](../../../../user-guide/query-tracker/about.md) using SPYT.

When working with YTsaurus, `yt` is used as a database ID, and `ytTable:/` is used as a file system. The latter can be omitted, so the below pair of queries is equivalent:
