...
The first part of this section describes the skiff serialization library (`yt/core/skiff`) which helps encode structured data, including data that is not YTsaurus tables.
...
- Support all the types supported by YTsaurus.
...
- All the children of the root node of a table's skiff schema must be named. YTsaurus uses them to determine which field to use for values from this or that column.
