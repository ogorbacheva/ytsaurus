...
The YTsaurus system supports transactions, but differs in a number of ways from the classic [transaction processing model](https://en.wikipedia.org/wiki/Online_transaction_processing):
...
YTsaurus uses [pessimistic locks](#locks), so possible conflicts are detected as they occur — when locks are acquired and objects are created within transactions rather than when a transaction is completed.
