...
YTsaurus enables you to specify [sorted dynamic tables](../../../user-guide/dynamic-tables/sorted-dynamic-tables.md) as output tables of map-reduce operations (the so-called bulk insert).
...
You can take the service column names from the [api](https://github.com/ytsaurus/ytsaurus/blob/879946a82c766da78612ab4fe3637f7ea4da8292/yt/yt/client/tablet_client/public.h#L152).

`$change_type` accepts values from the [ERowModificationType](https://github.com/ytsaurus/ytsaurus/blob/879946a82c766da78612ab4fe3637f7ea4da8292/yt/yt/client/api/public.h#L53) enum. Only `Write` and `Delete` values are allowed.
...
The value flags accept the bit mask of values of the [EUnversionedUpdateDataFlags](https://github.com/ytsaurus/ytsaurus/blob/879946a82c766da78612ab4fe3637f7ea4da8292/yt/yt/client/tablet_client/public.h#L142) enum. The `missing | aggregate` combination makes no sense, but it is acceptable. If no flags are specified for a column, they are considered equal to 0.
...
In extended format, you can also create intermediate static tables which will then be inserted into dynamic tables using sort or merge, but this is very situational.
