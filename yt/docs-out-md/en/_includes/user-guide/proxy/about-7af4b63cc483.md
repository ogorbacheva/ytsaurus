...
This section contains information about the different interfaces for working with the YTsaurus system: [HTTP](#http_proxy) and [RPC](#rpc_proxy) proxies.
...
- Read or write requests (for example, `read-table`, `write-table`) can potentially transfer a significantly larger amount of data, resulting in a bigger load on YTsaurus cluster nodes. Such queries are therefore processed by so-called heavy proxies (data proxies), the `data` role.
...
To get examples of how to interract with YTsaurus via the HTTP interface, check the [HTTP proxy](../../../user-guide/proxy/http.md) section.
