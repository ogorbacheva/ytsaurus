...
This section demonstrates how to work with YTsaurus via the HTTP proxy using the example of running the [Word Count](https://en.wikipedia.org/wiki/Word_count) task.

You can find a full description of all YTsaurus commands in the [Commands](../../../api/commands.md) section.

{% note info "Note" %}

To understand the material presented in this section, you need to know the [HTTP protocol](https://en.wikipedia.org/wiki/HTTP) structure, the principles of working via HTTP, and the basics of [data storage](../../../user-guide/storage/cypress.md) in the YTsaurus system.
...
Data in the YTsaurus system is stored in [tables](../../../user-guide/storage/static-tables.md), so you need to create a table. The table contains two columns: row number and row contents. Use the [create](../../../api/commands.md#create) command to create a table.
...
In the YTsaurus system, to run a Map operation, there is a [command](../../../api/commands.md#map) with the same name. An important thing to consider is the requirement that an output table must exist **before** an operation is started. You can also create a table using the [create](../../../api/commands.md#create) command.
