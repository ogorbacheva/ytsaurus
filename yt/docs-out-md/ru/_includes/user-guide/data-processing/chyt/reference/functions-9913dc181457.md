...
В системе YTsaurus предусмотрено 3 формата представления &mdash; `binary`, `text` и `pretty`.
...
#### Получение версии YTsaurus { #yt_version }

  `ytVersion()` &mdash; получить строковое представление версии кода YTsaurus, используемого в текущей версии CHYT.
...
### ytTables { #yt_tables }
  ...
  * **Обратите внимание**, что ввиду особенностей хранения данных в YTsaurus, беззнаковые числовые типы являются несовместимыми со знаковыми (т.е. типы `int32` и `uint32` несовместимы).
...
### concatYtTables { #concat_yt_tables }
  ...
  `concatYtTables(table1, [table2, [...]])` &mdash; прочитать объединение нескольких YTsaurus таблиц.
