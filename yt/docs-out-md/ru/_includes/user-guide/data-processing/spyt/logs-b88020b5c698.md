...
При запуске кластера можно настроить отправку логов с [воркеров](../../../../user-guide/data-processing/spyt/cluster/cluster-desc.md#spark-standalone) в YTsaurus таблицы. Логи будут записываться по пути `{cluster_path}/logs/worker_log`, под каждую дату будет создаваться отдельная таблица. Для включения отправки логов при запуске кластера необходимо использовать следующие опции:
