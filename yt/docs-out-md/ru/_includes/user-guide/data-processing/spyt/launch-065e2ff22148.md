...
Для использования History server в этом режиме его необходимо поднять отдельно согласно [инструкции](../../../../user-guide/data-processing/spyt/cluster/shs.md). Для получения диагностической информации также можно воспользоваться логами YTsaurus операций. Следует учесть следующее:
...
- В настоящий момент драйвер и экзекьюторы запускаются в разных YTsaurus операциях, соответственно логи нужно смотреть и в той, и в другой.
