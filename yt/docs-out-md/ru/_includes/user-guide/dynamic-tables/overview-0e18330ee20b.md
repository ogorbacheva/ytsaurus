...
Динамические таблицы поддерживают транзакционность по модели [MVCC](https://en.wikipedia.org/wiki/Multiversion_concurrency_control). Работа с ними не требует общения с мастер-сервером YTsaurus, что даёт дополнительные возможности для масштабирования.
