...
С каждым объектом в системе YTsaurus связан набор **атрибутов**. Множество атрибутов представляет собой словарь ключ-значение, который доступен для чтения и изменения. Именами атрибутов могут выступать произвольные непустые строки. Значениями — произвольные [YSON](../../../user-guide/storage/yson.md)-структуры.
