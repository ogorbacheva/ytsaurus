...
vcsPath: ru/yql/syntax/join.md
sourcePath: ru/yql/syntax/join.md
---
...
Выполнение `JOIN` начинается с вычислениия ключей и сохранения их значений в отдельную колонку. Такое преобразование происходит ещё на уровне SQL парсера и является общим для всех бэкендов (YTsaurus, DQ и т. п.)
...
Такая конвертация нужна, поскольку ключи сложных типов не поддерживаются в качестве ключей reduce операций в YTsaurus.
...
Из map стадии строки с одинаковым ключом попадают в одну YTsaurus reduce джобу, в которой и происходит `JOIN`.

Если необходимо, результирующая таблица из reduce стадии объединяется с выходными таблцами из map стадии с помощью отдельной YTsaurus Merge операции.
...
Если сортирована только одна таблица, а размер другой не превышает `yt.JoinMergeUnsortedFactor * <размер сортированной таблицы>`, стратегия SortedJoin также выбирается, при этом несортированная таблица сортируется отдельной YTsaurus операцией.
