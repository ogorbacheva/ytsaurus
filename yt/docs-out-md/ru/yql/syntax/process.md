...
vcsPath: ru/yql/syntax/process.md
sourcePath: ru/yql/syntax/process.md
---
# PROCESS

Преобразовать входную таблицу с помощью UDF на [C++](../udf/cpp.md), [Python](../udf/python.md) или [лямбда функции](expressions.md#lambda), которая применяется последовательно к каждой строке входа и имеет возможность для каждой строки входа создать ноль, одну или несколько строк результата (аналог Map в терминах MapReduce).
