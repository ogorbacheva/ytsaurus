...
vcsPath: ru/yql/syntax/pragma/yt.md
sourcePath: ru/yql/syntax/pragma/yt.md
---
# YT-прагмы {#yt}
...
Позволяет управлять TTL [файлового кэша](../../faq/temp.md) YTsaurus. Значение 0 отключает использование TTL для файлового кеша.
...
| Тип значения | По умолчанию | Статическая /<br/>динамическая |
| --- | --- | --- |
| Строка, см. [документацию](https://ytsaurus.tech/docs/ru/user-guide/storage/compression) | zstd_5 | Динамическая |

Настройки компрессии создаваемых через YQL таблиц.

Published являются таблицы, указанные в [INSERT INTO](../insert_into.md) все остальные считаются временными (Temporary). Также кодек, указанный как Temporary, используется для промежуточных данных в рамках одной YTsaurus операции, например слитного MapReduce.

## yt.PublishedErasureCodec / yt.TemporaryErasureCodec

| Тип значения | По умолчанию | Статическая /<br/>динамическая |
| --- | --- | --- |
| Строка, см. [документацию](https://ytsaurus.tech/docs/ru/user-guide/storage/replication#erasure) | none | Динамическая |
...
Управляет процессом [фонового сжатия таблиц](https://ytsaurus.tech/docs/ru/user-guide/storage/regular-system-processes#nightly_compress), чтобы они занимали меньше места.
...
Позволяет управлять [TTL создаваемых операцией таблиц](https://ytsaurus.tech/docs/ru/user-guide/storage/cypress#TTL).
...
Если флаг установлен, то при вычислении хеша из метаданных таблиц исключается YTsaurus [revision](https://ytsaurus.tech/docs/ru/user-guide/storage/cypress). Таким образом QueryCache не инвалидируется при изменении содержимого входных таблиц.
...
Позволяет управлять [TTL создаваемых операцией таблиц в кеше запросов](https://ytsaurus.tech/docs/ru/user-guide/storage/cypress).
...
Управление [одноименной настройкой YTsaurus](https://ytsaurus.tech/docs/ru/user-guide/data-processing/operations/automerge), помогающей уменьшить потребление квоты на число chunk'ов. `yt.TemporaryAutoMerge` действует для всех YT-операций, за исключением merge внутри YtPublish узла.
...
Множитель для оценки потребления процессора скриптовых UDF (включая [Python UDF](../../udf/python.md). Влияет на разбиение MapReduce-операций на джобы. Может быть переопределён специализированными прагмами `yt.PythonCpu` / `yt.JavascriptCpu` для конкретного типа UDF.
...
Множитель для оценки потребления процессора [Python UDF](../../udf/python.md), влияет на разбиение MapReduce-операций на джобы.
...
Включает использование поколоночных статистик для точной оценки размеров джобов при запуске операций поверх таблиц с колоночными выборками. См. [документацию](https://ytsaurus.tech/docs/ru/user-guide/data-processing/operations/operations-options#common_options).
...
Задаёт коэффициент резервирования памяти для джобов, [см. документацию](https://ytsaurus.tech/docs/ru/user-guide/data-processing/operations/operations-options#memory_reserve_factor).
...
Задаёт настройку `user_job_memory_digest_lower_bound` в спеке операции. Про настройку можно почитать в [документации](https://ytsaurus.tech/docs/ru/user-guide/data-processing/scheduler/memory-digest#nastrojki-digest).
...
Задаёт приблизительное соотношение выхода к входу map-стадии в совмещенной MapReduce операции, [см. документацию](https://ytsaurus.tech/docs/ru/user-guide/data-processing/operations/mapreduce)
...
Приостанавливать операцию в случае возникновения ошибки "Account limit exceeded" в джобах, [см. документацию](https://ytsaurus.tech/docs/ru/user-guide/data-processing/operations/operations-options#common_options).
...
Задаёт произвольную структурированную информацию, связанную с операцией. См. [документацию](https://ytsaurus.tech/docs/ru/user-guide/data-processing/operations/operations-options).
...
Задаёт словарь, описывающий клиента, посредством которого была запущена операция. См. [документацию](https://ytsaurus.tech/docs/ru/user-guide/data-processing/operations/operations-options).
...
Задаёт человекочитаемую информацию, отображаемую в веб-интерфейсе на странице операции. См. [документацию](https://ytsaurus.tech/docs/ru/user-guide/data-processing/operations/operations-options).
...
Задаёт количество [спекулятивно исполняемых джобов]({{yql.pages.syntax.pragma.speculative-job}}) в YTsaurus операциях. По умолчанию используются настройки YTsaurus кластера.
...
Позволяет управлять [TTL кешируемых бинарных артефактов](https://ytsaurus.tech/docs/ru/user-guide/storage/cypress#TTL). Работает только в паре с `yt.BinaryTmpFolder`. Каждое использование бинарного артефакта в запросе продлевает его TTL.
...
Установить на вновь создаваемых таблицах атрибут `@media`. При его наличии назначает [на каких медиумах в YTsaurus](https://ytsaurus.tech/docs/ru/user-guide/storage/media#naznachenie-mediuma) будут храниться чанки таблицы.
...
Установить на вновь создаваемых таблицах атрибут `@primary_medium`. При его наличии назначает [первичный медиум в YTsaurus](https://ytsaurus.tech/docs/ru/user-guide/storage/media#primary) на который будет осуществляться запись чанков. По умолчанию YTsaurus устанавливает первичный медиум `"default"`.
...
Установить медиум, используемый для промежуточных данных в операциях (Sort, MapReduce). Подробности см. в [документации](https://ytsaurus.tech/docs/ru/user-guide/data-processing/operations/sort).
...
Задаёт [первичный медиум в YTsaurus](https://ytsaurus.tech/docs/ru/user-guide/storage/media#primary) для Published, Temporary таблиц и промежуточных данных в операциях. Равносильно одновременной установке прагм `yt.IntermediateDataMedium`, `yt.PublishedPrimaryMedium` и `yt.TemporaryPrimaryMedium`.
...
Задаёт использование указанного [сетевого проекта](https://ytsaurus.tech/docs/ru/user-guide/data-processing/operations/mtn) в джобах для всех map-reduce операций в запросе (включая evaluation стадию).
