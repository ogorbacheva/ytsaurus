...
**{{product-name}}** ({% if lang == "en" %}ЛФШ: [вайт^j^изáwрʊс], {% endif %}читается {{ipa}}) — платформа распределённого хранения и обработки больших объемов данных с поддержкой [MapReduce](http://ru.wikipedia.org/wiki/MapReduce), распределённой файловой системой и NoSQL key-value базой данных.
