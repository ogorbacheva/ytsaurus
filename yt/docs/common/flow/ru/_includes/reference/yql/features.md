...
Данная страница описывает конструкции YQL, специфичные для работы с Flow. Общий синтаксис YQL описан в [документации YQL]({{ yql-docs-root }}/{{ lang }}/yql/{{ docs-revision-query }}).

{% endnote %}

## Построчное преобразование потока (мап) {#map}

Основная конструкция — `INSERT INTO ... SELECT ... FROM`. Читает [стрим]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) из упорядоченной динамической таблицы (очереди {{product-name}}), применяет преобразование и записывает результат в другую очередь.
...
- [Быстрый старт (YQL)]({{ flow-docs-root }}/{{ lang }}/tutorials/yql/quick-start{{ docs-revision-query }})
