...
Коннектор к [статическим таблицам {{product-name}}]({{ core-docs-root }}/{{ lang }}/concepts/storage/static-tables{{ docs-revision-query }}).

Код коннектора находится [здесь]({{source-root}}/yt/yt/flow/library/cpp/connectors/static_table).

Статические таблицы &mdash; особый вид источника. В нём изначально нет [партиций]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#partition), у строк в них нет таймстемпов записи, сами таблицы как правило иммутабельны и в них не происходит дозаписи. При этом часто читать нужно бесконечную последовательность таблиц. Так же "запись" в этот источник идёт очень крупными блоками, поэтому читать из него необходимо с ограничением скорости, чтобы не отнимать ресурсы у более важных [сорсов]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#source) [пайплайна]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline).

В связи с этим основная сложность этого источника лежит в [контроллере]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#controller), которому необходимо понять, какие таблицы читать, какие таймстемпы ([SystemTimestamp]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timestamps-and-watermarks), [EventTimestamp]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timestamps-and-watermarks)) для них выбрать, что делать, если таблица неожиданно исчезает и так далее.
...
{% include [NYT_NFlow_TUnitedParameters_NYT_NFlow_NStaticTableConnector_TSource](../../../../reference/configuration/NYT_NFlow_TUnitedParameters_NYT_NFlow_NStaticTableConnector_TSource.md) %}

##### Динамическая спека:

{% include [NYT_NFlow_TDynamicUnitedParameters_NYT_NFlow_NStaticTableConnector_TSource](../../../../reference/configuration/NYT_NFlow_TDynamicUnitedParameters_NYT_NFlow_NStaticTableConnector_TSource.md) %}
...
{% include [NYT_NFlow_TUnitedParameters_NYT_NFlow_NStaticTableConnector_TArrivalOrderTableSink](../../../../reference/configuration/NYT_NFlow_TUnitedParameters_NYT_NFlow_NStaticTableConnector_TArrivalOrderTableSink.md) %}

##### Динамическая спека:

{% include [NYT_NFlow_TDynamicUnitedParameters_NYT_NFlow_NStaticTableConnector_TArrivalOrderTableSink](../../../../reference/configuration/NYT_NFlow_TDynamicUnitedParameters_NYT_NFlow_NStaticTableConnector_TArrivalOrderTableSink.md) %}

## Импорт статической таблицы в стейт {#import-into-state}

Типичный сценарий: батч-процесс (YQL{% if audience == "internal" %}, Nirvana{% endif %}, &hellip;) периодически пересобирает справочник в виде статической YT-таблицы, а реалтайм-пайплайн должен джойнить свой поток против свежей версии этого справочника. Коннектор `static_table` поддерживает этот сценарий "из коробки", позволяя загрузить такую таблицу в [стейт]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}) пайплайна с произвольной бизнес-логикой над каждой строкой перед записью.
...
3. Enricher-computation, висящий на потоке событий, читает значения из того же стейта по ключу события через read-only [joiner]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}#external-state-joiner) и эмитит обогащённое сообщение в синк.
...
В пайплайне `path` коннектора external-state ссылается на симлинк, а не на конкретную версию. Лукап выполняется read-only [joiner]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}#external-state-joiner)-ом (`TSimpleExternalStateJoiner`); по умолчанию joiner перечитывает значение из YT на каждый лукап, поэтому переключение симлинка сразу видно пайплайну без рестарта. Подробнее про подготовку динамической таблицы и её схему &mdash; в [sorted-dynamic-table.md]({{ flow-docs-root }}/{{ lang }}/reference/connectors/sorted-dynamic-table{{ docs-revision-query }}).
...
{% if audience == "internal" %}Один из таких форматов в Аркадии &mdash; **vinyl**. Пример сборки и использования есть в [`ads/core/library/cpp/vinyl`]({{source-root}}/ads/core/library/cpp/vinyl): отдельный батч-процесс собирает на YT файл vinyl, выкатывает в нужное место, а пайплайн получает свежую версию файла через механизм [Resource]({{ flow-docs-root }}/{{ lang }}/concepts/spec{{ docs-revision-query }}) (на каждый воркер) и читает из него на каждом сообщении.{% endif %}
...
- [Список коннекторов]({{ flow-docs-root }}/{{ lang }}/concepts/connectors/overview{{ docs-revision-query }})
- [Sorted Dynamic Table]({{ flow-docs-root }}/{{ lang }}/reference/connectors/sorted-dynamic-table{{ docs-revision-query }})
- [Стейтфул computations]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }})
