...
[Пайплайн]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) выполняет джойн [потоков]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) хитов (hit) и действий (action) на временном окне. Для каждого хита система ожидает связанные действия (show, click) в течение заданного временного интервала, после чего генерирует объединенное событие.
...
Подробное описание задачи и логики джойна — в [описании C++ версии]({{ flow-docs-root }}/{{ lang }}/tutorials/cpp/examples/wait_click_join{{ docs-revision-query }}).
...
Сообщения, пришедшие с event-временем меньше текущего [вотермарка]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timestamps-and-watermarks), считаются опоздавшими и игнорируются. Подробнее про вотермарки — в разделе [Watermarks & Timers]({{ flow-docs-root }}/{{ lang }}/concepts/watermarks{{ docs-revision-query }}).

### [Таймеры]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer) для закрытия окна
...
[Стейт]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state) хранится во внешней динамической таблице. `PayloadBuilder` позволяет обновлять отдельные поля стейта без полной пересериализации:
...
Пример использует [Spring Boot интеграцию]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/spring{{ docs-revision-query }}) для конфигурации. Компьютейшен `join` регистрируется аннотацией `@FlowComputation` на классе `JoinProcessFunction`:
...
- `external_state_managers` — top-level секция с описанием [External State]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/external-state{{ docs-revision-query }}) (на одном уровне с `parameters`). Ключ (`"/join-state"`) — имя стейта, начинающееся с `/`; то же имя передаётся в дескриптор `StateDescriptors.external("/join-state")`. `external_state_manager_class_name` — зарегистрированный класс менеджера (`NYT::NFlow::TSimpleExternalStateManager` для стандартного варианта). `parameters/path` — путь к динамической таблице {{product-name}}; схема ключевых колонок таблицы должна совпадать с `group_by_schema`.
...
Интеграционное тестирование Java-пайплайнов выполняется так же, как и для C++ пайплайнов — с запуском полного пайплайна, включая C++ воркеры, очереди и стримы. Подробнее см. [Интеграционные тесты]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/testing{{ docs-revision-query }}).
