...
Поддержка вычислений на Java и Kotlin во Flow реализуется через механизм [компаньонов]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#companion). Код на Java или Kotlin выполняется в отдельном gRPC-процессе, который взаимодействует с C++ [воркером]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#worker).
...
Для создания [компьютейшена]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) на Java необходимо выбрать подходящий билдер, соответствующий [типу Computation-a в C++]({{ flow-docs-root }}/{{ lang }}/concepts/companion{{ docs-revision-query }}#vidy-computation-ov-dlya-raboty-s-kompanonami):
...
- **Computation id** — по нему происходит маппинг запросов между [worker]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#worker)-ом и компаньоном.
...
- `RowFunction` — получает [сообщения]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#message) и [таймеры]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer) по одному, предоставляет методы `onMessage` и `onTimer`.
...
Метод `getStreams()` позволяет зарегистрировать типизированные [стримы]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) через `FlowStreams.typed(...)`, чтобы SDK автоматически сериализовал и десериализовал сообщения в Java-объекты.
...
- [Примеры]({{ flow-docs-root }}/{{ lang }}/tutorials/java/examples/wordcount{{ docs-revision-query }})
