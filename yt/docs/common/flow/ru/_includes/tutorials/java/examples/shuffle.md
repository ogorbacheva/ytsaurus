...
[Пайплайн]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) читает [поток]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) событий, группирует их по ключу и подсчитывает количество уникальных событий с использованием внешнего [стейта]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state) (ExternalStateAccessor). Пример демонстрирует конфигурацию [компаньона]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#companion) через Spring Boot.
...
Процессная функция для source-[компьютейшена]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) `reader`. Выполняет парсинг и трансформацию входных данных. Ниже показана упрощённая версия — в [реальном коде]({{source-root}}/yt/yt/flow/examples/java/shuffle) дополнительно выполняется JSON-парсинг поля `data` с помощью Jackson `ObjectMapper`:
...
Процессная функция с использованием [ExternalStateAccessor]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/state{{ docs-revision-query }}#external-state) для подсчета количества событий:
