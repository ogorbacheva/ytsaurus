...
[Пайплайн]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) расширяет [AsyncRequest]({{ flow-docs-root }}/{{ lang }}/tutorials/java/examples/async_request{{ docs-revision-query }}): обработчик запросов теперь поддерживает повторные попытки с задержкой. При неудаче `RequestProcessorFunction` сохраняет состояние запроса во внутреннем [стейте]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state) и устанавливает таймер на 5 секунд; успех определяется по предикату `(requestId + failedAttempts) % 3 == 0`.
...
Идентична `StateKeeperFunction` из [AsyncRequest]({{ flow-docs-root }}/{{ lang }}/tutorials/java/examples/async_request{{ docs-revision-query }}): обрабатывает стримы `event` и `response`, накапливает `total_length` во внешнем стейте. Логика ретраев полностью инкапсулирована в `RequestProcessorFunction`.
