...
YQL over Flow позволяет описать [пайплайн]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) потоковой обработки данных в виде декларативного SQL-запроса — без написания кода на [C++]({{ flow-docs-root }}/{{ lang }}/tutorials/cpp/quick-start{{ docs-revision-query }}), [Java]({{ flow-docs-root }}/{{ lang }}/tutorials/java/quick-start{{ docs-revision-query }}), [Go]({{ flow-docs-root }}/{{ lang }}/tutorials/go/quick-start{{ docs-revision-query }}) или [Python]({{ flow-docs-root }}/{{ lang }}/tutorials/python/quick-start{{ docs-revision-query }}). Пайплайн запускается как ванилла операция на выбранном кластере {{product-name}}.
...
- [Документация YQL]({{ yql-docs-root }}/{{ lang }}/yql/{{ docs-revision-query }}) — полный справочник по синтаксису YQL
...
Пример: построчное преобразование [стрима]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) (мап).
...
Описание всех поддержанных конструкций YQL см. в разделе [Поддержанные конструкции]({{ flow-docs-root }}/{{ lang }}/reference/yql/features{{ docs-revision-query }}).
...
- [Поддержанные конструкции]({{ flow-docs-root }}/{{ lang }}/reference/yql/features{{ docs-revision-query }})
