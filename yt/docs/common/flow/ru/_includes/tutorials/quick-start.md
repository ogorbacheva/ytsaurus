...
1. [Controller]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#controller) &mdash; управляет жизненным циклом пайплайна;
1. [Worker]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#worker) &mdash; читает из источников, выполняет вычисления;
1. {{product-name}} Cluster &mdash; хранит системные таблицы в директории пайплайна.

Каждый пайплайн требует свой набор экземпляров Controller и Worker, рабочую директорию в [Cypress]({{ core-docs-root }}/{{ lang }}/concepts/storage/cypress{{ docs-revision-query }}), набор системных динамических таблиц.
...
1. [YT token]({{ core-docs-root }}/{{ lang }}/reference/storage/auth{{ docs-revision-query }}) для кластера {{product-name}} с динамическими таблицами{% if audience == "internal" %} (см. [список кластеров](../../user-guide/dynamic-tables/clusters.md)){% endif %}, например {{flow-example-cluster}}.
...
Сначала создаётся Cypress-объект типа `pipeline` вместе с набором служебных динамических таблиц, необходимых для работы Flow (см. раздел [Объект Pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/pipeline-object{{ docs-revision-query }})).

{% if audience == "internal" %}

В Yandex-инфраструктуре это делает [YtSync]({{ flow-docs-root }}/{{ lang }}/concepts/pipeline-object{{ docs-revision-query }}#yt-sync):
...
В опенсорсе для этого используется готовый helper [yt_sync_mini]({{ flow-docs-root }}/{{ lang }}/concepts/pipeline-object{{ docs-revision-query }}#yt-sync-mini):
...
Структуру [внутренних таблиц]({{ flow-docs-root }}/{{ lang }}/concepts/pipeline-object{{ docs-revision-query }}#internal_tables), созданных вместе с объектом `pipeline`, можно найти тут:
...
Полный список полей &mdash; в [TVanillaConfig]({{ flow-docs-root }}/{{ lang }}/reference/configuration/all_yson_structs{{ docs-revision-query }}#NYT_NFlow_TVanillaConfig) (см. также [TVanillaTaskConfig]({{ flow-docs-root }}/{{ lang }}/reference/configuration/all_yson_structs{{ docs-revision-query }}#NYT_NFlow_TVanillaTaskConfig)), подробный разбор запуска &mdash; в разделе [Запуск пайплайна в Vanilla-операции]({{ flow-docs-root }}/{{ lang }}/how-to-guides/deployment/launch-vanilla{{ docs-revision-query }}).

## Что дальше

Для более глубокого погружения в фреймворк следуйте инструкциям раздела [С чего начать]({{ flow-docs-root }}/{{ lang }}/concepts/getting-started{{ docs-revision-query }}).
