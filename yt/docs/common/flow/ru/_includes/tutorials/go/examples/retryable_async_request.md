...
Расширение примера [Async Request]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/async_request{{ docs-revision-query }}): обработчик запросов повторяет каждую неудачную попытку через [таймеры]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer). Запрос целиком сохраняется во внутреннем [стейте]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state) и повторяется с фиксированной задержкой, пока внешний сервис на него не ответит.
...
- `injector` — нативный сорс (`TSwiftPassthroughOrderedSourceComputation`), объявленный прямо в [спеке]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec): он читает очередь и публикует события в [стрим]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) `event`. Go-кода у него нет.
...
- Счётчик попыток живёт в стейте вместе с данными запроса, поэтому переживает рестарт [воркера]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#worker): после перезапуска повтор продолжается с того же места.
