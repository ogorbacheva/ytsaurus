...
Пример [пайплайна]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline), реализующего асинхронный поход во внешний сервис: один [компьютейшен]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) превращает события в запросы и накапливает ответы во внешнем [стейте]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state), другой обслуживает запросы без стейта. Go-реализация того же сценария, что и [C++ пример]({{ flow-docs-root }}/{{ lang }}/tutorials/cpp/examples/async_request{{ docs-revision-query }}).

[Исходный код]({{source-root}}/yt/yt/flow/examples/go/async_request)

## Структура {#structure}

Компаньон обслуживает два компьютейшена, `injector` остаётся нативным сорсом из [спеки]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec):
...
Цикл `event → request → response → state` замыкается между двумя компьютейшенами. Событие отвечается запросом, а не сразу результатом, поэтому обслуживающая сторона никогда не задерживает обработку: ответ приходит позже, отдельным [сообщением]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#message), и только тогда стейт ключа сдвигается.
...
- **Stateless-компьютейшен**: `requestProcessor` не использует стейт и сгруппирован по `request_id`, а не по ключу события, поэтому запросы одного ключа расходятся по всем [партициям]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#partition) и масштабируются независимо.
- **Зависимость стримов**: `streams_dependency` в спеке объявляет, что `request` порождается из `event` — воркер учитывает это при продвижении [вотермарков]({{ flow-docs-root }}/{{ lang }}/concepts/watermarks{{ docs-revision-query }}).
