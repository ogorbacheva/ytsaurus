...
Пример [пайплайна]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) из двух Go-[компьютейшенов]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation): source-компьютейшен парсит JSON и отправляет типизированные [сообщения]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#message), transform-компьютейшен ведёт подсчёт событий во внешнем [стейте]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state). Между ними стоят нативные passthrough-компьютейшены, перегруппирующие событие по четырём разным ключам.

[Исходный код]({{source-root}}/yt/yt/flow/examples/go/shuffle)

## Структура {#structure}

- `reader` (source, `TSwiftOrderedSourceCompanionComputation`) — `eventMapper`: парсит JSON из колонки `data` и отправляет типизированное сообщение в [стрим]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) `event`.
- `shuffle_a` … `shuffle_d` — нативные `TSwiftPassthroughComputation`, объявленные в [спеке]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec). Каждый перегруппирует поток по своему ключу (`key_a` … `key_d`) и публикует его в отдельный стрим `event_a` … `event_d`. Go-кода у них нет.
...
- Пайплайн из нескольких компьютейшенов: [сорс]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#source) на `flow.NewRowSourceComputation` плюс трансформ на `flow.NewRowComputation`. Оба регистрируются одним `pipeline.Add`.
