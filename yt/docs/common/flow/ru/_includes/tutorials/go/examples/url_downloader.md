...
Пример [пайплайна]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline), который группирует входящие URL по хосту и обрабатывает их пакетно по [таймеру]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer). Показывает типичный паттерн «накопить в [стейте]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state) → обработать по таймеру → очистить стейт».

[Исходный код]({{source-root}}/yt/yt/flow/examples/go/url_downloader)

## Структура {#structure}

Пайплайн состоит из двух [компьютейшенов]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation):

- `url_reader` — нативный сорс (`TSwiftPassthroughOrderedSourceComputation`), объявленный прямо в [спеке]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec): он читает очередь и публикует сообщения с полями `host` и `url` в [стрим]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) `urls`. Go-кода у него нет.
...
- Ключ стейта по хосту задаётся `group_by_schema` из [спеки]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec), так что каждая партиция компьютейшена обрабатывает URL одного хоста изолированно.
