...
Пример join-[пайплайна]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline), который объединяет [стримы]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) показов (`hit`) и действий (`action`) по общему ключу, используя [таймеры]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer) и внешний [стейт]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state). Go-реализация аналогичного [примера на C++]({{ flow-docs-root }}/{{ lang }}/tutorials/cpp/examples/wait_click_join{{ docs-revision-query }}).

[Исходный код]({{source-root}}/yt/yt/flow/examples/go/wait_click_join)

## Структура {#structure}

Пайплайн состоит из трёх [компьютейшенов]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation):

- `hit_reader` и `action_reader` — нативные сорсы (`TSwiftPassthroughOrderedSourceComputation`), объявленные прямо в [спеке]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec): они читают две очереди и публикуют стримы `hit` и `action`. Go-кода у них нет. Каждый сорс объявляет `watermark_strategy`, поэтому [вотермарк]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timestamps-and-watermarks) пайплайна считается по времени события.
...
- Фильтрация запоздавших данных: `msg.EventTimestamp < rt.MinWatermark()` отбрасывает сообщения из уже закрытых окон — иначе для одного показа опубликовался бы второй join-результат. Подробнее о [вотермарках]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timestamps-and-watermarks).
