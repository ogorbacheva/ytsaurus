...
Поддержка вычислений на Python во Flow реализуется через механизм [компаньонов]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#companion). Python-код выполняется в отдельном gRPC-процессе, который взаимодействует с C++ [воркером]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#worker).
...
Python SDK предоставляет единый подход для настройки компаньона — класс `Pipeline`. Он позволяет зарегистрировать [компьютейшены]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) и запустить gRPC-сервер компаньона:
...
Метод `pipeline.add(computation_id, function)` регистрирует функцию обработки для компьютейшена с указанным идентификатором. Идентификатор должен совпадать с `computation_id` в [спеке]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec) пайплайна.

## Computation и SourceComputation {#computation-and-source}

Для создания [компьютейшена]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) на Python необходимо выбрать подходящий режим регистрации, соответствующий [типу Computation-а в C++]({{ flow-docs-root }}/{{ lang }}/concepts/companion{{ docs-revision-query }}#vidy-computation-ov-dlya-raboty-s-kompanonami):
...
- **computation_id** — по нему происходит маппинг запросов между [воркером]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#worker) и компаньоном.
- **fn** — функция с логикой обработки [сообщений]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#message). Может быть экземпляром `RowFunction`, `BatchFunction`, обычной функцией или классом с методом `on_message`/`on_messages`.

Фильтрация сообщений в source-компьютейшенах выполняется через флаг [distribute]({{ flow-docs-root }}/{{ lang }}/how-to-guides/python/distribute{{ docs-revision-query }}) при эмите сообщения из Process Function.

## Process Function {#process-function}

Есть два вида ProcessFunction:

- `RowFunction` — получает [сообщения]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#message) и [таймеры]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer) по одному, предоставляет методы `on_message` и `on_timer`.
- `BatchFunction` — получает весь батч сообщений и таймеров, предоставляет методы `on_messages` и `on_timers`.

Подробнее — в разделе [Computation (Python)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/python/computation{{ docs-revision-query }}).
...
Подробнее — в разделе [Флаг distribute (Python)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/python/distribute{{ docs-revision-query }}).

## Node companion {#node-companion}

Точка входа в Python-компаньон — файл `__main__.py`. В нём необходимо сконфигурировать компьютейшены через `Pipeline` и вызвать `pipeline.run()`. Функция `main` из [WordCount]({{ flow-docs-root }}/{{ lang }}/tutorials/python/examples/wordcount{{ docs-revision-query }}) (в самом файле она вызывается из стандартного `if __name__ == "__main__":`):
...
- [Примеры]({{ flow-docs-root }}/{{ lang }}/tutorials/python/examples/wordcount{{ docs-revision-query }})
