...
Пример [пайплайна]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) из двух [компьютейшенов]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation): source-компьютейшен парсит JSON-данные и отправляет типизированные [сообщения]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#message), transform-компьютейшен ведёт подсчёт событий во внешнем [стейте]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state).

[Исходный код]({{source-root}}/yt/yt/flow/examples/python/shuffle)

## Структура

- `reader` (source) -- `EventMapper`: парсит JSON из поля `data` и отправляет типизированные сообщения в [стрим]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) `event`.
...
- [Source]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#source)-компьютейшен с `source=True` для чтения из внешнего источника.
