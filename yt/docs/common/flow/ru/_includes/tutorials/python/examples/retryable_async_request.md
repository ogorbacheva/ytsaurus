...
Расширение примера [AsyncRequest]({{ flow-docs-root }}/{{ lang }}/tutorials/python/examples/async_request{{ docs-revision-query }}): обработчик запросов поддерживает автоматические повторные попытки через [таймеры]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer). При неудаче запрос сохраняется во внутреннем [стейте]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state) и повторяется через фиксированный интервал, вплоть до `MAX_RETRIES` попыток.
