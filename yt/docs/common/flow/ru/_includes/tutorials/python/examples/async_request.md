...
Пример двухкомпонентного [пайплайна]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline), реализующего асинхронную обработку запросов: один [компьютейшен]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) маршрутизирует события в запросы и накапливает ответы, другой — выполняет вычисление без [стейта]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state). Python-реализация аналогичного [C++ примера]({{ flow-docs-root }}/{{ lang }}/tutorials/cpp/examples/async_request{{ docs-revision-query }}).
...
1. **`state`** (`StateKeeperFunction`) — stateful-компьютейшен, который:
   ...
   - Принимает ответы из стрима `response` и накапливает суммарную длину (`total_length`) во внешнем [стейте]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state).
...
- **Двухкомпонентный пайплайн**: `pipeline.add("state", ...)` и `pipeline.add("processor", ...)` регистрируют компьютейшены, стримы между которыми описываются в [спеке]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec).
