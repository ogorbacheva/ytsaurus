...
[Пайплайн]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) демонстрирует выполнение внешних вызовов (подобных HTTP-запросам) из [компьютейшенов]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) с управлением скоростью загрузки (троттлинг), шардированием по хостам и управлением [стейтом]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state).
...
4. Ставит [таймер]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer) для периодической проверки результатов через `GetNextHostCheck`
...
При ребалансировке [партиций]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#partition) URL, не попавшие в лимит `PersistLimit`, будут потеряны. Значение лимита следует выбирать с учетом допустимых потерь.
