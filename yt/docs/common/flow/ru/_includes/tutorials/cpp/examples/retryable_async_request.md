...
[Пайплайн]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) аналогичен [Async Request]({{ flow-docs-root }}/{{ lang }}/tutorials/cpp/examples/async_request{{ docs-revision-query }}), но добавляет логику повторных попыток (ретраев) с использованием [таймеров]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer) и внутреннего `YsonState`.
...
- хранить внутренний [стейт]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state) для отслеживания числа неудачных попыток;
...
Полностью аналогичен `TStateKeeper` из примера [Async Request]({{ flow-docs-root }}/{{ lang }}/tutorials/cpp/examples/async_request{{ docs-revision-query }}): принимает входные события и ответы, хранит аккумулированный результат во внешнем стейте.
...
В [спеке]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec) для `TRequestProcessor` необходимо зарегистрировать секцию `timers` для поддержки повторных попыток.
