...
Первый компьютейшн - `Reader`. Он занимается чтением и первичным преобразованием данных из очереди. Вот части [спеки]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec), связанные с данным компьютейшном:
...
- Для управления `Computation` необходимо использовать `NYT::NFlow::TQueueSourceController` - так как нам нужно определять число и настройки [партиций]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#partition) на базе входной сортированной динамической таблицы.
...
- Для работы со [стейтом]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state) мы используем `TSimpleExternalStateManager`, который предоставляет прямой доступ к таблице. Мы заводим поле с менеджером и регистрируем его в рамках реализации метода `DoInit()`.
