...
В этом разделе пошагово описано, как реализовать свой первый [пайплайн]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) на C++ с использованием Flow. В качестве примера рассматривается задача подсчёта слов (word count): чтение текстовых сообщений из очереди, разбиение на слова и подсчёт вхождений каждого слова.

## Предварительные требования

- Чекаут [репозитория]({{source-root}}).
- Настроенный `ya make` (сборочная система).
- Ознакомление с [основными понятиями]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}) Flow.
...
Подробнее о конвертации сообщений см. раздел [Computation (C++)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/computation{{ docs-revision-query }}), секция «TYsonMessage».

### 2. Определите стейт {#define-state}

Если [компьютейшен]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) работает со [стейтом]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state), определите класс-наследник от `TStateBase`:
...
Подробнее о работе со стейтами см. [Работа со стейтами (C++)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/state{{ docs-revision-query }}).

### 3. Реализуйте [Source]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#source) Computation {#implement-source}
...
Обратите внимание: `TSwiftOrderedSourceComputation` не материализует сами сообщения в YT, сохраняя лишь метаинформацию для восстановления. Подробнее см. [Computation (C++)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/computation{{ docs-revision-query }}#tswiftorderedsourcecomputation).
...
Подробнее о формате спеки см. [Spec & DynamicSpec]({{ flow-docs-root }}/{{ lang }}/concepts/spec{{ docs-revision-query }}).
...
- Объект пайплайна с [внутренними таблицами Flow]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#inner-pipeline-tables).
...
Детально про релизы и управление пайплайном можно прочитать в разделе [Релизы и управление пайплайном]({{ flow-docs-root }}/{{ lang }}/how-to-guides/deployment/update-pipeline{{ docs-revision-query }}#release-and-configure-basic-rules).
