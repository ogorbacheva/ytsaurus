...
- [Стейт]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}) обновляется атомарно вместе с обработкой сообщения.
...
Для каждой созданной [джобы]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#job) контроллер создаёт специальную транзакцию — `Lease`. Джоба коммитит любые данные, используя эту транзакцию как `PrerequisiteTransaction`. Lease обладает свойством барьерности: после отмены Lease любая попытка закоммитить транзакцию с её указанием гарантированно завершается неуспехом. Это позволяет контроллеру надёжно прекращать произвольные джобы и тут же запускать новые над теми же [партициями]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#partition).

#### 2. Дедупликация входных сообщений {#input-dedup}

- **Внутренние потоки** (`input`): дедупликация по `message_id` с помощью внутренней таблицы `input_messages` или её компактного варианта `compact_input_messages`. Компактный вариант выбирается автоматически, когда не включён `experimental_enable_non_uint_key` (партиционирование идёт исключительно по `uint64`-хеш-колонке); поведение можно переопределить параметром `use_compact_input_messages`. Старые `message_id` удаляются из этой таблицы с использованием [SystemWatermark]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timestamps-and-watermarks).
- **Источники** (`source`): дедупликация по оффсетам — оффсеты консьюмера продвигаются только после успешного коммита [эпохи]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#epoch).
- **[Таймеры]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer)**: результат обработки таймера коммитится в той же транзакции, что и удаление этого таймера.

#### 3. Атомарность выходных данных {#output-atomicity}

Способ доставки выходных сообщений зависит от типа [синка]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#sink):
...
**Для [Swift]({{ flow-docs-root }}/{{ lang }}/concepts/swift{{ docs-revision-query }})-компьютейшенов** (`TSwiftMapComputation`, `TSwiftOrderedSourceComputation`) детерминизм строго обязателен. В Swift-компьютейшенах результаты вычислений не материализуются между эпохами и могут быть пересчитаны при рестарте. Если разные попытки вычисления одного и того же сообщения дают разные результаты, возможно смешение результатов от разных попыток, потеря или дублирование данных.
...
Обновления [внутреннего стейта]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}) Flow (YSON-state, External State через `IExternalStateManager`) атомарны и безопасны. Однако если компьютейшен модифицирует данные в сторонней системе (базе данных, кеше), эти изменения не участвуют в транзакции эпохи и могут быть продублированы.

### Что Flow гарантирует автоматически {#automatic-guarantees}

- Внутренние потоки между [компьютейшенами]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#computation) — exactly-once.
- Обновления [стейта]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}) — атомарны с обработкой сообщения.
- Обработка и удаление [таймера]({{ flow-docs-root }}/{{ lang }}/concepts/timers{{ docs-revision-query }}) — в одной транзакции.
- Продвижение оффсетов [сорсов]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#source) — только после успешного коммита эпохи.
...
Flow **не гарантирует** глобальный порядок обработки сообщений. Однако существуют гарантии для производных сообщений с совпадающими [ключами]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#key) в рамках одной цепочки [lineage]({{ flow-docs-root }}/{{ lang }}/concepts/lineage{{ docs-revision-query }}).

Подробности — в разделе [Порядок обработки сообщений]({{ flow-docs-root }}/{{ lang }}/concepts/ordering{{ docs-revision-query }}).

## Влияние коннекторов на гарантии {#connectors}

Каждый [коннектор]({{ flow-docs-root }}/{{ lang }}/concepts/connectors/overview{{ docs-revision-query }}) имеет свой набор гарантий, зависящий от типа подключения (source/sink) и варианта синка (sync/async/at-least-once).
...
Подробнее — [Queue]({{ flow-docs-root }}/{{ lang }}/reference/connectors/queue{{ docs-revision-query }}).
...
Подробнее — [Static Table]({{ flow-docs-root }}/{{ lang }}/reference/connectors/static-table{{ docs-revision-query }}).
...
- Синк отсутствует. Сортированная динамическая таблица, используемая как источник сервис-лога, зачастую наполняется самим пайплайном через работу с [внешним стейтом]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}#external-state). Например, компьютейшен типа `TTransformComputation` обновляет записи таблицы на каждой эпохе.

Подробнее — [Сервис-лог]({{ flow-docs-root }}/{{ lang }}/reference/connectors/servicelog{{ docs-revision-query }}).
...
Расширение над коннектором [Queue]({{ flow-docs-root }}/{{ lang }}/reference/connectors/queue{{ docs-revision-query }}), добавляющее батчинг и сжатие. Гарантии идентичны базовому коннектору Queue:
...
- [Пайплайн]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) переживает выпадение отдельных машин и датацентров.
...
- [Автоматическая балансировка партиций]({{ flow-docs-root }}/{{ lang }}/concepts/overview{{ docs-revision-query }}) перераспределяет нагрузку при изменении топологии кластера.

## См. также

- [Обзор Flow]({{ flow-docs-root }}/{{ lang }}/concepts/overview{{ docs-revision-query }})
...
- [Stateful-обработка]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }})
