...
Детальнее в [спеке]({{ flow-docs-root }}/{{ lang }}/concepts/spec{{ docs-revision-query }}#watermark-strategy).
...
Данный модуль для source [компьютейшенов]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) пытается дать оценку снизу для `EventTimestamp` будущих событий по всем `output` потокам . Алгоритм устроен так:

1. Оценивается `EventWatermark` для каждой [партиции]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#partition) входной очереди независимо.
...
4. Недоступные партиции, а также партиции, в которые не идёт записи, могут ограниченно игнорироваться. Настройки этих эвристик можно найти в [описании спеки]({{ flow-docs-root }}/{{ lang }}/concepts/spec{{ docs-revision-query }}#watermark-generator)
...
Также планируется создать протокол для передачи между разными [пайплайнами]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) информации про `Watermark`.
...
Таймеры тесно связаны с вотермарками: именно `EventWatermark` (по умолчанию) определяет момент срабатывания таймера. Подробнее о том, как устроены таймеры, их конфигурации и API — в разделе [Таймеры]({{ flow-docs-root }}/{{ lang }}/concepts/timers{{ docs-revision-query }}).

## Конфигурация {#configuration}

Настройка `WatermarkStrategy` (`EventTimestampAssigner`, `WatermarkGenerator`, `WatermarkAlignment`) описана в разделе [Spec]({{ flow-docs-root }}/{{ lang }}/concepts/spec{{ docs-revision-query }}#watermark-strategy).
