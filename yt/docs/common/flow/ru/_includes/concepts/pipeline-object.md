...
Связанные разделы: [Pipeline в глоссарии]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline), [Spec, DynamicSpec и Config]({{ flow-docs-root }}/{{ lang }}/concepts/spec{{ docs-revision-query }}), [Stateful-обработка]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}), [Внутренние таблицы пайплайна]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#inner-pipeline-tables).
...
|-------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `input_messages`                    | Индекс входных сообщений [transform computation'ов]({{ flow-docs-root }}/{{ lang }}/concepts/computation{{ docs-revision-query }}#ttransformcomputation), использующийся для дедупликации                                                                                                                                                                                        |
| `compact_input_messages`            | Компактный индекс входных сообщений. Используется по умолчанию для всех computation'ов, кроме случая, когда включён `experimental_enable_non_uint_key`; поведение переопределяется параметром `use_compact_input_messages` в [TComputationSpec]({{ flow-docs-root }}/{{ lang }}/reference/configuration/all_yson_structs{{ docs-revision-query }}#NYT_NFlow_TComputationSpec) |
...
| `states`                            | Пользовательские и служебные [стейты]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}), сохраняемые по [ключу]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#key)                                                                                                                                                                                                                       |
| `partition_states`                  | Стейты сохраняемые по партиции                                                                                                                                                                                                                                                                                    |
| `timers`                            | [Таймеры]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer) пользовательского кода                                                                                                                                                                                                                                                               |
| `controller_logs`                   | Логи событий [Controller]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#controller) в категории PublicFlowController                                                                                                                                                                                                                                |
...
Таблицы `input_messages` и `compact_input_messages` не накапливают историю: записи о входных сообщениях нужны только для дедупликации (exactly-once) и удаляются после обработки. Controller продвигает [SystemWatermark]({{ flow-docs-root }}/{{ lang }}/concepts/watermarks{{ docs-revision-query }}) этих таблиц, и строки с уже обработанными сообщениями физически удаляются механизмом очистки динамических таблиц &mdash; в том числе после завершения пайплайна. Поэтому пустая таблица `input_messages` &mdash; это штатная очистка, а не потеря данных.
...
При смене схемы [внутренних таблиц](#internal_tables) в новой версии Flow апгрейд формата выполняется отдельной миграцией &mdash; см. [Внутренние таблицы пайплайна]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#inner-pipeline-tables) и [Базовые правила выкатки]({{ flow-docs-root }}/{{ lang }}/how-to-guides/deployment/update-pipeline{{ docs-revision-query }}#release-and-configure-basic-rules).

Если пайплайн использует [External State]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}) (пользовательские таблицы за пределами узла), их создание и эволюция схем &mdash; ответственность пользователя. {% if audience == "internal" %}В Yandex-инфраструктуре для этого используется [YtSync]({{yt-sync-docs}}/).{% else %}Операции выполняются стандартными командами `yt create table ... --attributes '{dynamic=true; schema=...}'` и `yt mount-table` &mdash; см. примеры в разделе [Команда create]({{ core-docs-root }}/{{ lang }}/how-to-guides/storage/cypress-example{{ docs-revision-query }}#create).{% endif %}

## См. также { #see_also }

- [Глоссарий: Pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline)
- [Внутренние таблицы пайплайна]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#inner-pipeline-tables)
- [Базовые правила выкатки пайплайна]({{ flow-docs-root }}/{{ lang }}/how-to-guides/deployment/update-pipeline{{ docs-revision-query }}#release-and-configure-basic-rules)
{% if audience != "internal" %}
