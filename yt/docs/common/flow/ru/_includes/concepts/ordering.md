...
В распределённой потоковой системе строгий глобальный порядок всех сообщений обеспечить непрактично: данные поступают из множества источников, обрабатываются параллельно в разных [партициях]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#partition) и [компьютейшенах]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#computation). Flow решает эту задачу иначе — через механизм `AlignmentTimestamp`.
...
- **Внутри одного [стрима]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream)**: сообщения обрабатываются в порядке `(AlignmentTimestamp, message_id)` — полный детерминированный порядок.
- **Между разными [стримами]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream)**: используется сливающая очередь приоритетов по [StabilizedEventTimestamp](#stabilized-event-timestamp) (вычисляется на основе `AlignmentTimestamp`) с поправкой на [stream_delays]({{ flow-docs-root }}/{{ lang }}/concepts/spec{{ docs-revision-query }}#inputordering); при равенстве — по `TaskId`.
...
Помимо [SystemTimestamp и EventTimestamp]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timestamps-and-watermarks), каждое сообщение во Flow несёт поле `AlignmentTimestamp` — временную метку, используемую для выравнивания прогресса обработки между потоками и партициями.
...
- ключи в их [lineage]({{ flow-docs-root }}/{{ lang }}/concepts/lineage{{ docs-revision-query }}) совпадают (то есть [ключи группировки]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#key) во всех промежуточных компьютейшенах по пути от источника до текущего компьютейшена одинаковы),
