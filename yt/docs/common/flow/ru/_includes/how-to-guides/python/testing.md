...
Данная страница описывает **юнит-тестирование** компонентов Python-[пайплайна]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline), а также **интеграционное тестирование** полного пайплайна через `FlowTestPythonBase`.

{% endnote %}

## Общая архитектура тестирования {#architecture}

В продакшене C++ [воркер]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#worker) отправляет gRPC-запросы [компаньону]({{ flow-docs-root }}/{{ lang }}/concepts/companion{{ docs-revision-query }}), передавая [сообщения]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#message), таймеры, [стейты]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state) и [watermark-и]({{ flow-docs-root }}/{{ lang }}/concepts/watermarks{{ docs-revision-query }}). Компаньон вызывает `Computation.do_process()`, который делегирует обработку в `ProcessFunction`.
...
Помимо `PEERDIR` на `integration_test_base`, интеграционному тесту нужны рецепт кластера, `DEPENDS` на бинарь пайплайна и `flow_server`, а также `DATA` со спекой. Полный `ya.make` теста из [WordCount]({{ flow-docs-root }}/{{ lang }}/tutorials/python/examples/wordcount{{ docs-revision-query }}):
