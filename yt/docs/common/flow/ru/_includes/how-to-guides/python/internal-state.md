...
Internal State — механизм работы с внутренним [состоянием (стейтом)]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state), хранящимся во внутренних таблицах Flow. В отличие от [External State]({{ flow-docs-root }}/{{ lang }}/how-to-guides/python/external-state{{ docs-revision-query }}), пользователю не нужно самостоятельно создавать таблицы — Flow управляет ими автоматически.

Подробнее про `StateAccessor` и общие принципы работы со стейтом: [StateAccessor]({{ flow-docs-root }}/{{ lang }}/how-to-guides/python/state-accessor{{ docs-revision-query }}).
...
Python SDK предоставляет три вида аксессоров для работы с Internal State, различающихся форматом сериализации. Для работы с внешним стейтом используется отдельный аксессор — [ExternalStateAccessor]({{ flow-docs-root }}/{{ lang }}/how-to-guides/python/external-state{{ docs-revision-query }}).
...
Здесь стейт привязан к ключу [сообщения]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#message) (определяемому через `group_by_schema` в [спеке]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec)). Для каждого уникального слова хранится свой независимый счётчик.
...
- [Пример WordCount]({{ flow-docs-root }}/{{ lang }}/tutorials/python/examples/wordcount{{ docs-revision-query }})
