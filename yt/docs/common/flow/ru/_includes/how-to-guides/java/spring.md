...
Один класс с `@SpringBootApplication` служит обеими точками входа: роль процесса выбирается по переменной среды `YT_FLOW_MODE`, как описано в разделе [Точка входа]({{ flow-docs-root }}/{{ lang }}/tutorials/java/quick-start{{ docs-revision-query }}). Отдельный класс для runner-а не нужен.
