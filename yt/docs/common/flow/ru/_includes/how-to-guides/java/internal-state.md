...
Internal State — механизм работы с внутренним состоянием (стейтом), хранящимся во внутренних таблицах Flow. В отличие от [External State]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/external-state{{ docs-revision-query }}), пользователю не нужно самостоятельно создавать таблицы — Flow управляет ими автоматически.

Подробнее про `StateAccessor` и работу со стейтом: [State Accessor]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/state-accessor{{ docs-revision-query }}).
...
|----------|--------|----------|
| [YsonStateAccessor]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/internal-state{{ docs-revision-query }}#yson-state-accessor) | YSON | Сериализация через `@YTreeObject` аннотации |
| [ProtoStateAccessor]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/internal-state{{ docs-revision-query }}#proto-state-accessor) | Protobuf | Сериализация через Protobuf |
| [DefaultStateAccessor]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/internal-state{{ docs-revision-query }}#default-state-accessor) | Произвольный | Пользовательские serializer/deserializer |
| [RawStateAccessor]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/internal-state{{ docs-revision-query }}#raw-state-accessor) | `byte[]` | Без сериализации (сырые байты) |
| [NoOpStateAccessor]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/internal-state{{ docs-revision-query }}#noop-state-accessor) | - | Хранит только факт наличия стейта |
...
`NoOpStateAccessor` хранит только факт наличия стейта для [ключа]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#key), без какого-либо payload. Полезен для отслеживания уже обработанных ключей (дедупликация).
