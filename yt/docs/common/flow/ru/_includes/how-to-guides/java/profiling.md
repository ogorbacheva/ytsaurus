...
JFR записывается на диск чанками по 30 МБ в директорию, заданную переменной окружения `YT_FLOW_COMPANION_LOG_DIR`. Данные хранятся до 24 часов и ограничены общим размером 1 ГБ. Управление JFR осуществляется через [Переменные окружения]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/env-variables{{ docs-revision-query }}):
