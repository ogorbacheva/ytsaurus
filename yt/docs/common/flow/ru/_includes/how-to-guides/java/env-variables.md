...
Подробнее о JVM-опциях по умолчанию, профилировании и настройке см. в разделе [Профилирование Java companion]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/profiling{{ docs-revision-query }}).
...
|| `YT_FLOW_COMPANION_JFR_OPTS` | Пользовательские JFR-опции через пробел, заменяющие значения по умолчанию. Игнорируется, если задана `YT_FLOW_COMPANION_JFR_DISABLED`. | [см. profiling]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/profiling{{ docs-revision-query }}#jfr-defaults) ||
|| `YT_FLOW_COMPANION_GC_LOG_DISABLED` | Если установлена в `1`, отключает логирование GC. | GC-логирование включено ||
|| `YT_FLOW_COMPANION_GC_LOG_OPTS` | Пользовательские опции логирования GC через пробел, заменяющие значения по умолчанию. Игнорируется, если задана `YT_FLOW_COMPANION_GC_LOG_DISABLED`. | [см. profiling]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/profiling{{ docs-revision-query }}#gc-defaults) ||
