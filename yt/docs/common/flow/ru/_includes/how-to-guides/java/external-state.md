...
External State в Java SDK Flow (Java и Kotlin) представлен классом `ExternalStateAccessor`, который предоставляет типизированный доступ к строкам внешней динамической таблицы. Стейт привязан к ключу сообщения (`group_by_schema`). Подробнее про `StateAccessor` и работу со стейтом: [State Accessor]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/state-accessor{{ docs-revision-query }}).

{% note info %}

Если нужен **только read-only** доступ к внешнему стейту (join по ключу с кэшированием по TTL, без модификации), на стороне фреймворка существует отдельный механизм — **External State Joiner**. Он доступен и в Java/Kotlin (см. [Read-only joiner](#read-only-joiner)), и в C++ ([External State Joiner]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/state{{ docs-revision-query }}#external-state-joiner)) и объявляется в спеке компьютейшена в top-level секции `external_state_joiners` (на одном уровне с `external_state_managers`).
...
Подробнее о Internal State см. [Internal State]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/internal-state{{ docs-revision-query }}).
...
`ExternalStateAccessor` для конкретного [ключа]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#key) получается через `RuntimeContext` (который наследует `StatefulContext`):
