...
Полный пример юнит-теста для `JoinProcessFunction` из проекта [wait_click_join]({{ flow-docs-root }}/{{ lang }}/tutorials/java/examples/wait_click_join{{ docs-revision-query }}). В этом варианте `PipelineContext` создаётся вручную, без Spring-контейнера.
