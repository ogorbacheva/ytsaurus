...
На этой странице описаны особенности реализации Computation на C++. Языконезависимое описание концепции см. в разделе [Computation]({{ flow-docs-root }}/{{ lang }}/concepts/computation{{ docs-revision-query }}).
...
Классы, содержащие `Swift` в названии, реализуют принцип [Swift]({{ flow-docs-root }}/{{ lang }}/concepts/swift{{ docs-revision-query }}). Подробнее — в разделе [Swift]({{ flow-docs-root }}/{{ lang }}/concepts/swift{{ docs-revision-query }}).
...
- Метод `DoSync` может использоваться для сохранения данных вручную в транзакцию `YT`. Однако, более правильно использовать [Sink]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#sink) или [ExternalState]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/state{{ docs-revision-query }}#external-state). Прямая работа с `DoSync` в `SwiftComputation` может привести к нежелаемому поведению. В `Transform` метод является безопасным, однако менее удобным.
- У каждого `Computation` есть семейство методов `DoProcess`. Они принимают либо `IInputContextPtr input` с методами `GetMessages` и `GetTimers`, либо конкретное сообщение или [таймер]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer). А объект `IOutputCollectorPtr output` предназначен для сбора выходных сообщений и таймеров — подробнее см. [OutputCollector](#output-collector).
...
- Всё выполнение кода в рамках Computation'ов строго однопоточно и выполняется в рамках `GetContext()->SerializedInvoker`. Многопоточность достигается за счёт увеличения числа [партиций]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#partition). Если тем не менее нужно выполнить некоторый код многопоточно, то стоит использовать `GetContext()->PoolInvoker`, но при этом необходимо дождаться результатов выполнения в рамках соответствующего метода.
...
| `output->AddTimer(timer)` | Добавить [таймер]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer) |
| `output->SetParents(parentIds)` | Задать parent ID для отслеживания [lineage]({{ flow-docs-root }}/{{ lang }}/concepts/lineage{{ docs-revision-query }}). Возвращает новый `IOutputCollectorPtr` с привязанным контекстом lineage |
...
{% include notitle [_](../../../../reference/configuration/NYT_NFlow_TMessageSerializer.md) %}
...
{% include notitle [_](../../../../reference/configuration/NYT_NFlow_EProcessingMode.md) %}

### TPassthroughComputation

Наследник `TTransformComputation`. Реализует [passthrough-компьютейшен]({{ flow-docs-root }}/{{ lang }}/concepts/computation{{ docs-revision-query }}#passthrough) — создан в основном для демонстрации возможностей.
...
* `input`-стримы;
* [таймеры]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer);
* [key-visitor-стримы]({{ flow-docs-root }}/{{ lang }}/concepts/key_visitor{{ docs-revision-query }});
...
Логику можно не наследовать от класса, а написать как [process function]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/process-functions{{ docs-revision-query }}) и указать в спеке адаптер `NYT::NFlow::TProcessFunctionTransformOrderedSourceComputation`: он исполняет функцию поверх `TTransformOrderedSourceComputation` — с той же материализацией выхода, теми же стейтами и той же валидацией спеки.

Пример — `NYT::NFlow::NExample::TLogParserProcessFunction` из [`examples/cpp/log_parser`]({{source-root}}/yt/yt/flow/examples/cpp/log_parser): разбирает строку лога на записи, эмитит YSON-структуру `TLogRecordMessage` (`level`, `text`, `worst_level_so_far`) и ведёт стейт `TWorstSeverityState` — бегущий максимум severity по партиции источника. Подробнее, вместе с полным исходным кодом, — в разделе [Log Parser]({{ flow-docs-root }}/{{ lang }}/tutorials/cpp/examples/log_parser{{ docs-revision-query }}).
...
Пример — `NYT::NFlow::NExample::TProtoLogParserComputation` из [`examples/cpp/proto_parser`]({{source-root}}/yt/yt/flow/examples/cpp/proto_parser): разбирает `TLogRecordProto`, эмитит `TLogRecordMessage` (`level`, `text`, `seen_at_level`) и ведёт стейт `TLevelCountsState` — счётчик записей каждого уровня по партиции источника. Счётчик неидемпотентен к повторной обработке и корректен ровно потому, что стейт коммитится в одной транзакции со смещением `source`. Подробнее — в разделе [Proto Parser]({{ flow-docs-root }}/{{ lang }}/tutorials/cpp/examples/proto_parser{{ docs-revision-query }}).
...
- Использует `watermark_strategy/watermark_generator` для оценки [вотермарков]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timestamps-and-watermarks) входных источников.
...
Подробнее о доступных коннекторах см. раздел [Коннекторы]({{ flow-docs-root }}/{{ lang }}/concepts/connectors/overview{{ docs-revision-query }}).

### Как работает батчинг и партиционирование? {#batching-partitions}

Каждая партиция обрабатывается строго однопоточно. Многопоточность достигается за счёт увеличения числа партиций (`partition_count` в спеке). Батчевые методы `DoProcess(IInputContextPtr input, IOutputCollectorPtr output)` получают все сообщения и таймеры за текущую [эпоху]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#epoch), что позволяет оптимизировать обработку.
...
Общая нагрузка = записей на партицию * количество партиций * частота эпох. Для [пайплайна]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) с 1000 партиций и эпохой в 1 секунду `TTransformComputation` создаст ~2000-4000 записей/с.

## Состояния пайплайна {#pipeline-state}

Возможные состояния пайплайна (тип `EPipelineState`):

{% include notitle [_](../../../../reference/configuration/NYT_NFlow_EPipelineState.md) %}
