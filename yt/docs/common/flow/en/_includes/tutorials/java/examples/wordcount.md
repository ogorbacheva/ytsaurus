...
The [pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) reads a [stream]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) of words and counts how many times each word appears, using a YSON state. This example shows how to configure a [companion]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#companion) with Spring Boot.
...
- `reader` is a SourceComputation without a processing function. Reading and parsing happen on the C++ [worker]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#worker) side.
...
This is the processing function that counts words using the [YsonStateAccessor]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/state{{ docs-revision-query }}#yson-state):
