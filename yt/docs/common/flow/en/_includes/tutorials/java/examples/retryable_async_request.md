...
The [pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) extends [AsyncRequest]({{ flow-docs-root }}/{{ lang }}/tutorials/java/examples/async_request{{ docs-revision-query }}): the request handler now supports retry attempts with a delay. If a failure occurs, `RequestProcessorFunction` saves the request state in the internal [state]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state) and sets a timer for 5 seconds; success is determined by the predicate `(requestId + failedAttempts) % 3 == 0`.
...
This is identical to the `StateKeeperFunction` from [AsyncRequest]({{ flow-docs-root }}/{{ lang }}/tutorials/java/examples/async_request{{ docs-revision-query }}): it processes the `event` and `response` streams and accumulates `total_length` in the external state. The retry logic is fully encapsulated in `RequestProcessorFunction`.
