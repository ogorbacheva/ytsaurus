...
The [pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) reads the [stream]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) of events, groups them by key, and counts the number of unique events using an external [state]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state) (ExternalStateAccessor). This example shows how to configure a [companion]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#companion) with Spring Boot.
...
This is a process function for the source-[computation]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) `reader`. It parses and transforms the input data. The version below is simplified. In the [actual code]({{source-root}}/yt/yt/flow/examples/java/shuffle), you also parse the `data` field as JSON using Jackson’s `ObjectMapper`:
...
This is a process function that uses [ExternalStateAccessor]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/state{{ docs-revision-query }}#external-state) to count the number of events:
