...
The [pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) performs a join of [streams]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) of hits and actions within a time window. For each hit, the system waits for related actions (show, click) within a specified time interval, then generates a merged event.
...
For a detailed description of the task and join logic, see the [C++ version description]({{ flow-docs-root }}/{{ lang }}/tutorials/cpp/examples/wait_click_join{{ docs-revision-query }}).
...
Messages that arrive with an event time less than the current [watermark]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timestamps-and-watermarks) are considered late and are ignored. For more details about watermarks, see the [Watermarks & Timers]({{ flow-docs-root }}/{{ lang }}/concepts/watermarks{{ docs-revision-query }}) section.

### [Timers]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer) for closing the window
...
The [state]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state) is stored in an external dynamic table. `PayloadBuilder` lets you update individual state fields without full re-serialization:
...
{% include notitle [_](../../../how-to-guides/java/_field_order_warning.md) %}
...
The example uses the [Spring Boot integration]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/spring{{ docs-revision-query }}) for configuration. The `join` computation is registered with the `@FlowComputation` annotation on the `JoinProcessFunction` class:
...
You run integration testing for Java pipelines the same way as for C++ pipelines: by launching the full pipeline, including C++ workers, queues, and streams. For details, see [Integration tests]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/testing{{ docs-revision-query }}).
