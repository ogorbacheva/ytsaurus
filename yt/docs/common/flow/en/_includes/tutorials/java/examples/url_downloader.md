...
The [pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) groups incoming URLs by host, accumulates them in the internal [state]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state), and processes them in batches based on a timer. The timer triggers 5 seconds after a URL is added to the queue and emits the results to the output [stream]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation).
