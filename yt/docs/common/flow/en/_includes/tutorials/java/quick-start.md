...
This is the class with the `main` method for starting the pipeline. `SimpleRunnerProgram` is the Java equivalent of [NYT::NFlow::TSimpleRunnerProgram]({{ flow-docs-root }}/{{ lang }}/how-to-guides/deployment/basic-rules{{ docs-revision-query }}#launch-flow) and accepts the same configuration files and environment variables.
...
- [Examples]({{ flow-docs-root }}/{{ lang }}/tutorials/java/examples/wordcount{{ docs-revision-query }})
