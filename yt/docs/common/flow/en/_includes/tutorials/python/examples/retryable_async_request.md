...
This extends the [AsyncRequest]({{ flow-docs-root }}/{{ lang }}/tutorials/python/examples/async_request{{ docs-revision-query }}) example: the request handler supports automatic retries using [timers]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer). If a request fails, it’s saved in the internal [state]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state) and retried after a fixed interval, up to `MAX_RETRIES` attempts.
