...
You implement Python computations in Flow through the [companion]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#companion) mechanism. Python code runs in a separate gRPC process that interacts with the C++ [worker]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#worker).
...
Any Flow [pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) consists of three components:

- `Runner` — starts the pipeline and sets a new version of the [spec]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec).
- `Controller` — manages the pipeline’s operation.
- `Worker` — performs the actual data processing.

## Pipeline API {#pipeline-api}

The Python SDK provides a unified approach to configure a companion: the `Pipeline` class. It lets you register [computations]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) and start the companion’s gRPC server:
...
The `pipeline.add(computation_id, function)` method registers a processing function for the computation with the specified ID. The ID must match the `computation_id` in the pipeline’s [spec]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec).

## Computation and SourceComputation {#computation-and-source}

To create a [computation]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) in Python, choose the appropriate registration mode that matches the [C++ Computation type]({{ flow-docs-root }}/{{ lang }}/concepts/companion{{ docs-revision-query }}#computation-types):
...
- **computation_id** — this is used to map requests between the [worker]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#worker) and the companion.
- **fn** — a function with the logic for processing [messages]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#message). It can be an instance of `RowFunction`, `BatchFunction`, a regular function, or a class with the `on_message`/`on_messages` method.

You filter messages in source computations by using the [distribute]({{ flow-docs-root }}/{{ lang }}/how-to-guides/python/distribute{{ docs-revision-query }}) flag when emitting a message from a Process Function.

## Process Function {#process-function}

There are two types of ProcessFunction:

- `RowFunction` — receives [messages]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#message) and [timers]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer) one at a time and provides the `on_message` and `on_timer` methods.
...
For more details, see the [distribute flag (Python)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/python/distribute{{ docs-revision-query }}) section.

## Node companion {#node-companion}

The entry point to the Python companion is the `__main__.py` file. In it, you must configure the computations via `Pipeline` and call `pipeline.run()`. The `main` function from [WordCount]({{ flow-docs-root }}/{{ lang }}/tutorials/python/examples/wordcount{{ docs-revision-query }}) (in the file itself, it’s called from the standard `if __name__ == "__main__":`):
...
Required parameters: `pool` and `worker.count`. For the other fields (`cpu_limit`, `memory_limit`, number of Controllers, etc.), reasonable default values exist — see the full list of fields and their descriptions in [TVanillaConfig]({{ flow-docs-root }}/{{ lang }}/reference/configuration/all_yson_structs{{ docs-revision-query }}#NYT_NFlow_TVanillaConfig) and [TVanillaTaskConfig]({{ flow-docs-root }}/{{ lang }}/reference/configuration/all_yson_structs{{ docs-revision-query }}#NYT_NFlow_TVanillaTaskConfig).
...
- [Examples]({{ flow-docs-root }}/{{ lang }}/tutorials/python/examples/wordcount{{ docs-revision-query }})
