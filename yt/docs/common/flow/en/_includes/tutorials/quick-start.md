...
- [Controller]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#controller) — manages the pipeline’s lifecycle.
- [Worker]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#worker) — reads from sources and performs computations.
- {{product-name}} Cluster — stores system tables in the pipeline directory.

Each pipeline needs its own set of Controller and Worker instances, a working directory in [Cypress]({{ core-docs-root }}/{{ lang }}/concepts/storage/cypress{{ docs-revision-query }}), and a set of system dynamic tables.
...
- A [YT token]({{ core-docs-root }}/{{ lang }}/reference/storage/auth{{ docs-revision-query }}) for the {{product-name}} cluster with dynamic tables{% if audience == "internal" %} (see the [list of clusters](../../user-guide/dynamic-tables/clusters.md)){% endif %}, for example, {{flow-example-cluster}}.
...
First, a Cypress object of type `pipeline` is created, along with the set of helper dynamic tables that Flow needs to run (see the [Pipeline Object]({{ flow-docs-root }}/{{ lang }}/concepts/pipeline-object{{ docs-revision-query }}) section).

{% if audience == "internal" %}

In the Yandex infrastructure, [YtSync]({{ flow-docs-root }}/{{ lang }}/concepts/pipeline-object{{ docs-revision-query }}#yt-sync) does this:
...
In the open-source version, you use the ready-made helper [yt_sync_mini]({{ flow-docs-root }}/{{ lang }}/concepts/pipeline-object{{ docs-revision-query }}#yt-sync-mini):
...
You can find the structure of the [internal tables]({{ flow-docs-root }}/{{ lang }}/concepts/pipeline-object{{ docs-revision-query }}#internal_tables) that were created with the `pipeline` object here:
...
See [Logs]({{ flow-docs-root }}/{{ lang }}/how-to-guides/operations/view-logs{{ docs-revision-query }}) for how to read the controller and worker logs.
...
![](../../../_images/flow_noop_pipeline.png =600x230){ .center }
...
For the full list of fields, see [TVanillaConfig]({{ flow-docs-root }}/{{ lang }}/reference/configuration/all_yson_structs{{ docs-revision-query }}#NYT_NFlow_TVanillaConfig) (also see [TVanillaTaskConfig]({{ flow-docs-root }}/{{ lang }}/reference/configuration/all_yson_structs{{ docs-revision-query }}#NYT_NFlow_TVanillaTaskConfig)).

## What’s next

For a deeper dive into the framework, follow the instructions in the [Getting started]({{ flow-docs-root }}/{{ lang }}/concepts/getting-started{{ docs-revision-query }}) section.