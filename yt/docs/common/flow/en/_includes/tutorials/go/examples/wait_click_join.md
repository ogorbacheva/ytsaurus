...
An example of a join [pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) that joins the [streams]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) of impressions (`hit`) and actions (`action`) by a shared key, using [timers]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer) and an external [state]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state). This is a Go implementation of the similar [C++ example]({{ flow-docs-root }}/{{ lang }}/tutorials/cpp/examples/wait_click_join{{ docs-revision-query }}).

[Source code]({{source-root}}/yt/yt/flow/examples/go/wait_click_join)

## Structure {#structure}

The pipeline consists of three [computations]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation):

- `hit_reader` and `action_reader` — native sources (`TSwiftPassthroughOrderedSourceComputation`) declared directly in the [spec]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec): they read two queues and publish the `hit` and `action` streams. They have no Go code. Each source declares a `watermark_strategy`, so the [watermark]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timestamps-and-watermarks) of the pipeline is computed from the event time.
...
- Filtering out late data: `msg.EventTimestamp < rt.MinWatermark()` drops the messages of already closed windows — otherwise a second join result would be published for one impression. For details, see [watermarks]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timestamps-and-watermarks).
