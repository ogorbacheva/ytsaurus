...
An extension of the [Async Request]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/async_request{{ docs-revision-query }}) example: the request handler retries every failed attempt through [timers]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer). The whole request is stored in the internal [state]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state) and retried with a fixed delay until the external service answers it.
...
- `injector` — a native source (`TSwiftPassthroughOrderedSourceComputation`) declared directly in the [spec]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec): it reads the queue and publishes the events to the `event` [stream]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation). It has no Go code.
...
- The attempt counter lives in the state together with the request data, so it survives a [worker]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#worker) restart: after a restart the retries continue from the same place.
