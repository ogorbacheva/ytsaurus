...
An example of a [pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) that makes an asynchronous call to an external service: one [computation]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) turns events into requests and accumulates the responses in an external [state]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state), and another one serves the requests without any state. This is a Go implementation of the same scenario as the [C++ example]({{ flow-docs-root }}/{{ lang }}/tutorials/cpp/examples/async_request{{ docs-revision-query }}).

[Source code]({{source-root}}/yt/yt/flow/examples/go/async_request)

## Structure {#structure}

The companion serves two computations; `injector` stays a native source declared in the [spec]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec):
...
- **A stateless computation**: `requestProcessor` uses no state and is grouped by `request_id` rather than by the event key, so the requests of one key spread over all [partitions]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#partition) and scale independently.
- **Stream dependency**: `streams_dependency` in the spec declares that `request` is produced from `event` — the worker takes this into account when advancing [watermarks]({{ flow-docs-root }}/{{ lang }}/concepts/watermarks{{ docs-revision-query }}).
