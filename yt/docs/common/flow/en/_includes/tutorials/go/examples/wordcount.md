...
The simplest example of a stateful [pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) in Go: counting the occurrences of every word in an internal YSON [state]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state).
...
- `reader` — a native source (`TSwiftPassthroughOrderedSourceComputation`) declared directly in the [spec]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec): it reads the queue and publishes the rows to the `words` [stream]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation). It has no Go code.
...
- The state key is defined by `group_by_schema` from the [spec]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec) — in this case, by the `word` field.
