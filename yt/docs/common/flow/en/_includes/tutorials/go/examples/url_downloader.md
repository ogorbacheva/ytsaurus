...
An example of a [pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) that groups incoming URLs by host and processes them in batches by a [timer]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer). It shows the typical pattern "accumulate in the [state]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state) → process by a timer → clear the state".

[Source code]({{source-root}}/yt/yt/flow/examples/go/url_downloader)

## Structure {#structure}

The pipeline consists of two [computations]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation):

- `url_reader` — a native source (`TSwiftPassthroughOrderedSourceComputation`) declared directly in the [spec]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec): it reads the queue and publishes messages with the `host` and `url` fields to the `urls` [stream]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation). It has no Go code.
...
- The state key by host is set by `group_by_schema` from the [spec]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec), so every partition of the computation processes the URLs of one host in isolation.
