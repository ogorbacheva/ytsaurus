...
An example of a [pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) that enriches events from a queue with data from a read-only external [state]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state). The reference table is attached through `TSimpleExternalStateJoiner`, and the Go computation looks a row up by the event key.
