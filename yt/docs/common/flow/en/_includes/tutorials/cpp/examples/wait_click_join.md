...
1. An action can occur several hours after a hit. However, this [pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) only considers actions where action_time < hit_time + wait_for_actions.
...
- You explicitly set `watermark_generator/out_of_orderness_bound` to `10s`. This is a key parameter for the heuristic that estimates the [watermark]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timestamps-and-watermarks). This value is only for tests; for real tasks, choose it based on the properties of the stream you’re reading.

You can read about all the settings in detail [here]({{ flow-docs-root }}/{{ lang }}/concepts/spec{{ docs-revision-query }}#watermark-strategy).

Here’s an example [spec]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec) for `action_reader`:
...
- In `group_by_schema`, in addition to `hit_id` and `hit_time`, we include `hash`. This is for the correct operation of the [partitioning]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#partition) algorithm.
- The pipeline needs a [timer]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer) to close the hit, so we register `timer` in `timers`. We don’t specify extra settings because timers, by default, use `event_time` and the input streams.
- To send the `joined_event` to an ordered dynamic table (which might be on another cluster), we use an asynchronous `TQueueSink`.

The join itself is implemented as a [process function]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/process-functions{{ docs-revision-query }}) called `TJoinFunction` (a subclass of `IProcessFunction` that processes messages and timers element by element). It’s executed by the built-in `TProcessFunctionComputation`. In the spec, you set it via `processing_function`, and `wait_for_actions` is passed through `processing_function_parameters`. We recommend reading the code in the repository because it’s continuously improved.
...
- `action_queue` and `hit_queue` — input [queues]({{ core-docs-root }}/{{ lang }}/reference/dynamic-tables/queues{{ docs-revision-query }}#data_model).
- `consumer` — a [consumer]({{ core-docs-root }}/{{ lang }}/reference/dynamic-tables/queues{{ docs-revision-query }}#data_model) for reading the input queues and notifying the queue broker which messages are fully processed and no longer needed by the system.
- `output_queue` — the output queue with the joined input logs.
- `producer` — a producer for [exactly-once]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#exactly-once) writes to the output queue.
- `state` — a [table]({{ core-docs-root }}/{{ lang }}/concepts/dynamic-tables/sorted-dynamic-tables{{ docs-revision-query }}) for storing the key profile.
