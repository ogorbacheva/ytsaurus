...
Both units of user logic are written as [process functions]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/process-functions{{ docs-revision-query }}) (subclasses of `IProcessFunction`). The function itself is lightweight and testable, and its execution mode is selected by the built-in `Computation` adapter in the spec via the `processing_function` field.
...
Since the executing `Computation` is a source (`TSwiftOrderedSourceComputation`), the output messages are not stored in {{product-name}} — only the metadata required for deterministic operation is saved. For more details about computation types, see the [Computations]({{ flow-docs-root }}/{{ lang }}/concepts/computation{{ docs-revision-query }}) section.

### TWordCountFunction

`TWordCountFunction` is a process function (`IProcessFunction`) that is executed by `TProcessFunctionComputation` (the transform adapter). It uses `TSimpleExternalStateManager` to work with the [state]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state). For each input word, it:

- Gets the current counter value from the external state table.
- Increases the counter by 1.
- Writes the updated value back.

For more details about working with state, see the [State]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}) section.
