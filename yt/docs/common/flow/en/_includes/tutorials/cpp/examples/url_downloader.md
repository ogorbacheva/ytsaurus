...
The [pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) shows how you run external calls (similar to HTTP requests) from [computations]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation). You control the download speed (throttling), shard by host, and manage the [state]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state).
...
4. You set a [timer]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer) to periodically check results via `GetNextHostCheck`.
...
During [partition]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#partition) rebalancing, URLs that don’t fit within the `PersistLimit` will be lost. Choose the limit value considering the acceptable loss.
