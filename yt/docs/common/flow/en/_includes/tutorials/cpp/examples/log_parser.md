...
This example shows [`TTransformOrderedSourceComputation`]({{ flow-docs-root }}/{{ lang }}/concepts/computation{{ docs-revision-query }}#ttransformorderedsourcecomputation) (for details, see [Computation (C++)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/computation{{ docs-revision-query }}#ttransformorderedsourcecomputation)): the [pipeline]({{source-root}}/yt/yt/flow/examples/cpp/log_parser) reads log lines from a queue and parses them right when reading the source, without an intermediate passthrough computation and a `TTransformComputation`. The parsing logic itself is written as a [process function]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/process-functions{{ docs-revision-query }}) and runs under the built-in adapter. The example also shows reading from a `Source` and maintaining your own durable state that survives restarts (see [State](#state)).
...
The user logic is written as a [process function]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/process-functions{{ docs-revision-query }}): a subclass of `IProcessFunction` that doesn’t depend on the `Computation` object, so you can cover it with unit tests without a cluster ([unittest]({{source-root}}/yt/yt/flow/examples/cpp/log_parser/unittest/log_parser_process_function_ut.cpp)). It’s executed by the built-in `TProcessFunctionTransformOrderedSourceComputation` adapter, which also sets the mode to ordered source (see the [list of adapters]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/process-functions{{ docs-revision-query }}#how-it-works)).
...
You can still inherit from the class: that’s how the [Proto parser]({{ flow-docs-root }}/{{ lang }}/tutorials/cpp/examples/proto_parser{{ docs-revision-query }}) example is written, on top of the `TProtoTransformOrderedSourceComputation<TProto>` helper over `TTransformOrderedSourceComputation`. The adapter’s spec validator is the same as the base class’s: a non-empty `group_by_schema`, timers, key-visitor streams, and `external_state_managers` are rejected in either variant (for the full [list of limitations]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/computation{{ docs-revision-query }}#ttransformorderedsourcecomputation)).

### The parser computation spec

Two fields of the `parser` computation spec link the function to the adapter (see [Registration]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/process-functions{{ docs-revision-query }}#registration)):
...
The framework synchronizes this state in the same epoch transaction as the advance of the source offset (see [Computation]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/computation{{ docs-revision-query }}#ttransformorderedsourcecomputation)); the function itself does nothing for this. Therefore, user state is exactly-once correct and doesn’t have to be idempotent under reprocessing: an ordinary, “naive” counter of processed records would be just as correct here. The example keeps a running maximum severity simply because it’s a natural aggregate for such a pipeline, not because it’s in any way safer than a counter.
...
- [Getting started (C++)]({{ flow-docs-root }}/{{ lang }}/tutorials/cpp/quick-start{{ docs-revision-query }})
- [Process function (C++)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/process-functions{{ docs-revision-query }})
- [Computation (C++)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/computation{{ docs-revision-query }}#ttransformorderedsourcecomputation)
- [Computation (concept)]({{ flow-docs-root }}/{{ lang }}/concepts/computation{{ docs-revision-query }}#ttransformorderedsourcecomputation)
