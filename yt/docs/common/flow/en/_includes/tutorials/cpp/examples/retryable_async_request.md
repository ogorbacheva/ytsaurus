...
The [pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) is similar to [Async Request]({{ flow-docs-root }}/{{ lang }}/tutorials/cpp/examples/async_request{{ docs-revision-query }}), but it adds retry logic using [timers]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer) and the internal `YsonState`.
...
- Store the internal [state]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state) to track the number of failed attempts.
...
It’s fully similar to `TStateKeeper` from the [Async Request]({{ flow-docs-root }}/{{ lang }}/tutorials/cpp/examples/async_request{{ docs-revision-query }}) example: it accepts incoming events and responses and stores the accumulated result in the external state.
...
In the [spec]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec) for `TRequestProcessor`, you must register the `timers` section to support retries.
