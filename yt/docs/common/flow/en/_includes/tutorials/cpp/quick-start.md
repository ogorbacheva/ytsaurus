...
In this section, you’ll learn step by step how to implement your first [pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) in C++ using Flow. As an example, we’ll walk through a word count task: reading text messages from a queue, splitting them into words, and counting the occurrences of each word.

## Prerequisites

- Check out the [repository]({{source-root}}).
- Set up `ya make` (the build system).
- Familiarize yourself with the [basic concepts]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}) of Flow.
...
For more details on message conversion, see the [Computation (C++)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/computation{{ docs-revision-query }}) section, “TYsonMessage”.

### 2. Define the state {#define-state}

If a [computation]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) works with a [state]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state), define a class that inherits from `TStateBase`:
...
For more details on working with states, see [Working with states (C++)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/state{{ docs-revision-query }}).

### 3. Implement the [Source]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#source) Computation {#implement-source}
...
Note that `TSwiftOrderedSourceComputation` doesn’t materialize the messages themselves in YT; it only stores metadata for recovery. For more details, see [Computation (C++)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/computation{{ docs-revision-query }}#tswiftorderedsourcecomputation).
...
For more details on the spec format, see [Spec & DynamicSpec]({{ flow-docs-root }}/{{ lang }}/concepts/spec{{ docs-revision-query }}).
...
- A pipeline object with [Flow inner tables]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#inner-pipeline-tables).
...
For detailed information about releases and pipeline management, read the [Releases and pipeline management]({{ flow-docs-root }}/{{ lang }}/how-to-guides/deployment/basic-rules{{ docs-revision-query }}) section.
...
- [Spec & DynamicSpec]({{ flow-docs-root }}/{{ lang }}/concepts/spec{{ docs-revision-query }})
