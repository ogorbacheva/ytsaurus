...
The [pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) demonstrates a pattern for asynchronous external requests using `TSwiftMapComputation`. You send events to the input, convert them into requests, process them with a deterministic processor, and accumulate the results in the [state]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state).
