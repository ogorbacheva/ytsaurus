...
- The [state]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}) is updated atomically along with message processing.
...
For each [job]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#job) you create, the controller sets up a special transaction — `Lease`. The job commits any data using this transaction as `PrerequisiteTransaction`. Lease has a barrier property: after Lease is canceled, any attempt to commit a transaction that references it is guaranteed to fail. This lets the controller reliably stop arbitrary jobs and immediately start new ones on the same [partitions]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#partition).

#### 2. Input message deduplication {#input-dedup}

- **Internal streams** (`input`): deduplication by `message_id` using the internal `input_messages` table or its compact variant `compact_input_messages`. The compact variant is selected automatically when `experimental_enable_non_uint_key` is disabled (partitioning is done exclusively by a `uint64` hash column); you can override this behavior with the `use_compact_input_messages` parameter. Older `message_id` values are removed from this table using [SystemWatermark]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timestamps-and-watermarks).
- **Sources** (`source`): deduplication by offsets — consumer offsets advance only after a successful [epoch]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#epoch) commit.
- **[Timers]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer)**: the result of timer processing is committed in the same transaction as the timer’s deletion.

#### 3. Output data atomicity {#output-atomicity}

The way output messages are delivered depends on the [sink]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#sink) type:
...
- **For [Swift]({{ flow-docs-root }}/{{ lang }}/concepts/swift{{ docs-revision-query }}) computations** (`TSwiftMapComputation`, `TSwiftOrderedSourceComputation`) determinism is strictly required. In Swift computations, computation results aren’t materialized between epochs and can be recalculated on restart. If different attempts to compute the same message yield different results, you might mix results from different attempts, or lose or duplicate data.
...
Updates to Flow’s [internal state]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}) (YSON-state, External State via `IExternalStateManager`) are atomic and safe. However, if a computation modifies data in an external system (a database, cache), those changes aren’t part of the epoch transaction and might be duplicated.

### What Flow guarantees automatically {#automatic-guarantees}

- Internal streams between [computations]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#computation) — exactly-once.
- [State]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}) updates — atomic with message processing.
- Timer processing and deletion — in the same transaction.
- Advancing [source]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#source) offsets — only after a successful epoch commit.
...
Flow **doesn’t guarantee** global message processing order. However, there are guarantees for derived messages with matching [keys]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#key) within a single [lineage]({{ flow-docs-root }}/{{ lang }}/concepts/lineage{{ docs-revision-query }}) chain.

For details, see the [Message processing order]({{ flow-docs-root }}/{{ lang }}/concepts/ordering{{ docs-revision-query }}) section.

## How connectors affect guarantees {#connectors}

Each [connector]({{ flow-docs-root }}/{{ lang }}/concepts/connectors/overview{{ docs-revision-query }}) has its own set of guarantees, which depend on the connection type (source/sink) and the sink variant (sync/async/at-least-once).
...
For more details, see [Queue]({{ flow-docs-root }}/{{ lang }}/reference/connectors/queue{{ docs-revision-query }}).
...
For more details, see [Static Table]({{ flow-docs-root }}/{{ lang }}/reference/connectors/static-table{{ docs-revision-query }}).
...
- No sink. The sorted dynamic table used as a service log source is often populated by the pipeline itself via [external state]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}#external-state) operations. For example, a `TTransformComputation` type computation updates table records at each epoch.

For more details, see [Service log]({{ flow-docs-root }}/{{ lang }}/reference/connectors/servicelog{{ docs-revision-query }}).
...
An extension over the [Queue]({{ flow-docs-root }}/{{ lang }}/reference/connectors/queue{{ docs-revision-query }}) connector that adds batching and compression. Guarantees are identical to the base Queue connector:
...
- The [pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) survives the failure of individual machines and data centers.
...
- [Automatic partition balancing]({{ flow-docs-root }}/{{ lang }}/concepts/overview{{ docs-revision-query }}) redistributes load when the cluster topology changes.

## See also

- [Flow overview]({{ flow-docs-root }}/{{ lang }}/concepts/overview{{ docs-revision-query }})
