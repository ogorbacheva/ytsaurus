...
For more details, see the [spec]({{ flow-docs-root }}/{{ lang }}/concepts/spec{{ docs-revision-query }}#watermark-strategy).
...
This module for source [computations]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) tries to provide a lower-bound estimate for the `EventTimestamp` of future events across all `output` streams. The algorithm works as follows:

1. You estimate `EventWatermark` for each [partition]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#partition) of the input queue independently.
...
4. Unavailable partitions, as well as partitions that aren’t receiving writes, can be ignored to a limited extent. You can find the settings for these heuristics in the [spec description]({{ flow-docs-root }}/{{ lang }}/concepts/spec{{ docs-revision-query }}#watermark-generator).
...
We also plan to create a protocol for passing `Watermark` information between different [pipelines]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline).
...
Timers are closely tied to watermarks: by default, `EventWatermark` determines when a timer fires. For more details on how timers work, their configuration, and the API, see the [Timers]({{ flow-docs-root }}/{{ lang }}/concepts/timers{{ docs-revision-query }}) section.

## Configuration {#configuration}

You configure `WatermarkStrategy` (`EventTimestampAssigner`, `WatermarkGenerator`, `WatermarkAlignment`) in the [Spec]({{ flow-docs-root }}/{{ lang }}/concepts/spec{{ docs-revision-query }}#watermark-strategy) section.
