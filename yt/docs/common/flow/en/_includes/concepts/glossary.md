...
From the {{product-name}} perspective, a pipeline is represented as a Cypress object of type `pipeline`. For details about the object structure, creation methods, and management, see the [Pipeline Object]({{ flow-docs-root }}/{{ lang }}/concepts/pipeline-object{{ docs-revision-query }}) section.
...
The order of messages in a stream is not guaranteed in general. However, for derived messages with matching [keys](#key) along the entire [lineage](#lineage) chain, there are guarantees of relative order. For more details, see the [Message Processing Order]({{ flow-docs-root }}/{{ lang }}/concepts/ordering{{ docs-revision-query }}) section.
...
For more details, see the [Computation]({{ flow-docs-root }}/{{ lang }}/concepts/computation{{ docs-revision-query }}) section.
...
For more details, see [Computation]({{ flow-docs-root }}/{{ lang }}/concepts/computation{{ docs-revision-query }}#passthrough).
...
For more details, see the [Swift]({{ flow-docs-root }}/{{ lang }}/concepts/swift{{ docs-revision-query }}) section.
...
A set of [message](#message) field values defined via `group_by_schema` in the [spec]({{ flow-docs-root }}/{{ lang }}/concepts/spec{{ docs-revision-query }}) of a [computation](#computation). All messages with the same key are routed to a single [partition](#partition) — each partition is assigned a key range `[LowerKey; UpperKey)`. The key is the isolation unit for [state](#state), [timers](#timer), and [ordering guarantees]({{ flow-docs-root }}/{{ lang }}/concepts/ordering{{ docs-revision-query }}#ordering-guarantees).
...
The collection of input [messages](#message) and [timers](#timer) from which specific output results of a [computation](#computation) were derived. The framework uses lineage to compute system output messages and to ensure [ordering guarantees]({{ flow-docs-root }}/{{ lang }}/concepts/ordering{{ docs-revision-query }}#ordering-guarantees) for derived messages.

For more details, see the [Lineage]({{ flow-docs-root }}/{{ lang }}/concepts/lineage{{ docs-revision-query }}) section.
...
- `TriggerTimestamp` — the trigger moment. When the [EventWatermark]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timestamps-and-watermarks) (or another configured watermark) exceeds this value, the timer is passed to the computation for processing.
...
For more details, see the [Timers]({{ flow-docs-root }}/{{ lang }}/concepts/timers{{ docs-revision-query }}) section.
...
For more details, see the [Stateful Processing]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}) section.
...
A component that connects the pipeline to an external system by reading messages from the connector’s [source](#source) and/or writing messages to the connector’s [sink](#sink). A connector can include both a [source](#source) and a [sink](#sink) (for example, in the case of [QYT]({{ flow-docs-root }}/{{ lang }}/reference/connectors/queue{{ docs-revision-query }})) or only a [source](#source) (for example, in the case of [static table]({{ flow-docs-root }}/{{ lang }}/reference/connectors/static-table{{ docs-revision-query }})).
...
For more details about connectors, see the [Connectors Documentation]({{ flow-docs-root }}/{{ lang }}/concepts/connectors/overview{{ docs-revision-query }}).
...
A separate process launched on the same host as the [worker](#worker) and executing user code in [Python]({{ flow-docs-root }}/{{ lang }}/tutorials/python/quick-start{{ docs-revision-query }}), [Java or Kotlin]({{ flow-docs-root }}/{{ lang }}/tutorials/java/quick-start{{ docs-revision-query }}). It interacts with the [worker](#worker) via gRPC. It lets you implement [computations](#computation) in languages other than C++.

For more details, see the [Companion]({{ flow-docs-root }}/{{ lang }}/concepts/companion{{ docs-revision-query }}) section.
...
- [EventTimestamp]({{ flow-docs-root }}/{{ lang }}/concepts/watermarks{{ docs-revision-query }}#eventtimestamp) — the true time of the event associated with the message.
- [SystemTimestamp]({{ flow-docs-root }}/{{ lang }}/concepts/watermarks{{ docs-revision-query }}#systemtimestamp) — the time when the specific message was created. For messages generated inside Flow, `SystemTimestamp` takes the “{{product-name}} time”. For `source` streams, it’s the time when the message appeared in the remote system.
- [AlignmentTimestamp]({{ flow-docs-root }}/{{ lang }}/concepts/ordering{{ docs-revision-query }}#alignment-timestamp) — a timestamp used to align the processing progress of partitions. It’s computed automatically.
- [StabilizedEventTimestamp]({{ flow-docs-root }}/{{ lang }}/concepts/ordering{{ docs-revision-query }}#stabilized-event-timestamp) — a timestamp computed based on `AlignmentTimestamp`; it’s used as a non-decreasing approximation of `EventTimestamp` across the message stream (with a matching set of [keys](#key) in the [lineage](#lineage)).
...
[Watermark]({{ flow-docs-root }}/{{ lang }}/concepts/watermarks{{ docs-revision-query }}) is a timestamp indicating that the system no longer expects older events to arrive. Each stream in the system maintains current `SystemWatermark` and `EventWatermark` values.
...
By default, Flow ensures exactly-once semantics for event processing. The mechanism is based on barrier Lease transactions, deduplication of input messages by `message_id` (for internal streams) and by offsets (for sources), and the atomicity of output data via [epoch]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#epoch) transactions. If needed, you can relax the semantics to at-least-once or at-most-once.

For more details, see the [Processing Guarantees]({{ flow-docs-root }}/{{ lang }}/concepts/guarantees{{ docs-revision-query }}) section.
...
Update the [static](*static_spec_upd) and/or dynamic [spec]({{ flow-docs-root }}/{{ lang }}/concepts/spec{{ docs-revision-query }}) of the pipeline.
...
- `completed` — a final state: all pipeline sources were [finite]({{ flow-docs-root }}/{{ lang }}/how-to-guides/python/testing{{ docs-revision-query }}) (`finite = true`), and all messages from them have been processed. You can’t exit this state — you’ll need to recreate the pipeline. This state most often appears in [integration tests]({{ flow-docs-root }}/{{ lang }}/how-to-guides/python/testing{{ docs-revision-query }}), and it can also occur in production pipelines if the deployment steps are executed in the wrong order (for example, if a source was mistakenly marked as finite) or if an incorrect spec is set.
...
The full list of internal tables and their purposes are provided in the section [Pipeline Object → Internal tables]({{ flow-docs-root }}/{{ lang }}/concepts/pipeline-object{{ docs-revision-query }}#internal_tables).
