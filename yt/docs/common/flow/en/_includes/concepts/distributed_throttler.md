...
Use the distributed throttler to globally limit the rate across the entire [pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline). The Controller holds a [token bucket](https://en.wikipedia.org/wiki/Token_bucket) for each named throttler; [Jobs]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#job) request a quota from the Controller before processing messages or performing any custom user action.
...
- Evenly distribute bandwidth across [partitions]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#partition) of a single [`Computation`]({{ flow-docs-root }}/{{ lang }}/concepts/computation{{ docs-revision-query }}).
- Slow down reading from a source.

## Priority by lag {#priority}

Each Job attaches its timestamp to the quota request — the same timestamp that determines lag in the input buffer. The server takes the minimum value across all `stabilized_event_timestamp + stream_delay` from [input streams]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream) and `read_alignment_timestamp` from [source streams]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#source). The server ranks requests by this timestamp in ascending order: it grants quota first to those that are lagging more. If there’s a load imbalance across [partitions]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#partition) of a single [`Computation`]({{ flow-docs-root }}/{{ lang }}/concepts/computation{{ docs-revision-query }}), the lagging ones automatically get more quota, while the leading ones slow down.
...
- [Spec, DynamicSpec and Config]({{ flow-docs-root }}/{{ lang }}/concepts/spec{{ docs-revision-query }})
