...
Related sections: [Pipeline in the glossary]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline), [Spec, DynamicSpec and Config]({{ flow-docs-root }}/{{ lang }}/concepts/spec{{ docs-revision-query }}), [Stateful processing]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}), [Pipeline internal tables]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#inner-pipeline-tables).
...
| Table name | Purpose |
| --- | --- |
| `input_messages` | Index of input messages for [transform computations]({{ flow-docs-root }}/{{ lang }}/concepts/computation{{ docs-revision-query }}#ttransformcomputation), used for deduplication. |
| `compact_input_messages` | Compact index of input messages. It’s used by default for all computations, except when `experimental_enable_non_uint_key` is enabled; the behavior is overridden by the `use_compact_input_messages` parameter in [TComputationSpec]({{ flow-docs-root }}/{{ lang }}/reference/configuration/all_yson_structs{{ docs-revision-query }}#NYT_NFlow_TComputationSpec). |
...
| `states` | User and service [states]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}) stored by [key]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#key). |
| `partition_states` | States stored by partition. |
| `timers` | [Timers]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer) of the user code. |
| `controller_logs` | Event logs of the [Controller]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#controller) in the PublicFlowController category. |
...
When the schema of the [internal tables](#internal_tables) changes in a new Flow version, the format upgrade is performed via a separate migration — see [Pipeline internal tables]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#inner-pipeline-tables) and [Basic rollout rules]({{ flow-docs-root }}/{{ lang }}/how-to-guides/deployment/basic-rules{{ docs-revision-query }}).

If the pipeline uses [External State]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}) (user tables outside the node), creating them and evolving their schemas is your responsibility. {% if audience == "internal" %}In the Yandex infrastructure, [YtSync]({{yt-sync-docs}}/) is used for this.{% else %}You perform the operations using standard commands like `yt create table ... --attributes '{dynamic=true; schema=...}'` and `yt mount-table` — see examples in the [Create command]({{ core-docs-root }}/{{ lang }}/how-to-guides/storage/cypress-example{{ docs-revision-query }}#create) section.{% endif %}

## See also { #see_also }

- [Glossary: Pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline)
- [Pipeline internal tables]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#inner-pipeline-tables)
- [Basic pipeline rollout rules]({{ flow-docs-root }}/{{ lang }}/how-to-guides/deployment/basic-rules{{ docs-revision-query }})
{% if audience != "internal" %}
