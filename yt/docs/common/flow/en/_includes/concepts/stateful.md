...
You access state inside a [computation]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) through three components:

- **State** — user data tied to a [key]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#key) and stored in a {{product-name}} dynamic table.
...
The accessor is valid only within the current [epoch]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#epoch). Don't store it in computation fields or reuse it across epochs — you must get the state through the client again in each epoch.
...
For correctness and performance, we strongly recommend that you use, as the [group_by_schema]({{ flow-docs-root }}/{{ lang }}/concepts/spec{{ docs-revision-query }}#computation) for a [computation]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation), the schema of the first key columns of the dynamic table with states (strictly a prefix of the key columns). This ensures that:

- Only one [partition]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#partition) handles events for a single key (correctness).
...
- **C++**: the client `TMutableStateKeyClient<TState>` (read-write) or `TJoinedStateKeyClient<TState>` (read-only) returns the accessor `TStateAccessor<TState>` / `TConstStateAccessor<TState>`; the same key client works with both internal and external states. [Learn more →]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/state{{ docs-revision-query }})
- **Java**: YsonStateAccessor, ProtoStateAccessor, ExternalStateAccessor. [Learn more →]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/state{{ docs-revision-query }})
- **Python**: ctx.state(), ctx.external_state(), ctx.proto_state(). [Learn more →]({{ flow-docs-root }}/{{ lang }}/how-to-guides/python/state{{ docs-revision-query }})
