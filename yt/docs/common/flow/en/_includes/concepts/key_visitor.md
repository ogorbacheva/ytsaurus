...
Stateful computations accumulate an [internal state]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state) per key while processing incoming messages. You often need to periodically scan this state—even without an incoming message for a specific key. Common tasks include:
...
Standard timers (see [Timers]({{ flow-docs-root }}/{{ lang }}/concepts/timers{{ docs-revision-query }})) aren’t suitable here. You must register them for each key in advance, but the set of keys can grow without an explicit “new key appeared” event. For example, keys may arrive only via state stores, not through a message stream.
...
| `TSwiftMapComputation` | ✓ (only for state handling: emitting to output from `DoProcessVisit` is forbidden, see [Swift]({{ flow-docs-root }}/{{ lang }}/concepts/swift{{ docs-revision-query }}#swift-map)) |
...
This is the basis of the reconciliation pattern: a periodic scan aligns the computation’s own state with the external table—keys present in the table are updated, and keys missing from it are deleted. Requirements for the table, behavior when the source is unavailable, and a code example are in the [TStaticTableKeyVisitorJoiner]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/state{{ docs-revision-query }}#static-table-key-visitor-joiner) section.
