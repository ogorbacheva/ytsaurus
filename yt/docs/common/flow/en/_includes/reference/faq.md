...
When you roll out a new binary with a stop but without [drain]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#start-stop-pause-pipeline):
...
- You might get duplicate events or lose events if the logic for generating output messages in [Swift]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#swift) computations changes.
