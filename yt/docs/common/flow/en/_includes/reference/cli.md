...
Use the standard `yt` CLI tool with the `flow` mode to work with Flow [Pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline). This section lists the main commands. You can view all available options for each command by using the `--help` command.
