...
This page describes YQL constructs specific to working with Flow. The general YQL syntax is described in the [YQL documentation]({{ yql-docs-root }}/{{ lang }}/yql/{{ docs-revision-query }}).

{% endnote %}

## Row-wise stream transformation (map) {#map}

The main construct is `INSERT INTO ... SELECT ... FROM`. It reads a [stream]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) from an ordered dynamic table (a {{product-name}} queue), applies a transformation, and writes the result to another queue.
...
- [Quick start (YQL)]({{ flow-docs-root }}/{{ lang }}/tutorials/yql/quick-start{{ docs-revision-query }})