...
Connector to [static tables in {{product-name}}]({{ core-docs-root }}/{{ lang }}/concepts/storage/static-tables{{ docs-revision-query }}).

The connector code is [here]({{source-root}}/yt/yt/flow/library/cpp/connectors/static_table).

Static tables are a special type of source. They don’t have [partitions]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#partition) initially, their rows don’t have write timestamps, and the tables themselves are usually immutable with no incremental writes. At the same time, you often need to read an endless sequence of tables. Also, “writes” to this source come in very large blocks, so you must read from it with rate limiting to avoid taking resources away from more important [sources]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#source) in the [pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline).

Because of this, the main challenge with this source lies in the [controller]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#controller), which needs to figure out which tables to read, which timestamps ([SystemTimestamp]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timestamps-and-watermarks), [EventTimestamp]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timestamps-and-watermarks)) to choose for them, what to do if a table unexpectedly disappears, and so on.
...
3. The enricher-computation, which sits on the event stream, reads values from the same state by the event’s key using a read-only [joiner]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}#external-state-joiner) and emits the enriched message to the sink.
...
Each new version of a static table is a **full reload** of the entire reference dataset into state. This scheme doesn’t automatically clean up deleted rows: if a row is missing in the new version, you must explicitly remove it from state — for example, by tagging rows with a version and periodically deleting outdated ones using the [key visit]({{ flow-docs-root }}/{{ lang }}/concepts/key_visitor{{ docs-revision-query }}) mechanism.
...
In the pipeline, the `path` of the external-state connector points to the symlink, not to a specific version. The lookup is performed by a read-only [joiner]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}#external-state-joiner) (`TSimpleExternalStateJoiner`); by default, the joiner re-reads the value from YT on each lookup, so the symlink switch is immediately visible to the pipeline without a restart. For more details on preparing the dynamic table and its schema, see [sorted-dynamic-table.md]({{ flow-docs-root }}/{{ lang }}/reference/connectors/sorted-dynamic-table{{ docs-revision-query }}).
...
{% if audience == "internal" %}One such format in Arcadia is **vinyl**. An example of building and using it is in [`ads/core/library/cpp/vinyl`]({{source-root}}/ads/core/library/cpp/vinyl): a separate batch process builds a vinyl file on YT, deploys it to the required location, and the pipeline gets the latest file version via the [Resource]({{ flow-docs-root }}/{{ lang }}/concepts/spec{{ docs-revision-query }}) mechanism (per worker) and reads from it on each message.{% endif %}
...
- [List of connectors]({{ flow-docs-root }}/{{ lang }}/concepts/connectors/overview{{ docs-revision-query }})
- [Sorted Dynamic Table]({{ flow-docs-root }}/{{ lang }}/reference/connectors/sorted-dynamic-table{{ docs-revision-query }})
- [Stateful computations]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }})