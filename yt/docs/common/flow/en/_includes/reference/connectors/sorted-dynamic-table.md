...
Use this connector to write data to [sorted dynamic tables in {{product-name}}]({{ core-docs-root }}/{{ lang }}/concepts/dynamic-tables/sorted-dynamic-tables{{ docs-revision-query }}).

{% note info "Attention" %}

In most cases, use [StateManager]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}) to work with sorted dynamic tables, not this connector.
