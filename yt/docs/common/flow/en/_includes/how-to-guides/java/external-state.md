...
For more details about Internal State, see [Internal State]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/internal-state{{ docs-revision-query }}).
...
You get the `ExternalStateAccessor` for a specific [key]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#key) via `RuntimeContext` (which inherits `StatefulContext`):
...
- `message` / `timer` — the message or timer for whose [key]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#key) you need to get the state.
