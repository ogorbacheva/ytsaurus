...
For more details about default JVM options, profiling, and configuration, see the section [Java companion profiling]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/profiling{{ docs-revision-query }}).
...
|| `YT_FLOW_COMPANION_JFR_OPTS` | Custom JFR options separated by a space, replacing the default values. Ignored if `YT_FLOW_COMPANION_JFR_DISABLED` is set. | [see profiling]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/profiling{{ docs-revision-query }}#jfr-defaults) ||
|| `YT_FLOW_COMPANION_GC_LOG_DISABLED` | If set to `1`, disables GC logging. | GC logging is enabled ||
|| `YT_FLOW_COMPANION_GC_LOG_OPTS` | Custom GC logging options separated by a space, replacing the default values. Ignored if `YT_FLOW_COMPANION_GC_LOG_DISABLED` is set. | [see profiling]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/profiling{{ docs-revision-query }}#gc-defaults) ||
