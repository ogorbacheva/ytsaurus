...
The `distribute` flag is a per-message flag that you set when you add an output message to [SourceComputation]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/computation{{ docs-revision-query }}#sourcecomputation). It controls whether the message is published further along the processing graph.
