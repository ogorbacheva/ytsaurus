...
Internal State is a mechanism for working with internal state stored in Flow’s internal tables. Unlike [External State]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/external-state{{ docs-revision-query }}), you don’t need to create the tables yourself — Flow manages them automatically.

Learn more about `StateAccessor` and working with state: [State Accessor]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/state-accessor{{ docs-revision-query }}).

General information about stateful processing is in the [Stateful processing]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}) section.
...
|----------|--------|-------------|
| [YsonStateAccessor]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/internal-state{{ docs-revision-query }}#yson-state-accessor) | YSON | Serialization via `@YTreeObject` annotations |
| [ProtoStateAccessor]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/internal-state{{ docs-revision-query }}#proto-state-accessor) | Protobuf | Serialization via Protobuf |
| [DefaultStateAccessor]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/internal-state{{ docs-revision-query }}#default-state-accessor) | Custom | User-defined serializer/deserializer |
| [RawStateAccessor]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/internal-state{{ docs-revision-query }}#raw-state-accessor) | `byte[]` | No serialization (raw bytes) |
| [NoOpStateAccessor]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/internal-state{{ docs-revision-query }}#noop-state-accessor) | — | Stores only the fact that the state exists |
...
`NoOpStateAccessor` stores only the fact that a state exists for a [key]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#key), with no payload. Use it to track already processed keys (for deduplication).
...
You must declare the names of internal states in the `internal_states` section of the [computation]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) parameters in the static spec:
