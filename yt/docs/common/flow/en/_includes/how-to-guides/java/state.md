...
This page describes details for Java and Kotlin when working with states. General concepts are described in the [Stateful computations]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}) section.
...
You also have access to **ProtoStateAccessor**, **DefaultStateAccessor**, **RawStateAccessor**, and **NoOpStateAccessor**. For more details on all types, see the [Internal State]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/internal-state{{ docs-revision-query }}) section.
...
Parameters:
- `"state-name"` — the state name; it must match the name registered in the [spec]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec) of the [computation]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) (`parameters/internal_states`).
- `message` — the current message from which you extract the [grouping key]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#key).
...
When you process timers, the state is available via the `timer` object, which contains the [grouping key]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#key):
...
For more details on `group_by_schema` and its impact on state handling, see the [Stateful computations]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}) section.
...
The `external_state_manager_class_name` field specifies the registered external state manager class. For a typical scenario, this is `"NYT::NFlow::TSimpleExternalStateManager"`. For more details on available managers, see the [C++ documentation]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/state{{ docs-revision-query }}#external-state).
...
- [Getting started (Java)]({{ flow-docs-root }}/{{ lang }}/tutorials/java/quick-start{{ docs-revision-query }})
