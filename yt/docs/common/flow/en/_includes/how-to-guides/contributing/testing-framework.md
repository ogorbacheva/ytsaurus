...
This page describes how to set up a workstation and configure low-level parameters for contributors who are improving Flow itself and running its integration test suite. If you're a pipeline author testing your code, see [Testing (C++)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/testing{{ docs-revision-query }}), [Testing (Java)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/testing{{ docs-revision-query }}), and [Testing (Python)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/python/testing{{ docs-revision-query }}) — these pages include the launch parameters and techniques you need for pipeline tests.
...
If you need the UI of the local {{product-name}} for debugging, you’ll need network access — see the [relevant section]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/testing{{ docs-revision-query }}#debug-ui-yt) in the documentation for pipeline authors.

## Flow test parameters {#test-param}

In addition to the general parameters (see [Testing (C++)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/testing{{ docs-revision-query }}#debug-test-framework)), these parameters are useful when you’re working on Flow itself — they affect the framework’s internal behavior:
