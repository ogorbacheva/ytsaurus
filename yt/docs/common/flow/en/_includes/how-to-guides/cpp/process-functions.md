...
The classic way to write a [Computation]({{ flow-docs-root }}/{{ lang }}/concepts/computation{{ docs-revision-query }}) in C++ is to inherit from `TTransformComputation` (or `TSwiftMapComputation` / `TSwiftOrderedSourceComputation`) and override the `DoProcessMessage`, `DoProcessTimer`, `DoProcessVisit`, and `DoInit` methods. In this approach, your custom logic becomes tightly coupled with the `Computation` object: it inherits dozens of protected methods and can only be constructed from a fully built `TComputationContext` (which includes {{product-name}} clients, stores, the state manager, and so on). As a result, it’s nearly impossible to test this logic in isolation with unit tests.
...
The process function isn’t run directly. Instead, a built-in `Computation` adapter executes it. You specify this adapter in the spec via the `computation_class_name` field (see [Registration](#registration)). The adapter gives the function the same environment as a regular `Computation`: the same spec, the same stores and states, and the same epoch-processing logic. That’s why a pipeline with a process function provides the same [processing guarantees]({{ flow-docs-root }}/{{ lang }}/concepts/guarantees{{ docs-revision-query }}) (including exactly-once) as a manually written `Computation`.
...
- `IProcessFunction` — element-wise processing (the most common case). The worker calls a method for each entity in the epoch (similar to `TTransformComputation::DoProcessMessage`, etc.). Override the methods you need; all are no-op by default:
    ...
    - `ProcessTimer(timer, output, context)` — handles a single [timer]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer).
...
The `distribute` flag in `output->AddMessage(message, distribute)` mirrors the `OutputCollector` semantics in `Computation`: for source, a message with `distribute = false` isn’t published but is still considered when evaluating the [watermark]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timestamps-and-watermarks); for other `Computation` types, a message with `distribute = false` is simply discarded.
...
| Method | Description |
| --- | --- |
| `GetWatermark(streamId)` / `GetInputEventWatermark()` | Event-time [watermarks]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timestamps-and-watermarks) |
...
| `MakeTimer(key, streamId, trigger, event)` | Creates a [timer]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer) |
...
For more details on state types, see the [Working with states]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/state{{ docs-revision-query }}) section.
...
For more details on the general approach to testing C++ computations, see the [Testing]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/testing{{ docs-revision-query }}) section.
...
- [Testing (C++)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/testing{{ docs-revision-query }})
