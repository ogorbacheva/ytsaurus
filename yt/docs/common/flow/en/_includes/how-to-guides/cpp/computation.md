...
Use this page to learn about the specifics of implementing Computation in C++. For a language-agnostic description of the concept, see the [Computation]({{ flow-docs-root }}/{{ lang }}/concepts/computation{{ docs-revision-query }}) section.
...
Classes that include `Swift` in their name implement the [Swift]({{ flow-docs-root }}/{{ lang }}/concepts/swift{{ docs-revision-query }}) principle. See the [Swift]({{ flow-docs-root }}/{{ lang }}/concepts/swift{{ docs-revision-query }}) section for more details.
...
- You can use the `DoSync` method to manually save data to a `YT` transaction. However, it’s better to use [Sink]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#sink) or [ExternalState]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/state{{ docs-revision-query }}#external-state). Directly working with `DoSync` in `SwiftComputation` can lead to undesirable behavior. In `Transform`, the method is safe but less convenient.
- Each `Computation` has a family of `DoProcess` methods. They accept either `IInputContextPtr input` with `GetMessages` and `GetTimers` methods, or a specific message or [timer]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer). The `IOutputCollectorPtr output` object is for collecting output messages and timers — see [OutputCollector](#output-collector) for more details.
...
- All code execution within Computations is strictly single-threaded and runs within `GetContext()->SerializedInvoker`. You achieve multithreading by increasing the number of [partitions]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#partition). If you still need to run some code multithreaded, use `GetContext()->PoolInvoker`, but make sure to wait for the execution results within the corresponding method.
...
| `output->AddTimer(timer)` | Add a [timer]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer) |
| `output->SetParents(parentIds)` | Set the parent ID for tracking [lineage]({{ flow-docs-root }}/{{ lang }}/concepts/lineage{{ docs-revision-query }}). Returns a new `IOutputCollectorPtr` with the attached lineage context |
...
This is a child of `TTransformComputation`. It implements a [passthrough computation]({{ flow-docs-root }}/{{ lang }}/concepts/computation{{ docs-revision-query }}#passthrough) and is mainly created to demonstrate capabilities.
...
* `input` streams;
* [timers]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer);
* [key-visitor streams]({{ flow-docs-root }}/{{ lang }}/concepts/key_visitor{{ docs-revision-query }});
...
- It uses `watermark_strategy/watermark_generator` to estimate [watermarks]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timestamps-and-watermarks) of input sources.
...
For more details about available connectors, see the [Connectors]({{ flow-docs-root }}/{{ lang }}/concepts/connectors/overview{{ docs-revision-query }}) section.

### How do batching and partitioning work? {#batching-partitions}

Each partition is processed strictly single-threaded. You achieve multithreading by increasing the number of partitions (`partition_count` in the spec). Batch methods `DoProcess(IInputContextPtr input, IOutputCollectorPtr output)` receive all messages and timers for the current [epoch]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#epoch), which lets you optimize processing.
...
Total load = records per partition × number of partitions × epoch frequency. For a [pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) with 1000 partitions and an epoch of 1 second, `TTransformComputation` will create ~2000–4000 records/s.
...
- [Working with states (C++)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/state{{ docs-revision-query }})
