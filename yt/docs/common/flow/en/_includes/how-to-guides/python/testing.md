...
This page describes **unit testing** of Python-[pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) components, as well as **integration testing** of the full pipeline via `FlowTestPythonBase`.

{% endnote %}

## General testing architecture {#architecture}

In production, the C++ [worker]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#worker) sends gRPC requests to the [companion]({{ flow-docs-root }}/{{ lang }}/concepts/companion{{ docs-revision-query }}), passing [messages]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#message), timers, [states]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state), and [watermarks]({{ flow-docs-root }}/{{ lang }}/concepts/watermarks{{ docs-revision-query }}). The companion calls `Computation.do_process()`, which delegates processing to `ProcessFunction`.
...
In addition to `PEERDIR` for `integration_test_base`, the integration test needs a cluster recipe, `DEPENDS` for the pipeline binary and `flow_server`, and `DATA` with the spec. The complete `ya.make` for the test from [WordCount]({{ flow-docs-root }}/{{ lang }}/tutorials/python/examples/wordcount{{ docs-revision-query }}):
...
- [Distribute flag (Python)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/python/distribute{{ docs-revision-query }})
- [Testing (Java)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/testing{{ docs-revision-query }})
- If you're extending Flow itself — [Pipeline testing framework]({{ flow-docs-root }}/{{ lang }}/how-to-guides/contributing/testing-framework{{ docs-revision-query }}).
