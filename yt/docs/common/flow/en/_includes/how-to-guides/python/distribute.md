...
The `distribute` flag is a per-message flag that you set when you add an output [message]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#message) to [SourceComputation]({{ flow-docs-root }}/{{ lang }}/how-to-guides/python/computation{{ docs-revision-query }}#sourcecomputation). It controls whether the message is published further along the processing graph.

The `distribute` flag ensures:

- Correct [watermark]({{ flow-docs-root }}/{{ lang }}/concepts/watermarks{{ docs-revision-query }}) evaluation: messages with `distribute=False` are still accounted for by the watermark generator (unlike filtering in `on_message`, which can break the watermark).
...
- [The distribute flag (Java)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/java/distribute{{ docs-revision-query }})
