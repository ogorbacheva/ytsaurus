...
Internal State is a mechanism for working with internal [state]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state) stored in Flow’s internal tables. Unlike [External State]({{ flow-docs-root }}/{{ lang }}/how-to-guides/python/external-state{{ docs-revision-query }}), you don’t need to create tables manually—Flow manages them automatically.

Learn more about `StateAccessor` and general principles for working with state: [StateAccessor]({{ flow-docs-root }}/{{ lang }}/how-to-guides/python/state-accessor{{ docs-revision-query }}).
...
The Python SDK provides three types of accessors for working with Internal State, which differ by serialization format. To work with external state, use a separate accessor: [ExternalStateAccessor]({{ flow-docs-root }}/{{ lang }}/how-to-guides/python/external-state{{ docs-revision-query }}).
...
Here, the state is tied to the [message]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#message) key (defined via `group_by_schema` in the [spec]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec)). Each unique word has its own independent counter.
...
- [Working with states (Python)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/python/state{{ docs-revision-query }}) — brief overview
...
- [WordCount example]({{ flow-docs-root }}/{{ lang }}/tutorials/python/examples/wordcount{{ docs-revision-query }})
