...
* [Launch in a Vanilla operation]({{ flow-docs-root }}/{{ lang }}/how-to-guides/deployment/launch-vanilla{{ docs-revision-query }}) — the controllers and workers start within a single {{product-name}} vanilla operation; no separate long-running deployment is needed. This is the simplest method.
...
When you launch a pipeline for the first time or update it, you often need to change the configuration of objects in {{product-name}} — for example, add new tables, delete old ones, or modify the schema of existing tables. You can see how to do this in the [wait_click_join example]({{ flow-docs-root }}/{{ lang }}/tutorials/cpp/examples/wait_click_join{{ docs-revision-query }}#yt-sync).{% if audience == "internal" %} The most convenient way to perform these operations is [YtSync]({{yt-sync-docs}}/); see its own documentation for details.{% endif %}

Updating the configuration of objects in {{product-name}}, especially running [migrations]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#migration), is a potentially risky operation. Therefore, it’s the Flow user’s responsibility to run it. The rules you need to follow are described below.
...
Regardless of how you manage objects in {{product-name}}, follow these rules when you deploy a [pipeline release]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#release-pipeline):
...
- Include a diff check in your release pipeline (in the procedure for deploying a new pipeline release){% if audience == "internal" %} (the [dump-diff scenario in YtSync]({{yt-sync-docs}}/getting_started#dostupnye-scenarii#dump-diff-scenario)){% endif %} between the current state of the objects (their existence, schema, and attributes) in {{product-name}} and the state required for the new pipeline version to work{% if audience == "internal" %} (this state is described in `YtSync`){% endif %}.
    ...
    When you deploy a new release, the schemas of Flow’s internal tables might change. Stop the pipeline and run the [migration]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#migration) before you update it.

    {% endnote %}

- Perform all [migrations]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#migration) only while the pipeline is fully stopped.
...
- Try to keep the table settings consistent across different [environments]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#environment) (production/pre-production/testing), or at least avoid unnecessary differences.
