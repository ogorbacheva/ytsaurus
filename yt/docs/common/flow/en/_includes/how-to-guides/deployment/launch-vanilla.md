...
This is the simplest way to run Flow: you don’t need a separate long-running deployment of controllers and workers — they start inside a single {{product-name}} [Vanilla operation]({{ core-docs-root }}/{{ lang }}/reference/data-processing/operations/vanilla{{ docs-revision-query }}). To enable this type of run, just add the `vanilla` block to `pipeline.yson`.

## What you’ll need {#prerequisites}

You need a configuration file:

* `pipeline.yson` — [runner config]({{ flow-docs-root }}/{{ lang }}/concepts/spec{{ docs-revision-query }}#config) with the pipeline spec. For the pipeline to run in a Vanilla operation, it must include a `vanilla` block with `enable = %true` (see [How to enable](#enable)). You don’t need a separate `config.yson` — the node config inside the jobs is built automatically.
...
For the full list of fields, see [TVanillaConfig]({{ flow-docs-root }}/{{ lang }}/reference/configuration/all_yson_structs{{ docs-revision-query }}#NYT_NFlow_TVanillaConfig) and [TVanillaTaskConfig]({{ flow-docs-root }}/{{ lang }}/reference/configuration/all_yson_structs{{ docs-revision-query }}#NYT_NFlow_TVanillaTaskConfig).
...
- [Basic rollout rules]({{ flow-docs-root }}/{{ lang }}/how-to-guides/deployment/basic-rules{{ docs-revision-query }})
- [Basic pipeline operations]({{ flow-docs-root }}/{{ lang }}/how-to-guides/operations/manage-pipeline{{ docs-revision-query }})
...
- [Logs]({{ flow-docs-root }}/{{ lang }}/how-to-guides/operations/view-logs{{ docs-revision-query }})
