...
You can look at `logs/flow.log` of a live job through the [job shell]({{ core-docs-root }}/{{ lang }}/how-to-guides/problems/jobshell-and-slowjobs{{ docs-revision-query }}); `<job_id>` is taken from the vanilla operation page (the `controller` and `worker` tasks):
...
The `zstd` utility may not be present in the job: the set of utilities is determined by the task image. If it isn’t there, add a layer containing it through the task’s `layers` or `system_layer_path` — see [Additional parameters]({{ flow-docs-root }}/{{ lang }}/how-to-guides/deployment/launch-vanilla{{ docs-revision-query }}#advanced-config).

The logging level and composition are overridden through the `node_config` patch in the `vanilla` block, the `logging` section — see [TLogManagerConfig]({{ flow-docs-root }}/{{ lang }}/reference/configuration/all_yson_structs{{ docs-revision-query }}#NYT_NLogging_TLogManagerConfig).
