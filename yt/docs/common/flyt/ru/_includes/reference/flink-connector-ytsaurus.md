...
Apache Flink Connector {{product-name}} — это коннектор для потоковой и пакетной обработки данных на Apache Flink. Работает с {% if audience == "public" %}[сортированными динамическими таблицами]({{ core-docs-root }}/{{ lang }}/concepts/dynamic-tables/sorted-dynamic-tables{{ docs-revision-query }}){% else %}[сортированными динамическими таблицами]({{ docs_root }}/ru/core/concepts/dynamic-tables/sorted-dynamic-tables){% endif %} {{product-name}} и поддерживает запись, чтение ограниченных потоков и Lookup-операции.
...
Для локальной разработки и тестирования рекомендуется воспользоваться официальной документацией по {% if audience == "public" %}[установке кластера {{product-name}} через Kind]({{ core-docs-root }}/{{ lang }}/tutorials/quick-start{{ docs-revision-query }}){% else %}[установке кластера {{product-name}} через Kind]({{ docs_root }}/ru/core/tutorials/quick-start){% endif %}. Для продакшн процессов следуйте {% if audience == "public" %}[руководству администратора {{product-name}}]({{ core-docs-root }}/{{ lang }}/how-to-guides/install-ytsaurus{{ docs-revision-query }}){% else %}[руководству администратора {{product-name}}]({{ docs_root }}/ru/core/how-to-guides/install-ytsaurus){% endif %}.

{% note info %}

`flink-connector-ytsaurus` использует {% if audience == "public" %}[Java-клиент {{product-name}}]({{ core-docs-root }}/{{ lang }}/how-to-guides/api/java/examples{{ docs-revision-query }}){% else %}[Java-клиент {{product-name}}]({{ docs_root }}/ru/core/how-to-guides/api/java/examples){% endif %}. Перед началом работы уточните, какой адрес прокси требуется указать в параметре `proxy` для вашего окружения.
...
Подробнее о схемах {{product-name}} см. {% if audience == "public" %}[в документации]({{ core-docs-root }}/{{ lang }}/reference/storage/static-schema{{ docs-revision-query }}){% else %}[во внутренней документации]({{ docs_root }}/ru/core/reference/storage/static-schema){% endif %}.
...
Коннектор поддерживает как синхронные, так и асинхронные Lookup-операции с возможностью кеширования. Lookup-операции используются для обогащения потока данными из внешней таблицы по ключу. В терминах SQL это соответствует сценарию `Lookup Join`. Подробнее о lookup join см. {% if audience == "public" %}[в разделе Lookup Join]({{ core-docs-root }}/{{ lang }}/reference/dynamic-tables/dyn-query-language{{ docs-revision-query }}#lookup-joins){% else %}[во внутреннем разделе Lookup Join]({{ docs_root }}/ru/core/reference/dynamic-tables/dyn-query-language#lookup-joins){% endif %}.
...
- {% if audience == "public" %}[Сортированные динамические таблицы]({{ core-docs-root }}/{{ lang }}/concepts/dynamic-tables/sorted-dynamic-tables{{ docs-revision-query }}){% else %}[Сортированные динамические таблицы]({{ docs_root }}/ru/core/concepts/dynamic-tables/sorted-dynamic-tables){% endif %} — подробнее об устройстве таблиц, с которыми работает коннектор;
- {% if audience == "public" %}[YSON-форматтер для Flink]({{ flyt-docs-root }}/{{ lang }}/reference/flink-yson{{ docs-revision-query }}){% else %}[YSON-форматтер для Flink]({{ docs_root }}/ru/flyt/reference/flink-yson){% endif %} — если нужна работа с YSON напрямую в задачах Flink;
- {% if audience == "public" %}[Java-клиент {{product-name}}]({{ core-docs-root }}/{{ lang }}/how-to-guides/api/java/examples{{ docs-revision-query }}){% else %}[Java-клиент {{product-name}}]({{ docs_root }}/ru/core/how-to-guides/api/java/examples){% endif %} — низкоуровневый API, на котором построен коннектор.
