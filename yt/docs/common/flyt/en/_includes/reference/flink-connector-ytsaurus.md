...
Apache Flink Connector {{product-name}} is a connector for streaming and batch data processing on Apache Flink. It works with {% if audience == "public" %}[sorted dynamic tables]({{ core-docs-root }}/{{ lang }}/concepts/dynamic-tables/sorted-dynamic-tables{{ docs-revision-query }}){% else %}[sorted dynamic tables]({{ docs_root }}/ru/core/concepts/dynamic-tables/sorted-dynamic-tables){% endif %} in {{product-name}} and supports writes, reading bounded streams, and Lookup operations.
...
For local development and testing, follow the official documentation to {% if audience == "public" %}[install the {{product-name}} cluster via Kind]({{ core-docs-root }}/{{ lang }}/tutorials/quick-start{{ docs-revision-query }}){% else %}[install the {{product-name}} cluster via Kind]({{ docs_root }}/ru/core/tutorials/quick-start){% endif %}. For production deployments, follow {% if audience == "public" %}[the {{product-name}} Admin Guide]({{ core-docs-root }}/{{ lang }}/how-to-guides/install-ytsaurus{{ docs-revision-query }}){% else %}[the internal {{product-name}} Admin Guide]({{ docs_root }}/ru/core/how-to-guides/install-ytsaurus){% endif %}.

{% note info %}

`flink-connector-ytsaurus` uses {% if audience == "public" %}[the Java {{product-name}} client]({{ core-docs-root }}/{{ lang }}/how-to-guides/api/java/examples{{ docs-revision-query }}){% else %}[the Java {{product-name}} client]({{ docs_root }}/ru/core/how-to-guides/api/java/examples){% endif %}. Before you start, verify which proxy address you need to specify in the `proxy` parameter for your environment.
...
For more information about {{product-name}} schemas, see {% if audience == "public" %}[the official documentation]({{ core-docs-root }}/{{ lang }}/reference/storage/static-schema{{ docs-revision-query }}){% else %}[the internal documentation]({{ docs_root }}/ru/core/reference/storage/static-schema){% endif %}.
...
Lookup operations enrich a stream with data from an external table by key. In SQL terms, this corresponds to the `Lookup Join` scenario. For more information about lookup joins, see {% if audience == "public" %}[Lookup Join]({{ core-docs-root }}/{{ lang }}/reference/dynamic-tables/dyn-query-language{{ docs-revision-query }}#lookup-joins){% else %}[the internal Lookup Join documentation]({{ docs_root }}/ru/core/reference/dynamic-tables/dyn-query-language#lookup-joins){% endif %}.
...
- {% if audience == "public" %}[Sorted dynamic tables]({{ core-docs-root }}/{{ lang }}/concepts/dynamic-tables/sorted-dynamic-tables{{ docs-revision-query }}){% else %}[Sorted dynamic tables]({{ docs_root }}/ru/core/concepts/dynamic-tables/sorted-dynamic-tables){% endif %} — learn more about the tables the connector works with;
- {% if audience == "public" %}[YSON formatter for Flink]({{ flyt-docs-root }}/{{ lang }}/reference/flink-yson{{ docs-revision-query }}){% else %}[YSON formatter for Flink]({{ docs_root }}/ru/flyt/reference/flink-yson){% endif %} — if you need to work with YSON directly in Flink jobs;
- {% if audience == "public" %}[{{product-name}} Java client]({{ core-docs-root }}/{{ lang }}/how-to-guides/api/java/examples{{ docs-revision-query }}){% else %}[{{product-name}} Java client]({{ docs_root }}/ru/core/how-to-guides/api/java/examples){% endif %} — the low-level API the connector is built on.
