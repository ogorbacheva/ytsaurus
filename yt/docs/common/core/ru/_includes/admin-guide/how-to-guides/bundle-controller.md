...
Для работы скрипта необходимо сохранить токен в `~/.yt/token` или воспользоваться альтернативными способами конфигурации токена, описанными в [документации]({{ docs_root }}/core/user-guide/how-to-guides/api/python/userdoc#configuration_token) по Python API. Адрес кластера указывается в переменной `YT_PROXY` или через аргумент `--proxy`.
...
## Учения бандла { #drills }

Для имитации отключения бандла на отдельном кластере (например, при учениях по проверке отказоустойчивости) у бандла есть режим учений (drills mode). Режим включается атрибутом `enable_drills_mode` в `bundle_controller_target_config` того кластера, который нужно «выключить».

Включение режима учений:

```bash
yt --proxy <cluster> set //sys/tablet_cell_bundles/<bundle_name>/@bundle_controller_target_config/enable_drills_mode %true
```

Выключение режима учений:

```bash
yt --proxy <cluster> set //sys/tablet_cell_bundles/<bundle_name>/@bundle_controller_target_config/enable_drills_mode %false
```

Флаг устанавливается на бандле на том кластере, который вы хотите прикрыть. Например, чтобы провести учения для бандла на `<cluster-name>`, выставьте `enable_drills_mode = %true` в этом бандле на `<cluster-name>`.

### Как это работает { #drills-how }

При включении режима учений Bundle controller:

- снимает с нод бандла `node_tag_filter`, что вытесняет таблет-целлы с нод, и переводит бандл на отдельный drills-фильтр вида `<zone>/<bundle_name>_drills_mode_on`;
- отключает проверки исправности таблет-целлов (`mute_tablet_cells_check` и `mute_tablet_cell_snapshots_check`), чтобы не поднимать алерты во время учений.

При выключении (`enable_drills_mode = %false`) эти действия откатываются в обратном порядке: возвращается обычный `node_tag_filter`, затем, по истечении grace-периода, вновь включаются проверки исправности.

Пока режим учений активен, на бандле горит алерт `bundle_has_drills_mode_enabled`.

### Ограничения и меры предосторожности { #drills-caveats }

Использование режима учений может быть опасно при работе с асинхронными репликами. Если во время учений реплика внезапно исчезает, транзакции асинхронной репликации могут «залипать» и мешать таблетам отмонтироваться — в таком состоянии таблеты надолго уходят в `freezing`.

Риск сохраняется, даже если вы не отмонтируете таблицы вручную: фоновая балансировка таблетов может отмонтировать их сама и триггерить ту же проблему.
