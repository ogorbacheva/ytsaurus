...
4. Включена [загрузка снапшотов]({{ docs_root }}/core/admin-guide/how-to-guides/persistence-uploader#uploading-snapshots-to-cypress) и [access-логов]({{ docs_root }}/core/admin-guide/how-to-guides/logging#structured_log_delivery) мастера в Кипарис.
...
5. Установлен [Cron Helm-чарт]({{ docs_root }}/core/admin-guide/how-to-guides/install-cron#process_master_snapshot), в котором включена задача `process_master_snapshot`. Эта задача должна отработать хотя бы один раз и создать необходимые директории и таблицы, а именно: `//sys/admin/snapshots/snapshot_exports` и `//sys/admin/snapshots/user_exports`.
6. Установлен [CHYT]({{ chyt-docs-root }}/{{ lang }}/admin-guide/how-to-guides/install-chyt{{ docs-revision-query }}) и [создана Клика]({{ chyt-docs-root }}/{{ lang }}/user-guide/how-to-guides/try-chyt{{ docs-revision-query }}). Сохраните название клики в переменную окружения `CHYT_CLIQUE_NAME`. Для простоты в примере будет использоваться клика с названием `ch_public`, создаваемая по умолчанию.
...
Создайте пользователей [согласно инструкции]({{ docs_root }}/core/user-guide/reference/storage/auth):
...
- Для Access Log Viewer:
  ...
  Укажите для CHYT клики [словарь]({{ chyt-docs-root }}/{{ lang }}/user-guide/reference/cliques/configuration{{ docs-revision-query }}#clickhouse_config), который сможет обращаться к Bulk ACL Checker'у:
