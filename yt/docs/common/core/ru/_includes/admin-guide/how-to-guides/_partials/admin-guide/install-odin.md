...
Odin - сервис, осуществляющий качественный мониторинг работы {{product-name}}-кластера. Подробнее можно почитать в разделе [Мониторинг]({{ docs_root }}/core/admin-guide/how-to-guides/monitoring#odin).
...
* специальный пользователь-робот `robot-odin` с выписанным для него токеном (см. раздел [Управление токенами]({{ docs_root }}/core/user-guide/reference/storage/auth#token-management)):
...
UI должен быть установлен как helm-chart (см. [инструкцию по установке]({{ docs_root }}/core/admin-guide/how-to-guides/install-ytsaurus#ui)).
...
Инструкция по обновлению Odin подробно описана в статье [Обновление odin]({{ docs_root }}/core/admin-guide/how-to-guides/update/update-odin).
