...
Task-прокси предоставляет доступ к веб-сервисам, развёрнутым в операциях {{product-name}}. Подробнее можно почитать в разделе [Task-прокси]({{ docs_root }}/core/user-guide/reference/proxy/task).
...
- специальный пользователь-робот `robot-task-proxy` с выписанным для него токеном (см. раздел [Управление токенами]({{ docs_root }}/core/user-guide/reference/storage/auth#token-management)):
   ```sh
...
Инструкция по обновлению Task-прокси подробно описана в статье [Обновление Task-прокси]({{ docs_root }}/core/admin-guide/how-to-guides/update/update-task-proxy).
