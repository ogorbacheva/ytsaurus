...
Ниже описано, как выполнить обновление с использованием оператора версии **0.32.0** и новее. О различиях со старыми версиями смотрите в разделе [Переход с enableFullUpdate на updatePlan]({{ docs_root }}/core/admin-guide/how-to-guides/update/update_strategy#migration-from-enablefullupdate).
...
: Обновляйте кластер по порядку мажорных версий, не пропуская промежуточные: сначала `25.1 → 25.2`, затем `25.2 → 25.3`. Пропуск версии может нарушить работу кластера. Внутри целевой мажорной версии вы можете выбрать самую свежую минорную и получить последние обновления — например, `25.2.0 → 25.3.2`. {{ core-admin-guide-how-to-guides-update-update-ytsaurus-md-audience-1 }}
...
: Перед обновлением кластера обновите оператор. Его версия должна быть совместима с целевой версией кластера.{{ core-admin-guide-how-to-guides-update-update-ytsaurus-md-audience-2 }}

Во время обновления кластер может быть недоступен.
: По умолчанию кластер полностью недоступен на время обновления — все операции и запросы прерываются. Планируйте обновление на период минимальной нагрузки. Если кластер должен оставаться доступным, выберите другую стратегию — смотрите [Стратегии обновления]({{ docs_root }}/core/admin-guide/how-to-guides/update/update_strategy).
...
{{ core-admin-guide-how-to-guides-update-update-ytsaurus-md-audience-3 }}
...
[Компоненты с собственным релизным циклом](#additional-components-ref) — Query Tracker, YQL-агент и другие — при этом не обновляются. Как их обновить, смотрите в разделе [Обновление отдельных компонентов](#partial-update). Перед обновлением проверьте совместимость этих компонентов с целевой версией кластера{{ core-admin-guide-how-to-guides-update-update-ytsaurus-md-audience-4 }}
...
{{ core-admin-guide-how-to-guides-update-update-ytsaurus-md-audience-5 }}
...
Обновление может занять некоторое время, так как оператор сначала скачивает новый Docker-образ и только потом запускает обновлённые компоненты. Чтобы образы были готовы заранее и обновление проходило быстрее, настройте [предзагрузку образов]({{ docs_root }}/core/admin-guide/how-to-guides/update/update_strategy#image-heater).
...
- **Командой CLI** — [yt admin remove-master-unrecognized-options]({{ docs_root }}/core/admin-guide/reference/cli-admin#remove-master-unrecognized-options). Этот способ поддерживает флаг `--dry` для предварительного просмотра изменений.
...
Подробнее о стратегии, доступности компонентов и рекомендуемом количестве инстансов — в разделе [Стратегия `RollingUpdate`]({{ docs_root }}/core/admin-guide/how-to-guides/update/update_strategy#configure-rolling).
...
Подробнее о предзагрузке — в разделе [Предзагрузка образов]({{ docs_root }}/core/admin-guide/how-to-guides/update/update_strategy#image-heater).

{% endcut %}

Описание всех полей `updatePlan` — в разделе [Настройка поля updatePlan]({{ docs_root }}/core/admin-guide/how-to-guides/update/update_strategy#updateplan).
