...
![](../../../../../_images/admin-guide/how-to-guides/encryption-diagram.svg)
...
В отличие от других компонентов, CHYT запускается внутри [vanilla-операции]({{ docs_root }}/core/user-guide/reference/data-processing/operations/vanilla) {{product-name}}, поэтому сертификаты передаются особым образом:
...
- Все настройки безопасности, включая сертификаты и параметры шифрования, передаются в операцию через механизм [secure_vault]({{ docs_root }}/_includes/admin-guide/*secure_vault).
