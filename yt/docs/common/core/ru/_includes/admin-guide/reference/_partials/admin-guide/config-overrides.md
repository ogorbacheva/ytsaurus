...
Оператор {{product-name}} автоматически генерирует статические конфигурационные файлы для всех компонентов кластера &mdash; на основе [спецификации]({{ docs_root }}/core/admin-guide/how-to-guides/prepare-spec) `Ytsaurus`. Если необходимо точечно изменить параметры, которые не вынесены в поля этой спецификации (например, чтобы включить debug-логирование для диагностики или настроить RPC-таймауты), используйте поле `configOverrides`. Этот механизм позволяет наложить свои настройки поверх конфигурации, сгенерированной оператором.
...
{{ core-admin-guide-reference-config-overrides-md-audience-1 }}
