...
Некоторым командам нужны дополнительные зависимости (`kubernetes>=31.0.0` для `logs k8s`, `docker>=7.1.0` для `metrics replay`). Они устанавливаются вместе с [extra-набором `admin`]({{ docs_root }}/core/user-guide/how-to-guides/api/python/start#extras):
...
Чаще всего это требуется [после обновления кластера]({{ docs_root }}/core/admin-guide/how-to-guides/update/update-ytsaurus#operator): в новой версии {{product-name}} часть полей конфига может выйти из употребления, и тогда они остаются в `//sys/@config` как нераспознанные.
