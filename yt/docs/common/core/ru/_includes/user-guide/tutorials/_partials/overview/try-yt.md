...
- В данном руководстве вы будете запускать {{product-name}} кластер в минимальной конфигурации — без гарантий отказоустойчивости.
  ...
  Примеры того, как настраивать конфигурацию кластера, приведены в [Руководстве администратора]({{ docs_root }}/core/admin-guide/how-to-guides/prepare-spec).
...
Независимо от способа установки, в результате будут подняты необходимые компоненты системы: [мастер-сервер]({{ docs_root }}/_includes/overview/*about-master), [планировщик]({{ docs_root }}/_includes/overview/*about-scheduler), [YQL]({{ docs_root }}/_includes/overview/*about-yql), [Query Tracker]({{ docs_root }}/_includes/overview/*about-qt) и другие. Все последующие примеры из этого руководства — создание таблиц, загрузка данных и запуск MapReduce — не зависят от способа установки кластера и будут одинаковыми как для Docker, так и в случае Minikube.
...
- Docker {selected}
  ...
  3. Чтобы поднять кластер, запустите скрипт:
     ...
     Этот скрипт создаст и запустит [docker-контейнеры]({{ docs_root }}/_includes/overview/*about-docker-containers), в которых будет развёрнут {{product-name}}. Если всё прошло успешно, появится сообщение:
     ```
  ...
  4. Чтобы убедиться, что всё работает, запустите команду:
     ...
     - `yt.backend` — в этом контейнере подняты компоненты {{product-name}} кластера.
       ...
       - [Мастер-сервер]({{ docs_root }}/core/admin-guide/reference/components#master) — отвечает за отказоустойчивое хранение метаданных кластера. К этим данным относится информация о пользователях системы, о хранимых объектах и о местоположении самих данных.
       - Нода — объединяет в себе функциональность [Data-ноды]({{ docs_root }}/core/admin-guide/reference/components#data-nodes) и [Exec-ноды]({{ docs_root }}/core/admin-guide/reference/components#exec-nodes). Отвечает за хранение данных, за работу динамических таблиц и занимается исполнением джобов.
       - [Планировщик]({{ docs_root }}/core/admin-guide/reference/components#planirovshik) (шедулер) — отвечает за планирование процесса обработки данных (Map, Reduce и так далее).
       - [Контроллер-агент]({{ docs_root }}/core/admin-guide/reference/components#planirovshik) — отвечает за планирование джобов конкретных операций.
       - [HTTP-прокси]({{ docs_root }}/core/admin-guide/reference/components#proxy) — сервер, через который осуществляется общение с внешним миром.
       - [YQL-агент]({{ docs_root }}/core/admin-guide/reference/components#yql-agent) — движок исполнения SQL подобных запросов.
       - [Очередь {{product-name}}]({{ docs_root }}/core/user-guide/reference/dynamic-tables/queues) — упорядоченные динамические таблицы.
       - [Query Tracker]({{ docs_root }}/core/user-guide/explanation/query-tracker/about) — компонент для запуска запросов на разных SQL диалектах (YQL, QT, CHYT, SPYT).
...
- Minikube
  ...
  ![](../../../../../_images/user-guide/tutorials/try-yt-podman-settings.png)
...
- Kind
  ...
  - Для работы через [RPC proxy]({{ docs_root }}/core/user-guide/reference/proxy/rpc) по адресу `localhost:8082` в отдельном терминале выполните команду:
...
Самый удобный способ взаимодействия с системой {{product-name}} — из консоли. Утилита [{{product-name}} СLI]({{ docs_root }}/core/user-guide/reference/api/cli/cli) не устанавливается при развёртывании кластера, её нужно установить в системе отдельно.
...
- Minikube
  ...
  Здесь токен задаётся в переменной окружения — это сделано намеренно, для простоты и наглядности примера. В реальных задачах так делать не стоит, для работы с токенами в {{product-name}} есть специальные команды. Подробнее читайте в разделе [Аутентификация]({{ docs_root }}/core/user-guide/reference/storage/auth#upravlenie-tokenami).
...
В {{product-name}} все данные хранятся в [таблицах]({{ docs_root }}/core/user-guide/explanation/storage/static-tables). Поэтому для начала создайте таблицу:
...
Значение `16-64ca-10191-47007b7d` — это идентификатор созданного узла [Кипариса]({{ docs_root }}/_includes/overview/*about-cypress). Идентификаторы узлов полезны при работе с [транзакциями]({{ docs_root }}/core/user-guide/explanation/storage/transactions) в {{product-name}}. В рамках текущего примера данные идентификаторы вам не понадобятся.
...
![](../../../../../_images/user-guide/tutorials/try-yt-new-table.png)

### 3. Запишите данные { #insert-data }

Теперь запишите данные в таблицу, вызвав команду [write-table]({{ docs_root }}/core/user-guide/reference/api/cli/commands#write-table):
...
Другой способ прочитать таблицу — выполнить [SELECT]({{ yql-docs-root }}/{{ lang }}/yql/syntax/select/index{{ docs-revision-query }}) запрос в веб-интерфейсе. Для этого перейдите во вкладку **Queries** и введите запрос:

```sql
SELECT * FROM `//home/input_table`;
```

![](../../../../../_images/user-guide/tutorials/try-yt-hello-world-select.png)
...
3. Нажмите кнопку **Add setting** и в полях укажите значения: "cluster" и "primary" соответственно. Нажмите галочку.
   ...
   ![](../../../../../_images/user-guide/tutorials/try-yt-fix-ui-cluster.png)
...
Теперь запишите данные в исходную таблицу, вызвав команду [write-table]({{ docs_root }}/core/user-guide/reference/api/cli/commands#write-table):

```bash prompt="$"
$ cat source.tsv | yt write-table //home/mapreduce_input --format dsv
```
Чтобы проверить, что в таблицу записались данные, можно прочитать таблицу с помощью команды [read-table]({{ docs_root }}/core/user-guide/reference/api/cli/commands#read-table). В квадратных скобках указан полуинтервал — получить первые шесть строк таблицы:
...
Запустите MapReduce операцию с помощью команды [map-reduce]({{ docs_root }}/core/user-guide/reference/api/cli/commands#map-reduce):
...
![](../../../../../_images/user-guide/tutorials/try-yt-operations.png)
...
![](../../../../../_images/user-guide/tutorials/try-yt-jupyter.png)

{% endcut %}

{% cut "Веб-интерфейс" %}

Тут вы сможете ознакомиться с возможностями веб-интерфейса {{product-name}}: изучить файловую систему, посмотреть на список пулов, позапускать запросы в [Query Tracker]({{ docs_root }}/core/user-guide/explanation/query-tracker/about).

Ссылка на веб-интерфейс поднятого кластера будет в письме.

![](../../../../../_images/user-guide/tutorials/try-yt-ui.png)
...
[*about-cypress]: Кипарис — это распределённая файловая система и хранилище метаинформации. В Кипарисе хранятся таблицы и файлы. Подробнее читайте в [документации]({{ docs_root }}/core/user-guide/explanation/storage/cypress).
...
[*about-qt]: Инструмент для запуска SQL-подобных запросов. Query Tracker доступен в веб-интерфейсе — в разделе **Queries**. Подробнее читайте в [документации]({{ docs_root }}/core/user-guide/explanation/query-tracker/about).
...
[*about-master]: Отвечает за отказоустойчивое хранение метаданных кластера. К ним относится информация о пользователях системы, о хранимых объектах и о местоположении самих данных. Подробнее читайте в [документации]({{ docs_root }}/core/admin-guide/reference/components#master).

[*about-scheduler]: Отвечает за распределение ресурсов между операциями и за процесс их исполнения на кластере. Подробнее в [документации]({{ docs_root }}/core/user-guide/explanation/data-processing/scheduler/scheduler-and-pools).
