...
В данном разделе собрана информация о дополнительных параметрах запросов. Подробнее о командах читайте в разделе [Команды]({{ docs_root }}/core/user-guide/reference/api/commands).
...
- `prerequisite_transactions` — список [транзакций]({{ docs_root }}/core/user-guide/explanation/storage/transactions), которые обязаны существовать на момент выполнения запроса.
...
В [немутирующих]({{ docs_root }}/core/user-guide/reference/api/commands#concepts) запросах к [Кипарису]({{ docs_root }}/core/user-guide/explanation/storage/cypress) можно указать источник данных для чтения при помощи параметра `read_from`.
