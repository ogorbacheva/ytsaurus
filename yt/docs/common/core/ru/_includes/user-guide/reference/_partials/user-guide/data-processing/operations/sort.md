...
* `intermediate_data_medium` (`default`) — [тип носителя (медиум)]({{ docs_root }}/core/user-guide/reference/storage/media), на котором хранятся чанки промежуточных данных сортировки;
...
* `schema_inference_mode` (auto) — режим определения схемы. Доступные значения: auto, from_input, from_output. Подробнее можно прочитать в разделе [Схема данных]({{ docs_root }}/core/user-guide/reference/storage/static-schema#schema_inference);
