...
##  Основные отличия YSON от JSON { #main-differences }
...
## Синтаксические отличия { #syntax-differeces }
...
Специальные токены:
`;`, `=`, `#`, `[`, `]`, `{`, `}`, `<`, `>`, `)`, `/`, `@`, `!`, `+`, `^`, `:`, `,`, `~`.
Не все эти символы используются в YSON, некоторые используются в [YPath]({{ docs_root }}/core/user-guide/reference/storage/ypath).
