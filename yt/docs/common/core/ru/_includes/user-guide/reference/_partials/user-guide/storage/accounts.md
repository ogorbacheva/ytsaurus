...
Все типы квот для хранения данных привязаны к **аккаунту** проекта. Аккаунт — это служебный объект в [Кипарисе]({{ docs_root }}/core/user-guide/explanation/storage/cypress). Все аккаунты имеют уникальные имена. При этом они находятся в отдельном пространстве имён, так что имя пользователя и аккаунта могут совпадать. Имя аккаунта хранится в атрибуте `name`, доступном как на чтение, так и на запись.
...
{{ core-user-guide-reference-storage-accounts-md-audience-1 }}

{{ core-user-guide-reference-storage-accounts-md-audience-2 }}Вложенные аккаунты создаются из командой строки или в веб-интерфейсе {{product-name}}.
...
2. Указать {{ core-user-guide-reference-storage-accounts-md-audience-3 }}название нового аккаунта, имя родительского аккаунта, в который будет вложен создаваемый аккаунт, а также ответственных за новый аккаунт пользователей. При необходимости можно также запросить автоматическое создание домашней директории, соответствующей аккаунту.
...
Чтобы создать аккаунт, в атрибуте `name` необходимо указать его имя, а в `parent_name` — имя родителя. Если имя родителя не указано, будет создан аккаунт верхнего уровня. {{ core-user-guide-reference-storage-accounts-md-audience-4 }}
...
В случае ошибки `argument command: invalid choice` следует обновить утилиту `yt`. Подробнее можно прочитать в разделе {{ core-user-guide-reference-storage-accounts-md-audience-5 }}.
...
{{ core-user-guide-reference-storage-accounts-md-audience-6 }}
