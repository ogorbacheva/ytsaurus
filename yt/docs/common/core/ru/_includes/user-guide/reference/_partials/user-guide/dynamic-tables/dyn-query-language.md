...
При наличии в запросе ключевого слова WITH INDEX, индексная таблица будет использоваться для координации, а индексная таблица, указанная после FROM, будет подключена к ней по общим колонкам. См. также [вторичные индексы]({{ docs_root }}/core/user-guide/reference/dynamic-tables/secondary-indices).
...
Можно указывать подзапрос в секции `FROM`. В таком случае внутренний запрос будет выполняться распределенно, а внешний запрос будет выполняться на проксе.
...
### Подзапрос по иерахическим данным

По данным списочных типов можно запускать подзапрос. Это может быть полезно, если нужно отфильтровать список или как-то его преобразовать.
Для этого в секции `FROM` подзапроса нужно указать колонку списочного типа и alias, через который будет обеспечиваться доступ к элементам списка.

Указание колонки должно быть взято в скобки. Это нужно для разрешения синтаксической неоднозначности между запросом по таблице и запросом по колонке списочного типа.

Также можно запускать запрос по набору списков. Для этого нужно перечислить колонки списочного типа и alias к ним через запятую.
В случае набора списков будет происходить склеивание списков с помощью операции zip. Списки должны быть одинакового размера.

Для использования данной функциональности нужно указать опцию запроса `expression_builder_version=2`.

Синтаксис секции `FROM` подзапроса:
```sql
...
Списки данных могут возникать в процессе агрегации во внешнем запросе. Для агрегации данных в список используется агрегатная функция `array_agg(value, ignore_nulls)`.
В подзапросе по иерархическим данным можно использовать колонки, alias и выражения из внешнего запроса.
...
В данном примере в выражении `li + a` идентификатор `a` раскрывается в выражение `t.k + 1` из внешнего запроса.
Во вложенном запросе можно использовать агрегатные функции по данным из группировки внешнего запроса - выражение `sum(t.s)`.

Группировка может использовать как во внешнем так и во внутреннем запросе:
```sql
...
Выражение `sum(t.s)` будет относится ко внешней группировке, так колонка `t.s` не является ключом группировки и должна использоваться под агрегатной функцией.
...
Выражение из колонки внутреннего запроса и агрегатного выражения внешнего запроса `ls + sum(t.s) AS x` используется в качестве ключа группировки внутреннего запроса.
...
Во внутреннем запросе идентификатор `b` раскрывается в использование агрегатного выражения `sum(2)` из внешней группировки.
...
#### Работа с [YSON]({{ docs_root }}/core/user-guide/reference/storage/formats#yson)
...
1. `try_get_int64`, `get_int64`, `try_get_uint64`, `get_uint64`, `try_get_double`, `get_double`, `try_get_boolean`, `get_boolean`, `try_get_string`, `get_string`, `try_get_any`, `get_any`
    ...
    - `path` — строка, содержащая путь до нужного поля в формате [YPATH]({{ docs_root }}/core/user-guide/reference/storage/ypath).
...
4. `any_to_yson_string(yson) :: any -> string`
    Преобразует значение типа `any` в строку, содержащую его text-[YSON]({{ docs_root }}/core/user-guide/reference/storage/yson) представление.
...
Входные данные для запроса и варианты их разбиения определяются структурой основной таблицы, указанной в запросе, и предикатом фильтрации. В настоящее время поддержано исполнение запросов поверх **[динамических]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/overview)** (сортированных и упорядоченных) таблиц.

При работе с динамическими сортированными таблицами разбиение данных двухуровневое: изначально таблица разбивается на _таблеты_, и запрос отправляется на машины, обслуживающие выбранные таблеты, а затем на каждой машине таблет разбивается далее на партиции для обеспечения параллельности исполнения внутри каждой машины. Подробнее о таблетах можно узнать в разделе [Динамические таблицы]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/overview).
