...
Для понимания изложенного в разделе материала требуется знание устройства [HTTP протокола](https://ru.wikipedia.org/wiki/HTTP), принципов работы через HTTP, основ [хранения данных]({{ docs_root }}/core/user-guide/explanation/storage/cypress) в системе {{product-name}}.
...
Word Count — это типичная задача, которую считают в любой системе [MapReduce]({{ docs_root }}/core/user-guide/reference/data-processing/operations/mapreduce). Общая схема следующая:

1. По исходному тексту выполняется [Map]({{ docs_root }}/core/user-guide/reference/data-processing/operations/map) операция, выдающая для каждого слова пару `(<слово>, 1)`.
2. Результат сортируется по первой координате.
3. По первой координате выполняется [Reduce]({{ docs_root }}/core/user-guide/reference/data-processing/operations/reduce) операция, суммирующая вторую координату. На выходе получается набор пар `(<слово>, <количество упоминаний слова>)`.

Полное описание всех доступных команд {{product-name}} можно найти в разделе [Команды]({{ docs_root }}/core/user-guide/reference/api/commands).

## Перед началом работы { #prerequisites }
{{ core-user-guide-reference-proxy-http-md-audience-1 }}

## Установка переменных окружения { #export-vars }

{{ core-user-guide-reference-proxy-http-md-audience-2 }}

## Создание исходной таблицы {#prepare}

Прежде всего необходимо создать рабочий каталог, в котором будут размещаться данные для нашего примера. Для создания каталогов в Кипарисе используется команда [create]({{ docs_root }}/core/user-guide/reference/api/commands#create):
...
Данные в системе {{product-name}} хранятся в [таблицах]({{ docs_root }}/core/user-guide/explanation/storage/static-tables), поэтому  необходимо создать таблицу:
...
{{ core-user-guide-reference-proxy-http-md-audience-3 }}
...
Для записи данных в таблицу используется команда [write_table]({{ docs_root }}/core/user-guide/reference/api/commands#write_table). Обратите внимание — запросы на загрузку данных выполняются не через обычную прокси `YT_PROXY`, а через [тяжёлую прокси](#get-data-proxy-list):
...
Чтобы загрузить данные в другом формате, например в JSON, необходимо добавить немного больше опций. Подробности можно прочитать в разделе [Форматы]({{ docs_root }}/core/user-guide/reference/storage/formats#json).
...
При написании своего клиента пользователю необходимо эпизодически запрашивать список [тяжёлых прокси]({{ docs_root }}/core/user-guide/reference/proxy/http-reference#hosts) через запрос `/hosts`. В ответе возвращается упорядоченный по приоритету список тяжёлых прокси. Приоритет прокси определяется динамически и зависит от её загрузки (CPU+I/O). Хорошая стратегия — раз в минуту или раз в несколько запросов перезапрашивать список `/hosts` и менять текущую прокси, к которой задаются запросы.
...
Для запуска операции [Map]({{ docs_root }}/core/user-guide/reference/data-processing/operations/map) или [Reduce]({{ docs_root }}/core/user-guide/reference/data-processing/operations/reduce) потребуется загрузить в систему скрипт, который нужно будет исполнить. Скрипт для данной задачи размещен {{ core-user-guide-reference-proxy-http-md-audience-4 }} {{proxy-pages.http.mapreduce-example}}. Пусть локально он сохранён в файл `exec.py`.

Для загрузки файлов в систему {{product-name}} используется команда [write_file]({{ docs_root }}/core/user-guide/reference/api/commands#write_table). Перед загрузкой необходимо создать узел типа «файл», аналогично созданию таблицы на предыдущем шаге:
...
Важный момент, который стоит учитывать: требование существования выходной таблицы **до** запуска операции. Создадим таблицу с помощью команды [create]({{ docs_root }}/core/user-guide/reference/api/commands#create):
...
Сортировка запускается командой [start_operation]({{ docs_root }}/core/user-guide/reference/api/commands#start_operation) с параметром `operation_type`, равным `sort`. Входная спецификация для [Sort]({{ docs_root }}/core/user-guide/reference/data-processing/operations/sort) немного похожа на спецификации для [Map]({{ docs_root }}/core/user-guide/reference/data-processing/operations/map) и [Reduce]({{ docs_root }}/core/user-guide/reference/data-processing/operations/reduce).
...
Запуск Reduce операции выполняется с помощью команды [start_operation]({{ docs_root }}/core/user-guide/reference/api/commands#start_operation) с параметром `operation_type`, равным `reduce`:
...
В /api/v3 все операции обработки данных (Map, Reduce, Sort и так далее) запускались не командой `start_operation`, а с помощью одноимённых команд — [map]({{ docs_root }}/core/user-guide/reference/api/commands#map), [reduce]({{ docs_root }}/core/user-guide/reference/api/commands#reduce), [sort]({{ docs_root }}/core/user-guide/reference/api/commands#sort). Например, так выглядит запуск Map операции:
