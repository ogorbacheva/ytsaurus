...
В данном разделе собрана информация о командной строке {{product-name}}: [описание](#description), [способы установки]({{ docs_root }}/core/user-guide/how-to-guides/api/cli/install), [примеры использования]({{ docs_root }}/core/user-guide/how-to-guides/api/cli/examples).

## Описание { #description }

{{product-name}} CLI (Command Line Interface) — самый удобный способ взаимодействия с системой {{product-name}} из консоли. CLI реализует функциональность всех [команд]({{ docs_root }}/core/user-guide/reference/api/commands), поддерживаемых {{product-name}}. Имя команды задается первым аргументом, далее идут её опции. Для получения справки по CLI достаточно выполнить команду:
...
Более подробно о токенах и аутентификации в разделе [Аутентификация]({{ docs_root }}/core/user-guide/reference/storage/auth).
...
Более подробно о возможных настройках в разделе [Конфигурация клиентской библиотеки]({{ docs_root }}/core/user-guide/how-to-guides/api/python/userdoc#configuration).
...
- [Форматы данных]({{ docs_root }}/core/user-guide/reference/storage/formats), принимаемых на входе команд и возникающих на выходе, могут быть заданы через следующие переменные окружения:
