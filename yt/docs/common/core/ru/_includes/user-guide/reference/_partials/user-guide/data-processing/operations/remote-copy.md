...
Скорость передачи данных между кластерами квотируется. Подробнее см. в разделе [Квотирование межкластерной сетевой полосы]({{ docs_root }}/core/user-guide/reference/data-processing/operations/cluster-throttlers).
...
* `schema_inference_mode` (auto) — режим определения схемы. Доступные значения: `auto`, `from_input`, `from_output`. Подробности в разделе [Схема данных]({{ docs_root }}/core/user-guide/reference/storage/static-schema#schema_inference).
...
Операция RemoteCopy (`remote_copy`) поддержана только для [сортированных таблиц]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/sorted-dynamic-tables). Для копирования упорядоченной таблицы следует сконвертировать её в статическую. Скопировать на другой кластер упорядоченную таблицу с сохранением таблетов невозможно.

{{ core-user-guide-reference-data-processing-operations-remote-copy-md-audience-2 }}
