...
В некоторых {{product-name}}-операциях вычисления настолько просты, что заводить для каждой из них класс с использованием [C++ API]({{ docs_root }}/core/user-guide/explanation/api/cpp/description) кажется излишним. В этом случае можно воспользоваться для описания операций [лямбда-выражениями](https://ru.wikipedia.org/wiki/Лямбда-выражение).
...
{{ core-user-guide-reference-api-cpp-lambda-md-audience-1 }} и включите в программу заголовок:

```
#include yt/cpp/mapreduce/library/lambda/yt_lambda.h
```

[Инициализируйте {{product-name}}]({{ docs_root }}/core/user-guide/explanation/api/cpp/description#init) и получите `client`. Передавайте `client` или [транзакцию]({{ docs_root }}/core/user-guide/explanation/api/cpp/description#transactions) первым параметром в описанные в данном документе функции.

Функции этой библиотеки хорошо сочетаются с использованием записей в формате [protobuf]({{ docs_root }}/core/user-guide/reference/api/cpp/protobuf).
...
Вариант обычных операций, где [Reduce]({{ docs_root }}/core/user-guide/reference/data-processing/operations/reduce) имеет на входе и выходе один и тот же тип, а также ассоциативен. В этом случае можно дополнительно упростить реализацию Reduce-преобразования, а также бесплатно использовать ее в качестве combiner.
...
| **Параметр** | **Описание**                                                     |
| -------- | ------------------------------------------------------------ |
| `T`      | Тип записи (TNode или конкретный тип [protobuf-сообщения]({{ docs_root }}/core/user-guide/reference/api/cpp/protobuf)) |
...
Данная функция делает классический [Map]({{ docs_root }}/core/user-guide/reference/data-processing/operations/map), т. е. читает каждую запись из таблицы одного формата и пишет ее в таблицу другого формата. Некоторые записи не попадают в конечную таблицу.
