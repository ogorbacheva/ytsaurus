...
Управлять чтением и записью можно через [форматы табличных данных]({{ docs_root }}/core/user-guide/reference/storage/formats) и с помощью конфигов `TableReader`, `TableWriter` и `ControlAttributes`.
...
При чтении входных таблиц с других кластеров (удалённое чтение) скорость чтения ограничивается квотами межкластерной сетевой полосы. Подробнее см. в разделе [Квотирование межкластерной сетевой полосы]({{ docs_root }}/core/user-guide/reference/data-processing/operations/cluster-throttlers).
...
`table_writer` позволяет на этапе создания таблицы в операции или при запуске операции [Merge]({{ docs_root }}/core/user-guide/reference/data-processing/operations/merge) управлять различными параметрами хранения.
...
- `enable_range_index` (`false`) — присылать контрольные записи с номером диапазона среди заказанных диапазонов входной таблицы (поле `range_index` типа `int64` в атрибутах контрольной записи); подробнее можно прочитать в разделе [YPATH]({{ docs_root }}/core/user-guide/reference/storage/ypath#known_attributes);
...
Подробнее про `table_index` в разных форматах можно прочитать в разделе [Переключение таблиц]({{ docs_root }}/core/user-guide/how-to-guides/data-processing/operations/table-switch).
