#### **Q: Как добавить колонку в таблицу {{product-name}}?** {#add-column-to-table}
...
------
#### **Q: Получаю ошибку при попытке изменить схему таблицы - Changing «strict» from «false» to «true» is not allowed. Что делать?** {#change-schema-strict-error}

**A:** Нельзя поменять схему у непустой таблицы со слабой на строгую, так как для валидации подобного действия необходимо прочитать всю таблицу, чтобы убедиться, что данные соответствуют новой схеме. Проще всего создать новую таблицу и перезаписать данные из старой через read + write или запустив операцию [Merge]({{ docs_root }}/core/user-guide/reference/data-processing/operations/merge).

------
#### **Q: Как авторизоваться при работе с YT из консоли?** {#yt-console-authorization}

**A:** Необходимо сохранить токен нужного пользователя в файл `~/.yt/token`.

------
#### **Q: Можно ли уменьшить коэффициент репликации у временных таблиц в native C++ wrapper?** {#reduce-replication-factor}

**A:** Нет, в wrapper такая возможность не предусмотрена.

------
#### **Q: При использовании python wrapper возникает ошибка «ImportError: Bad magic number in ./modules/yt/__init__.pyc». Что делать?** {#bad-magic-number}
...
Версия python может отличаться на разных узлах кластера. Надёжнее всего использовать свой [образ ФС]({{ docs_root }}/core/user-guide/reference/data-processing/layers/layer-paths) для запуска джобов.

------
#### **Q: Каковы накладные расходы при чтении небольших диапазонов в таблицах и при чтении небольших таблиц?** {#reading-small-ranges-overhead}
...
------
#### **Q: Можно ли эффективно организовать хранение байтовых чисел (hex) в ключах вместо строк?** {#store-hex-numbers-keys}

**A:** Можно при использовании формата [YSON]({{ docs_root }}/core/user-guide/reference/storage/formats#yson) или [JSON]({{ docs_root }}/core/user-guide/reference/storage/formats#json).

------
#### **Q: Каковы уровни логирования в консольном клиенте и как они задаются?** {#logging-levels-console-client}
...
------
#### **Q: При чтении с помощью команды read небольшой таблицы клиент повисает при перезапросах. В чем может быть дело?** {#client-hangs-requeries}

**A:** Одна из возможных частых причин — слишком большое число (мелких) чанков в таблице. Следует запустить операцию [Merge]({{ docs_root }}/core/user-guide/reference/data-processing/operations/merge), указав опцию `--spec '{force_transform=true}'`. При появлении таких таблиц в результате работы операций консольный клиент выдает предупреждение, содержащее, помимо прочего, команду, которую можно выполнить для укрупнения чанков таблицы. Также можно указать опцию `auto_merge_output={action=merge}`, в этом случае слияние будет выполняться автоматически.

------
#### **Q: Операция завершилась с ошибкой «Account "tmp" is over disk space (или chunk) limit», в чем дело?** {#account-tmp-over-limit}
...
Вы можете [установить `cron`](https://ytsaurus.tech/docs/ru/admin-guide/install-cron) со скриптом `clear_tmp` для того, чтобы очищать [пространство временных данных](https://ytsaurus.tech/docs/ru/yql/faq/temp) по мере заполнения.

------
#### **Q: Операция упала с ошибкой «Account "intermediate" is over disk space (или chunk) limit», в чем дело?** {#account-intermediate-over-limit}
...
Если вы не указывали `intermediate_data_account` (смотрите раздел [Настройки операций]({{ docs_root }}/core/user-guide/reference/data-processing/operations/operations-options), [Sort]({{ docs_root }}/core/user-guide/reference/data-processing/operations/sort), [MapReduce]({{ docs_root }}/core/user-guide/reference/data-processing/operations/mapreduce)). Это значит, что вы делите данный аккаунт со всеми. Чтобы избежать этой проблемы, укажите `intermediate_data_account`.

------
#### **Q: Является ли чтение таблицы (или файла) в {{product-name}} консистентной операцией? Что будет, если читать таблицу и одновременно с этим её удалить?** {#consistent-read-operation}
...
------
#### **Q: При запуске клиента получаю ошибку "Cannot determine backend type: either driver config or proxy url should be specified". Что делать?** {#backend-type-determination}

**A:** Проверьте, указана ли переменная окружения `YT_PROXY=<cluster-name>`.

------

#### **Q: Что делать, если запрос Python обёртки падает с "gaierror(-2, 'Name or service not known')"?** {#name-service-unknown}
