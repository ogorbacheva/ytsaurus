...
- **Отключение перенаправления**: токен `&` может использоваться для подавления перенаправления по [символической ссылке]({{ docs_root }}/core/user-guide/explanation/storage/links).
  Примеры:
...
Префикс и суффикс пути позволяют закодировать атрибуты в более компактном виде. Обе эти части используют лексику [YSON]({{ docs_root }}/core/user-guide/reference/storage/yson). Подробнее об их устройстве:

### Префикс { #rich_ypath_prefix }

Часть `<prefix>` либо пуста, либо задает атрибуты в виде map-fragment в формате [YSON]({{ docs_root }}/core/user-guide/reference/storage/yson).
...
Существует [команда]({{ docs_root }}/core/user-guide/reference/api/commands#parse_ypath) `parse_ypath` драйвера, позволяющая канонизировать сложный YPath.
