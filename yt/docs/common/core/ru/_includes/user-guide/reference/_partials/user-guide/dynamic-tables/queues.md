...
Связь между консьюмерами и очередями обеспечивается объектами *регистраций*. {{ core-user-guide-reference-dynamic-tables-queues-md-audience-1 }}
...
{{ core-user-guide-reference-dynamic-tables-queues-md-audience-2 }}

### Создание очереди

Создание очереди ничем не отличается от создания обычной упорядоченной динамической таблицы.

Для наибольшей полноты графиков/статистик рекомендуется добавлять в схему таблицы колонки [timestamp]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/ordered-dynamic-tables#timestamp_column) и [cumulative_data_weight]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/ordered-dynamic-tables#cumulative_data_weight_column).
...
Оба Cypress аргумента обеих команд имеют тип [rich YPath]({{ docs_root }}/core/user-guide/reference/storage/ypath#rich_ypath) и поддерживают указание имени кластера. В ином случае используется кластер, на котором выполняется команда.
...
Параметр очереди в данном методе представляет из себя [rich YPath]({{ docs_root }}/core/user-guide/reference/storage/ypath#rich_ypath) и поддерживает указание кластера, на котором она расположена. В ином случае используется кластер, на котором выполняется команда.
...
Такая настройка не конфликтует с [существующими]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/ordered-dynamic-tables#remove_old_data) настройками TTL динамических таблиц: можно настроить тримминг по vital consumers и `max_data_ttl=36h, min_data_versions=0`, чтобы, помимо удаления данных по оффсетам, всегда хранить не более трёх дней данных.

### Графики и статусы

{{ core-user-guide-reference-dynamic-tables-queues-md-audience-3 }}
...
{{ core-user-guide-reference-dynamic-tables-queues-md-audience-4 }}

## Пример использования

{{ core-user-guide-reference-dynamic-tables-queues-md-audience-5 }}

<small>Листинг 1 — Пример использования API очередей</small>

```bash
# Create queue on <queue-cluster>.
$ yt --proxy <queue-cluster> create table //tmp/$USER-test-queue --attributes '{dynamic=true;schema=[{name=data;type=string};{name="$timestamp";type=uint64};{name="$cumulative_data_weight";type=int64}]}'
2826e-2b1e4-3f30191-dcd2013e

# Create queue_consumer on <consumer-cluster>.
$ yt --proxy <consumer-cluster> create queue_consumer //tmp/$USER-test-consumer

# OR: Create queue_consumer on <consumer-cluster> as table with explicit schema specification.
$ yt --proxy <consumer-cluster> create table //tmp/$USER-test-consumer --attributes '{dynamic=true;treat_as_queue_consumer=true;schema=[{name=queue_cluster;type=string;sort_order=ascending;required=true};{name=queue_path;type=string;sort_order=ascending;required=true};{name=partition_index;type=uint64;sort_order=ascending;required=true};{name=offset;type=uint64;required=true};{name=meta;type=any;required=false}]}'
18a5b-28931-3ff0191-35282540

# Register consumer for queue.
$ yt --proxy <queue-cluster> register-queue-consumer //tmp/$USER-test-queue "<cluster=<consumer-cluster>>//tmp/$USER-test-consumer" --vital

# Check registrations for queue.
$ yt --proxy <queue-cluster> list-queue-consumer-registrations --queue-path //tmp/$USER-test-queue
[
  {
    "queue_path" = <
...
$ yt --proxy <queue-cluster> get //tmp/$USER-test-queue/@queue_status
{
    "partition_count" = 1;
    ...
    "registrations" = [
        {
            "consumer" = "<consumer-cluster>://tmp/bob-test-consumer";
            "vital" = %true;
            "queue" = "<queue-cluster>://tmp/bob-test-queue";
        };
    ];
...
$ yt --proxy <queue-cluster> get //tmp/$USER-test-queue/@queue_partitions
[
    {
        "error" = {
            "attributes" = {
...
$ yt --proxy <consumer-cluster> get //tmp/$USER-test-consumer/@queue_consumer_status
{
    "queues" = {
        "<queue-cluster>://tmp/bob-test-queue" = {
            "error" = {
                "attributes" = {
                ...
                "code" = 1;
                "message" = "Queue \"<queue-cluster>://tmp/bob-test-queue\" snapshot is missing";
            };
        };
    };
    "registrations" = [
...
$ yt --proxy <queue-cluster> mount-table //tmp/$USER-test-queue
$ yt --proxy <consumer-cluster> mount-table //tmp/$USER-test-consumer
...
$ yt --proxy <queue-cluster> set //tmp/$USER-test-queue/@auto_trim_config '{enable=true}'

# Write rows without exactly once semantics.
$ for i in {1..20}; do echo '{data=foo}; {data=bar}; {data=foobar}; {data=megafoo}; {data=megabar}' | yt insert-rows --proxy <queue-cluster> //tmp/$USER-test-queue --format yson; done;

# Check that queue status reflects writes.
$ yt --proxy <queue-cluster> get //tmp/$USER-test-queue/@queue_status/write_row_count_rate
{
...
$ yt --proxy <consumer-cluster> pull-queue-consumer //tmp/$USER-test-consumer "<cluster=<queue-cluster>>//tmp/$USER-test-queue" --partition-index 0 --offset 0 --max-row-count 5 --format "<format=text>yson"
...
$ yt --proxy <consumer-cluster> advance-queue-consumer //tmp/$USER-test-consumer "<cluster=<queue-cluster>>//tmp/$USER-test-queue" --partition-index 0 --old-offset 0 --new-offset 42
...
# Create queue producer on <queue-cluster>.
$ yt --proxy <queue-cluster> create queue_producer //tmp/$USER-test-producer
309db-eb36-3f30191-f83f27c0

# Create queue producer session.
$ yt --proxy <queue-cluster> create-queue-producer-session --queue-path //tmp/$USER-test-queue --producer-path //tmp/$USER-test-producer --session-id session_123
{
  "epoch" = 0;
  "sequence_number" = -1;
  "user_meta" = #;
...
$ echo '{data=value1;"$sequence_number"=1};{data=value2;"$sequence_number"=2}' | yt --proxy <queue-cluster> push-queue-producer //tmp/$USER-test-producer //tmp/$USER-test-queue --session-id session_123 --epoch 0 --input-format yson
{
...
# Check written rows.
$ yt --proxy <queue-cluster> pull-queue //tmp/$USER-test-queue --offset 100 --partition-index 0 --format "<format=pretty>yson"
{
    "$tablet_index" = 0;
    "$row_index" = 100;
    "data" = "value1";
...
$ echo '{data=value2;"$sequence_number"=2};{data=value3;"$sequence_number"=10}' | yt --proxy <queue-cluster> push-queue-producer //tmp/$USER-test-producer //tmp/$USER-test-queue --session-id session_123 --epoch 0 --input-format yson
{
...
{{ core-user-guide-reference-dynamic-tables-queues-md-audience-6 }}
