...
Операция MapReduce аналогична операции [Sort]({{ docs_root }}/core/user-guide/reference/data-processing/operations/sort), но позволяет выполнять пользовательский скрипт mapper до выполнения partitioning джобов и пользовательский скрипт reducer после выполнения sorting-джобов.
...
У слитной операции MapReduce по сравнению с комбинацией операций [Map]({{ docs_root }}/core/user-guide/reference/data-processing/operations/map) + [Sort]({{ docs_root }}/core/user-guide/reference/data-processing/operations/sort) + [Reduce]({{ docs_root }}/core/user-guide/reference/data-processing/operations/reduce) есть несколько плюсов:
...
Общие параметры для всех типов операций описаны в разделе [Настройки операций]({{ docs_root }}/core/user-guide/reference/data-processing/operations/operations-options).
...
* `intermediate_data_medium` (default) — [тип носителя (медиум)]({{ docs_root }}/core/user-guide/reference/storage/media), на который будут записаны чанки промежуточных данных.
...
* `pivot_keys` — список ключей, по которым будет производиться партицирование данных для reduce стадии. Опция полностью аналогична соответствующей [опции]({{ docs_root }}/core/user-guide/reference/data-processing/operations/reduce) для операции reduce.
