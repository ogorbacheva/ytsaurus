...
Система поддерживает символические ссылки (`soft links`) в [Кипарисе]({{ docs_root }}/core/user-guide/explanation/storage/cypress). Такая ссылка представляет собой узел типа `link`, ссылающийся на другой объект `target`.
...
Чтобы создать символическую ссылку, используйте [команду]({{ docs_root }}/core/user-guide/reference/api/commands#link) `link`. В качестве аргументов сначала укажите путь целевого узла, затем путь узла ссылки:
...
Помимо [атрибутов]({{ docs_root }}/core/user-guide/reference/storage/attributes), общих для всех типов узлов, узлы-ссылки содержат атрибуты:
