...
Для таблиц право `read` даёт доступ к данным, однако видимый набор колонок и строк может быть дополнительно сужен [колоночными ACL]({{ docs_root }}/core/user-guide/reference/storage/columnar-acl) и [построчным доступом]({{ docs_root }}/core/user-guide/reference/storage/row-level-security): даже имея право `read` на всю таблицу, пользователь должен быть явно допущен к чтению конкретных колонок или строк.
...
Даёт возможность [зарегистрировать консьюмера очереди]({{ docs_root }}/core/user-guide/reference/dynamic-tables/queues#register_queue_consumer). ACE с этим правом имеет два особых правила: в нём должна быть указана витальность через атрибут `vital` (`%true` или `%false`), и оно должно содержать `register_queue_consumer` как единственное право. Например:
...
- использование [Job Shell]({{ docs_root }}/core/user-guide/how-to-guides/problems/jobshell-and-slowjobs) требует одновременно прав `read` и `manage` , так как наличие доступа через консоль позволяет читать или произвольным образом менять состояние джоба.
...
Для получения доступа к существующему аккаунту или директории в {{product-name}} обратитесь к {{ core-user-guide-explanation-storage-access-control-p2-md-audience-1 }}.
