...
Реплицированными могут быть как [сортированные]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/overview), так и [упорядоченные]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/ordered-dynamic-tables) динамические таблицы. Сами реплики могут быть **синхронными** и **асинхронными**. Успешный коммит транзакции записи гарантирует, что данные попали в синхронную реплику и в очередь репликации. В асинхронные реплики данные будут записаны в фоне. Подробнее можно прочитать в разделе [Репликация](#replication).
...
Если хочется добавить реплику, сделав полную копию данных уже существующей реплики, то стоит воспользоваться {{ core-user-guide-explanation-dynamic-tables-replicated-dynamic-tables-md-audience-1 }} по копированию реплики.
...
| errors               | Error[*]({{ docs_root }}/_includes/user-guide/dynamic-tables/*error)           | Список ошибок при репликации на данную реплику              |
...
| tablets                   | TabletReplicaInfo[*]({{ docs_root }}/_includes/user-guide/dynamic-tables/**) | Описание состояния таблетов; позиция в списке соответствует индексу таблета, как в самой таблице |
