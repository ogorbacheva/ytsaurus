...
В данном разделе описаны транзакции применительно к динамическим таблицам. Транзакционная модель статических таблиц описана в разделе [Транзакции статических таблиц]({{ docs_root }}/core/user-guide/explanation/storage/transactions).
...
У таблиц имеется атрибут `atomicity`, который определяет гарантии на применяемые транзакции. Данный атрибут можно изменять только для [отмонтированных]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/overview#mount_table) таблиц. Значения атрибута могут быть следующие:
