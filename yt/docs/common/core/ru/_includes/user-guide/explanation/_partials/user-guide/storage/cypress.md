...
* С каждым узлом в дереве связан [набор атрибутов]({{ docs_root }}/core/user-guide/reference/storage/attributes), в том числе задаваемых пользователем.
* Дерево является [транзакционным]({{ docs_root }}/core/user-guide/explanation/storage/transactions).
* В качестве узла Кипариса могут быть не только файлы и директории, но и [другие объекты]({{ docs_root }}/core/user-guide/explanation/storage/objects). 

По аналогии с файловой системой в Кипарисе поддерживается [система контроля доступов]({{ docs_root }}/core/user-guide/explanation/storage/access-control).

### Что еще важно знать {#what-else-is-important}

* Корневым элементом Кипариса является `/`, который имеет тип **map_node** (то есть является директорией). Адресация узлов в Кипарисе осуществляется с помощью [YPath]({{ docs_root }}/core/user-guide/reference/storage/ypath).

* Количество дочерних узлов директории ограничено. По умолчанию лимит равен 50000, но зависит от конфигурации кластера. 

* Глубина Кипариса в теории не ограничена, но стоит помнить о том, что в [YPath]({{ docs_root }}/core/user-guide/reference/storage/ypath) существует лимит на глубину разрешения пути.
...
С Кипарисом можно работать с помощью [CLI]({{ docs_root }}/core/user-guide/reference/api/cli/cli).

{% endnote %}

## Атрибуты узлов Кипариса { #attributes }

У узлов Кипариса есть общие для всех объектов [атрибуты]({{ docs_root }}/core/user-guide/reference/storage/attributes). Также есть дополнительные атрибуты, представленные в таблице:

| **Атрибут**                | **Тип**            | **Значение**                                                                                                                  |
| -------------------------- | ------------------ | ------------------------------------------------------------                                                                  |
| `parent_id`                | `string`           | Идентификатор узла-родителя (отсутствует в корне)                                                                             |
| `locks`                    | `array<Lock>`      | [Список блокировок]({{ docs_root }}/core/user-guide/explanation/storage/transactions), взятых на узел                                                               |
| `lock_mode`                | `LockMode`         | Текущий [режим блокировки]({{ docs_root }}/core/user-guide/explanation/storage/transactions) узла (зависит от транзакции)                                           |
...
| `annotation`               | `string`           | Человекочитаемая [аннотация]({{ docs_root }}/core/user-guide/reference/storage/annotations) объекта                                                               |

Каждый узел имеет свой дескриптор, отвечающий за управление доступом, поэтому среди его атрибутов есть `inherit_acl`, `acl` и `owner`. Подробнее можно прочитать в разделе [Система контроля доступов]({{ docs_root }}/core/user-guide/explanation/storage/access-control).
...
Обращение к атрибутам при этом не учитывается (обращением к атрибуту считается указание полного пути к атрибуту внутри [YPath]({{ docs_root }}/core/user-guide/reference/storage/ypath)). Также не учитываются запросы с включенной опцией `suppress_access_tracking`. 
...
* Чтобы выставлять данные атрибуты для узла, необходимо иметь [право]({{ docs_root }}/core/user-guide/explanation/storage/access-control) `write` на сам узел, как и для многих других атрибутов, а также `remove` для данного узла и всего его поддерева, так как фактически заказывается удаление, хоть и отложенное. Чтобы удалить данные атрибуты, достаточно права `write`.
