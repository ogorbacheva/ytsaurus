...
В разделе описаны основные [типы объектов]({{ docs_root }}/core/user-guide/explanation/storage/objects#object_types) системы {{product-name}}. Представлена таблица с ключевыми [атрибутами каждого объекта]({{ docs_root }}/core/user-guide/explanation/storage/objects#attributes) и описано понятие [идентификатора объекта]({{ docs_root }}/core/user-guide/explanation/storage/objects#object_ids).

## Типы объектов { #object_types }

Основные типы объектов в {{product-name}}:

* [Файлы]({{ docs_root }}/core/user-guide/explanation/storage/objects#files);
* [Таблицы]({{ docs_root }}/core/user-guide/explanation/storage/objects#tables);
* [Ссылки]({{ docs_root }}/core/user-guide/explanation/storage/objects#links);
* [YSON-документы]({{ docs_root }}/core/user-guide/explanation/storage/objects#yson_doc);
* [Примитивные типы]({{ docs_root }}/core/user-guide/explanation/storage/objects#primitive_types);
* [Системные объекты](#internal_objects).

Описание объектов занимает большую часть метаинформации, которая хранится в реплицированном виде в памяти мастер-серверов [Кипариса]({{ docs_root }}/core/user-guide/explanation/storage/cypress). Кипарис содержит в себе различную системную информацию, а также данные о том, где хранятся пользовательские данные.

Для любого узла Кипариса может присутствовать несколько версий, поскольку в контексте каждой [транзакции]({{ docs_root }}/core/user-guide/explanation/storage/transactions) состояние узла может выглядеть по-разному.

### Файлы { #files }

Это узлы Кипариса, имеющие тип **file** и предназначенные для хранения больших бинарных данных в системе.
Файлы состоят из множества чанков — [являются владельцами чанков]({{ docs_root }}/core/user-guide/explanation/storage/chunks#attributes).
...
Подробнее можно прочитать в разделе [Файлы]({{ docs_root }}/core/user-guide/explanation/storage/files).

### Таблицы { #tables }

Это узлы типа **table**, предназначенные для хранения больших пользовательских данных. Таблицы бывают двух типов: [Статические]({{ docs_root }}/core/user-guide/explanation/storage/static-tables) и [Динамические]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/overview). Логически таблица представляет из себя последовательность строк. Каждая строка состоит из колонок.
Значения колонок типизированы и могут содержать произвольные структурированные данные, поддерживаемые языком [YSON]({{ docs_root }}/core/user-guide/reference/storage/yson).
...
Подробнее можно прочитать в разделе [Ссылки]({{ docs_root }}/core/user-guide/explanation/storage/links).
...
Поддерживаются запросы и модификации внутри документа. Адресация внутри документа осуществляется с помощью языка [YPath]({{ docs_root }}/core/user-guide/reference/storage/ypath).

Подробнее можно прочитать в разделе [YSON-документы]({{ docs_root }}/core/user-guide/explanation/storage/yson-docs).

Подробнее про формат YSON можно прочитать в разделе [YSON.]({{ docs_root }}/core/user-guide/reference/storage/yson)
...
| -------------- | ------------------------------------------------------------ |
| `transaction`   | [Транзакция]({{ docs_root }}/core/user-guide/explanation/storage/transactions) |
| `chunk`   | Чанк           |
| `chunk_list`  | Чанклист    |
