...
YSON-документ — это узел [Кипариса]({{ docs_root }}/core/user-guide/explanation/storage/cypress), имеющий тип  **document** и предназначенный для хранения произвольных YSON-структур.
...
Работать с документами можно с помощью стандартных [команд]({{ docs_root }}/core/user-guide/reference/api/commands):  `get`, `list`, `exists`, `set`, `remove`, как и с другими объектами в Кипарисе.
...
Адресация внутри документа осуществляется с помощью языка [YPath]({{ docs_root }}/core/user-guide/reference/storage/ypath).
...
| ----------- | ------- | ------------------------------------------------------------ |
| `value`     | `any`   | Полное содержимое документа. Атрибут позволяет задать содержимое документа при создании. Атрибут является [opaque]({{ docs_root }}/core/user-guide/reference/storage/attributes#system_attr), то есть при чтении всех атрибутов узла без фильтра он будет отображаться как [entity]({{ docs_root }}/core/user-guide/reference/storage/yson#entity). |
