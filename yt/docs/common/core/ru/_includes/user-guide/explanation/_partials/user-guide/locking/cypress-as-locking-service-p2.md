...
В системе {{product-name}} есть [транзакции]({{ docs_root }}/core/user-guide/explanation/storage/transactions), которые бывают мастерными и таблетными. Мастерные транзакции реализуют пессимистическую модель блокировок, то есть взятие блокировки на узел Кипариса происходит синхронно и при этом проверяются возможные конфликты.
...
Для запуска процесса под блокировкой можно использовать команду [run-command-with-lock]({{ docs_root }}/core/user-guide/reference/api/cli/commands#run-command-with-lock), а можно реализовать логику со взятием блокировки самостоятельно в коде приложения.
