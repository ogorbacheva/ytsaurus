...
Утилита поставляется вместе с пакетом [Python API]({{ docs_root }}/core/user-guide/how-to-guides/api/python/start). Поддерживаются все типы джобов, в которых запускается пользовательский код.
...
3. После того как команда отработает, в папке `job_ffe4ff5c-d2fac8c1-3fe0384-816a7fd0` будут лежать следующие файлы:
    ...
    * Файл `command` содержит команду для запуска джоба. Так как операция была запущена с помощью [Python API]({{ docs_root }}/core/user-guide/how-to-guides/api/python/start), код будет иметь вид:
