...
Для этого при запуске операции в спецификации нужно указывать [ACL]({{ docs_root }}/core/user-guide/explanation/storage/access-control), описывающий пользователей и группы, которые могут выполнять действия над операцией и джобами.
...
Если вам захочется поменять `owners` какого-то Job Shell уже запущенной операции, можно воспользоваться командой [update_operation_parameters]({{ docs_root }}/core/user-guide/reference/api/commands#update_operation_parameters--update_operation_parameters), указав новый список `owners` конкретного Job Shell:
