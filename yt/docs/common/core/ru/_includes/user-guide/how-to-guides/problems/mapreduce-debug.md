...
{% include [Локальная эмуляция запуска джоба](../_partials/user-guide/problems/mapreduce-debug/local-emulation.md) %}

{% include [Получение текущего stderr выполняющегося джоба](../_partials/user-guide/problems/mapreduce-debug/stderr-running.md) %}
...
- В SDK на других языках
  ...
  Можно передать настройку `stderr_table_path` напрямую в спецификацию операции. Описание данной опции см. в разделе [Настройки операций]({{ docs_root }}/core/user-guide/reference/data-processing/operations/operations-options).

{% endlist %}

{% include [Stderr-таблица](../_partials/user-guide/problems/mapreduce-debug/stderr-table.md) %}
