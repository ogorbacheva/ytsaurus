...
Перед запуском примеров необходимо получить [YT-токен]({{ docs_root }}/core/user-guide/reference/storage/auth).

Запускать примеры нужно из-под `Linux` без аргументов.

{{ core-user-guide-how-to-guides-api-cpp-examples-md-audience-1 }}
...
Уровень логирования устанавливается с помощью переменной окружения такой командой (см. так же [документацию]({{ docs_root }}/core/user-guide/explanation/api/cpp/description#logging)):
...
Если известно, как устроена таблица, то может быть удобно воспользоваться форматом [Protobuf]({{ docs_root }}/core/user-guide/reference/api/cpp/protobuf), который:
...
Некоторые достаточно простые операции можно представить в виде лямбда-выражений C++ с пустым capture-list (т. е. не захватывающих переменных) с использованием библиотеки [yt/cpp/mapreduce/library/lambda]({{ docs_root }}/core/user-guide/reference/api/cpp/lambda).
...
[Reducer]({{ docs_root }}/core/user-guide/reference/data-processing/operations/reduce) пишется естественным образом.
...
Входные таблицы те же, что и в предыдущем примере: с пользователями и роботами{{ core-user-guide-how-to-guides-api-cpp-examples-md-audience-2 }}
...
В {{product-name}} есть слитная операция [MapReduce]({{ docs_root }}/core/user-guide/reference/data-processing/operations/mapreduce), которая работает несколько быстрее нежели [Map]({{ docs_root }}/core/user-guide/reference/data-processing/operations/map) + [Sort]({{ docs_root }}/core/user-guide/reference/data-processing/operations/sort) + [Reduce]({{ docs_root }}/core/user-guide/reference/data-processing/operations/reduce). Чтобы по таблице с пользователями ещё раз посчитать статистику, сколько раз встречается то или иное имя, перед подсчётом нормализируем имена, приведя их к нижнему регистру. Это нужно, чтобы люди с именами `АРКАДИЙ` и `Аркадий` считались как одно.
...
Есть возможность настраивать параметры операции, перегружая метод `IJob::PrepareOperation`, подробности см. в разделе [Описание]({{ docs_root }}/core/user-guide/explanation/api/cpp/description#prepare_operation).
...
![](../../../../../../_images/user-guide/how-to-guides/api/cpp/reduce.svg){ .center}

![](../../../../../../_images/user-guide/how-to-guides/api/cpp/join_reduce.svg){ .center}
...
Во время работы операция накапливает множество статистик (читайте [документацию]({{ docs_root }}/core/user-guide/how-to-guides/problems/jobstatistics)).
...
{{product-name}} поддерживает [динамические таблицы]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/overview). Wrapper С++ предоставляет возможность совершать базовые операции с такими таблицами.
