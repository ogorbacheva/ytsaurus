...
В пользовательском коде операций [Map]({{ docs_root }}/core/user-guide/reference/data-processing/operations/map) и [Reduce]({{ docs_root }}/core/user-guide/reference/data-processing/operations/reduce) можно получать данные о том, из какой таблицы пришла запись, и выбирать, в какую таблицу отправить запись. Реализуется это с помощью **[дескрипторов](#descriptors)** или через **[переключатели таблиц](#switches)**.
...
Когда у операции типа [MapReduce]({{ docs_root }}/core/user-guide/reference/data-processing/operations/mapreduce) не указан mapper, на входе reducer не будет данных о том, из каких входных таблиц пришли записи.
...
Схему настройки переключателей входных таблиц можно прочесть в разделе [Настройки ввода/вывода]({{ docs_root }}/core/user-guide/reference/storage/io-configuration).

Форма переключателей зависит от формата выходных данных.

### YSON { #yson }

В [YSON]({{ docs_root }}/core/user-guide/reference/storage/yson) и для входных и для выходных таблиц в потоке записей может встретиться [entity]({{ docs_root }}/core/user-guide/reference/storage/yson#entity) с атрибутами. Это управляющие команды, встроенные в поток данных. Например: `table_index=N`, где N — integer. Эта команда «переключает» поток таким образом, что последующие записи относятся к другой таблице с индексом N. Это верно и для входных, и для выходных таблиц.
...
[DSV]({{ docs_root }}/core/user-guide/reference/storage/formats#dsv) не поддерживает переключение выходных таблиц.
...
При установке опции `enable_table_index=true` для таблиц в формате [SCHEMAFUL_DSV]({{ docs_root }}/core/user-guide/reference/storage/formats#schemaful_dsv) во всех строках таблицы первым полем, перед колонами схемы, будет записан индекс таблицы.
