...
- Имя команды (строка);
- Описание входного и выходного форматов ([YSON-строка]({{ docs_root }}/core/reference/storage/data-types#yson) с атрибутами);
...
- Данные являются бинарным потоком (`binary`). Например, содержимое [файлов]({{ docs_root }}/core/concepts/storage/files);
- Данные [структурированы]({{ docs_root }}/core/reference/storage/formats#structured_data) (`structured`). Например, обычные узлы [Кипариса]({{ docs_root }}/core/concepts/storage/cypress);
- Данные являются табличным потоком (`tabular`). Например, содержимое [таблиц]({{ docs_root }}/core/concepts/storage/objects#tables).
...
Если команда работает со структурированными или табличными данными, для неё следует указать [формат]({{ docs_root }}/core/reference/storage/formats). Для входного потока данных служит параметр `input_format`, для выходного потока параметр `output_format` соответственно.

В случае структурированных данных поддерживаются форматы `yson` (используется по умолчанию) и `json`. Для табличных данных существует множество различных [форматов]({{ docs_root }}/core/reference/storage/formats).
...
Также имеется возможность указывать пререквизиты у запросов, а также для легких немутирующих запросов указывать, откуда производить чтение. Подробную информацию можно найти в разделе [Параметры запросов]({{ docs_root }}/core/reference/api/query-parameters).

## Работа с транзакциями

Подробную информацию про транзакции можно найти в статье [Транзакции]({{ docs_root }}/core/concepts/storage/transactions).
...
Подробную информацию про дерево метаинформации можно найти в статье [Кипарис]({{ docs_root }}/core/concepts/storage/cypress).
...
| `type`              | Да          |                           | [Тип узла]({{ docs_root }}/core/concepts/storage/objects).                                                                                                                                                                                                    |
...
| `lock_existing`     | Нет          | `false`                     | Установить на указанный узел [exclusive-блокировку]({{ docs_root }}/core/concepts/storage/transactions#locks), даже если он уже существует. Используется только вместе с `ignore_existing`. Если невозможно установить блокировку, команда завершится ошибкой. |
...
| `waitable`      | Нет          | `false`                     | В случае конфликта не завершаться ошибкой, а встать в очередь. Была ли блокировка установлена фактически, можно узнать с помощью атрибута `state` объекта блокировки. См. [Транзакции]({{ docs_root }}/core/concepts/storage/transactions#locking_queue). |
...
Блокировки автоматически снимаются по завершении транзакции. Не следует использовать команду `unlock` без веской необходимости. См. [Транзакции]({{ docs_root }}/core/concepts/storage/transactions#lock_operations).
...
| `lock_existing`               | Нет          | `false`                     | Установить на узел по пути `destination_path` [exclusive-блокировку]({{ docs_root }}/core/concepts/storage/transactions#locks), даже если он уже существует. Используется только вместе с `ignore_existing`. В случае невозможности установить блокировку команда завершится ошибкой. |
...
| `preserve_expiration_time`    | Нет          | `false`                     | Копировать ли атрибут [`expiration_time`]({{ docs_root }}/core/concepts/storage/cypress#TTL) или оставлять его пустым.                                                                                                                                                           |
| `preserve_expiration_timeout` | Нет          | `false`                     | Копировать ли атрибут [`expiration_timeout`]({{ docs_root }}/core/concepts/storage/cypress#TTL) или оставлять его пустым.                                                                                                                                                        |
...
Подробную информацию про права доступа можно найти в разделе [Контроль доступа]({{ docs_root }}/core/concepts/storage/access-control).
...
Подробную информацию про файлы можно найти в статье [Файлы]({{ docs_root }}/core/concepts/storage/files).
...
Подробную информацию про файловый кеш можно найти в разделе [Файловый кеш]({{ docs_root }}/core/reference/storage/file-cache).
...
Подробную информацию про статические таблицы можно найти в разделе [Статические таблицы]({{ docs_root }}/core/concepts/storage/static-tables).

Подробную информацию про динамические таблицы можно найти в разделе [Динамические таблицы]({{ docs_root }}/core/concepts/dynamic-tables/overview).

### write_table { #write_table }

Свойства команды: **Мутирующая**, **Тяжелая**.

Применимость: [Статические таблицы]({{ docs_root }}/core/concepts/storage/static-tables).
...
| `table_writer` | Нет      | Из конфигурации драйвера  | [Опции записи таблицы]({{ docs_root }}/core/reference/storage/io-configuration). |
...
Применимость: [Статические]({{ docs_root }}/core/concepts/storage/static-tables) и [динамические]({{ docs_root }}/core/concepts/dynamic-tables/overview) таблицы.
...
| ------------------ | ------------- | ------------------------- | ------------------------------------------------------------ |
| `path`               | Да      |                           | Путь до таблицы в Кипарисе; путь *должен* существовать.      |
| `table_reader`       | Нет      | Из конфигурации драйвера  | [Опции чтения таблицы]({{ docs_root }}/core/reference/storage/io-configuration#table_reader). |
| `control_attributes` | Нет      | Из конфигурации драйвера  | [Конфигурация контрольных атрибутов чтения]({{ docs_root }}/core/reference/storage/io-configuration#control_attributes). |
...
Применимость: Статические [таблицы с бинарными данными]({{ docs_root }}/core/concepts/storage/blobtables).
...
| ---------------------- | ------------- | ------------------------- | ------------------------------------------------------------ |
| `path`                   | Да      |                           | Путь до таблицы в Кипарисе; путь *должен* существовать.      |
| `table_reader`           | Нет      | Из конфига драйвера       | [Опции чтения таблицы]({{ docs_root }}/core/reference/storage/io-configuration). |
...
Применимость: [Динамические таблицы]({{ docs_root }}/core/concepts/dynamic-tables/overview).

Семантика:

- Выполнить SQL-подобный запрос для динамической таблицы в соответствии с [поддерживаемыми возможностями]({{ docs_root }}/core/reference/dynamic-tables/dyn-query-language);
...
| `aggregate`  | Нет    | `false`                   | Если выставлено `false`, [агрегирующие колонки]({{ docs_root }}/core/concepts/dynamic-tables/sorted-dynamic-tables#aggr_columns) перезатерты новым значением. Если выставлено `true`, такие колонки применят дельту из входных данных. |
| `atomicity`  | Нет    | `full`                    | Поддерживаются значения `none` и `full`. Если выставлено `none`, запись будет происходить на каждом таблете, независимо от других. Если выставлено`full`, будут записаны либо все переданные строки, либо ничего. [Подробнее]({{ docs_root }}/core/concepts/dynamic-tables/sorted-dynamic-tables). |
...
Применимость: [Упорядоченные]({{ docs_root }}/core/concepts/dynamic-tables/ordered-dynamic-tables) динамические таблицы.
...
Применимость: [Сортированные]({{ docs_root }}/core/concepts/dynamic-tables/sorted-dynamic-tables) динамические таблицы.

Семантика:

- Форсированно запускает итерацию [фонового балансировщика таблетов]({{ docs_root }}/core/how-to-guides/dynamic-tables/resharding), т.е. решардирует таблеты, выходящие за границы `min_tablet_size`–`max_tablet_size`;
...
Применимость: [Статические]({{ docs_root }}/core/concepts/storage/static-tables) и [динамические таблицы]({{ docs_root }}/core/concepts/dynamic-tables/overview).
...
- При изменении схемы происходят [разнообразные проверки корректности]({{ docs_root }}/core/reference/storage/static-schema), т.к. система должна быть уверена, что существующие данные соответствуют схеме;
...
| `upstream_replica_id` | Нет    |                           | Если указано, то изменяет идентификатор объекта-реплики на метакластере. Подробнее можно прочесть в разделе [Реплицированные]({{ docs_root }}/core/concepts/dynamic-tables/replicated-dynamic-tables) динамические таблицы. |
...
Применимость: [Реплицированные]({{ docs_root }}/core/concepts/dynamic-tables/replicated-dynamic-tables) динамические таблицы.
...
Подробную информацию про запуск операций обработки данных можно найти в разделе [Обработка данных]({{ docs_root }}/core/reference/data-processing/operations/overview).

Все операции выполняются асинхронно, указанные команды лишь запускают их. Чтобы узнать, закончилась операция или нет, запросите [статус операции]({{ docs_root }}/core/reference/data-processing/operations/overview#status) командой `get_operation`.
Также все команды для работы с операциями являются транзакционными. Это означает, что вся работа с таблицами в пределах операции будет делаться в рамках указанной транзакции при запуске операции.  Узел отвечающей операции (`//sys/operations/<OP_ID>`) обновляется [планировщиком]({{ docs_root }}/core/concepts/data-processing/scheduler/scheduler-and-pools) вне любых транзакций.
...
| `spec`           | Да      |                           | Спецификация операции. Подробнее в разделе [Настройка операций]({{ docs_root }}/core/reference/data-processing/operations/operations-options). |
...
- Подробное описание всех [параметров спецификации]({{ docs_root }}/core/reference/data-processing/operations/merge).
...
- Подробное описание всех [параметров спецификации]({{ docs_root }}/core/reference/data-processing/operations/erase).
...
- Подробное описание всех [параметров спецификации]({{ docs_root }}/core/reference/data-processing/operations/map).
...
- Подробное описании всех [параметров спецификации]({{ docs_root }}/core/reference/data-processing/operations/reduce).
...
- Подробное описании всех [параметров спецификации]({{ docs_root }}/core/reference/data-processing/operations/sort).
...
- Подробное описании всех [параметров спецификации]({{ docs_root }}/core/reference/data-processing/operations/mapreduce).
...
- Подробное описание всех [параметров спецификации]({{ docs_root }}/core/reference/data-processing/operations/remote-copy).
...
К операции можно обратиться либо через `operation_id`, либо через `operation_alias`. Подробнее про алиасы операций см. в разделе [Настройки операций]({{ docs_root }}/core/reference/data-processing/operations/operations-options#common_options).
...
| *parameters[scheduling_options_per_pool_tree]* | Нет      |                           | Словарь `{имя дерева: настройки планировщика для данного дерева}`. Описание настроек ниже. Подробнее про планировщик можно прочесть в разделе [Планировщик и пулы]({{ docs_root }}/core/concepts/data-processing/scheduler/scheduler-and-pools). |
...
| **Параметр** | **Обязательный** | **Описание**                                                 |
| ------------ | ------------- | ------------------------------------------------------------ |
| `path`       | Да      | Путь к параметру в спецификации операции в формате [YPath]({{ docs_root }}/core/reference/storage/ypath). |
...
Имея право `manage` на пул, можно совершать некоторые [действия над операциями]({{ docs_root }}/core/how-to-guides/data-processing/scheduler/manage-pools#allowed_pool_actions), запущенными в пуле, не обладая формальными правами на операцию. В таких случая команда выдаст, что у пользователя нет права, так как команда проверяет только права самой операции.
...
| `pool`                       | `string`          | Название [пула]({{ docs_root }}/core/concepts/data-processing/scheduler/scheduler-and-pools), в котором был запущен джоб.                                   |
| `pool_tree`                  | `string`          | Название [дерева пулов]({{ docs_root }}/core/concepts/data-processing/scheduler/scheduler-and-pools), в котором был запущен джоб. |
...
- Выполнить разбор переданного [YPath-пути]({{ docs_root }}/core/reference/storage/ypath), переложив все идентификаторы сложного YPath в атрибуты.
...
| ------------------ | ---------------- | ------------------------------------------------------------ |
| primitive\_types   | список строк     | [Примитивные типы]({{ docs_root }}/core/reference/storage/data-types#schema_primitive). |
| erasure\_codecs    | список строк     | [Erasure-кодеки]({{ docs_root }}/core/reference/storage/replication).|
| compression\_codecs| список строк     | [Кодеки сжатия]({{ docs_root }}/core/reference/storage/compression).|
