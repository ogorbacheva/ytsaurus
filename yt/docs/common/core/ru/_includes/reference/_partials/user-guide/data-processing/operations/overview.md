...
- **[Map]({{ docs_root }}/core/reference/data-processing/operations/map), [Reduce]({{ docs_root }}/core/reference/data-processing/operations/reduce), [MapReduce]({{ docs_root }}/core/reference/data-processing/operations/mapreduce)** — выполняют пользовательский код над входными данными;
- **[Sort]({{ docs_root }}/core/reference/data-processing/operations/sort)** — сортирует указанную на входе таблицу;
- **[Merge]({{ docs_root }}/core/reference/data-processing/operations/merge)** — выполняет слияние таблиц;
- **[Erase]({{ docs_root }}/core/reference/data-processing/operations/erase)** — удаляет из таблицы указанный диапазон данных;
- **[RemoteCopy]({{ docs_root }}/core/reference/data-processing/operations/remote-copy)** — копирует данные между кластерами;
- **[Vanilla]({{ docs_root }}/core/reference/data-processing/operations/vanilla)** — запускает набор **пользовательских скриптов** в нужном количестве на узлах кластера и поддерживает их жизнедеятельность.

У всех операций есть специальный параметр `spec`, в котором передаётся описание настроек операции, так называемая **спецификация**. Спецификация представляет собой иерархическую структуру, которая задана в [формате YSON]({{ docs_root }}/core/reference/storage/formats#yson). У каждого типа операции своя структура спецификации, её подробное описание можно найти в подразделах, посвященных конкретным типам операций.

**Пользовательский скрипт** — произвольная строка, которая будет выполнена посредством вызова `bash -c`, и набор параметров, описывающих детали выполнения. Работа пользовательского скрипта происходит в окружении, которое называется `sandbox`. Пользовательский скрипт выполняется от имени служебного пользователя, которого формально нет в системе (речь идет о пользователях системы Linux), и у которого есть права только на чтение/запись в текущую директорию и в директорию `tmp`. Подробное описание параметров пользовательского скрипта приведено в разделе [Настройки операций]({{ docs_root }}/core/reference/data-processing/operations/operations-options#user_script_options).

Операции запускаются в **вычислительных пулах**. Пулы организованы в древовидную структуру и позволяют распределять вычислительные ресурсы кластера. Подробнее можно прочитать в разделе [Планировщик и пулы]({{ docs_root }}/core/concepts/data-processing/scheduler/scheduler-and-pools).
...
- `pending` — операция находится в очереди на запуск. В каждом [пуле]({{ docs_root }}/core/concepts/data-processing/scheduler/scheduler-and-pools) есть ограничение на количество выполняющихся операций, и когда оно превышено, операции выстраиваются в очередь на запуск;
...
- `reviving` — операция восстанавливается после перезапуска [планировщика]({{ docs_root }}/core/concepts/data-processing/scheduler/scheduler-and-pools);
...
- `aborted` — джоб был прерван. Как правило, такое состояние является следствием **вытеснения** (preemption) или сетевых ошибок. Подробнее о вытеснении в разделе [Планировщик и пулы]({{ docs_root }}/core/concepts/data-processing/scheduler/scheduler-and-pools);
- `failed` — джоб завершился с ошибкой. Как правило, такое состояние является следствием ошибок в пользовательском скрипте. В таком случае, у данного джоба будет лог с ошибками (stderr), который можно прочитать. Также данные джобы могут появляться, если закончилось место у используемого [аккаунта]({{ docs_root }}/core/concepts/storage/cypress), и джоб не может записать результат своей работы;
...
*  загрузка файлов в [Файловый кеш]({{ docs_root }}/core/reference/storage/file-cache);
...
3. Существует ограничение на количество джобов операций. Для [Map]({{ docs_root }}/core/reference/data-processing/operations/map), [Merge]({{ docs_root }}/core/reference/data-processing/operations/merge), [Reduce]({{ docs_root }}/core/reference/data-processing/operations/reduce), [RemoteCopy]({{ docs_root }}/core/reference/data-processing/operations/remote-copy) оно составляет 100 000 джобов. В случае [Reduce]({{ docs_root }}/core/reference/data-processing/operations/reduce) и сортированного [Merge]({{ docs_root }}/core/reference/data-processing/operations/merge) сформировать столько джобов может не получиться, так как у планировщика не будет достаточного количества семплов. Для операций с shuffle-стадией ([Sort]({{ docs_root }}/core/reference/data-processing/operations/sort) и [MapReduce]({{ docs_root }}/core/reference/data-processing/operations/mapreduce)) существует ограничение в 5 000 партиций. Подробнее о партициях можно узнать в соответствующих разделах, посвящённых конкретным типам операций;
...
Чтение данных с другого кластера (удалённое чтение) подчиняется ограничениям на межкластерную сетевую полосу. Подробнее см. в разделе [Квотирование межкластерной сетевой полосы]({{ docs_root }}/core/reference/data-processing/operations/cluster-throttlers).
