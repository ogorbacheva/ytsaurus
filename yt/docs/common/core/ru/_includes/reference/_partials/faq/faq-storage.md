...
**A:** Можно, примеры и ограничения можно найти в разделе [Схема таблиц]({{ docs_root }}/core/reference/storage/static-schema#create).
...
Сразу после выставления атрибута новые данные будут записываться на SSD, старые данные будут переложены в фоне. Подробнее о том, как влиять на данный процесс и отслеживать его динамику, в разделе [статические таблицы]({{ docs_root }}/core/concepts/storage/static-tables#medium).

------

#### **Q: Что делать, если чтение таблицы тормозит?** {#slow-table-read}

**A:** Про это есть [отдельная страница]({{ docs_root }}/core/how-to-guides/problems/slow-read).

------

#### **Q: Как уменьшить число используемых чанков в своей квоте?** {#reduce-chunk-usage}

**A:** Если эти чанки занимают таблицы (а это типичный случай), то следует запустить операцию [Merge]({{ docs_root }}/core/reference/data-processing/operations/merge) с параметром `combine_chunks = %true`.
...
**A:** Формат `YAMRED_DSV` больше не поддерживается. Стоит воспользоваться другим [форматом]({{ docs_root }}/core/reference/storage/formats#formaty-predstavleniya-tablichnyh-dannyh).