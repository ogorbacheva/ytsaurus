...
Для работы с [типом данных {{product-name}}]({{ docs_root }}/core/reference/storage/data-types) из первой колонки можно использовать соответствующий protobuf-тип из второй колонки в таблице ниже.
...
- у таблицы была задана [схема]({{ docs_root }}/core/reference/storage/static-schema);
- соответствующая колонка (в примере: `ColumnName`) имела {{product-name}}-тип [struct]({{ docs_root }}/core/reference/storage/data-types#schema_struct);
...
- у таблицы была задана [схема]({{ docs_root }}/core/reference/storage/static-schema);
- соответствующая колонка (в примере: `ListColumn`) имела {{product-name}}-тип [list]({{ docs_root }}/core/reference/storage/data-types#schema_list);
...
По умолчанию поля внутри `oneof`-группы соответствуют {{product-name}}-типу [variant]({{ docs_root }}/core/reference/storage/data-types#schema_variant). Например, сообщение ниже будет соответствовать структуре с полями `x` типа `int64` и `my_oneof` типа `variant<y: string, z: bool>`:
