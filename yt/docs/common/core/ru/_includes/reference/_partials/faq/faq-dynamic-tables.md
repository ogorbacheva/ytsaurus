...
**A:** В запросе участвует динамическая таблица, когда-то сконвертированная из статической. При этом не был указан параметр `block_size`. При получении подобной ошибки убедитесь, что выполняете все указания из [раздела]({{ docs_root }}/core/how-to-guides/dynamic-tables/mapreduce#convert_table), посвящённого конвертации статической таблицы в динамическую. Если размер блока получается большим, то следует увеличить `max_unversioned_block_size` до 32 мегабайт и перемонтировать таблицу. Так может быть, если в ячейках таблицы хранятся больше бинарные данные, а они целиком попадают в один блок.
...
**A:** Ошибка обычно возникает из‑за некорректных настроек при конвертации статической таблицы в динамическую. Убедитесь, что вы выполнили все указания из [раздела о конвертации]({{ docs_root }}/core/how-to-guides/dynamic-tables/mapreduce#convert_table).
...
**A:** Слишком большой таблет. Необходимо добиться, того чтобы у таблицы стало больше таблетов. Обратите внимание, что [автошардирование]({{ docs_root }}/core/how-to-guides/dynamic-tables/tablet-balancing) ограничивает число таблетов числом селлов, умноженном на значение параметра `tablet_balancer_config/tablet_to_cell_ratio`.
...
Таблеты могут перемещаться в другие селлы в результате [автоматического шардирования]({{ docs_root }}/core/how-to-guides/dynamic-tables/tablet-balancing). Такие перемещения можно увидеть на графике "Tablet balancer moves" на странице бандла. При необходимости вы можете изменить расписание автоматического шардирования. В этом случае нужно подобрать такое расписание, чтобы таблица продолжила держать вашу нагрузку.
...
1. В секции запроса `WHERE` указан префикс первичного ключа таблицы. Если это не сделано, то [отсечение входных данных]({{ docs_root }}/core/reference/dynamic-tables/dyn-query-language#input_data_cut) выполнить не получится, и запрос не получится эффективно выполнить (запрос будет выполняться, как full scan);
2. Если в запросе есть `JOIN`, убедитесь, что он выполняется [с использованием индекса]({{ docs_root }}/core/reference/dynamic-tables/dyn-query-language#joins).

Необходимо переписать запрос так, чтобы он работал эффективно. Для отладки запроса можно воспользоваться командой [explain]({{ docs_root }}/core/reference/dynamic-tables/dyn-query-language#explain_query).
...
**A:** Реплицированную таблицу нельзя очистить атомарно, но вы можете очистить отдельные реплики с помощью операции [Erase]({{ docs_root }}/core/reference/data-processing/operations/erase). Для этого понадобится [включить]({{ docs_root }}/core/how-to-guides/dynamic-tables/bulk-insert) так называемый «bulk insert» для всего кластера, выставив:
...
Если вам не требуется строгая консистентность, вы можете [ослабить]({{ docs_root }}/core/concepts/dynamic-tables/sorted-dynamic-tables#chtenie-stroki) уровень изоляции читающей транзакции.
...
В некоторых сценариях может помочь ослабление [гарантий]({{ docs_root }}/core/concepts/dynamic-tables/transactions#conflicts): использование режимов `atomicity=none` или shared write lock. Перед тем как пользоваться ослабленными гарантиями, обязательно изучите документацию и убедитесь, что побочные эффекты для вас не страшны.
...
Подробнее про настройки компакшна см. в разделе [Удаление старых данных]({{ docs_root }}/core/concepts/dynamic-tables/sorted-dynamic-tables#remove_old_data).
...
Чтобы убрать chunk view из таблицы необходимо сделать специальную форсированную компактификацию, выставив в аттрибут `forced_chunk_view_compaction_revision` значение 1, после чего выполнить `remount-table`. Более подробно см. в разделе [Форсированная компактификация]({{ docs_root }}/core/how-to-guides/dynamic-tables/compaction#forced_compaction).
