...
- Первая строка представляет [vanilla]({{ docs_root }}/core/reference/data-processing/operations/vanilla)-операцию с примером HTTP-сервера.
...
- Третья и четвёртая строки соответствуют [standalone]({{ docs_root }}/user-guide/data-processing/spyt/cluster/cluster-start)-кластеру SPYT, запущенному с History Server.
- Пятая строка относится к SPYT [direct submit]({{ docs_root }}/user-guide/data-processing/spyt/direct-submit/desc).
