...
В момент вычисления предиката игнорируются ограничения доступа к колонкам (см. [Управление доступом к колонкам таблиц]({{ docs_root }}/core/reference/storage/columnar-acl)). Поскольку доступом управляет владелец данных, это не является уязвимостью как таковой, однако может привести к утечке данных на основе различных метаданных, поэтому необходимо проявлять аккуратность в проектировании предикатов.
