...
Один из распространённых сценариев обработки данных — фильтрация большой таблицы из множества чанков для получения небольшого результата с помощью операции [Map]({{ docs_root }}/core/reference/data-processing/operations/map) без специальных флагов (Unordered Map). В силу устройства данного типа операций, фильтрация таблицы из `n` чанков в худшем случае может привести к `n` выходным чанкам вне зависимости от того, насколько мелкие чанки получаются в результате. Подобное поведение сказывается на потреблении чанковой квоты пользовательского аккаунта. Подробнее о квотировании можно узнать в разделе [Квоты]({{ docs_root }}/core/reference/storage/quotas).

Итоговые чанки можно объединить с помощью операции [Merge]({{ docs_root }}/core/reference/data-processing/operations/merge), но такой способ неудобен, так как требует запуска дополнительной операции, а также не спасает от того, что в пике требуется запас `n` чанков квоты, что для больших таблиц может составлять сотни тысяч чанков, тогда как чанковая квота является довольно дорогим ресурсом. Поэтому в {{product-name}} реализовано автоматическое слияние чанков на выходе из операции, которое действует «на опережение», объединяя выходные чанки до того, как их накопится достаточно много, чтобы израсходовать квоту.

Автоматическое слияние работает со следующими ограничениями:

* Доступно только для операций Unordered [Map]({{ docs_root }}/core/reference/data-processing/operations/map), [Sorted Reduce]({{ docs_root }}/core/reference/data-processing/operations/reduce).
...
* Недоступно, если задан параметр `row_count_limit`. Подробнее о данном параметре можно узнать в разделе [Настройки операций]({{ docs_root }}/core/reference/data-processing/operations/operations-options).

В случае, если операция попадает под указанные ограничения, пользователю необходимо самостоятельно запускать операцию [Merge]({{ docs_root }}/core/reference/data-processing/operations/merge).
