...
Коллективы джобов — это не то же самое, что [Gang-операции]({{ docs_root }}/core/reference/data-processing/operations/vanilla#gang-operations). Gang-операции перезапускают **все** джобы операции при перезапуске любого нерезервного джоба, тогда как коллективы джобов перезапускают только джобы внутри **одного** коллектива (который обрабатывает отдельную порцию данных).
...
[Gang-операции]({{ docs_root }}/core/reference/data-processing/operations/vanilla#gang-operations) — это разновидность Vanilla-операций, поэтому в случае, когда требуется запустить операцию другого типа, подойдут только операции с коллективом джобов.
...
- Gang-операции дают больше гарантий (которые, в частности, позволяют сделать перезапуск коллектива более оптимальным) (см. [Перезапуск Gang-операций]({{ docs_root }}/core/reference/data-processing/operations/vanilla#gang-operation-restart)).
- В отличие от Gang-операций, операции с коллективами джобов не поддерживают резервные джобы (см. [Резервные джобы в Gang-операциях]({{ docs_root }}/core/reference/data-processing/operations/vanilla#reserved-jobs)).
- Операции с коллективами джобов, в отличие от Gang-операций, могут иметь выходные таблицы (см. [Ограничения Gang-операций]({{ docs_root }}/core/reference/data-processing/operations/vanilla#gang-operation-limitations)).
...
|----------|-----------------------------------|
| [Map]({{ docs_root }}/core/reference/data-processing/operations/map) | В спецификации `mapper` |
| [Reduce]({{ docs_root }}/core/reference/data-processing/operations/reduce) | В спецификации `reducer` |
| [MapReduce]({{ docs_root }}/core/reference/data-processing/operations/mapreduce) | В спецификации `mapper`, `reducer` или `reduce_combiner` |
| [Vanilla]({{ docs_root }}/core/reference/data-processing/operations/vanilla) | В спецификации таска |
...
* **Несовместимость с gang_options**: Коллективы джобов нельзя использовать вместе с [Gang-операциями]({{ docs_root }}/core/reference/data-processing/operations/vanilla#gang-operations). При попытке указать оба параметра операция завершится с ошибкой.
...
* [Gang-операции]({{ docs_root }}/core/reference/data-processing/operations/vanilla#gang-operations)
* [Операция Map]({{ docs_root }}/core/reference/data-processing/operations/map)
* [Операция Reduce]({{ docs_root }}/core/reference/data-processing/operations/reduce)
* [Операция MapReduce]({{ docs_root }}/core/reference/data-processing/operations/mapreduce)
* [Операция Vanilla]({{ docs_root }}/core/reference/data-processing/operations/vanilla)
