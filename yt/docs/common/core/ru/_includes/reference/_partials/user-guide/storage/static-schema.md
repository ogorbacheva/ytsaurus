...
| `type_v3`    | `any`     | Альтернативное представление типа. Это поле позволяет задавать колонки со [сложными типами]({{ docs_root }}/core/reference/storage/data-types#schema). | Да, если не указан `type` |
...
| `expression` | `string`  | Выражение, определяющее значение поля вычислимой колонки. Только для ключевых колонок. Подробнее можно прочитать в разделе [Шардирование]({{ docs_root }}/core/how-to-guides/dynamic-tables/resharding). | Нет |
...
{{product-name}} поддерживает типы колонок, описанные в разделе [Типы данных]({{ docs_root }}/core/reference/storage/data-types).
...
Остальные ключи зависят от конкретного типа и описаны на странице [Типы данных]({{ docs_root }}/core/reference/storage/data-types).
...
- Производительность:
  - Если дополнительно включить поколоночный формат хранения [чанков]({{ docs_root }}/core/concepts/storage/chunks) `optimize_for = scan`, то данные будут храниться компактнее, а чтение подмножества колонок будет быстрее.
