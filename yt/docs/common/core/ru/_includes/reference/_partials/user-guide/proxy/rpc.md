...
Для работы через Python необходимо установить пакет с биндингами, pypi-пакет называется {{ core-user-guide-reference-proxy-rpc-md-audience-1 }}. После этого достаточно настроить клиент, указав тип бэкенда - `rpc` и, если нужно, указав подключение по ipv4.

{{ core-user-guide-reference-proxy-rpc-md-audience-2 }}
