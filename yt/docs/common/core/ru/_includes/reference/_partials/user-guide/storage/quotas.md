...
* Хранение:
  ...
  * квота на объем оперативной памяти мастер-серверов, потребляемый узлами (nodes) в дереве метаинформации;  отдельно не заказывается, выдаётся по договорённости. Учитываются метаданные чанков и неответвленных узлов, атрибуты неответвленных узлов, pivot-ключи [динамических таблиц]({{ docs_root }}/core/concepts/dynamic-tables/overview), а также некоторые другие данные. Эмпирическая оценка: 100-200 байт на узел и столько же на чанк. Указанные величины могут существенно варьироваться в зависимости от длины атрибутов и имён узлов, длины значений в ключевых колонках чанков и т.д.;
  * квота на количество [таблетов (шардов)]({{ docs_root }}/core/concepts/dynamic-tables/overview#tablets) динамических таблиц. Отдельно не заказывается, выдаётся по запросу. Квота выделяется на объект [таблет-селл бандл (tablet_cell_bundle)]({{ docs_root }}/core/concepts/dynamic-tables/concepts#tablet_cell_bundles)
* Обработка:
  ...
  * выделенные вычислительные мощности для выполнения запросов к [динамическим таблицам]({{ docs_root }}/core/concepts/dynamic-tables/overview) (write/select/lookup) — CPU, RAM. Сюда входит квота на объём данных в динамических таблицах, поднятых в память (tablet_static_memory). Квоты выделяются на объект [таблет-селл бандл (tablet_cell_bundle)]({{ docs_root }}/core/concepts/dynamic-tables/concepts#tablet_cell_bundles).
...
Подробнее о медиумах можно прочитать в разделе [Медиумы]({{ docs_root }}/core/reference/storage/media).
...
Вычислительные мощности для обработки данных привязаны к [пулам]({{ docs_root }}/core/concepts/data-processing/scheduler/scheduler-and-pools) (кроме tablet_cell_bundle).
