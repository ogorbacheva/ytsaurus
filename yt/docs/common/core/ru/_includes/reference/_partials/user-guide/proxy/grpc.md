...
gRPC запросы обслуживают [RPC-прокси]({{ docs_root }}/core/reference/proxy/rpc) {{product-name}}. Как следует из названия прокси (RPC) — от клиента сервер ожидает [protobuf](https://developers.google.com/protocol-buffers/) сообщение. Ответом сервера также будет сообщение. Каждая команда {{product-name}}, такая как «создать узел в Кипарисе», «записать строки» представлена в виде пары сообщений (с префиксами TReq и TRsp для запроса и ответа соответственно). Пример: `TReqCreateNode, TRspCreateNode` —  создание узла в Кипарисе. Сообщения и их описания можно посмотреть в proto файле.
