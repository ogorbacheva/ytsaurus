...
Скорость передачи данных между кластерами квотируется. Подробнее см. в разделе [Квотирование межкластерной сетевой полосы]({{ docs_root }}/core/reference/data-processing/operations/cluster-throttlers).

{% endnote %}

Общие параметры для всех типов операций описаны в разделе [Настройки операций]({{ docs_root }}/core/reference/data-processing/operations/operations-options).
...
* `schema_inference_mode` (auto) — режим определения схемы. Доступные значения: `auto`, `from_input`, `from_output`. Подробности в разделе [Схема данных]({{ docs_root }}/core/reference/storage/static-schema#schema_inference).
...
Операция RemoteCopy (`remote_copy`) поддержана только для [сортированных таблиц]({{ docs_root }}/core/concepts/dynamic-tables/sorted-dynamic-tables). Для копирования упорядоченной таблицы следует сконвертировать её в статическую. Скопировать на другой кластер упорядоченную таблицу с сохранением таблетов невозможно.
