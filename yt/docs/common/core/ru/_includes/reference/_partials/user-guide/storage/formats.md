...
Структурированные данные — дерево узлов с атрибутами, в котором узлы соответствуют [YSON-типам]({{ docs_root }}/core/reference/storage/yson) (map, list, простые типы) с атрибутами.
Структурированные данные можно получить при помощи команды [`get`]({{ docs_root }}/core/reference/api/commands#get).

Поддерживаемые форматы: `json`, `yson`, `yaml`.

Пример: узлы [Кипариса]({{ docs_root }}/core/concepts/storage/cypress), их атрибуты.
...
Примеры: входные и выходные данные операций, входные данные команды [`write_table`]({{ docs_root }}/core/reference/api/commands#write_table), выходные данные команды [`read_table`]({{ docs_root }}/core/reference/api/commands#read_table).
...
Примеры: входные данные команды [`upload`]({{ docs_root }}/core/reference/api/commands#write_file) и выходные данные команды [`download`]({{ docs_root }}/core/reference/api/commands#read_file).
...
Для всех команд [утилиты `yt`]({{ docs_root }}/core/reference/api/cli/cli), которые возвращают или принимают данные, необходимо указывать формат.
...
Подробная спецификация в разделе [YSON]({{ docs_root }}/core/reference/storage/yson).
...
- **complex_type_mode** (`named`) — режим представления композитных типов, структур и вариантов,
  возможные значения `named` или `positional`, подробнее в [разделе]({{ docs_root }}/core/reference/storage/data-types#yson).
- **string_keyed_dict_mode** (`positional`) - режим представления словарей со строковыми ключами, возможные значения `named` или `positional`, подробнее в [специальном разделе]({{ docs_root }}/core/reference/storage/data-types#yson).
- **decimal_mode** (`binary`) — режим представления типа `decimal`,
  возможные значения `text`, `binary`, подробнее в [разделе]({{ docs_root }}/core/reference/storage/data-types#yson).
- **time_mode** (`binary`) — режим представления типов `date`, `datetime`, `timestamp`, возможные значения `text` или `binary`, подробнее в [разделе]({{ docs_root }}/core/reference/storage/data-types#yson).
- **uuid_mode** (`binary`) — режим представления типа `uuid`, возможные значения `binary`, `text_yql` или `text_yt` подробнее в [разделе]({{ docs_root }}/core/reference/storage/data-types#yson).
...
Чтобы преобразовать YSON-строку в JSON-строку, нужно перевести каждый байт в unicode-символ с соответствующим номером и закодировать в UTF-8. Такое преобразование происходит, например, при чтении табличных данных командой [`read_table`]({{ docs_root }}/core/reference/api/commands#read_table).

При преобразовании JSON-строки в YSON-строку происходит проверка последовательности unicode-символов JSON-строки: каждый из них должен находиться в диапазоне от 0 до 255. Затем они конвертируются в соответствующие байты. Такое преобразование выполняется при записи табличных данных командой [`write_table`]({{ docs_root }}/core/reference/api/commands#write_table).
...
При преобразовании JSON-строки в YSON-строку выполняется UTF8-кодирование unicode-символов в байтовые последовательности. Такое преобразование выполняется командой [`write_table`]({{ docs_root }}/core/reference/api/commands#write_table).
...
- Представление сложных типов зависит от значения параметра **enable_complex_types**:
  - при значении `false` cложные типы представляются в виде [arrow::binary](https://arrow.apache.org/docs/cpp/api/datatype.html#_CPPv4N5arrow4Type4type6BINARYE), где хранится [YSON-представление]({{ docs_root }}/core/reference/storage/data-types#yson) этих типов.
...
Подробнее можно прочитать в разделе [Protobuf]({{ docs_root }}/core/reference/api/cpp/protobuf).
