...
Все типы квот для хранения данных привязаны к **аккаунту** проекта. Аккаунт — это служебный объект в [Кипарисе]({{ docs_root }}/core/concepts/storage/cypress). Все аккаунты имеют уникальные имена. При этом они находятся в отдельном пространстве имён, так что имя пользователя и аккаунта могут совпадать. Имя аккаунта хранится в атрибуте `name`, доступном как на чтение, так и на запись.
