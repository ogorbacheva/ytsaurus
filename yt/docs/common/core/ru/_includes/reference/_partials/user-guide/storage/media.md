...
- Ключ — `"data_parts_only"`, значение — флаг. На медиумах с `"data_parts_only" = %true` [erasure-coded]({{ docs_root }}/core/reference/storage/replication#erasure) объекты хранят только data-парты и не хранят erasure-парты. Например:
...
[Кипарис]({{ docs_root }}/core/concepts/storage/cypress) позволяет читать отдельные записи в словарях непосредственно, не считывая весь словарь целиком.
...
Атрибуты `primary_medium` и `media` являются [наследуемыми]({{ docs_root }}/core/reference/storage/attributes#system_attr). Это означает, что их можно устанавливать на директории, и тогда все создаваемые в поддереве владельцы чанков будут получать соответствующие значения атрибутов автоматически. Использование атрибута `primary_medium` является предпочтительным в большинстве случаев; использовать атрибут `media` таким образом не рекомендуется.
