...
Для наибольшей полноты графиков/статистик рекомендуется добавлять в схему таблицы колонки [timestamp]({{ docs_root }}/core/concepts/dynamic-tables/ordered-dynamic-tables#timestamp_column) и [cumulative_data_weight]({{ docs_root }}/core/concepts/dynamic-tables/ordered-dynamic-tables#cumulative_data_weight_column).
...
Оба Cypress аргумента обеих команд имеют тип [rich YPath]({{ docs_root }}/core/reference/storage/ypath#rich_ypath) и поддерживают указание имени кластера. В ином случае используется кластер, на котором выполняется команда.
...
Параметр очереди в данном методе представляет из себя [rich YPath]({{ docs_root }}/core/reference/storage/ypath#rich_ypath) и поддерживает указание кластера, на котором она расположена. В ином случае используется кластер, на котором выполняется команда.
