...
Решение — разбить прокси на группы с помощью [ролей]({{ docs_root }}/core/how-to-guides/cluster-access-proxy/overview#proxy-role). Это позволит разделить нагрузку между лёгкими и тяжёлыми запросами, а также изолировать ресурсы разных проектов и команд.
...
- Вы настраиваете Ingress/DNS на сервис группы `control`. Клиенты будут автоматически получать адреса Data-прокси через механизм [Discovery]({{ docs_root }}/core/how-to-guides/cluster-access-proxy/overview#discovery) и отправлять тяжёлые запросы напрямую на них.
