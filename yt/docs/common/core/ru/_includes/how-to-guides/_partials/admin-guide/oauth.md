...
В веб-интерфейсе {{product-name}} реализовано два типа аутентификации пользователей: по паролю и через [SSO](https://en.wikipedia.org/wiki/Single_sign-on). Как работать с паролями — написано в [Руководстве пользователя]({{ docs_root }}/core/reference/storage/auth). В данной статье описано, как сконфигурировать SSO.
