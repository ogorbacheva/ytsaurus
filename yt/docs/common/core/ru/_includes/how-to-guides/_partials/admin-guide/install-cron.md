...
* специальный пользователь-робот `robot-cron` с выписанным для него токеном (см. раздел [Управление токенами]({{ docs_root }}/core/reference/storage/auth#token-management)):
...
- Включенной [загрузки мастерных снапшотов в Кипарис]({{ docs_root }}/core/how-to-guides/persistence-uploader#uploading-snapshots-to-cypress).
...
Инструкция по обновлению Cron подробно описана в статье [Обновление cron]({{ docs_root }}/core/how-to-guides/update/update-cron).
