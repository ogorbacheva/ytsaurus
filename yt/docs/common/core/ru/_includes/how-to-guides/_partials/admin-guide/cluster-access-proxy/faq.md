...
Технически — можно, но для высоконагруженных инсталляций это не рекомендуется. Если настроить клиенты на работу без [Discovery]({{ docs_root }}/core/how-to-guides/cluster-access-proxy/overview#discovery) (только через Ingress), возникнут три проблемы:
...
- Проблемы с транзакциями. Необходима настройка [липких сессий]({{ docs_root }}/core/how-to-guides/cluster-access-proxy/manage-traffic#sticky-sessions).
...
Настройте подмену адресов через атрибут `//sys/http_proxies/@balancers` (см. раздел [Настройка подмены адресов]({{ docs_root }}/core/how-to-guides/cluster-access-proxy/network-isolation#set-discovery-step)).
