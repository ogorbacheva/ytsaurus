...
4. Включена [загрузка снапшотов]({{ docs_root }}/core/how-to-guides/persistence-uploader#uploading-snapshots-to-cypress) и [access-логов]({{ docs_root }}/core/how-to-guides/logging#structured_log_delivery) мастера в Кипарис.
...
5. Установлен [Cron Helm-чарт]({{ docs_root }}/core/how-to-guides/install-cron#process_master_snapshot), в котором включена задача `process_master_snapshot`. Эта задача должна отработать хотя бы один раз и создать необходимые директории и таблицы, а именно: `//sys/admin/snapshots/snapshot_exports` и `//sys/admin/snapshots/user_exports`.
...
Создайте пользователей [согласно инструкции]({{ docs_root }}/core/reference/storage/auth):
