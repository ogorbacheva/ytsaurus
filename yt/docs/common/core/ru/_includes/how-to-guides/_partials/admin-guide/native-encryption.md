...
В отличие от других компонентов, CHYT запускается внутри [vanilla-операции]({{ docs_root }}/core/reference/data-processing/operations/vanilla) {{product-name}}, поэтому сертификаты передаются особым образом:
