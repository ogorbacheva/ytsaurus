...
|`data/output/K/erasure_disk_space`| Заполняется только для таблиц с `erasure_codec != none`. Помимо `compressed_data_size` включает объем [parity]({{ docs_root }}/_includes/user-guide/problems/*parity) блоков. Подробнее про erasure-кодеки можно прочитать в [разделе]({{ docs_root }}/core/reference/storage/replication). В байтах. |
...
|`user_job/memory_reserve` | Объем памяти, гарантированный пользовательскому джобу в момент запуска. Когда фактическое потребление становится больше (но меньше `memory_limit`), джоб может быть прерван с пометкой `resource_overdraft`, если на узле кластера недостаточно памяти. Данную величину можно регулировать с помощью [опции]({{ docs_root }}/core/reference/data-processing/operations/operations-options) `memory_reserve_factor` в спецификации операции. В байтах. |
...
Простой случай: если `user_job/cpu/user ~= time/exec`, это означает, что причина медленной работы джоба в пользовательском коде. Следует подключиться к джобу с помощью [job shell]({{ docs_root }}/core/how-to-guides/problems/jobshell-and-slowjobs) и искать узкое место с помощью perf и gdb.
