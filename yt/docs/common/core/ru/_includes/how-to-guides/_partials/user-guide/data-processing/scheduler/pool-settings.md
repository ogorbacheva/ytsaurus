...
- `node_tag_filter` — (прежнее название `nodes_filter`) фильтр для выбора узлов кластера, принадлежащих дереву (подробнее читайте в разделе [Тегирование нод кластера]({{ docs_root }}/core/how-to-guides/node-tags));
...
- `integral_guarantees` — настройки управления [интегральными гарантиями]({{ docs_root }}/core/concepts/data-processing/scheduler/integral-guarantees) в данном дереве;
...
* `Starvation status` — флаг, указывающий, страдает ли операция. При наличии страдающей операции планировщик попытается прервать джобы нестрадающих операций (выполнить [вытеснение]({{ docs_root }}/core/concepts/data-processing/scheduler/preemption)), чтобы запустить аллокации страдающей операции;
