...
В данном разделе собраны наиболее популярные примеры по работе с [Кипарисом]({{ docs_root }}/core/concepts/storage/cypress) из командной строки (CLI), более подробное описание команд и их параметров можно найти в разделе [Команды]({{ docs_root }}/core/reference/api/commands).
...
Опция `--format` определяет формат выходных данных. Поддерживаются форматы `json` и `yson`. Подробнее можно прочитать в разделе [Форматы]({{ docs_root }}/core/reference/storage/formats). Опцию `--format` нельзя указывать вместе с опцией `-l`.
...
Данная команда создает узел указанного типа `<type>` по пути `<path>`. Команда возвращает идентификатор созданного объекта. Поддерживаемые объекты перечислены в разделе [Объекты]({{ docs_root }}/core/concepts/storage/objects).
...
Опцию `-l, --lock-existing` можно указывать вместе с `--ignore-existing`. Тогда на указанный узел будет взята [exclusive-блокировка]({{ docs_root }}/core/concepts/storage/transactions#locks), даже если он уже существует. (В случае конфликта блокировки команда завершится ошибкой.) Т.к. фактическое создание узла всегда неявно (автоматически) берет на него блокировку, опция позволяет сделать эффект команды при использовании `--ignore-existing` более предсказуемым.
...
Опция `--format` определяет формат выходных данных. Поддерживаются форматы `json` и `yson`. Подробнее можно прочитать в разделе [Форматы]({{ docs_root }}/core/reference/storage/formats).
...
Команда `set` присваивает указанное значение `value` в формате [YSON]({{ docs_root }}/core/reference/storage/yson) по указанному пути `<path>`.
...
Опция `--format` определяет формат значения `value`. Поддерживаются форматы `json` и `yson`. Подробнее можно прочитать в разделе [Форматы]({{ docs_root }}/core/reference/storage/formats).
...
Опция `--preserve-expiration-time` позволяет сохранить назначенное [время удаления]({{ docs_root }}/core/concepts/storage/cypress) копируемого узла.

Опция `--preserve-expiration-timeout` позволяет сохранить назначенный [интервал удаления]({{ docs_root }}/core/concepts/storage/cypress) копируемого узла.

Опция `--preserve-creation-time` позволяет сохранить [время создания]({{ docs_root }}/core/concepts/storage/cypress) копируемого узла. **Только для копирования.**
...
Данная команда создает [символическую ссылку]({{ docs_root }}/core/concepts/storage/links) по пути `<link_path>` на объект, находящийся по пути `<target_path>`.
...
Опция `-l, --lock-existing` гарантирует, что на ссылку будет установлена [exclusive-блокировка]({{ docs_root }}/core/concepts/storage/transactions#locks), даже если ссылка уже существует. (Имеет смысл только вместе с `--ignore-existing`; в случае невозможности установить блокировку команда завершится ошибкой).
...
Время работы данного запроса прямо пропорционально количеству чанков, из которых (суммарно) состоят входные узлы.  При этом во многих клиентских библиотеках существует ограничение на максимальное время работы команд, обычно это десятки секунд. Поэтому использовать concatenate стоит только в случае, если вы уверены, что по выходным путям мало чанков, порядка 10 000. В остальных случаях следует использовать операцию [Merge]({{ docs_root }}/core/reference/data-processing/operations/merge). Она не будет работать эффективнее (и на таблицах в десятки миллионы чанков может занимать многие минуты), но для клиента отслеживание ее работы будет осуществляться через периодический опрос статуса, что фактически снимает все ограничения по времени выполнения.
