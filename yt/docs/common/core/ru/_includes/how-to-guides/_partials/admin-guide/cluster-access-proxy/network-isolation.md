...
Если ваша сетевая инфраструктура позволяет обращаться к подам напрямую (например, используется CNI Calico с маршрутизацией BGP или AWS VPC CNI), дополнительная настройка {{product-name}} не требуется. Механизм [Discovery]({{ docs_root }}/core/how-to-guides/cluster-access-proxy/overview#discovery) вернёт внутренние FQDN подов (например, `hp-0.http-proxies.default.svc.cluster.local`).
...
Чтобы включить режим, добавьте поле `hostNetwork: true` в корневой уровень спецификации [оператора]({{ docs_root }}/core/how-to-guides/install-ytsaurus#operator):
...
Этот способ наиболее универсален для облачных сред. Прокси публикуются через сервисы (NodePort или LoadBalancer), а {{product-name}} конфигурируется так, чтобы механизм [Discovery]({{ docs_root }}/core/how-to-guides/cluster-access-proxy/overview#discovery) возвращал клиентам именно эти внешние адреса, а не внутренние имена (FQDN) подов.
