...
Task-прокси предоставляет доступ к веб-сервисам, развёрнутым в операциях {{product-name}}. Подробнее можно почитать в разделе [Task-прокси]({{ docs_root }}/core/reference/proxy/task).
...
- специальный пользователь-робот `robot-task-proxy` с выписанным для него токеном (см. раздел [Управление токенами]({{ docs_root }}/core/reference/storage/auth#token-management)):
   ```sh
...
Инструкция по обновлению Task-прокси подробно описана в статье [Обновление Task-прокси]({{ docs_root }}/core/how-to-guides/update/update-task-proxy).
