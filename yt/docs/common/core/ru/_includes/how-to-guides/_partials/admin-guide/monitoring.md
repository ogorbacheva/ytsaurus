...
Подробнее об установке и настройке Odin можно прочитать в разделе [Установка Odin]({{ docs_root }}/core/how-to-guides/install-odin).
...
Инструкции по настройке дашбордов в UI находятся в разделе ["настройки дашбордов {{product-name}} UI"]({{ docs_root }}/core/how-to-guides/install-monitoring#setup-ui).
...
Подробнее о настройке Grafana и интеграции с {{product-name}} читайте в соответствующем разделе [настройки Grafana]({{ docs_root }}/core/how-to-guides/install-monitoring#setup-grafana).
...
- `lost_vital_chunks` — проверяет, что счетчик lost_vital_chunks равен нулю из `//sys/lost_vital_chunks/@count`. Подробнее про vitality можно прочитать [по ссылке]({{ docs_root }}/core/concepts/storage/chunks#vitality);
...
- `yt_chunk_server_underreplicated_chunk_count` — число чанков, имеющих на дисках кластера меньше реплик, чем указано в атрибуте replication_factor. Постоянный рост underreplicated может говорить о том, что настройки [media]({{ docs_root }}/core/reference/storage/media) не позволяют выполнять условия replication_factor в текущей конфигурации кластера. Число underreplicated чанков можно посмотреть в //sys/underreplicated_chunks/@count;
...
Важно отслеживать [квоты]({{ docs_root }}/core/reference/storage/quotas) системных аккаунтов, таких как sys, tmp, intermediate, а также пользовательские квоты продуктовых процессов. Пример для аккаунта sys:
