...
8. Если у вас установлен Odin, проверьте, собираются ли его метрики:
    ...
    Сбор качественных метрик [Odin]({{ docs_root }}/core/how-to-guides/install-odin) осуществляется через отдельный `ServiceMonitor`, создаваемый самим чартом Odin.
    ...
    Если он отсутствует - в [настройках чарта]({{ docs_root }}/core/how-to-guides/install-odin#prepare-values) необходимо включить создание `ServiceMonitor`.
...
Для корректной работы мониторинга и интеграции с Grafana необходимо передать нужные адреса в UI {{product-name}}, установленный через [Helm-чарт]({{ docs_root }}/core/how-to-guides/install-ytsaurus#ui). При установке чарта мониторинга будет производиться автоматическая проверка наличия этих переменных.
