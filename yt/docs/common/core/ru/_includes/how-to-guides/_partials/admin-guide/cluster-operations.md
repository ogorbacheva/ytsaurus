...
Управление квотами для хранения данных осуществляется с помощью аккаунтов. Подробнее про аккаунты можно узнать [по ссылке]({{ docs_root }}/core/reference/storage/accounts).
...
У аккаунта есть [множество атрибутов]({{ docs_root }}/core/reference/storage/accounts#account_attributes), описывающих лимиты на ресурсы, их текущее использование и другие.
...
За распределение вычислительных ресурсов в кластере отвечает планировщик, но описание деревьев пулов и пулов (сущностей, которые хранят информацию о вычислительных квотах) хранится в Кипарисе. Рекомендуется прочитать [общую документацию]({{ docs_root }}/core/concepts/data-processing/scheduler/scheduler-and-pools) про устройство планировщика и [отдельную страницу про управление пулами]({{ docs_root }}/core/how-to-guides/data-processing/scheduler/manage-pools).
...
В модели системы поддержаны такие сущности, как пользователи и группы, а через атрибуты объекта можно управлять правами доступа. Подробнее о модели прав доступа рассказано в отдельном [разделе]({{ docs_root }}/core/concepts/storage/access-control).
...
| tags                    | list<string>      | Список [тегов]({{ docs_root }}/core/how-to-guides/node-tags) данной ноды                        |
...
| user_tags                 | list<string>     | Список дополнительных [тегов]({{ docs_root }}/core/how-to-guides/node-tags) ноды                |
