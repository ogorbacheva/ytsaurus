...
В данном разделе содержится информация о пакетной обработке запросов (batch-запросах), приведены примеры использования для [Python]({{ docs_root }}/core/how-to-guides/storage/batch-requests#python_api), [C++]({{ docs_root }}/core/how-to-guides/storage/batch-requests#c_plus_plus_api) и [Java]({{ docs_root }}/core/how-to-guides/storage/batch-requests#java_api) API.

При работе с системой {{product-name}} каждая команда порождает отдельный запрос к мастер-серверу. Запрос несет в себе накладные расходы, зачастую более высокие, чем непосредственно выполнение команды. Поэтому объединение нескольких команд в один запрос может существенно ускорить процессы, которые посылают много легких команд к [Кипарису]({{ docs_root }}/core/concepts/storage/cypress) и основную часть времени находятся в ожидании ответа. Такой запрос называется **Batch-запросом**.
...
Указать [формат]({{ docs_root }}/core/reference/storage/formats) возвращаемых данных отдельной команды через `batch_client` нельзя. Причина в том, что формат возвращаемых данных един для всего batch-запроса.
