...
Ниже описано, как выполнить обновление с использованием оператора версии **0.32.0** и новее. О различиях со старыми версиями смотрите в разделе [Переход с enableFullUpdate на updatePlan]({{ docs_root }}/core/how-to-guides/update/update_strategy#migration-from-enablefullupdate).
...
: По умолчанию кластер полностью недоступен на время обновления — все операции и запросы прерываются. Планируйте обновление на период минимальной нагрузки. Если кластер должен оставаться доступным, выберите другую стратегию — смотрите [Стратегии обновления]({{ docs_root }}/core/how-to-guides/update/update_strategy).
...
- **Командой CLI** — [yt admin remove-master-unrecognized-options]({{ docs_root }}/core/reference/cli-admin#remove-master-unrecognized-options). Этот способ поддерживает флаг `--dry` для предварительного просмотра изменений.
...
Подробнее о стратегии, доступности компонентов и рекомендуемом количестве инстансов — в разделе [Стратегия `RollingUpdate`]({{ docs_root }}/core/how-to-guides/update/update_strategy#configure-rolling).
...
Подробнее о предзагрузке — в разделе [Предзагрузка образов]({{ docs_root }}/core/how-to-guides/update/update_strategy#image-heater).

{% endcut %}

Описание всех полей `updatePlan` — в разделе [Настройка поля updatePlan]({{ docs_root }}/core/how-to-guides/update/update_strategy#updateplan).
