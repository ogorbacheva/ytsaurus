...
| `configOverrides`  | `optional<LocalObjectReference>` | Конфигмапа для переопределения генерируемых статических конфигов. Подробности см. в разделе [Переопределение конфигурации]({{ docs_root }}/core/reference/config-overrides). |
...
| `bootstrap` | BootstrapSpec | Настройки для первичного поднятия кластера, например, параметры [таблет-селл бандлов]({{ docs_root }}/core/concepts/dynamic-tables/concepts#tablet_cell_bundles) |
...
Подробнее про компоненты можно прочитать [в отдельном разделе]({{ docs_root }}/core/reference/components).
...
## Логирование
Корректная настройка логирования очень важна для диагностики проблем и при обращениях в поддержку. Рекомендации по настройке логирования собраны на отдельной [странице]({{ docs_root }}/core/how-to-guides/logging).

## Локации

Рекомендации по разметке дисков и конфигурации локаций собраны на отдельной [странице]({{ docs_root }}/core/how-to-guides/locations).
...
Оператор автоматически создает несколько [таблет-селл бандлов]({{ docs_root }}/core/concepts/dynamic-tables/concepts#tablet_cell_bundles) — `sys` и `default`.
