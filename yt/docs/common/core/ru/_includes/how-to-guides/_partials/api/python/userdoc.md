...
Пример можно найти в [разделе]({{ docs_root }}/core/how-to-guides/api/python/examples#gevent).
...
Конфигурация библиотеки выполняется одним из следующих способов:
  ...
  - Через файл, путь к которому указан в переменной окружения `YT_CONFIG_PATH`, по умолчанию `~/.yt/config`. Файл должен быть в формате [YSON]({{ docs_root }}/core/reference/storage/formats#yson) (с помощью переменной окружения `YT_CONFIG_FORMAT` можно изменить это поведение; поддерживаются форматы YSON и JSON). Пример содержания файла: `{proxy={url="<cluster_name>"}}`.
...
При использовании [CLI]({{ docs_root }}/core/reference/api/cli/cli) патч к конфигурации можно передать с помощью опции `--config`.
...
* `profiles` - словарь, задающий профили и соответствующие им настройки. Ключ словаря - это имя профиля, а значение — конфигурация профиля в стандратном [формате]({{ docs_root }}/core/how-to-guides/api/python/userdoc#configuration).
...
Команды в {{product-name}} делятся на легкие и тяжелые (посмотреть на список команд с точки зрения системы можно в разделе [Команды]({{ docs_root }}/core/reference/api/commands)).
...
2. Секция `concatenate_retries` отвечает за повторные запросы команды concatenate (см. [API]({{ docs_root }}/core/how-to-guides/api/python/start)) , которую нельзя назвать легкой, так как она может ходить к разным мастер-селлам, долго висеть, поэтому повторные запросы для легких команд ей не подходят.
...
- `code` (тип int) — HTTP код ошибки (смотрите [раздел]({{ docs_root }}/core/reference/api/error-codes)). Если не проставлен, то равен 1.
...
Подробнее про форматы можно прочитать [в разделе]({{ docs_root }}/core/reference/storage/formats).
...
Пути в {{product-name}} представляются в виде так называемого [YPath]({{ docs_root }}/core/reference/storage/ypath). В коде для этого используется класс [YPath](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.ypath.YPath), а также его наследники [TablePath](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.ypath.TablePath) и [FilePath](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.ypath.FilePath). В конструкторах последних двух классов можно указывать соответствующие YPath-атрибуты, например, `schema`, `start_index`, `end_index` для `TablePath` и `append` и `executable` для `FilePath`. Подробнее про `TablePath` можно прочитать в [разделе](#tablepath_class).
...
Пример использования с комментариями можно найти [в отдельном разделе]({{ docs_root }}/core/how-to-guides/api/python/examples#yson_string_proxy)

### Batch-запросы

Низкоуровневое API:

- [execute_batch](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.execute_batch) — принимает набор описаний запросов в виде списка и возвращает набор результатов. Простая обертка над [командой API]({{ docs_root }}/core/reference/api/commands#execute_batch).
...
Подробней про batch-запросы можно узнать в [tutorial]({{ docs_root }}/core/how-to-guides/storage/batch-requests).
...
Ряд опций в командах являются общими опциями для целых классов команд. Подробнее можно прочитать в разделе [Команды]({{ docs_root }}/core/reference/api/commands).

### Работа с Кипарисом { #cypress_commands }

- [get](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.get) — получить значение узла Кипариса. [Подробнее]({{ docs_root }}/core/how-to-guides/storage/cypress-example#get).

- [set](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.set) — записать значение в узел Кипариса. [Подробнее]({{ docs_root }}/core/how-to-guides/storage/cypress-example#set).

- [create](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.create) — создать пустой узел Кипариса типа `type` и атрибутами `attributes`. [Подробнее]({{ docs_root }}/core/how-to-guides/storage/cypress-example#create).

- [exists](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.exists) — проверить существование узла Кипариса. [Подробнее]({{ docs_root }}/core/how-to-guides/storage/cypress-example#exists).

- [remove](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.remove) — удалить узел Кипариса. [Подробнее]({{ docs_root }}/core/how-to-guides/storage/cypress-example#remove).

- [list](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.list) — получить список детей узла `path`. Опция `absolute` включает вывод абсолютных путей вместо относительных. [Подробнее]({{ docs_root }}/core/how-to-guides/storage/cypress-example#list).
...
- [copy](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.copy) и [move](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.move) — скопировать/переместить узел Кипариса. Подробнее про значение опций можно прочитать в [разделе]({{ docs_root }}/core/how-to-guides/storage/cypress-example#copy_move).

- [link](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.link) — сделать символьную ссылку на узел Кипариса. [Подробнее]({{ docs_root }}/core/how-to-guides/storage/cypress-example#link).
...
Для работы с вложенными атрибутами стоит использовать обычные глаголы Кипариса и навигацию с помощью [YPath]({{ docs_root }}/core/reference/storage/ypath).
...
Подробнее про файлы в Кипарисе можно прочитать в [разделе]({{ docs_root }}/core/concepts/storage/files).
...
- Файлы могут быть переданы в качестве аргументов операции. В таком случае они попадают в корень директории, где будут запускаться джобы. Подробнее можно прочитать в [разделе](#run_operation_commands) и [примере]({{ docs_root }}/core/how-to-guides/api/python/examples#files).
...
Подробнее про таблицы можно прочитать в разделе [Статические таблицы]({{ docs_root }}/core/concepts/storage/static-tables).
...
Все команды для работы с таблицами (в том числе операции) принимают в качестве входных и выходных таблиц не только строки, но и класс [TablePath](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.ypath.TablePath) (там, где это разумно). Данный класс представляет из себя путь к таблице с некоторыми модификаторами (обертка над [YPath]({{ docs_root }}/core/reference/storage/ypath) для таблиц). Его конструктор принимает:
...
- [write_table_structured](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.write_table_structured)
    ...
    Опция `table_writer` позволяет указать разные системные [параметры записи]({{ docs_root }}/core/reference/storage/io-configuration). Для записи сырых или сжатых данных используйте функцию `write_table`.
    ...
    В более сложных случаях может потребоваться изготавливать схему вручную, подробнее можно прочитать в [разделе](#table_schema) и [примере]({{ docs_root }}/core/how-to-guides/api/python/examples#table_schema).
...
- [read_table_structured](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.read_table_structured)
    ...
    Опция `table_reader` (dict) позволяет указать разные системные [параметры чтения]({{ docs_root }}/core/reference/storage/io-configuration#table_reader).
    ...
    Возвращаемый итератор поддерживает метод `.with_context()`, который возвращает итератор на пары вида `(row, ctx)`. Второй элемент позволяет получать индекс текущей строки и диапазона с помощью методов `ctx.get_row_index()` и `ctx.get_range_index()` (аналогичный итератор внутри джоба позволяет также получить индекс таблицы: `ctx.get_table_index()`). Про контекст в [обычном чтении]({{ docs_root }}/core/how-to-guides/api/python/examples#read_write) и [внутри операций]({{ docs_root }}/core/how-to-guides/api/python/examples#table_switches) есть примеры в туториале.
...
- [write_table](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.write_table)
    ...
    См. [пример]({{ docs_root }}/core/how-to-guides/api/python/examples#read_write_untyped) в соответствующем разделе.
...
- [read_table](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.read_table)
    ...
    Опция `table_reader` (dict) позволяет указать разные системные [параметры чтения]({{ docs_root }}/core/reference/storage/io-configuration#table_reader).
    Опция `control_attributes` (dict) позволяет при чтении заказать различные [контрольные атрибуты]({{ docs_root }}/core/reference/storage/io-configuration#control_attributes)
    ...
    смотрите [пример]({{ docs_root }}/core/how-to-guides/storage/examples#read_write_untyped) в соответствующем разделе.
...
4. Параллельная запись не будет эффективной, если парсер вашего формата медленный (например, в случае [schemaful_dsv]({{ docs_root }}/core/reference/storage/formats#schemaful_dsv)).
...
- [unlock](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.unlock) — снимает с указанного узла все [явные]({{ docs_root }}/core/concepts/storage/transactions#implicit_locks) локи, взятые текущей транзакцией. Как уже взятые, так и ожидающие в очереди. Если локов нет, не имеет эффекта. Если разблокировка невозможна (потому что заблокированная версия узла содержит изменения по сравнению с оригинальной версией), возвращает исключение.
...
Про доступные операции для работы с табличными данными можно в [разделе]({{ docs_root }}/core/reference/data-processing/operations/overview).
...
- [run_erase](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.run_erase) — [операция удаления данных из таблицы]({{ docs_root }}/core/reference/data-processing/operations/erase)
- [run_merge](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.run_merge) — [операция слияния/обработки данных в таблице без пользовательского кода]({{ docs_root }}/core/reference/data-processing/operations/merge)
- [run_sort](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.run_sort) — [операция сортировки данных]({{ docs_root }}/core/reference/data-processing/operations/sort). В случае, если не указан destination_table делается inplace-сортировка таблицы.
- [run_map_reduce](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.run_map_reduce) — [операция MapReduce]({{ docs_root }}/core/reference/data-processing/operations/mapreduce).
- [run_map](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.run_map) — [операция Map]({{ docs_root }}/core/reference/data-processing/operations/map).
- [run_reduce](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.run_reduce) — [операция Reduce]({{ docs_root }}/core/reference/data-processing/operations/reduce).
- [run_join_reduce](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.run_join_reduce) — [операция JoinReduce]({{ docs_root }}/core/reference/data-processing/operations/reduce#foreign_tables).
...
- [run_remote_copy](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.run_remote_copy) — [операция RemoteCopy]({{ docs_root }}/core/reference/data-processing/operations/remote-copy)
...
Имена методов соответствуют названиям опций в спецификации, описанным [здесь]({{ docs_root }}/core/reference/data-processing/operations/operations-options).
...
1. Spec builder'ы для заполнения [I/O параметров]({{ docs_root }}/core/reference/storage/io-configuration):
...
смотрите [пример в туториале]({{ docs_root }}/core/how-to-guides/api/python/examples#spec_builder).
...
Для всех операций с пользовательским кодом в отдельную опцию вынесен `memory_limit` и опции `format`, `input_format`,`output_format`, позволяющие специфицировать [формат входных/выходных данных]({{ docs_root }}/core/reference/storage/formats). Имеются параметры `yt_files` и `local_files` (и алиас `files`), позволяющие задать пути файлов в Кипарисе и пути к локальным файлам (соответственно), которые приедут в окружение джоба.
...
- [run_job_shell](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.run_job_shell) —  запустить job-shell для джоба. Проще использовать данную функцию с помощью аналогичной команды в [CLI]({{ docs_root }}/core/how-to-guides/api/cli/examples).
...
- [get_operation](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.operation_commands.get_operation) — получить информацию об операции по ее id. Возвращается `dict` с полями, аналогичными полям ответа [get_operation]({{ docs_root }}/core/reference/api/commands#get_operation).
- [list_operations](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.operation_commands.list_operations) — получить информацию про набор операций по фильтрам. Смысл полей аналогичен `get_operation`. Список фильтров см. в разделе [Команды]({{ docs_root }}/core/reference/api/commands#list_operations).
...
- [get_job](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.operation_commands.get_job) — получить информацию про джоб. Возвращается `dict` с полями, аналогичными полям ответа [get_job]({{ docs_root }}/core/reference/api/commands#get_job).
- [list_jobs](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.operation_commands.list_jobs) — получить информацию про набор джобов операции. Смысл полей в ответе аналогичен `get_job`. Список фильтров см. в разделе [Команды]({{ docs_root }}/core/reference/api/commands#list_jobs).
...
Для отладки невыполнившихся (failed) джобов удобно использовать [job tool]({{ docs_root }}/core/how-to-guides/problems/jobstatistics). Данная утилита позволяет подготовить окружение, аналогичное окружению джоба и запустить его для тех же входных данных.
...
[Динамические таблицы]({{ docs_root }}/core/concepts/dynamic-tables/overview) реализуют интерфейс точечного чтения и записи данных по ключу, поддерживающих транзакции и собственный диалект SQL.

Список команд для работы с содержимым динамических таблиц.
- [insert_rows](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.dynamic_table_commands.insert_rows) — [вставить]({{ docs_root }}/core/concepts/dynamic-tables/sorted-dynamic-tables#insert_rows) строки в динамическую таблицу
- [lookup_rows](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.dynamic_table_commands.lookup_rows) — [выбрать]({{ docs_root }}/core/concepts/dynamic-tables/sorted-dynamic-tables#chtenie-stroki) строки с заданными ключами
- [delete_rows](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.dynamic_table_commands.delete_rows) — [удалить]({{ docs_root }}/core/concepts/dynamic-tables/sorted-dynamic-tables#udalenie-stroki) строки с заданными ключами
- [select_rows](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.dynamic_table_commands.select_rows) — [выбрать]({{ docs_root }}/core/concepts/dynamic-tables/sorted-dynamic-tables#vypolnenie-zaprosa) строки, удовлетворяющие запросу на [диалекте SQL]({{ docs_root }}/core/reference/dynamic-tables/dyn-query-language)
- [lock_rows](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.dynamic_table_commands.lock_rows) — [заблокировать]({{ docs_root }}/core/concepts/dynamic-tables/sorted-dynamic-tables#blokirovka-stroki) строки с заданными ключами
- [trim_rows](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.dynamic_table_commands.trim_rows) — [удалить]({{ docs_root }}/core/concepts/dynamic-tables/ordered-dynamic-tables#trim) заданное количество строк из упорядоченной таблицы

Команды, связанные с [монтирование]({{ docs_root }}/core/concepts/dynamic-tables/overview#mount_table) таблиц.
...
Команды, связанные с [шардированием]({{ docs_root }}/core/how-to-guides/dynamic-tables/resharding).
...
Другие команды.
- [freeze_table](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.dynamic_table_commands.freeze_table) — [заморозить]({{ docs_root }}/core/concepts/dynamic-tables/overview#zamorozka) таблицу
- [unfreeze_table](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.dynamic_table_commands.unfreeze_table) — [разморозить]({{ docs_root }}/core/concepts/dynamic-tables/overview#zamorozka) таблицу
- [balance_tablet_cells](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.dynamic_table_commands.balance_tablet_cells) — распределить таблеты равномерно по [таблет-селлам]({{ docs_root }}/core/concepts/dynamic-tables/overview#tablet_cells)
- [get_tablet_infos](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.dynamic_table_commands.get_tablet_infos) — получить атрибуты [таблета]({{ docs_root }}/core/concepts/dynamic-tables/overview#tablets)
- [get_tablet_errors](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.dynamic_table_commands.get_tablet_errors) — получить ошибки [таблета]({{ docs_root }}/core/concepts/dynamic-tables/overview#tablets)
- [alter_table_replica](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.dynamic_table_commands.alter_table_replica) — [изменить атрибуты]({{ docs_root }}/core/concepts/dynamic-tables/replicated-dynamic-tables#nastrojki-replik) реплики [реплицированной таблицы]({{ docs_root }}/core/concepts/dynamic-tables/replicated-dynamic-tables)

### Другие команды { #etc_commands }

#### Преобразования таблиц

**Transform.** Иногда бывает необходимо переложить таблицу в новый кодек сжатия или erasure, переложить таблицу в [новый формат чанка]({{ docs_root }}/core/concepts/storage/chunks). В решении таких задач поможет функция [transform](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.transform).
...
- [{#T}]({{ docs_root }}/core/how-to-guides/api/python/examples#prepare_operation);
- [{#T}]({{ docs_root }}/core/how-to-guides/api/python/examples#grep).


### Описание типа данных { #prepare_operation }

Для указания входных и выходных типов строк в классе джоба можно использовать тайп хинты (смотрите примеры в туториале: [раз]({{ docs_root }}/core/how-to-guides/api/python/examples#simple_map), [два]({{ docs_root }}/core/how-to-guides/api/python/examples#multiple_input_reduce), [три]({{ docs_root }}/core/how-to-guides/api/python/examples#multiple_input_multiple_output_reduce) и [четыре]({{ docs_root }}/core/how-to-guides/api/python/examples#map_reduce_multiple_intermediate_streams)), либо переопределить метод [`.prepare_operation(self, context, preparer)`](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.prepare_operation.TypedJob.prepare_operation). Указание типов производится через методы объекта `preparer` типа [`OperationPreparer`](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.prepare_operation.OperationPreparer). Полезные методы:
...
смотрите примеры в туториале: [раз]({{ docs_root }}/core/how-to-guides/api/python/examples#prepare_operation) и [два]({{ docs_root }}/core/how-to-guides/api/python/examples#grep).

Если запускается MapReduce с несколькими промежуточными потоками, то требуется также переопределить метод [.get_intermediate_stream_count(self)](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.prepare_operation.TypedJob.get_intermediate_stream_count), вернув из него количество промежуточных потоков. Смотрите [пример]({{ docs_root }}/core/how-to-guides/api/python/examples#map_reduce_multiple_intermediate_streams).
...
Примеры можно найти в [туториале]({{ docs_root }}/core/how-to-guides/api/python/examples#job_decorators).
...
При запуске операции можно указать, какой [образ ФС]({{ docs_root }}/core/reference/data-processing/layers/layer-paths) необходимо подготовить перед запуском джобов.
Есть некоторый набор [готовых слоёв]({{ docs_root }}/core/reference/data-processing/layers/layer-paths#gotovye-sloi-v-kiparise), которые находятся по пути `//porto_layers`.
...
Примеры можно найти в [туториале]({{ docs_root }}/core/how-to-guides/api/python/examples#job_decorators_untyped).
...
2. В формате YSON имеется два целочисленных типа: [int64 и uint64]({{ docs_root }}/core/reference/storage/yson). В то время, как в python с точки зрения модели данных тип один и никаких ограничений не имеет. Поэтому при чтении данных беззнаковый тип представляется как YsonUint64, в то время, как знаковый представляется обычным int-ом. При записи int-ов сделана автоматика: числа в диапазоне [-2^63, 2^63) представляются как знаковые, а числа в диапазоне [2^63, 2^64) как беззнаковые. Но всегда можно явно указать тип, создав явно Yson-объект.
...
Кроме потока записей при чтении из таблицы (или в джобе) можно заказать разные [контрольные атрибуты]({{ docs_root }}/core/reference/storage/io-configuration#control_attributes). Работа с контрольными атрибутами зависит от формата, например в форматах, отличных от Yamr, JSON и YSON, большинство контрольных атрибутов не представимы.
...
Примеры переключения между выходными таблицами с `table_index` при `control_attributes_mode`, равном `iterator` и `row_fields`, а также пример получения индекса текущей строки таблицы в reducer-е с помощью `context`, можно найти [здесь]({{ docs_root }}/core/how-to-guides/api/python/examples#table_switches_untyped)

#### Другие форматы { #other_formats }

По умолчанию библиотека проводит сериализацию через [YSON формат]({{ docs_root }}/core/reference/storage/formats#yson), так как именно он обеспечивает корректное и однозначное представление любых данных, хранящихся в таблицах {{product-name}}. Для эффективной работы с YSON-форматом в python необходима [нативная библиотека](#yson_bindings). В случае отсутствия данной библиотеки на кластере вы получите ошибку при выполнении джоба (fallback на python-библиотеку для YSON не делается из-за того, что она очень медленная и неэффективная). В такой ситуации вы можете переключиться на использование [JSON-формата]({{ docs_root }}/core/reference/storage/formats#json).
