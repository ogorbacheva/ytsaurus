...
Для работы скрипта необходимо сохранить токен в `~/.yt/token` или воспользоваться альтернативными способами конфигурации токена, описанными в [документации]({{ docs_root }}/core/how-to-guides/api/python/userdoc#configuration_token) по Python API. Адрес кластера указывается в переменной `YT_PROXY` или через аргумент `--proxy`.
