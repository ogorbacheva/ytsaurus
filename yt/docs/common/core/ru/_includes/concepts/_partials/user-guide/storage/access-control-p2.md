...
Даёт возможность [зарегистрировать консьюмера очереди]({{ docs_root }}/core/reference/dynamic-tables/queues#register_queue_consumer). ACE с этим правом имеет два особых правила: в нём должна быть указана витальность через атрибут `vital` (`%true` или `%false`), и оно должно содержать `register_queue_consumer` как единственное право. Например:
...
- использование [Job Shell]({{ docs_root }}/core/how-to-guides/problems/jobshell-and-slowjobs) требует одновременно прав `read` и `manage` , так как наличие доступа через консоль позволяет читать или произвольным образом менять состояние джоба.
