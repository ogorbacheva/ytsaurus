...
Динамические таблицы бывают [сортированные]({{ docs_root }}/core/concepts/dynamic-tables/sorted-dynamic-tables) и [упорядоченные]({{ docs_root }}/core/concepts/dynamic-tables/ordered-dynamic-tables). Помимо этого, для каждого вида таблиц реализован механизм репликации. Подробнее о репликации можно узнать в разделе [реплицированные динамические таблицы]({{ docs_root }}/core/concepts/dynamic-tables/replicated-dynamic-tables).
...
Более подробное описание архитектуры и модели данных приведено в отдельных разделах, посвящённых [сортированным]({{ docs_root }}/core/concepts/dynamic-tables/sorted-dynamic-tables) и [упорядоченным]({{ docs_root }}/core/concepts/dynamic-tables/ordered-dynamic-tables) динамическим таблицам.
...
| atomicity   | Atomicity | [Режим атомарности]({{ docs_root }}/core/concepts/dynamic-tables/transactions#atomicity). По умолчанию `full`.  |
...
- [настраивать фоновую компактификацию]({{ docs_root }}/core/how-to-guides/dynamic-tables/compaction#attributes);
- [устанавливать TTL]({{ docs_root }}/core/concepts/dynamic-tables/sorted-dynamic-tables#remove_old_data) — время жизни отдельных значений;
- [управлять автоматическим шардированием]({{ docs_root }}/core/how-to-guides/dynamic-tables/tablet-balancing).
...
Кроме того, динамические таблицы наследуют [ограничения]({{ docs_root }}/core/concepts/storage/static-tables#limitations) статических таблиц.
