...
Посмотреть [Примеры использования]({{ docs_root }}/core/how-to-guides/api/cpp/examples).
...
Логирование включается переменной окружения `YT_LOG_LEVEL`, которая может принимать одно из следующих значений: `ERROR`, `WARNING` (для совместимости с [Python API]({{ docs_root }}/core/how-to-guides/api/python/userdoc), эквивалентен `ERROR`), `INFO`, `DEBUG`.
...
Если нужно в рамках одной программы иметь клиенты, работающие под разными [токенами авторизации]({{ docs_root }}/core/reference/storage/auth), можно воспользоваться опциями:
...
Более подробно об устройстве транзакций можно почитать в разделе [Транзакции]({{ docs_root }}/core/concepts/storage/transactions).

## TNode { #tnode }

[TNode]({{source-root}}/yt/cpp/mapreduce/interface/node.h)  — это основной класс, обеспечивающий динамическое DOM-представление [YSON-документа]({{ docs_root }}/core/concepts/storage/yson-docs). Используется для работы с Кипарисом, передачи дополнительных спецификаций операций, как один из способов кодирования строк таблицы и т. д.
...
Подробнее о командах для работы см. в разделе [Работа с деревом метаинформации]({{ docs_root }}/core/how-to-guides/storage/cypress-example). Реализовано в виде интерфейса `ICypressClient`, от которого `IClientBase` наследуется.
...
[Создать]({{ docs_root }}/core/reference/api/commands#create) узел. [См. аналог в CLI]({{ docs_root }}/core/how-to-guides/storage/cypress-example#create).
...
| `type`        | `ENodeType`  | -          | [Тип]({{ docs_root }}/core/concepts/storage/objects#object_types) создаваемого узла: <br/>  `NT_STRING` — строка (`string_node`);<br/> `NT_INT64` — целое знаковое число (`int64_node`); <br/>`NT_UINT64` — целое беззнаковое число (`uint64_node`); <br/>`NT_DOUBLE` — вещественное число (`double_node`); <br/>`NT_BOOLEAN` — булево значение (`boolean_node`); <br/>`NT_MAP` — словарь в Кипарисе (ключи — строки, значения — другие узлы, `map_node`); <br/> `NT_FILE` — [файл]({{ docs_root }}/core/concepts/storage/objects#files) (`file`); <br/>`NT_TABLE` — [таблица]({{ docs_root }}/core/concepts/storage/objects#tables) (`table`);<br/> `NT_DOCUMENT` — [документ]({{ docs_root }}/core/concepts/storage/objects#yson_doc) (`document`); <br/>`NT_REPLICATED_TABLE` — [реплицированная таблица]({{ docs_root }}/core/concepts/dynamic-tables/replicated-dynamic-tables) (`replicated_table`); <br/>`NT_TABLE_REPLICA` — (`table_replica`); |
...
| `Attributes`  | `TNode`      | none        | [Атрибуты]({{ docs_root }}/core/reference/storage/attributes) создаваемого узла. |

### Remove

[Удалить]({{ docs_root }}/core/reference/api/commands#remove) узел. [См. аналог в CLI]({{ docs_root }}/core/how-to-guides/storage/cypress-example#remove).
...
[Проверить существование]({{ docs_root }}/core/reference/api/commands#exists) объекта по указанному пути.
...
[Получить содержимое узла Кипариса]({{ docs_root }}/core/reference/api/commands#get) в формате  `TNode`. [См. аналог в CLI]({{ docs_root }}/core/how-to-guides/storage/cypress-example#get).
...
[Записать новое содержимое узла Кипариса]({{ docs_root }}/core/reference/api/commands#set). [См. аналог в CLI]({{ docs_root }}/core/how-to-guides/storage/cypress-example#set).
...
[Получить список потомков узла]({{ docs_root }}/core/reference/api/commands#list). [См. аналог в CLI]({{ docs_root }}/core/how-to-guides/storage/cypress-example#list).
...
[Скопировать узел Кипариса по новому адресу]({{ docs_root }}/core/reference/api/commands#copy). [См. аналог в CLI]({{ docs_root }}/core/how-to-guides/storage/cypress-example#copy_move).
...
[Перенести узел по новому адресу]({{ docs_root }}/core/reference/api/commands#move). [См. аналог в CLI]({{ docs_root }}/core/how-to-guides/storage/cypress-example#copy_move).
...
[Создать]({{ docs_root }}/core/reference/api/commands#link) [символическую ссылку]({{ docs_root }}/core/concepts/storage/links) на объект по новому адресу. [См. аналог в CLI]({{ docs_root }}/core/how-to-guides/storage/cypress-example#link).
...
| `Attributes`  | `TNode`      | none       | [Атрибуты]({{ docs_root }}/core/reference/storage/attributes) для `linkPath` виде `TNode`, записываются в случае создания. |

### Concatenate

[Склеить]({{ docs_root }}/core/reference/api/commands#concatenate) набор файлов или таблиц (в том порядке, в котором указаны пути). [См. аналог в CLI]({{ docs_root }}/core/how-to-guides/storage/cypress-example#concatenate).

Слияние данных происходит на уровне метаинформации исключительно на мастер-сервере [Кипариса]({{ docs_root }}/core/concepts/storage/cypress) .
...
Для представления записи TNode должен иметь тип [map]({{ docs_root }}/core/concepts/storage/objects#primitive_types) и представлять собой отображение имён колонок в YSON-значения.

{{ core-user-guide-explanation-api-cpp-description-md-audience-3 }}

### Protobuf

Подробнее о Protobuf см. [Protobuf-представление таблиц]({{ docs_root }}/core/reference/api/cpp/protobuf).
...
Есть поддержка вложенных структур, в этом случае используется бинарная протобуфная сериализация, т.е. в колонке таблицы будет храниться строка с бинарной protobuf сериализацией [вложенного сообщения]({{ docs_root }}/core/reference/api/cpp/protobuf#embedded). Во вложенных структурах могут быть `repeated` поля, они будут сериализованы бинарно вместе с остальной частью вложенного сообщения.
...
- [Преобразования типов]({{ docs_root }}/core/reference/storage/data-types) при записи: `EnableStringToAllConversion`, `EnableAllToStringConversion`, `EnableIntegralTypeConversion%` (`uint64` <-> `int64`, включена по умолчанию), `EnableIntegralToDoubleConversion`, `EnableTypeConversion` (все указанные выше одновременно).

### Дополнительные опции

Если необходима ещё более тонкая настройка параметров чтения/записи, можно использовать структуры `TTableWriterOptions` и `TTableReaderOptions`. В них присутствует поле `Config`, в него в формате `TNode` можно передавать дополнительные настройки, соответствующие параметрам [table_writer]({{ docs_root }}/core/reference/storage/io-configuration#table_writer) и [table_reader]({{ docs_root }}/core/reference/storage/io-configuration#table_reader).
...
В качестве `mapper` можно указать нулевой указатель, тогда операция запустится без стадии Map, вторая версия функции дополнительно запустит стадию [reduce_combiner]({{ docs_root }}/core/reference/data-processing/operations/reduce#reduce_combiner).
...
Получить [информацию об операции]({{ docs_root }}/core/reference/api/commands#get_operation).
...
Получить список операций, [удовлетворяющих фильтрам]({{ docs_root }}/core/reference/api/commands#list_operation).
...
Обновить runtime parameters [запущенной операции]({{ docs_root }}/core/reference/api/commands#update_operation_parameters).
...
Получить [информацию о джобе]({{ docs_root }}/core/reference/api/commands#get_job).
...
Получить джобы, [удовлетворяющие указанным фильтрам]({{ docs_root }}/core/reference/api/commands#list_jobs).
...
Получить вход [запущенного или упавшего джоба]({{ docs_root }}/core/reference/api/commands#get_job_input).
...
Получить fail context [упавшего джоба]({{ docs_root }}/core/reference/api/commands#get_job_fail_context).
...
Получить stderr [запущенного или упавшего джоба]({{ docs_root }}/core/reference/api/commands#get_job_stderr).
...
[Скопировать файл в кеш]({{ docs_root }}/core/reference/api/commands#put_file_to_cache). Возвращает путь к файлу в кеше.
...
[Получить путь к файлу в кеше]({{ docs_root }}/core/reference/api/commands#get_file_from_cache). Возвращает путь или `Nothing`, если файла с таким MD5 нет.
