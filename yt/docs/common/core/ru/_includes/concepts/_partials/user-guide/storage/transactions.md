...
В данном разделе описаны транзакции, применительно к [Кипарису]({{ docs_root }}/core/concepts/storage/cypress) и [статическим таблицам]({{ docs_root }}/core/concepts/storage/static-tables), [блокировки](#locks) и [версионированность](#versioning) объектов Кипариса.
Транзакционная модель динамических таблиц описана в разделе [Мультиверсионность и транзакционность динамических таблиц]({{ docs_root }}/core/concepts/dynamic-tables/transactions).
...
Транзакционность в пределах мастер-серверов распространяется на версионируемые [объекты]({{ docs_root }}/core/concepts/storage/objects). В качестве примера таких объектов могут выступать файлы, таблицы и каталоги.
...
Помимо атрибутов, [присущих всем объектам]({{ docs_root }}/core/concepts/storage/objects#attributes), транзакции имеют следующие атрибуты:
...
- `snapshot` - транзакция получает возможность читать, но не модифицировать узел. Блокировка используется, чтобы взять read-only копию состояния узла Кипариса в контексте транзакции и зафиксировать состояние данного узла.
  ...
  ​    Snapshot-блокировка берется только на сам узел, но не на путь до него в Кипарисе. Если продолжать обращаться к узлу по его пути, можно получить новый узел, размещенный по тому же пути.
      Чтобы гарантированно обратиться к snapshot-версии узла, используйте `id` данного узла, который возвращает [команда]({{ docs_root }}/core/reference/api/commands#lock) `lock`.
...
При запуске операций планировщик создает набор транзакций, чтобы обеспечить определенную атомарность обработки данных в операции. Подробнее про схему работы можно прочитать в разделе [Транзакционность обработки данных]({{ docs_root }}/core/reference/data-processing/operations/overview#transactions).
