...
Данные, хранящиеся в [таблицах]({{ docs_root }}/core/concepts/storage/static-tables), [файлах]({{ docs_root }}/core/concepts/storage/files) и журналах, разбиты на части, называемые чанками. Чанк хранит непрерывный диапазон данных.
...
Физически данные чанков хранятся на узлах кластера. Мастер-сервера {{product-name}} отслеживают расположение чанков, а также поддерживают их в [реплицированном]({{ docs_root }}/core/reference/storage/replication) состоянии.

Мастер-сервера {{product-name}} хранят только метаданные. Тем не менее, метаданные имеют существенный размер, поэтому, чтобы ограничить потребление памяти мастер-серверами, количество чанков, доступных аккаунту, [квотируется]({{ docs_root }}/core/reference/storage/quotas).
...
Размер блока можно изменить в настройках [`table_writer`]({{ docs_root }}/core/reference/storage/io-configuration), атрибут `block_size`.

Как правило, чанк занимает на узле кластера сотни мегабайт или единицы гигабайт.
Размер создаваемых чанков можно настраивать через секцию [`table_writer`]({{ docs_root }}/core/reference/storage/io-configuration), атрибут `desired_chunk_size`.
...
Построчный формат подходит для [динамических таблиц]({{ docs_root }}/core/concepts/dynamic-tables/overview), из которых данные выбирают по ключу.
...
Для **динамической таблицы** аналогичную задачу решает forced compaction. Перед его использованием обязательно изучите статью [Фоновая компактификация]({{ docs_root }}/core/how-to-guides/dynamic-tables/compaction#forced_compaction).
...
| `compression_statistics`  | `CompressionStatistics` | Статистика по типам использованных [кодеков сжатия]({{ docs_root }}/core/reference/storage/compression) и размерам данных. | Да |
| `erasure_statistics`      | `ErasureStatistics`     | Статистика по типам использованных [erasure-кодеков]({{ docs_root }}/core/reference/storage/replication) и размерам данных. | Да |
...
| `replication_factor`      | `integer`               | Коэффициент [репликации]({{ docs_root }}/core/reference/storage/replication) (для erasure-кодирования равен 1). | Да |
| `compression_codec`       | `string`                | Имя [кодека сжатия]({{ docs_root }}/core/reference/storage/compression).            | Да |
| `erasure_codec`           | `string`                | Имя [erasure-кодека]({{ docs_root }}/core/reference/storage/replication).           | Да |
| `vital`                   | `bool`                  | Являются ли чанки этого узла [витальными](#vitality)   | Нет
| `media`                   | `Media`                 | Коэффициенты репликации и другие настройки [медиумов]({{ docs_root }}/core/reference/storage/media). | Да |
| `primary_medium`          | `string`                | [Первичный медиум]({{ docs_root }}/core/reference/storage/media#primary).           | Да |
