...
Кроме статических таблиц, система {{product-name}} поддерживает динамические таблицы. Подробнее о видах и возможностях динамических таблиц можно прочитать в разделе [Динамические таблицы]({{ docs_root }}/core/concepts/dynamic-tables/overview).

## Общее описание { #common }

Статические таблицы — это классический тип таблиц в системе {{product-name}}.
Физически такие таблицы разбиты на части ([чанки]({{ docs_root }}/core/concepts/storage/chunks)), каждая часть содержит фрагмент записей таблицы.
...
- [слияние]({{ docs_root }}/core/reference/data-processing/operations/merge);
- [удаление]({{ docs_root }}/core/reference/data-processing/operations/erase).
...
| `optimize_for` | `string`        | Формат физического хранения таблицы в чанках: `lookup` (построчный) и `scan` (поколоночный). Если не указать этот атрибут при создании таблицы, будет использоваться `lookup` формат. Рекомендуется явно указывать `scan`, чтобы получить дополнительные оптимизации при хранении и чтении данных. [Подробнее]({{ docs_root }}/core/concepts/storage/chunks#optimize_for)                                                                                      | Нет              |

Кроме того, таблица является объектом, который владеет чанками, то есть имеет соответствующие атрибуты, представленные в [таблице]({{ docs_root }}/core/concepts/storage/chunks#attributes).

## Схема статических таблиц { #schema }

Схема статической таблицы представляет из себя список описаний колонок. Подробное описание формата схемы приведено в разделе [Схема данных]({{ docs_root }}/core/reference/storage/static-schema).
...
- Максимальный вес строки  — 128 мегабайт. Вес строки — сумма длин всех значений в данной строке в байтах. При этом длины значений считаются в зависимости от типа:
  ...
  - `boolean` — 1 байт;
  - `string` — длина строки;
  - `any` — длина структуры сериализованной в binary [yson]({{ docs_root }}/core/reference/storage/yson), в байтах;
  - `null` — 0 байт.
