...
- В данном руководстве вы будете запускать {{product-name}} кластер в минимальной конфигурации — без гарантий отказоустойчивости.
  ...
  Примеры того, как настраивать конфигурацию кластера, приведены в [Руководстве администратора]({{ docs_root }}/core/how-to-guides/prepare-spec).
...
- Docker {selected}
  ...
  4. Чтобы убедиться, что всё работает, запустите команду:
     ...
     - `yt.backend` — в этом контейнере подняты компоненты {{product-name}} кластера.
       ...
       - [Мастер-сервер]({{ docs_root }}/core/reference/components#master) — отвечает за отказоустойчивое хранение метаданных кластера. К этим данным относится информация о пользователях системы, о хранимых объектах и о местоположении самих данных.
       - Нода — объединяет в себе функциональность [Data-ноды]({{ docs_root }}/core/reference/components#data-nodes) и [Exec-ноды]({{ docs_root }}/core/reference/components#exec-nodes). Отвечает за хранение данных, за работу динамических таблиц и занимается исполнением джобов.
       - [Планировщик]({{ docs_root }}/core/reference/components#planirovshik) (шедулер) — отвечает за планирование процесса обработки данных (Map, Reduce и так далее).
       - [Контроллер-агент]({{ docs_root }}/core/reference/components#planirovshik) — отвечает за планирование джобов конкретных операций.
       - [HTTP-прокси]({{ docs_root }}/core/reference/components#proxy) — сервер, через который осуществляется общение с внешним миром.
       - [YQL-агент]({{ docs_root }}/core/reference/components#yql-agent) — движок исполнения SQL подобных запросов.
       - [Очередь {{product-name}}]({{ docs_root }}/core/reference/dynamic-tables/queues) — упорядоченные динамические таблицы.
       - [Query Tracker]({{ docs_root }}/core/concepts/query-tracker/about) — компонент для запуска запросов на разных SQL диалектах (YQL, QT, CHYT, SPYT).
...
- Kind
  ...
  - Для работы через [RPC proxy]({{ docs_root }}/core/reference/proxy/rpc) по адресу `localhost:8082` в отдельном терминале выполните команду:
...
Самый удобный способ взаимодействия с системой {{product-name}} — из консоли. Утилита [{{product-name}} СLI]({{ docs_root }}/core/reference/api/cli/cli) не устанавливается при развёртывании кластера, её нужно установить в системе отдельно.
...
- Minikube
  ...
  Здесь токен задаётся в переменной окружения — это сделано намеренно, для простоты и наглядности примера. В реальных задачах так делать не стоит, для работы с токенами в {{product-name}} есть специальные команды. Подробнее читайте в разделе [Аутентификация]({{ docs_root }}/core/reference/storage/auth#upravlenie-tokenami).
...
В {{product-name}} все данные хранятся в [таблицах]({{ docs_root }}/core/concepts/storage/static-tables). Поэтому для начала создайте таблицу:
...
Значение `16-64ca-10191-47007b7d` — это идентификатор созданного узла [Кипариса]({{ docs_root }}/_includes/overview/*about-cypress). Идентификаторы узлов полезны при работе с [транзакциями]({{ docs_root }}/core/concepts/storage/transactions) в {{product-name}}. В рамках текущего примера данные идентификаторы вам не понадобятся.
...
Теперь запишите данные в таблицу, вызвав команду [write-table]({{ docs_root }}/core/reference/api/cli/commands#write-table):
...
Теперь запишите данные в исходную таблицу, вызвав команду [write-table]({{ docs_root }}/core/reference/api/cli/commands#write-table):

```bash prompt="$"
$ cat source.tsv | yt write-table //home/mapreduce_input --format dsv
```
Чтобы проверить, что в таблицу записались данные, можно прочитать таблицу с помощью команды [read-table]({{ docs_root }}/core/reference/api/cli/commands#read-table). В квадратных скобках указан полуинтервал — получить первые шесть строк таблицы:
...
Запустите MapReduce операцию с помощью команды [map-reduce]({{ docs_root }}/core/reference/api/cli/commands#map-reduce):
...
Тут вы сможете ознакомиться с возможностями веб-интерфейса {{product-name}}: изучить файловую систему, посмотреть на список пулов, позапускать запросы в [Query Tracker]({{ docs_root }}/core/concepts/query-tracker/about).
...
[*about-cypress]: Кипарис — это распределённая файловая система и хранилище метаинформации. В Кипарисе хранятся таблицы и файлы. Подробнее читайте в [документации]({{ docs_root }}/core/concepts/storage/cypress).
...
[*about-qt]: Инструмент для запуска SQL-подобных запросов. Query Tracker доступен в веб-интерфейсе — в разделе **Queries**. Подробнее читайте в [документации]({{ docs_root }}/core/concepts/query-tracker/about).
...
[*about-master]: Отвечает за отказоустойчивое хранение метаданных кластера. К ним относится информация о пользователях системы, о хранимых объектах и о местоположении самих данных. Подробнее читайте в [документации]({{ docs_root }}/core/reference/components#master).

[*about-scheduler]: Отвечает за распределение ресурсов между операциями и за процесс их исполнения на кластере. Подробнее в [документации]({{ docs_root }}/core/concepts/data-processing/scheduler/scheduler-and-pools).
