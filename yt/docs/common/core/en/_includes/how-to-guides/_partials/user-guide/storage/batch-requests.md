...
You cannot specify the [format]({{ docs_root }}/core/reference/storage/formats) of the returned data by a separate command via `batch_client`. The reason is that the format of the returned data is the same for the entire batch request.
