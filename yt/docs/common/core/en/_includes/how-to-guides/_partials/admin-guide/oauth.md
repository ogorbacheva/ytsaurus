...
The {{product-name}} web interface offers two types of user authentication: password-based authentication and single sign-on ([SSO](https://en.wikipedia.org/wiki/Single_sign-on)). To learn more about using passwords, see the [User manual]({{ docs_root }}/core/reference/storage/auth). This article explains how to configure SSO.
