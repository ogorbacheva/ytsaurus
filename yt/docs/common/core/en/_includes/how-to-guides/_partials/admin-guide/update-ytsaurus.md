...
Before updating {{product-name}}, you first need to update the operator to the latest release. New stable images might not work with an outdated operator, and the operator itself might not work if it has an unstable {{product-name}} version like `dev`. You can find a list of stable images on the [Releases]({{ docs_root }}/core/reference/releases) page.
...
Before updating the operator, make sure the cluster is healthy. For example, it shouldn't have any [LVCs]({{ docs_root }}/core/how-to-guides/problems#lvc), and all [tablet cell bundles]({{ docs_root }}/core/how-to-guides/problems#tabletcellbundles) must be alive.
...
1. Launch the chart update:
    ...
    ```
   For a list of available operator versions, see the [release page]({{ docs_root }}/core/reference/releases#kubernetes-operator).
...
You can remove unrecognized master options either manually, by using the [CLI command `yt admin remove-master-unrecognized-options`]({{ docs_root }}/core/reference/cli-admin#remove-master-unrecognized-options), or by running the [standalone script of the same name](https://github.com/ytsaurus/ytsaurus/tree/main/yt/yt/scripts/remove_master_unrecognized_options).
