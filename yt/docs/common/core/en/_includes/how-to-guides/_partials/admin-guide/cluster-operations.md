...
You can manage quotas for data storage using accounts. You can read more about accounts [here]({{ docs_root }}/core/reference/storage/accounts).
...
Account has [lots of attributes]({{ docs_root }}/core/reference/storage/accounts#account_attributes) for describing resource limits, current resource usage, and more.
...
The scheduler is responsible for allocating computational resources in the cluster, but the descriptions of pool trees and pools (entities that store information about computational quotas) are stored in Cypress. We recommend reading the general [scheduler documentation]({{ docs_root }}/core/concepts/data-processing/scheduler/scheduler-and-pools) as well as our [help page on pool management]({{ docs_root }}/core/how-to-guides/data-processing/scheduler/manage-pools).
...
Users and groups are presented by user and group objects, while access rights are managed by the acl attribute. For more information about the access rights model, see the relevant [section]({{ docs_root }}/core/concepts/storage/access-control).
...
| tags | list<string> | List of [tags]({{ docs_root }}/core/how-to-guides/node-tags) for this node |
...
| user_tags | list<string> | List of additional node [tags]({{ docs_root }}/core/how-to-guides/node-tags) |
