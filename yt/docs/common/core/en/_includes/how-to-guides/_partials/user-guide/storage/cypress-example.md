...
This section includes the most popular [Cypress]({{ docs_root }}/core/concepts/storage/cypress) command-line (CLI) code examples. For a more detailed description of the commands and their options, please see the Commands section.
...
The `--format` option defines the output data format. Supported formats are `json` and `yson`. For more information, see [Formats]({{ docs_root }}/core/reference/storage/formats). The `--format` option is incompatible with `-l`.
...
This command creates a node of the type specified in `<type>` in the path specified in `<path>`. The command returns the identifier of the object created. Supported objects are listed in the [Objects]({{ docs_root }}/core/concepts/storage/objects) section.
...
The `-l, --lock-existing` option is compatible with `--ignore-existing`. If indicated together, an [exclusive lock]({{ docs_root }}/core/concepts/storage/transactions#locks) will be taken out on the specified node even if it already exists. (In the event of a lock conflict, the command will return an error). Since actually creating a node always takes out an implicit (automatic) lock on the node, the option makes the outcome of the command more predictable when using `--ignore-existing`.
...
The `--format` option defines the output data format. Supported formats are `json` and `yson`. For more information, see [Formats]({{ docs_root }}/core/reference/storage/formats).
...
The `set` command assigns the specified `value` in [YSON]({{ docs_root }}/core/reference/storage/yson) format using the specified `<path>`.
...
The `--format` option defines the `value` format. Supported formats are `json` and `yson`. For more information, see [Formats]({{ docs_root }}/core/reference/storage/formats).
...
The `--preserve-expiration-time` option helps keep the specified [delete time]({{ docs_root }}/core/concepts/storage/cypress) of the node being copied.

The `--preserve-expiration-timeout` option helps keep the specified [delete interval]({{ docs_root }}/core/concepts/storage/cypress) of the node being copied.

The `--preserve-creation-time` option helps keep the [create time]({{ docs_root }}/core/concepts/storage/cypress) of the node being copied. **For copying only.**
...
This command creates a [symbolic link]({{ docs_root }}/core/concepts/storage/links) in `<link_path>` to an object found in `<target_path>`.
...
The `-l, --lock-existing` option guarantees that an [exclusive lock]({{ docs_root }}/core/concepts/storage/transactions#locks) will be taken out on the link even if it already exists. (Only useful with `--ignore-existing`; if a lock cannot be set, the command will return an error).
