...
Cluster throttlers limit incoming inter-cluster traffic on {{product-name}} MR clusters. Throttling applies to [RemoteCopy]({{ docs_root }}/core/reference/data-processing/operations/remote-copy) operations and MapReduce operations that read input tables from remote clusters.

For more information about how throttling affects user operations, see [Inter-cluster Network Bandwidth Throttling]({{ docs_root }}/core/reference/data-processing/operations/cluster-throttlers).
