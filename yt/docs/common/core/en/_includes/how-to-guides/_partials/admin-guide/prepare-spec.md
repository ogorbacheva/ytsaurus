...
| `configOverrides` | `optional<LocalObjectReference>` | A ConfigMap for overriding generated static configs. See [Configuration overrides]({{ docs_root }}/core/reference/config-overrides) for details. |
...
| `bootstrap` | BootstrapSpec | Settings for initially deploying the cluster (for example, [tablet cell bundle parameters]({{ docs_root }}/core/concepts/dynamic-tables/concepts#tablet_cell_bundles)) |
...
You can read more about components in [this section]({{ docs_root }}/core/reference/components).
...
## Logging
A proper logging configuration is essential for diagnosing problems and facilitating support. For recommendations on setting up logging, see [this page]({{ docs_root }}/core/how-to-guides/logging).

## Locations

There are recommendations for disk layout and location configuration on a separate [page]({{ docs_root }}/core/how-to-guides/locations).
...
The operator automatically creates multiple [tablet cell bundles]({{ docs_root }}/core/concepts/dynamic-tables/concepts#tablet_cell_bundles): `sys` and `default`.
