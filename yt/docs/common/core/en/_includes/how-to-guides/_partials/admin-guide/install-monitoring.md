...
8. If you have Odin installed, check if its metrics are being collected:
    ...
    Collection of qualitative metrics from [Odin]({{ docs_root }}/core/how-to-guides/install-odin) is carried out through a separate `ServiceMonitor` created by the Odin chart itself.
    ...
    If it is missing, you need to enable `ServiceMonitor` creation in the [chart settings]({{ docs_root }}/core/how-to-guides/install-odin#prepare-values).
...
For monitoring and Grafana integration to work properly, you need to pass the required addresses to the {{product-name}} UI installed via the [Helm chart]({{ docs_root }}/core/how-to-guides/install-ytsaurus#ui). When installing the monitoring chart, these variables will be automatically checked for existence.
