...
Task proxy provides access to web services deployed in {{product-name}} operations. For more details, see the [Task proxy]({{ docs_root }}/core/reference/proxy/task) section.
...
- a special robot user `robot-task-proxy` with an issued token (see the [token management]({{ docs_root }}/core/reference/storage/auth#token-management) section):
   ```sh
