...
You need to get a [{{product-name}} token]({{ docs_root }}/core/reference/storage/auth) before running the examples.
...
The log level is set using an environment variable with this command (more info is available in the [documentation]({{ docs_root }}/core/concepts/api/cpp/description#logging)):
...
If you know the structure of the table, you may use the [Protobuf]({{ docs_root }}/core/reference/api/cpp/protobuf) format, which:
...
Some simple operations can be represented as C++ lambda expressions with an empty capture-list (not capturing variables) using the [yt/cpp/mapreduce/library/lambda]({{ docs_root }}/core/reference/api/cpp/lambda) library.
...
The [reducer]({{ docs_root }}/core/reference/data-processing/operations/reduce) is straightforward.
...
{{product-name}} has the [MapReduce]({{ docs_root }}/core/reference/data-processing/operations/mapreduce) merged operation, which works a bit faster than [Map]({{ docs_root }}/core/reference/data-processing/operations/map) + [Sort]({{ docs_root }}/core/reference/data-processing/operations/sort) + [Reduce]({{ docs_root }}/core/reference/data-processing/operations/reduce). Let's return to one of our first examples and once again get the statistics of how many times a name occurs in our table with users. Before we do that, we first need to standardize the names by converting all of them to lowercase so that people with the names `ARCHIE` and `Archie` are counted as one.
...
You can set up the operation parameters by overloading the `IJob::PrepareOperation` method. For more information, see [Description]({{ docs_root }}/core/concepts/api/cpp/description#prepare_operation).
...
A running operation gathers a lot of statistics (see the [documentation]({{ docs_root }}/core/how-to-guides/problems/jobstatistics)).
...
{{product-name}} supports [dynamic tables]({{ docs_root }}/core/concepts/dynamic-tables/overview). You can perform basic operations with such tables by using the C++ wrapper.
