...
The utility comes as part of the [Python API]({{ docs_root }}/core/how-to-guides/api/python/start) package. All types of jobs that run custom code are supported.
...
3. Once the command has finished, the folder `job_ffe4ff5c-d2fac8c1-3fe0384-816a7fd0` will contain the following files:
    ...
    * The `command` file contains the command that runs the job. Since the operation was run using the [Python API]({{ docs_root }}/core/how-to-guides/api/python/start), the code will look like this:
