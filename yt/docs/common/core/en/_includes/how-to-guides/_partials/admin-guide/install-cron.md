...
* a special robot user `robot-cron` with a token issued for it (see the [Token management]({{ docs_root }}/core/reference/storage/auth#token-management) section):
...
To work, it requires:
- Enabled [loading of master snapshots into Cypress]({{ docs_root }}/core/how-to-guides/persistence-uploader#uploading-snapshots-to-cypress).
