...
You can read about updating the operator in [that section]({{ docs_root }}/core/how-to-guides/update/update-ytsaurus#operator).
...
[Prepare the `Ytsaurus` specification]({{ docs_root }}/core/how-to-guides/prepare-spec) for the resource and upload it to K8s ([sample specification](https://github.com/ytsaurus/ytsaurus-k8s-operator/blob/main/config/samples/cluster_v1_demo.yaml)):
```bash
...
* prepared a special robot-user for the {{product-name}} UI and ready to provide its token (see the [Token management]({{ docs_root }}/core/reference/storage/auth#token-management) section).
