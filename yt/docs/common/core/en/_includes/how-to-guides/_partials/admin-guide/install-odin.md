...
Odin is a service that provides qualitative monitoring of a {{product-name}} cluster. For more details, see the [Monitoring]({{ docs_root }}/core/how-to-guides/monitoring#odin) section.
...
* a dedicated service user `robot-odin` with an issued token (see [Token Management]({{ docs_root }}/core/reference/storage/auth#token-management)):
