...
Dynamic tables are divided into [tablets]({{ docs_root }}/core/concepts/dynamic-tables/overview#tablets) (shards), which are the unit of concurrency.

#### Sorted tables

Each tablet of a [sorted table]({{ docs_root }}/core/concepts/dynamic-tables/sorted-dynamic-tables) is responsible for a certain range of keys. Tablet boundary keys (also called pivot keys) are available in the `pivot_keys` table attribute. A tablet with the *k* index is responsible for the keys between the *k* boundary key (inclusive) and *k*+1 boundary key (not inclusive).
...
Each tablet of an [ordered table]({{ docs_root }}/core/concepts/dynamic-tables/ordered-dynamic-tables) is an independent queue. To reshard an ordered table, you must specify the desired number of tablets. For more information, see [Sharding ordered tables]({{ docs_root }}/core/concepts/dynamic-tables/ordered-dynamic-tables).

#### Replicated tables
Physically, each tablet of a [replicated table]({{ docs_root }}/core/concepts/dynamic-tables/replicated-dynamic-tables) is a queue and stores the replication log. Despite this, the sharding logic is the same as for regular tables.
...
Use the [reshard-table]({{ docs_root }}/core/reference/api/commands#reshard_table) command to reshard a table. Before running the command, the table must be unmounted.
...
When the `tablet_count` parameter is specified, the system attempts to automatically select appropriate boundary keys. If the table is relatively small (less than 200 MB per tablet), there may be fewer total tablets than requested. In this case, we recommend using the [enable_slicing]({{ docs_root }}/core/reference/api/commands#reshard_table) option to refine the boundary key search.
