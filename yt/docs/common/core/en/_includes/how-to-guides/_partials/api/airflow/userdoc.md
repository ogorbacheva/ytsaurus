...
This section provides instructions on how to connect to a {{product-name}} cluster in Apache Airflow and how to use operators to interact with Cypress, work with static tables, and execute queries in [Query Tracker]({{ docs_root }}/core/concepts/query-tracker/about).
...
6. **Extra**: Optionally, you can specify [Client Config]({{ docs_root }}/core/how-to-guides/api/python/userdoc#configuration):
    ```
    {
        "client_config": {
...
All operators use the Python SDK. If an error occurs during operator execution, see the [Python SDK documentation]({{ docs_root }}/core/how-to-guides/api/python/userdoc).
...
Operator for retrieving the contents of a Cypress node. It implements the [get]({{ docs_root }}/core/reference/api/commands#get) functionality from the Python SDK.
...
Operator for writing new contents to a Cypress node. It implements the [set]({{ docs_root }}/core/reference/api/commands#set) functionality from the Python SDK.
...
Operator for removing a Cypress node. It implements the [remove]({{ docs_root }}/core/reference/api/commands#remove) functionality from the Python SDK.
...
Operator for creating an empty Cypress node of the `node_type` with `attributes`. It implements the [create]({{ docs_root }}/core/reference/api/commands#create) functionality from the Python SDK.
...
Operator for getting a list of descendants of the `path` node. The `absolute` option enables the output of absolute paths instead of relative paths. It implements the [list]({{ docs_root }}/core/reference/api/commands#list) functionality from the Python SDK.
...
Operator for reading data from a static table. It implements the [read_table]({{ docs_root }}/core/reference/api/commands#read_table) functionality from the Python SDK.

This operator can write data to the object storage if you pass the `object_storage_path` parameter, which is an object of the [ObjectStorage](https://airflow.apache.org/docs/apache-airflow/stable/core-concepts/objectstorage.html) type. Otherwise, the table will be written to XCom. When writing data to the object storage, you need to specify the desired `object_storage_format` (see [Table data representation formats]({{ docs_root }}/core/reference/storage/formats#table_formats)).
...
Operator for executing queries in Query Tracker with different syntax. You can learn more about Query Tracker [here]({{ docs_root }}/core/concepts/query-tracker/about). It implements the [run_query](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.run_query) functionality from the Python SDK.
