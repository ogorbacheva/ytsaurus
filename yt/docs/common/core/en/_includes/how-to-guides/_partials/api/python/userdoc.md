...
You can find an example in this [section]({{ docs_root }}/core/how-to-guides/api/python/examples#gevent).
...
When using [CLI]({{ docs_root }}/core/reference/api/cli/cli), you can pass the configuration patch using the `--config` option.
...
* `profiles`: A dict where a key is a profile name and a value is a profile configuration in standard [format]({{ docs_root }}/core/how-to-guides/api/python/userdoc#configuration).
...
Commands in {{product-name}} are classified into light and heavy (to get the system's view on the list of commands, see [Commands]({{ docs_root }}/core/reference/api/commands)).
...
2. The `concatenate_retries` section regulates the retries of the concatenate command (see [API]({{ docs_root }}/core/how-to-guides/api/python/start)). This isn't a light command because it may access different master cells and spend a long time there. That's why you can't use the retry settings for light commands in this case.
...
- `code` (int type): HTTP error code  (see the [section]({{ docs_root }}/core/reference/api/error-codes)). If omitted, it's equal to 1.
...
To learn more about the formats, see the [section]({{ docs_root }}/core/reference/storage/formats).
...
An example of use with comments is [in a separate section]({{ docs_root }}/core/how-to-guides/api/python/examples#yson_string_proxy).

### Batch requests

Low-level API:

- [execute_batch](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.execute_batch): Accepts a set of request descriptions in the list format and returns a set of results. A simple wrapper for the [API command]({{ docs_root }}/core/reference/api/commands#execute_batch).
...
For more information about batch requests, see the [tutorial]({{ docs_root }}/core/how-to-guides/storage/batch-requests).
...
Some command options are shared by command classes. For more information, see [Commands]({{ docs_root }}/core/reference/api/commands).

### Working with Cypress { #cypress_commands }

- [get](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.get): Get the value of the Cypress node. [Read more]({{ docs_root }}/core/how-to-guides/storage/cypress-example#get).

- [set](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.set): Write a value to the Cypress node. [Read more]({{ docs_root }}/core/how-to-guides/storage/cypress-example#set).

- [create](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.create): Create an empty Cypress node of the `type` and with `attributes`. [Read more]({{ docs_root }}/core/how-to-guides/storage/cypress-example#create).

- [exists](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.exists): Check whether the Cypress node exists. [Read more]({{ docs_root }}/core/how-to-guides/storage/cypress-example#exists).

- [remove](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.remove): Delete the Cypress node. [Read more]({{ docs_root }}/core/how-to-guides/storage/cypress-example#remove).

- [list](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.list): Get a list of children for the `path` node. The `absolute` option enables the output of absolute paths instead of relative paths. [Read more]({{ docs_root }}/core/how-to-guides/storage/cypress-example#list).
...
- [copy](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.copy) and [move](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.move): Copy/move the Cypress node. To learn more about the option value, see [section]({{ docs_root }}/core/how-to-guides/storage/cypress-example#copy_move).

- [link](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.link): create a symlink to a Cypress node. [Read more]({{ docs_root }}/core/how-to-guides/storage/cypress-example#link).
...
To access nested attributes, use regular Cypress verbs and navigation using [YPath]({{ docs_root }}/core/reference/storage/ypath).
...
For more information about files in Cypress, see the [section]({{ docs_root }}/core/concepts/storage/files).
...
- Files can be transmitted as arguments of an operation. In that case, they are written to the root of the directory where your jobs will be run. For more information, see the [section](#run_operation_commands) and the [example]({{ docs_root }}/core/how-to-guides/api/python/examples#files).
...
For more information about tables, see [Static tables]({{ docs_root }}/core/concepts/storage/static-tables).
...
- [write_table_structured](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.write_table_structured)
    ...
    With the `table_writer` options, you can specify a number of system [write parameters]({{ docs_root }}/core/reference/storage/io-configuration). To write raw or compressed data, use the `write_table` function.
    ...
    In more complex cases, you might need to build the schema manually. For more information, see [this section](#table_schema) and the [example]({{ docs_root }}/core/how-to-guides/api/python/examples#table_schema).
...
- [read_table_structured](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.read_table_structured)
    ...
    The `table_reader` (dict) option enables you to specify a number of system [read parameters]({{ docs_root }}/core/reference/storage/io-configuration#table_reader).
    ...
    The iterator returned supports the `.with_context()` method that returns an iterator on the `(row, ctx)` pairs. The second item enables you to get the indexes of the current row and range using the `ctx.get_row_index()` and `ctx.get_range_index()` methods (a similar iterator inside the job  also enables you to get the table index: `ctx.get_table_index()`). See examples in the tutorial showing the context in [regular reading]({{ docs_root }}/core/how-to-guides/api/python/examples#read_write) and [inside operations]({{ docs_root }}/core/how-to-guides/api/python/examples#table_switches).
...
- [write_table](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.write_table)
    ...
    Learn the [example]({{ docs_root }}/core/how-to-guides/api/python/examples#read_write_untyped) in the dedicated section.
...
- [read_table](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.read_table)
    ...
    With the `control_attributes` (dict) option, you can request a number of [control attributes]({{ docs_root }}/core/reference/storage/io-configuration#control_attributes) when reading data.
    ...
    See the [example]({{ docs_root }}/core/how-to-guides/storage/examples#read_write_untyped) in the dedicated section.
...
4. Parallel writing won't be effective if the parser used for your format is slow (for example, in the event of [schemaful_dsv]({{ docs_root }}/core/reference/storage/formats#schemaful_dsv)).
...
To learn about the operations available for table data, see the [section]({{ docs_root }}/core/reference/data-processing/operations/overview).
...
- [run_erase](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.run_erase): [The operation that deletes data from a table]({{ docs_root }}/core/reference/data-processing/operations/erase)
- [run_merge](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.run_merge): [The operation to merge/process data in the table without running the custom code]({{ docs_root }}/core/reference/data-processing/operations/merge)
- [run_sort](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.run_sort): [Data sorting operation]({{ docs_root }}/core/reference/data-processing/operations/sort). If the destination table is omitted, inplace sorting of the table is performed.
- [run_map_reduce](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.run_map_reduce): [MapReduce operation]({{ docs_root }}/core/reference/data-processing/operations/mapreduce).
- [run_map](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.run_map): [Map operation]({{ docs_root }}/core/reference/data-processing/operations/map).
- [run_reduce](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.run_reduce): [Reduce operation]({{ docs_root }}/core/reference/data-processing/operations/reduce).
- [run_join_reduce](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.run_join_reduce): [JoinReduce operation]({{ docs_root }}/core/reference/data-processing/operations/reduce#foreign_tables).
...
- [run_remote_copy](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.run_remote_copy): [RemoteCopy operation]({{ docs_root }}/core/reference/data-processing/operations/remote-copy)
...
The names of the methods correspond to option names in the specification listed [here]({{ docs_root }}/core/reference/data-processing/operations/operations-options).
...
1. Spec builders for filling out [I/O parameters]({{ docs_root }}/core/reference/storage/io-configuration):
...
see the [example in the tutorial]({{ docs_root }}/core/how-to-guides/api/python/examples#spec_builder).
...
- [run_job_shell](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.run_job_shell): Run a job-shell for the job. The easiest way to use this function is with its analogous command in [CLI]({{ docs_root }}/core/how-to-guides/api/cli/examples).
...
- [get_operation](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.operation_commands.get_operation): Get information about the operation based on its ID. A `dict` is returned with fields similar to the fields in the [get_operation]({{ docs_root }}/core/reference/api/commands#get_operation) response.
- [list_operations](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.operation_commands.list_operations): Get information about a set of operations based on filters. The meaning of the fields is similar to `get_operation`. For a list of filters, see [Commands]({{ docs_root }}/core/reference/api/commands#list_operations).
...
- [get_job](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.operation_commands.get_job): Get information about the job. A `dict` is returned with fields similar to the fields in the [get_job]({{ docs_root }}/core/reference/api/commands#get_job) response.
- [list_jobs](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.operation_commands.list_jobs): Get information about the set of jobs for the operation. The meaning of fields in the response is similar to that in `get_job`. For a list of filters, see [Commands]({{ docs_root }}/core/reference/api/commands#list_jobs).
...
For debugging of failed jobs, it's convenient to use the [job tool]({{ docs_root }}/core/how-to-guides/problems/jobstatistics). This utility enables you to prepare the environment similar to the job environment and run it with the same input data.
...
[Dynamic tables]({{ docs_root }}/core/concepts/dynamic-tables/overview) implement an interface both for point reads and for key-based data writes with transaction support and a native SQL dialect.

The list of commands to access the content of dynamic tables.
- [insert_rows](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.dynamic_table_commands.insert_rows): [Insert]({{ docs_root }}/core/concepts/dynamic-tables/sorted-dynamic-tables#insert_rows) rows to a dynamic table
- [lookup_rows](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.dynamic_table_commands.lookup_rows): [Select]({{ docs_root }}/core/concepts/dynamic-tables/sorted-dynamic-tables#chtenie-stroki) the rows with the specified keys
- [delete_rows](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.dynamic_table_commands.delete_rows): [Delete]({{ docs_root }}/core/concepts/dynamic-tables/sorted-dynamic-tables#udalenie-stroki) the rows with the specified keys.
- [select_rows](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.dynamic_table_commands.select_rows): [Select]({{ docs_root }}/core/concepts/dynamic-tables/sorted-dynamic-tables#vypolnenie-zaprosa) the rows that match the query written in the [SQL dialect]({{ docs_root }}/core/reference/dynamic-tables/dyn-query-language)
- [lock_rows](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.dynamic_table_commands.lock_rows): [Lock]({{ docs_root }}/core/concepts/dynamic-tables/sorted-dynamic-tables#blokirovka-stroki) the rows with the given keys
- [trim_rows](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.dynamic_table_commands.trim_rows): [Delete]({{ docs_root }}/core/concepts/dynamic-tables/ordered-dynamic-tables#trim) the specified number of rows from the other table

Commands related to [mounting]({{ docs_root }}/core/concepts/dynamic-tables/overview#mount_table) of tables.
...
Commands related to [sharding]({{ docs_root }}/core/how-to-guides/dynamic-tables/resharding).
...
Other commands.
- [freeze_table](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.dynamic_table_commands.freeze_table): [Freeze]({{ docs_root }}/core/concepts/dynamic-tables/overview#zamorozka) the table
- [unfreeze_table](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.dynamic_table_commands.unfreeze_table): [Unfreeze]({{ docs_root }}/core/concepts/dynamic-tables/overview#zamorozka) the table
- [balance_tablet_cells](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.dynamic_table_commands.balance_tablet_cells): Spread the tablets evenly across [tablet cells]({{ docs_root }}/core/concepts/dynamic-tables/overview#tablet_cells)
- [get_tablet_infos](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.dynamic_table_commands.get_tablet_infos): Get the [tablet]({{ docs_root }}/core/concepts/dynamic-tables/overview#tablets)'s attributes
- [get_tablet_errors](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.dynamic_table_commands.get_tablet_errors): Get the errors that occurred on the [tablet]({{ docs_root }}/core/concepts/dynamic-tables/overview#tablets)
- [alter_table_replica](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.dynamic_table_commands.alter_table_replica): [Change attributes]({{ docs_root }}/core/concepts/dynamic-tables/replicated-dynamic-tables#nastrojki-replik) of a replica of the [replicated table]({{ docs_root }}/core/concepts/dynamic-tables/replicated-dynamic-tables)

### Other commands { #etc_commands }

#### Converting tables

**Transform.** Occasionally, you might need to transform your table to a new compression codec, erasure codec, or a [new chunk format]({{ docs_root }}/core/concepts/storage/chunks). Such tasks are solved by the [transform](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.client_impl.YtClient.transform) function.
...
Make sure to define, in the job class, the `__call__(self, row)` method (for the mapper) or `__call__(self, rows)` method (for the reducer). As input, this method accepts table rows (in the case of a reducer, a single call (`__call__`) corresponds to a set of rows with the same key). It has to return (**by using `yield`**) the rows that need to be written to the output table. If there are multiple output tables, use the wrapper class `yt.wrapper.OutputRow`, whose constructor accepts the row written and the `table_index` as a named parameter (see the [example]({{ docs_root }}/core/how-to-guides/api/python/examples#table_switches) in the tutorial).
...
To specify the input and output string types in the job class, you can use the hint type: [one]({{ docs_root }}/core/how-to-guides/api/python/examples#simple_map), [two]({{ docs_root }}/core/how-to-guides/api/python/examples#multiple_input_reduce), [three]({{ docs_root }}/core/how-to-guides/api/python/examples#multiple_input_multiple_output_reduce), and [four]({{ docs_root }}/core/how-to-guides/api/python/examples#map_reduce_multiple_intermediate_streams)) or override the method [`.prepare_operation(self, context, preparer)`](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.prepare_operation.TypedJob.prepare_operation). The types are specified using the methods of the `preparer` object of the [`OperationPreparer`](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.prepare_operation.OperationPreparer) type. Useful methods:
...
See the examples in the tutorial: [one]({{ docs_root }}/core/how-to-guides/api/python/examples#prepare_operation) and [two]({{ docs_root }}/core/how-to-guides/api/python/examples#grep).

If you run MapReduce with multiple intermediate streams, you also need to override the [.get_intermediate_stream_count(self)](https://pydoc.ytsaurus.tech/yt.wrapper.html#yt.wrapper.prepare_operation.TypedJob.get_intermediate_stream_count) method, returning the number of intermediate streams from it. See [example]({{ docs_root }}/core/how-to-guides/api/python/examples#map_reduce_multiple_intermediate_streams).
...
You can find examples in the [tutorial]({{ docs_root }}/core/how-to-guides/api/python/examples#job_decorators).
...
When running an operation, you can specify which [file system image]({{ docs_root }}/core/reference/data-processing/layers/layer-paths) needs to be prepared before running your jobs.
There is a certain set of [ready-to-use layers]({{ docs_root }}/core/reference/data-processing/layers/layer-paths#gotovye-sloi-v-kiparise) available at `//porto_layers`.
...
You can find examples in the [tutorial]({{ docs_root }}/core/how-to-guides/api/python/examples#job_decorators_untyped).
...
Apart from a stream of records, when reading data from a table (or from within a job), you can request various [control attributes]({{ docs_root }}/core/reference/storage/io-configuration#control_attributes). Working with control attributes is format-dependent: for example, most control attributes can't be represented in formats different from Yamr, JSON, or YSON.
...
Examples of switching between output tables with `table_index` at `control_attributes_mode` equal to `iterator` and `row_fields`. [Here]({{ docs_root }}/core/how-to-guides/api/python/examples#table_switches_untyped) you can find an example of how to get an index of the current table in the reducer using `context`.

#### Other formats { #other_formats }

By default, the library serializes data into [YSON format]({{ docs_root }}/core/reference/storage/formats#yson) because this format provides a proper and unambiguous representation for any data stored in {{product-name}} tables. To handle YSON format efficiently in Python, you need the [native library](#yson_bindings). If you don't add this library to the cluster, you will get a runtime error in your job (we didn't add a fallback to the Python's YSON library because it's very slow and inefficient). In this situation, you can switch over to the [JSON format]({{ docs_root }}/core/reference/storage/formats#json).
...
Together with the library to work with {{product-name}} clusters,  we also supply a library for [YSON format]({{ docs_root }}/core/reference/storage/yson). The library is available in the `yt.yson` module and implements the standard `load`, `loads`, `dump`, and `dumps` functions. In addition, it provides access to [Yson types](#structured_data). The library also implements the following generally useful functions:
