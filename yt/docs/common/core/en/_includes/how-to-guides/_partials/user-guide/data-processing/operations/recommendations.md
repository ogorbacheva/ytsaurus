...
A [chunk]({{ docs_root }}/core/concepts/storage/chunks) under 100 MB may be considered small. To prevent there being too many chunks in the system, you should look to have an average chunk size of at least 512 MB.
...
You need to set the `<mode>` attribute to `sorted` for a [sorted table]({{ docs_root }}/core/concepts/storage/static-tables#sorted_tables) and to `ordered` for an [unsorted]({{ docs_root }}/core/concepts/storage/static-tables#unsorted_tables) one. For more information on table types, please see [Static tables]({{ docs_root }}/core/concepts/storage/static-tables).
...
If you are using a [python library]({{ docs_root }}/core/how-to-guides/api/python/userdoc), you can specify the `auto_merge_output={action=merge}` configuration option which will make the library automatically aggregate the resulting tables if their chunks are too small.

## MapReduce vs Map+Sort+Reduce

For more information on MapReduce internals and on why it is normally faster than the Map+Sort+Reduce sequence, please see [MapReduce]({{ docs_root }}/core/reference/data-processing/operations/mapreduce).
...
2. The data is appended. A table is time-sorted, for instance. An additional amount of data comes in with all the key (time) values greater than all the key (time) values in the table. Then, a sort can be applied to this batch of data followed by a Reduce with `teleport=%true`. For more information on the option, please see the [Reduce]({{ docs_root }}/core/reference/data-processing/operations/reduce#foreign_tables) section.
...
To solve this problem, {{product-name}} has [Reduce combiners]({{ docs_root }}/core/reference/data-processing/operations/mapreduce#reduce_combiner.md) that help process large keys in several jobs in the reduce phase.
