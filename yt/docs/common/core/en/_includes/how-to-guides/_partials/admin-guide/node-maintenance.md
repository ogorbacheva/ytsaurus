...
```bash
$ yt add-maintenance --component="cluster_node" --type="pending_restart" --address="<node-address>"  --comment="my comment"
```

To make sure that the _restart pending_ flag is set for the node, use the following command
```bash
$ yt get //sys/cluster_nodes/<node-address>/@pending_restart
> true
```
...
```bash
$ yt remove-maintenance --component="cluster_node" --address="<node-address>" --id="<maintenance-id>"
```
