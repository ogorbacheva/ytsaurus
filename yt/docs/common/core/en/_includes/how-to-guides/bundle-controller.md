...
Set the flag on the bundle in the cluster which you want to cover. For example, if you waht to run drills for a bundle on `<cluster-name>`, you should set `enable_drills_mode = %true` for this bundle on `<cluster-name>`.
