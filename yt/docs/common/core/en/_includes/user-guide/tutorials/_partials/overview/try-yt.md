...
Regardless of the installation method, the required system components will be deployed in the process, including the [master server]({{ docs_root }}/_includes/overview/*about-master), [scheduler]({{ docs_root }}/_includes/overview/*about-scheduler), [YQL]({{ docs_root }}/_includes/overview/*about-yql), [Query Tracker]({{ docs_root }}/_includes/overview/*about-qt), and others. All examples in this guide — table creation, data upload, and running MapReduce — apply regardless of your preferred installation method and will be the same for both Docker and Minikube.
...
- Docker {selected}
  ...
  3. Run the script to deploy the cluster:
     ...
     The script creates and runs [docker containers]({{ docs_root }}/_includes/overview/*about-docker-containers) for deploying {{product-name}}. If the operation is successful, you'll see the following message:
     ```
  ...
  4. To make sure everything works correctly, run the following command:
     ...
     - `yt.backend`: Hosts {{product-name}} cluster components.
       ...
       - The [master server]({{ docs_root }}/core/admin-guide/reference/components#master) is responsible for fault-tolerant storage of the cluster's metadata. This includes information about system users, stored objects, and the location of the data itself.
       - The node combines the functionality of a [data node]({{ docs_root }}/core/admin-guide/reference/components#data-nodes) and an [exec node]({{ docs_root }}/core/admin-guide/reference/components#exec-nodes). It is responsible for data storage, dynamic tables, and job execution.
       - The [scheduler]({{ docs_root }}/core/admin-guide/reference/components#planirovshik) plans data processing operations, such as Map and Reduce.
       - The [controller agent]({{ docs_root }}/core/admin-guide/reference/components#planirovshik) schedules the jobs of individual operations.
       - The [HTTP proxy]({{ docs_root }}/core/admin-guide/reference/components#proxy) is a server used for external communication.
       - The [YQL agent]({{ docs_root }}/core/admin-guide/reference/components#yql-agent) is an execution engine for SQL-like queries.
       - The [{{product-name}} queue]({{ docs_root }}/core/user-guide/reference/dynamic-tables/queues) contains ordered dynamic tables.
       - [Query Tracker]({{ docs_root }}/core/user-guide/explanation/query-tracker/about) is a component for running queries in different SQL dialects (YQL, QT, CHYT, SPYT).
...
- Kind
  ...
  To access the cluster API at `localhost:8081`, run this command in a separate terminal:
   ...
   To work via [RPC proxy]({{ docs_root }}/core/user-guide/reference/proxy/rpc) at `localhost:8082`, run this command in a separate terminal:
...
The most convenient way to interact with the {{product-name}} system is through the console. The [{{product-name}} CLI]({{ docs_root }}/core/user-guide/reference/api/cli/cli) utility isn't installed as part of the cluster deployment process. You need to install it on your system separately.
...
In {{product-name}}, all data is stored in [tables]({{ docs_root }}/core/user-guide/explanation/storage/static-tables). Let's create one!
...
The value `16-64ca-10191-47007b7d` is the ID of the created [Cypress]({{ docs_root }}/_includes/overview/*about-cypress) node. Node IDs are useful when working with [transactions]({{ docs_root }}/core/user-guide/explanation/storage/transactions) in {{product-name}}. You won't need these IDs in this example.
...
Now write some data to the table by calling the [write-table]({{ docs_root }}/core/user-guide/reference/api/cli/commands#write-table) command:
...
Another way to read a table is by running a [SELECT]({{ yql-docs-root }}/{{ lang }}/yql/syntax/select/index{{ docs-revision-query }}) query in the web interface. To do this, go to the **Queries** tab and enter the following query:
...
Now write some data to the source table by calling the [write-table]({{ docs_root }}/core/user-guide/reference/api/cli/commands#write-table) command:

```bash prompt="$"
$ cat source.tsv | yt write-table //home/mapreduce_input --format dsv
```
To verify that the data has been written to the table, use the [read-table]({{ docs_root }}/core/user-guide/reference/api/cli/commands#read-table) command. The half-interval specified in the square brackets indicates that we want to get the first six rows of the table:
...
Run the MapReduce operation using the [map-reduce]({{ docs_root }}/core/user-guide/reference/api/cli/commands#map-reduce) command:
...
Here you can test out the features of the {{product-name}} web interface: explore the file system, see the list of pools, and run queries in [Query Tracker]({{ docs_root }}/core/user-guide/explanation/query-tracker/about).
...
[*about-cypress]: Cypress is a distributed file system and metadata storage. Cypress stores tables and files. To learn more, see the [documentation]({{ docs_root }}/core/user-guide/explanation/storage/cypress).
...
[*about-qt]: A tool for running SQL-like queries. Query Tracker is available in the web interface, under **Queries**. To learn more, see the [documentation]({{ docs_root }}/core/user-guide/explanation/query-tracker/about).
...
[*about-master]: The master server is responsible for fault-tolerant storage of the cluster's metadata. This includes information about system users, stored objects, and the location of the data itself. To learn more, see the [documentation]({{ docs_root }}/core/admin-guide/reference/components#master).

[*about-scheduler]: The scheduler is responsible for allocating resources between operations as well as for their execution on the cluster. To learn more, see the [documentation]({{ docs_root }}/core/user-guide/explanation/data-processing/scheduler/scheduler-and-pools).
