...
You can get data about which table an entry came from, and select which table to send it to, in the operation's [Map]({{ docs_root }}/core/user-guide/reference/data-processing/operations/map) and [Reduce]({{ docs_root }}/core/user-guide/reference/data-processing/operations/reduce) user code. This is done using **[descriptors](#descriptors)** or **[table switches](#switches)**.
...
When no mapper is specified for a [MapReduce]({{ docs_root }}/core/user-guide/reference/data-processing/operations/mapreduce) type operation, there will be no data about which input tables entries came from at the reducer input.
...
The schema of configuring switches for input tables can be read in the [Input/output settings]({{ docs_root }}/core/user-guide/reference/storage/io-configuration).

The switches form depends on the format of output data.

### YSON { #yson }

In [YSON]({{ docs_root }}/core/user-guide/reference/storage/yson), in the entry stream, you might encounter an [entity]({{ docs_root }}/core/user-guide/reference/storage/yson#entity) with attributes for both input and output tables.  These are control inputs that are built into the datastream. For example: `table_index=N`, where N is an integer. This command "switches" the stream in such a way that subsequent entries are connected to another table with index N. This is true for both input and output tables.
...
[DSV]({{ docs_root }}/core/user-guide/reference/storage/formats#dsv) does not support switching of input tables.
...
When installing the `enable_table_index=true` option for tables in [SCHEMAFUL_DSV]({{ docs_root }}/core/user-guide/reference/storage/formats#schemaful_dsv) format, the table index will be recorded in all lines of the table with the first field, before the schema columns.
