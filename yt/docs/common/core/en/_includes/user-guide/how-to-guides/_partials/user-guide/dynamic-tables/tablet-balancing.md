...
* Table sharding.
* Redistributing tablets between [tablet cells]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/concepts#tablet_cells).
...
You can configure balancing both on a per-table basis and for each [tablet cell bundle]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/concepts#tablet_cell_bundles). You can also configure balancing for groups of tables. Table configuration has a higher priority than group configuration, which in turn has a higher priority than bundle configuration.

The system has two types of balancers: the legacy balancer, which lives in the master process and supports only balancing by tablet size and a limited number of options, and the standalone balancer that also supports balancing by load. {{ core-user-guide-how-to-guides-dynamic-tables-tablet-balancing-md-audience-1 }}

{{ core-user-guide-how-to-guides-dynamic-tables-tablet-balancing-md-audience-2 }}
...
| --- | --- | --- | --- |
| [in-memory move]({{ docs_root }}/_includes/user-guide/dynamic-tables/*in-memory-move) | enable_in_memory_cell_balancer | enable_move | enable_auto_tablet_move |
| [ordinary move]({{ docs_root }}/_includes/user-guide/dynamic-tables/*ordinary-move) | enable_cell_balancer | enable_move | enable_auto_tablet_move |
| [parameterized move]({{ docs_root }}/_includes/user-guide/dynamic-tables/*parameterized-move) | - | enable_move | enable_auto_tablet_move |
| [reshard]({{ docs_root }}/_includes/user-guide/dynamic-tables/*reshard) | enable_tablet_size_balancer | enable_reshard | enable_auto_reshard |
| [parameterized reshard]({{ docs_root }}/_includes/user-guide/dynamic-tables/*parameterized-reshard) | - | enable_reshard | enable_auto_reshard |
