...
{{product-name}} enables you to specify [sorted dynamic tables]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/sorted-dynamic-tables) as output tables of map-reduce operations (the so-called bulk insert).
...
{{ core-user-guide-how-to-guides-dynamic-tables-bulk-insert-md-audience-1 }}
...
You can take the service column names from the [api]({{ core-user-guide-how-to-guides-dynamic-tables-bulk-insert-md-audience-2 }}).

`$change_type` accepts values from the [ERowModificationType]({{ core-user-guide-how-to-guides-dynamic-tables-bulk-insert-md-audience-3 }}) enum. Only `Write` and `Delete` values are allowed.
...
The value flags accept the bit mask of values of the [EUnversionedUpdateDataFlags]({{ core-user-guide-how-to-guides-dynamic-tables-bulk-insert-md-audience-4 }}) enum. The `missing | aggregate` combination makes no sense, but it is acceptable. If no flags are specified for a column, they are considered equal to 0.
...
{{ core-user-guide-how-to-guides-dynamic-tables-bulk-insert-md-audience-6 }}
