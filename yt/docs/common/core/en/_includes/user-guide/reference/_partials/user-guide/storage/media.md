...
- Key: `"data_parts_only"`; value: a flag. On media with `"data_parts_only" = %true`, [erasure coded]({{ docs_root }}/core/user-guide/reference/storage/replication#erasure) object only store data parts and do not store erasure parts. For example:
...
[Cypress]({{ docs_root }}/core/user-guide/explanation/storage/cypress) enables you to read individual records from a dictionary without reading the entire dictionary.
...
Attributes `primary_medium` and `media` are [inheritable]({{ docs_root }}/core/user-guide/reference/storage/attributes#system_attr). This means they can be set on a directory, a then every newly created chunk owner in the subtree will receive corresponding attribute values automatically. Using the `primary_medium` attribute is preferred in most cases; using `media` this way is not recommended.
