...
Communication between consumers and queues is enabled by *registration* objects. {{ core-user-guide-reference-dynamic-tables-queues-md-audience-1 }}
...
To ensure that your graphs and statistics are comprehensive, we recommend adding the [timestamp]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/ordered-dynamic-tables#timestamp_column) and [cumulative_data_weight]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/ordered-dynamic-tables#cumulative_data_weight_column) columns to the table schema.
...
Both Cypress arguments in both of these commands are of [rich YPath]({{ docs_root }}/core/user-guide/reference/storage/ypath#rich_ypath) type and can take cluster names. Otherwise, the command uses the cluster on which it was executed.
...
The queue parameter in this method is a [rich YPath]({{ docs_root }}/core/user-guide/reference/storage/ypath#rich_ypath) that can take the cluster on which the queue is located. Otherwise, the command uses the cluster on which it was executed.
