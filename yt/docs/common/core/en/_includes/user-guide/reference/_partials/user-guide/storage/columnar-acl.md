...
The general operation principles of the access control system in {{product-name}} are described in the [Access control]({{ docs_root }}/core/user-guide/explanation/storage/access-control) section.
...
This feature is only available for [schematic data.]({{ docs_root }}/core/user-guide/reference/storage/static-schema). In particular, we assume that `Cs` is a subset of all columns explicitly specified in the schema of table `T`.

Set `Cs` is usually specified by the user when setting a column filter, for example, via [YPath]({{ docs_root }}/core/user-guide/reference/storage/ypath). When no filter is specified, `Cs` are considered a set of all columns described in the table schema. If the table schema is not strict, additional columns can be found in the data, but no special checks are made for them.
