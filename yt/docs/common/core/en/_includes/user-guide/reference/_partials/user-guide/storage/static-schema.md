...
| `type_v3` | `any` | Alternative type representation. This field enables you to specify columns with [composite types]({{ docs_root }}/core/user-guide/reference/storage/data-types#schema). | Yes, if `type` is not set |
...
| `expression` | `string` | An expression defining the value of the computed column field. For key columns only. For more information, see [Sharding]({{ docs_root }}/core/user-guide/how-to-guides/dynamic-tables/resharding). | No |
...
{{product-name}} supports the column types described in the [Data types]({{ docs_root }}/core/user-guide/reference/storage/data-types) section.
...
Other keys depend on the specific type and are described on the [Data types]({{ docs_root }}/core/user-guide/reference/storage/data-types) page.
...
- Performance:
  - If you additionally enable the columnar format of storing [chunks]({{ docs_root }}/core/user-guide/explanation/storage/chunks) `optimize_for = scan`, data will be stored more compactly and reading a subset of columns will be faster.
  - Some table readers can work faster for tables with a schema.
- Convenience and more options:
  - {{ core-user-guide-reference-storage-static-schema-md-audience-1 }} is more convenient for working with schematized tables.
  - In [CHYT]({{ chyt-docs-root }}/{{ lang }}/user-guide/explanation/about-chyt{{ docs-revision-query }}), you cannot work with unschematized data.
