...
* Storage:
   ...
   * Quota on the master server RAM used by the nodes in the metadata tree. Not requested as a separate resource but rather issued subject to agreement. Takes into account chunk and trunk node metadata, trunk node attributes, [dynamic table]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/overview) pivot keys as well as other data. Rule-of-thumb estimate: 100 to 200 bytes per node and the same amount per chunk. The above values may vary greatly depending on attribute and node name lengths, the lengths of the values in chunk key columns, and so on.
   * Quota on the number of dynamic table [tablets (shards)]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/overview#tablets). Not requested separately but issued upon request. The quota is allocated for the [tablet_cell_bundle]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/concepts#tablet_cell_bundles) object.
* Processing:
...
For more information on media, see the [Media]({{ docs_root }}/core/user-guide/reference/storage/media) section.
...
Data processing computational resources are linked to [pools]({{ docs_root }}/core/user-guide/explanation/data-processing/scheduler/scheduler-and-pools) (except tablet_cell_bundle).
...
{{ core-user-guide-reference-storage-quotas-md-audience-1 }}

{% note info "Note" %}

Dynamic tables require a disk quota to write journals (medium ssd_journals) and snapshots (medium default){{ core-user-guide-reference-storage-quotas-md-audience-2 }}. The required quotas are a quota on file system objects (node_count) and a quota on the number of tablets (table shards, or tablet_count).

{% endnote %}

{{ core-user-guide-reference-storage-quotas-md-audience-3 }}
