...
All types of data storage quotas are linked to the project **account**. An account is a service object in [Cypress]({{ docs_root }}/core/user-guide/explanation/storage/cypress). All accounts have unique names. However, they are located in a separate namespace, so the username and account name may be the same. The account name is stored in the `name` attribute which is both readable and writable.
...
![](../../../../../../_images/user-guide/reference/storage/account_tree_view.png){ .center }
...
2. Specify {{ core-user-guide-reference-storage-accounts-md-audience-1 }}the name of the new account, the name of the parent account into which the created account will be nested, and the users responsible for the new account. If necessary, you can also request the automatic creation of a home directory corresponding to the account.

![](../../../../../../_images/user-guide/reference/storage/account_tree_create.png){ .center }
...
![](../../../../../../_images/user-guide/reference/storage/account_tree_transfer_resources.png){ .center }
...
In case of the `argument command: invalid choice` error, update the `yt` utility. For more information, see [Try YTsaurus]({{ docs_root }}/core/user-guide/tutorials/quick-start).
