...
For more information about path modifiers, see [YPath]({{ docs_root }}/core/user-guide/reference/storage/ypath).
...
Different types of objects have their own set of type-specific system attributes. For example, all objects have the `id` attribute, tables have the `compression_codec` and `compressed_data_size` attributes. For more information about the attributes inherent to all objects, see [Objects]({{ docs_root }}/core/user-guide/explanation/storage/objects).
...
System attributes can also be **opaque** (or **computed**). The value of an **opaque** attribute will be available only when explicitly requested with the `get` command, but will not be available when all attributes are requested. Specifically, when all attributes are requested, the value of an opaque attribute will be set to **entity**. Most often, attributes are declared as opaque when they have a large value (in terms of occupied memory) or take a long time to compute.{{ core-user-guide-reference-storage-attributes-md-audience-1 }} An example of an opaque attribute will be a list of all table chunks (the `chunk_ids` attribute) or the total usage of resources by Cypress subtree (the `recursive_resource_usage` attribute).

Below is a list of attributes of {{product-name}} system objects:

- [Attributes of any object]({{ docs_root }}/core/user-guide/explanation/storage/objects#attributes).
- [Attributes of Cypress nodes]({{ docs_root }}/core/user-guide/explanation/storage/cypress#attributes).
- [Attributes of static tables]({{ docs_root }}/core/user-guide/explanation/storage/static-tables#attributes).
- [Attributes of dynamic tables]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/overview#attributes).
- [Attributes of files]({{ docs_root }}/core/user-guide/explanation/storage/files#attributes).
- The [compression_codec]({{ docs_root }}/core/user-guide/reference/storage/compression#get_compression) and [erasure_codec]({{ docs_root }}/core/user-guide/reference/storage/replication#erasure) attributes for tables and files.
- [Attributes of transactions]({{ docs_root }}/core/user-guide/explanation/storage/transactions#attributes).
- [Attributes of links]({{ docs_root }}/core/user-guide/explanation/storage/links#attributes).
- [Attributes of accounts]({{ docs_root }}/core/user-guide/reference/storage/accounts#account_attributes).
- [Attributes of media]({{ docs_root }}/core/user-guide/reference/storage/media#atributy).
...
A popular scenario for using attributes is annotating the path to tables and files when making requests. For more information, see [Operation options]({{ docs_root }}/core/user-guide/reference/data-processing/operations/operations-options#path_attributes) and [YPath]({{ docs_root }}/core/user-guide/reference/storage/ypath#known_attributes).
