...
1. When writing to a table or file, data is split into parts — [chunks]({{ docs_root }}/core/user-guide/explanation/storage/chunks).
...
If the table is permanently written to, there is usually no need for [forced compression]({{ docs_root }}/core/user-guide/how-to-guides/dynamic-tables/compaction#forced_compaction). If you need to force compression, use `forced compaction`.
...
- Compression is usually not applied to files that are used as [symlinks]({{ docs_root }}/core/user-guide/explanation/storage/links) to chunks in an operation.
...
After the operation is complete, you will see `codec/cpu/encode`, `codec/cpu/decode`, and `compression_ratio` for each algorithm. For more information, see [Job stats]({{ docs_root }}/core/user-guide/how-to-guides/problems/jobstatistics#system_stats).
