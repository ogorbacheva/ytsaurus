...
* Vanilla operations can be used to start a distributed process: you can describe several types of jobs and specify how many jobs of each type there should be.
  ...
  [The {{product-name}} scheduler]({{ docs_root }}/core/user-guide/explanation/data-processing/scheduler/scheduler-and-pools) carries the function of keeping this distributed process running, working to have the required number of jobs of each type planned and completed, just like in any other {{product-name}} operation.
...
* A task description in a Vanilla operation is very similar to a user script description, but we should draw a distinction between these concepts, because they correspond to different things in the [Scheduler]({{ docs_root }}/core/user-guide/explanation/data-processing/scheduler/scheduler-and-pools).
...
When [reviving]({{ docs_root }}/core/user-guide/explanation/data-processing/reliability#revival) a gang operation, the system checks that all non-reserved jobs have been successfully restored. If this is not the case, the operation will be restarted without reusing allocations.
