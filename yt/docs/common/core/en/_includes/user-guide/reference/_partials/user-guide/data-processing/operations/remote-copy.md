...
The data transfer rate between clusters is subject to throttling. For more information, see [Inter-cluster Network Bandwidth Throttling]({{ docs_root }}/core/user-guide/reference/data-processing/operations/cluster-throttlers).
...
* `schema_inference_mode` (auto) — schema definition mode. Possible values: auto, from_input, from_output. To learn more, see [Data schema]({{ docs_root }}/core/user-guide/reference/storage/static-schema#schema_inference).
...
The RemoteCopy (`remote_copy`) operation is supported only for [sorted tables]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/sorted-dynamic-tables). To copy an ordered table, you need to convert it to a static one. It is impossible to copy an ordered table to another cluster with preserving tablets.

{{ core-user-guide-reference-data-processing-operations-remote-copy-md-audience-1 }}
