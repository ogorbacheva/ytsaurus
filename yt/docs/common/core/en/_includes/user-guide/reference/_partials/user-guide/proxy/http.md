...
You can find a full description of all {{product-name}} commands in the [Commands]({{ docs_root }}/core/user-guide/reference/api/commands) section.

{% note info "Note" %}

To understand the material presented in this section, you need to know the [HTTP protocol](https://en.wikipedia.org/wiki/HTTP) structure, the principles of working via HTTP, and the basics of [data storage]({{ docs_root }}/core/user-guide/explanation/storage/cypress) in the {{product-name}} system.
...
The Word Count example is a typical task for any [MapReduce]({{ docs_root }}/core/user-guide/reference/data-processing/operations/mapreduce) system. The general schema is as follows:

1. A [Map]({{ docs_root }}/core/user-guide/reference/data-processing/operations/map) operation is performed on the source text, producing a pair `(word, 1)` for each word.
2. The result is sorted by the first coordinate.
3. A [Reduce]({{ docs_root }}/core/user-guide/reference/data-processing/operations/reduce) operation that summarizes the second coordinate is performed on the first coordinate. The result is a set of pairs (`word, number of word mentions)`.

Further, the [curl](https://en.wikipedia.org/wiki/CURL) utility is used to work over HTTP. The environment variables listed below were set for convenience.{{ core-user-guide-reference-proxy-http-md-audience-1 }}
...
Data in the {{product-name}} system is stored in [tables]({{ docs_root }}/core/user-guide/explanation/storage/static-tables), so you need to create a table. The table contains two columns: row number and row contents. Use the [create]({{ docs_root }}/core/user-guide/reference/api/commands#create) command to create a table.
...
You need to upload the data into the table. To do this, use the [write_table]({{ docs_root }}/core/user-guide/reference/api/commands#write_table) command. The tab-separated format is used as the data format:
...
To upload, for example, JSON, you need to add a bit more options. For more information, see [Formats]({{ docs_root }}/core/user-guide/reference/storage/formats#json).
...
To run a [Map]({{ docs_root }}/core/user-guide/reference/data-processing/operations/map) or [Reduce]({{ docs_root }}/core/user-guide/reference/data-processing/operations/reduce) operation, you will need to upload a script that must be executed into the system. The script for this task is placed on [GitHub](https://github.com/ytsaurus/ytsaurus/tree/main/yt/cpp/mapreduce/examples/python-misc/http_proxy.py) (however, it is locally saved to `exec.py` ). The [write_file]({{ docs_root }}/core/user-guide/reference/api/commands#write_table) command is used to upload files to the {{product-name}} system. Before uploading, create a node of the "file" type, similar to creating the table at the previous step.
...
In the {{product-name}} system, to run a Map operation, there is a [command]({{ docs_root }}/core/user-guide/reference/api/commands#map) with the same name. An important thing to consider is the requirement that an output table must exist **before** an operation is started. You can also create a table using the [create]({{ docs_root }}/core/user-guide/reference/api/commands#create) command.
...
To run sorting, use the [sort]({{ docs_root }}/core/user-guide/reference/data-processing/operations/sort) command which is a bit like [Map]({{ docs_root }}/core/user-guide/reference/data-processing/operations/map) and [Reduce]({{ docs_root }}/core/user-guide/reference/data-processing/operations/reduce) in its input specification.
...
The Reduce operation is started using the [command]({{ docs_root }}/core/user-guide/reference/api/commands#reduce) with the same name.
