...
- **[Map]({{ docs_root }}/core/user-guide/reference/data-processing/operations/map), [Reduce]({{ docs_root }}/core/user-guide/reference/data-processing/operations/reduce), and [MapReduce]({{ docs_root }}/core/user-guide/reference/data-processing/operations/mapreduce)** execute user-provided code against input data.
- **[Sort]({{ docs_root }}/core/user-guide/reference/data-processing/operations/sort)** sorts input tables.
- **[Merge]({{ docs_root }}/core/user-guide/reference/data-processing/operations/merge)** merges tables.
- **[Erase]({{ docs_root }}/core/user-guide/reference/data-processing/operations/erase)** deletes specified data from a table.
- **[RemoteCopy]({{ docs_root }}/core/user-guide/reference/data-processing/operations/remote-copy)** copies data between clusters.
- **[Vanilla]({{ docs_root }}/core/user-guide/reference/data-processing/operations/vanilla)** launches an appropriate number of **user scripts** on cluster nodes and keeps them running.

Each operation has a `spec` parameter, which, as the name implies, conveys the operation's **specification** (aka its options). The specification is a hierarchical structure that exists in the [YSON format]({{ docs_root }}/core/user-guide/reference/storage/formats#yson). Each operation type has its own specification structure, whose detailed description can be found in the subsections dedicated to specific operation types.

**User script** is a user-defined string that is executed via a `bash -c` call, plus the parameters detailing the execution. User scripts run in a `sandbox` environment, where they are executed by a service user that is not formally present in the system (referring to Linux system users) and that only has read/write rights for the current directory and for the `tmp` directory. To learn more about user script parameters, see [Operation options]({{ docs_root }}/core/user-guide/reference/data-processing/operations/operations-options#user_script_options).

Operations are run in **compute pools**, which are structured as a tree and serve to allocate the cluster's compute resources. For more information, see [Scheduler and pools]({{ docs_root }}/core/user-guide/explanation/data-processing/scheduler/scheduler-and-pools).
...
- `pending` — the operation is queued for launch. Each [pool]({{ docs_root }}/core/user-guide/explanation/data-processing/scheduler/scheduler-and-pools) has a limit on the number of simultaneously running operations, and, if it is exceeded, operations are sent to a queue.
...
- `reviving` — the operation is being recovered following the restart of the [scheduler]({{ docs_root }}/core/user-guide/explanation/data-processing/scheduler/scheduler-and-pools).
...
- `aborted` — the job was interrupted. This normally occurs as a result of **preemption** or network errors. To learn more about preemption, see [Scheduler and pools]({{ docs_root }}/core/user-guide/explanation/data-processing/scheduler/scheduler-and-pools).
- `failed` — the job encountered an error. This state usually results from errors in the user script. An error log (stderr) for the failed job has been generated and can be viewed. Such cases may also occur if the [account]({{ docs_root }}/core/user-guide/explanation/storage/cypress) in use has run out of space, meaning the job cannot write the result of its work.
...
* Uploading files to the [File cache]({{ docs_root }}/core/user-guide/reference/storage/file-cache).
...
3. There is a limit on the number of jobs an operation can have. For [Map]({{ docs_root }}/core/user-guide/reference/data-processing/operations/map), [Merge]({{ docs_root }}/core/user-guide/reference/data-processing/operations/merge), [Reduce]({{ docs_root }}/core/user-guide/reference/data-processing/operations/reduce), and [RemoteCopy]({{ docs_root }}/core/user-guide/reference/data-processing/operations/remote-copy) it is 100,000 jobs. In the cases of [Reduce]({{ docs_root }}/core/user-guide/reference/data-processing/operations/reduce) and sorted [Merge]({{ docs_root }}/core/user-guide/reference/data-processing/operations/merge), it may be impossible to generate so many jobs due to the scheduler not having enough samples. For operations that include the shuffle stage ([Sort]({{ docs_root }}/core/user-guide/reference/data-processing/operations/sort) and [MapReduce]({{ docs_root }}/core/user-guide/reference/data-processing/operations/mapreduce)), there is a limit of 5,000 partitions. To learn more about partitions, see the corresponding sections dedicated to specific operation types.
...
Reading data from another cluster (remote read) is subject to inter-cluster network bandwidth limits. For more information, see [Inter-cluster Network Bandwidth Throttling]({{ docs_root }}/core/user-guide/reference/data-processing/operations/cluster-throttlers).
