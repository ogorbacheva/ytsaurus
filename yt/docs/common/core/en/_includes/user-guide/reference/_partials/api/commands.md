...
- Command name (string).
- Description of the input and output formats (a [YSON string]({{ docs_root }}/core/user-guide/reference/storage/data-types#yson) with attributes).
...
- No data (`null`).
- The data is a `binary` stream. For example, [file]({{ docs_root }}/core/user-guide/explanation/storage/files) contents.
- The data is [structured]({{ docs_root }}/core/user-guide/reference/storage/formats#structured_data) (`structured`). For example, regular [Cypress]({{ docs_root }}/core/user-guide/explanation/storage/cypress) nodes.
- The data is a `tabular` stream. For example, [table]({{ docs_root }}/core/user-guide/explanation/storage/objects#tables) contents.
...
If your command works with structured or tabular data, you need to specify a [format]({{ docs_root }}/core/user-guide/reference/storage/formats) for it. For the input stream, use the `input_format` parameter, and for the output stream, use the `output_format` parameter, respectively.

For structured data formats, `YSON` (default) and `JSON` formats are supported. For tabular data, there exist many [formats]({{ docs_root }}/core/user-guide/reference/storage/formats).
...
[Transactions]({{ docs_root }}/core/user-guide/explanation/storage/transactions) are an integral Cypress property. Many commands that interact with Cypress this way or another, are transactional. Each command or group of commands has a separate indication of whether it is transactional or not. Transactional commands support the following parameters:
...
You can also specify prerequisites for your queries. Moreover, for light non-mutating queries, you can also specify where to read the data from. For more information, see [Query parameters]({{ docs_root }}/core/user-guide/reference/api/query-parameters).

## Working with transactions

For more information about transactions, see the [Transactions]({{ docs_root }}/core/user-guide/explanation/storage/transactions) article.
...
For more information about the metainformation tree, see [Cypress]({{ docs_root }}/core/user-guide/explanation/storage/cypress).
...
| `type` | Yes |                           | [Node type]({{ docs_root }}/core/user-guide/explanation/storage/objects). |
...
| `lock_existing` | No | `false` | Set an [exclusive lock]({{ docs_root }}/core/user-guide/explanation/storage/transactions#locks) on the specified node even if it already exists. This parameter is only used together with `ignore_existing`. If the lock couldn't be set, the command fails. |
...
| `waitable` | No | `false` | In case of a conflict, it's doesn't fail but is enqueued. You can find out whether the lock was actually set using the `state` attribute of the lock object. See [Transactions]({{ docs_root }}/core/user-guide/explanation/storage/transactions#locking_queue). |
...
Locks are automatically removed at the end of the transaction. Do not use the `unlock` command unless absolutely needed. See [Transactions]({{ docs_root }}/core/user-guide/explanation/storage/transactions#lock_operations).
...
| `lock_existing` | No | `false` | Set an [exclusive lock]({{ docs_root }}/core/user-guide/explanation/storage/transactions#locks) on the node at `destination_path`, even if it already exists. This parameter is only used together with `ignore_existing`. If the command fails to set a lock, it returns an error. |
...
| `preserve_expiration_time` | No | `false` | Whether to copy the [`expiration_time`]({{ docs_root }}/core/user-guide/explanation/storage/cypress#TTL) attribute or leave it empty. |
| `preserve_expiration_timeout` | No | `false` | Whether to copy the [`expiration_timeout`]({{ docs_root }}/core/user-guide/explanation/storage/cypress#TTL) attribute or leave it empty. |
...
For more information about access permissions, see [Access control]({{ docs_root }}/core/user-guide/explanation/storage/access-control).
...
For more information about files, see the [Files]({{ docs_root }}/core/user-guide/explanation/storage/files) article.
...
To learn more about the file cache, see the [File cache]({{ docs_root }}/core/user-guide/reference/storage/file-cache) section.
...
To learn more about static tables, see the [Static tables]({{ docs_root }}/core/user-guide/explanation/storage/static-tables) section.

To learn more about dynamic tables, see the [Dynamic tables]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/overview) section.

### write_table { #write_table }

Command properties: **Mutating**, **Heavy**.

Scope: [Static tables]({{ docs_root }}/core/user-guide/explanation/storage/static-tables).
...
| `table_writer` | No | From the driver configuration | [Table write options]({{ docs_root }}/core/user-guide/reference/storage/io-configuration). |
...
Scope: [Static]({{ docs_root }}/core/user-guide/explanation/storage/static-tables) and [dynamic]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/overview) tables.
...
| ------------------ | ------------- | ------------------------- | ------------------------------------------------------------ |
| `path` | Yes |                           | Path to a table in Cypress. The path *must* exist. |
| `table_reader` | No | From the driver configuration | [Table's read options]({{ docs_root }}/core/user-guide/reference/storage/io-configuration#table_reader). |
| `control_attributes` | No | From the driver configuration | [Configuration of the control read attributes]({{ docs_root }}/core/user-guide/reference/storage/io-configuration#control_attributes). |
...
Scope: Static [tables with binary data]({{ docs_root }}/core/user-guide/explanation/storage/blobtables).
...
| ---------------------- | ------------- | ------------------------- | ------------------------------------------------------------ |
| `path` | Yes |                           | Path to a table in Cypress. The path *must* exist. |
| `table_reader` | No | From the driver config file | [Table's read options]({{ docs_root }}/core/user-guide/reference/storage/io-configuration). |
...
Scope: [Dynamic tables]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/overview).

Semantics:

- Execute an SQL-like query for a dynamic table in accordance with the [supported features]({{ docs_root }}/core/user-guide/reference/dynamic-tables/dyn-query-language).
...
| `aggregate` | No | `false` | If set to `false`, the [aggregating columns]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/sorted-dynamic-tables#aggr_columns) will be overwritten by the new value. If set to `true`, such columns will apply the delta from the source data. |
| `atomicity` | No | `full` | Supported values: `none` and `full`. If set to `none`, writing will occur on each tablet, independent of the others. If set to `full`, either all of the passed rows will be written, or nothing. [Read more]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/sorted-dynamic-tables). |
...
Scope: [Ordered]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/ordered-dynamic-tables) dynamic tables.
...
Scope: [Sorted]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/sorted-dynamic-tables) dynamic tables.

Semantics:

- Forces an iteration of a [background tablet balancer]({{ docs_root }}/core/user-guide/how-to-guides/dynamic-tables/resharding), that is, reshards the tablets that go beyond the `min_tablet_size`–`max_tablet_size` interval.
...
Scope: [Static]({{ docs_root }}/core/user-guide/explanation/storage/static-tables) and [dynamic tables]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/overview).
...
- Changing the schema involves [various validation checks]({{ docs_root }}/core/user-guide/reference/storage/static-schema) because the system has to make sure that the existing data matches the schema.
...
| `upstream_replica_id` | No |                           | If specified, it changes the ID of the replica object on the metacluster. For more information, see [Replicated dynamic tables]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/replicated-dynamic-tables). |
...
Scope: [Replicated]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/replicated-dynamic-tables) dynamic tables.
...
For more information about running data processing operations, see [Data processing]({{ docs_root }}/core/user-guide/reference/data-processing/operations/overview).

All the operations are run asynchronously, the specified commands only launch them. To find out whether the operation is complete or not, request the [operation status]({{ docs_root }}/core/user-guide/reference/data-processing/operations/overview#status) using the `get_operation` command.
All the commands used to work with operations are also transactional. It means that everything you do with tables in an operation will be executed within the specified transaction when you run the operation.  The node responsible for the operation (`//sys/operations/<OP_ID>`) is updated by the [scheduler]({{ docs_root }}/core/user-guide/explanation/data-processing/scheduler/scheduler-and-pools)outside of any transactions.
...
| `spec` | Yes |                           | Operation specification. For more information, see [Setting up operations]({{ docs_root }}/core/user-guide/reference/data-processing/operations/operations-options). |
...
- Detailed description of all [specification parameters]({{ docs_root }}/core/user-guide/reference/data-processing/operations/merge).
...
- Detailed description of all [specification parameters]({{ docs_root }}/core/user-guide/reference/data-processing/operations/erase).
...
- Detailed description of all [specification parameters]({{ docs_root }}/core/user-guide/reference/data-processing/operations/map).
...
- Detailed description of all [specification parameters]({{ docs_root }}/core/user-guide/reference/data-processing/operations/reduce).
...
- Detailed description of all [specification parameters]({{ docs_root }}/core/user-guide/reference/data-processing/operations/sort).
...
- Detailed description of all [specification parameters]({{ docs_root }}/core/user-guide/reference/data-processing/operations/mapreduce).
...
- Detailed description of all [specification parameters]({{ docs_root }}/core/user-guide/reference/data-processing/operations/remote-copy).
...
An operation can be accessed either via `operation_id` or `operation_alias`. For more information about operation aliases, see the section [Operation options]({{ docs_root }}/core/user-guide/reference/data-processing/operations/operations-options#common_options).
...
| *parameters[scheduling_options_per_pool_tree]* | No |                           | Dictionary `{tree name: scheduler settings for this tree}`. The settings are described below. To learn more about the scheduler, see [Scheduler and pools]({{ docs_root }}/core/user-guide/explanation/data-processing/scheduler/scheduler-and-pools). |
...
| **Parameter** | **Required** | **Description**                                              |
| ------------- | ------------ | ------------------------------------------------------------ |
| `path`        | Yes          | Path to the parameter in the operation specification in [YPath]({{ docs_root }}/core/user-guide/reference/storage/ypath) format. |
...
Having `manage` permission for a pool allows you to perform some [actions]({{ docs_root }}/core/user-guide/how-to-guides/data-processing/scheduler/manage-pools#allowed_pool_actions) on operations running in the pool without formal permissions for the operation. In such cases, the command will indicate that the user does not have permission, as it only checks the permissions for the operation itself.
...
| `pool` | `string` | Name of the [pool]({{ docs_root }}/core/user-guide/explanation/data-processing/scheduler/scheduler-and-pools) where the job was run. |
| `pool_tree` | `string` | Name of the [pool tree]({{ docs_root }}/core/user-guide/explanation/data-processing/scheduler/scheduler-and-pools) where the job was run. |
...
- Perform parsing of the passed [YPath]({{ docs_root }}/core/user-guide/reference/storage/ypath) by putting all IDs of the complex YPath into attributes.
...
| ------------------ | ---------------- | ------------------------------------------------------------ |
| primitive\_types | list of rows | [Primitive types]({{ docs_root }}/core/user-guide/reference/storage/data-types#schema_primitive). |
| erasure\_codecs | list of rows | [Erasure codecs]({{ docs_root }}/core/user-guide/reference/storage/replication). |
| compression\_codecs | list of rows | [Compression codecs]({{ docs_root }}/core/user-guide/reference/storage/compression). |
