...
For information about the available commands, see the [Commands]({{ docs_root }}/core/user-guide/reference/api/commands) section. The only difference is that the command names are written in the usual style for the JavaScript language, namely CamelCase.
