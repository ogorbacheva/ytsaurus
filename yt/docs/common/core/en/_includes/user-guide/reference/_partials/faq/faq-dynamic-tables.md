...
Full use of tablet transactions in Python API is only possible via RPC-proxy (`yt.config['backend'] = 'rpc'`). Use of tablet transactions over HTTP is currently not supported.{{ core-user-guide-reference-faq-md-audience-1-4a03e6e6 }}
...
**A:** The error usually occurs due to incorrect settings during the converting of a static table to a dynamic one. Make sure you have followed all the instructions from the [conversion section]({{ docs_root }}/core/user-guide/how-to-guides/dynamic-tables/mapreduce#convert_table).
...
**A:** The tablet is too large. Get the table to have more tablets. Note that [auto-sharding]({{ docs_root }}/core/user-guide/how-to-guides/dynamic-tables/tablet-balancing) limits the number of tablets to the number of cells multiplied by the value of the `tablet_balancer_config/tablet_to_cell_ratio` parameter.
...
**A:** The client sent a query to a cluster node that has an updated tablet revision. This can happen if the table was explicitly remounted or if the tablet was moved as a result of [automatic balancing]({{ docs_root }}/_includes/user-guide/dynamic-tables/tablet-balancing) (the tablet may have remained on the same node but changed its tablet cell). Send another query — the error will disappear once the cache is updated. Alternatively, you can reconfigure the balancing settings.
...
Tablets can move to other cells as a result of [automatic sharding]({{ docs_root }}/core/user-guide/how-to-guides/dynamic-tables/tablet-balancing). You can see these moves on the "Tablet balancer moves" graph on the bundle's page. If you want, you can set a different schedule for automatic sharding. Your new schedule must allow the table to continue sustaining your load.
...
Check the following:
1. The primary key prefix of the table must be specified in the `WHERE` query section. If this hasn't been done, [input data cutting off]({{ docs_root }}/core/user-guide/reference/dynamic-tables/dyn-query-language#input_data_cut) can't be performed, and the query can't be executed efficiently (it will run as a full scan).
2. If the query contains `JOIN`, make sure it's executed [using an index]({{ docs_root }}/core/user-guide/reference/dynamic-tables/dyn-query-language#joins).

You need to manually rewrite the query to make sure it runs efficiently. To debug the query, you can use the [explain]({{ docs_root }}/core/user-guide/reference/dynamic-tables/dyn-query-language#explain_query) command.
...
{{ core-user-guide-reference-faq-md-audience-2-4a03e6e6 }}

------
#### **Q: How do I clear a replicated table using a CLI?** {#clear-replicated-table-cli}

**A:** You can't clear a replicated table atomically, but you can clear individual replicas using the [Erase]({{ docs_root }}/core/user-guide/reference/data-processing/operations/erase) operation. To do this, you need to [enable]({{ docs_root }}/core/user-guide/how-to-guides/dynamic-tables/bulk-insert) bulk insert for the entire cluster by setting:
...
{{ core-user-guide-reference-faq-md-audience-3 }}
...
If you don't require strict consistency, you can [weaken]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/sorted-dynamic-tables#chtenie-stroki) the isolation level for the reading transaction.
...
In some scenarios, weakening [guarantees]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/transactions#conflicts) may help. For example, you can use `atomicity=none` mode or shared write lock mode. Before you weaken the guarantees, be sure to read the documentation to find out whether the side effects can impact you.
...
For more information about compaction settings, see the section [Deleting old data]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/sorted-dynamic-tables#remove_old_data).
...
To remove chunk views from a table, perform a special forced compaction by setting the `forced_chunk_view_compaction_revision` attribute to 1, and then execute `remount-table`. For more information, see [Forced compaction]({{ docs_root }}/core/user-guide/how-to-guides/dynamic-tables/compaction#forced_compaction).
{{ core-user-guide-reference-faq-md-audience-4 }}
