...
To work with the [{{product-name}} data type]({{ docs_root }}/core/user-guide/reference/storage/data-types) from the first column, you can use the corresponding protobuf type from the second column in the table below.
...
- The table must have a [schema]({{ docs_root }}/core/user-guide/reference/storage/static-schema).
- The corresponding column (in the example: `ColumnName`) must have the [struct]({{ docs_root }}/core/user-guide/reference/storage/data-types#schema_struct) {{product-name}} type.
...
- The table must have a [schema]({{ docs_root }}/core/user-guide/reference/storage/static-schema).
- The corresponding column (in the example: `ListColumn`) must have the [list]({{ docs_root }}/core/user-guide/reference/storage/data-types#schema_list) {{product-name}} type.
...
By default, fields within a `oneof` group correspond to the [variant]({{ docs_root }}/core/user-guide/reference/storage/data-types#schema_variant) {{product-name}} type. For example, the message below will correspond to the structure with the `x` field of the `int64` type and the `my_oneof` field of the `variant<y: string, z: bool>` type:
