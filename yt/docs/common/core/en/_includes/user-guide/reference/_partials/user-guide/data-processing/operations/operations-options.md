...
- `pool` — name of the [compute pool]({{ docs_root }}/core/user-guide/explanation/data-processing/scheduler/scheduler-and-pools) where the operation runs. If this parameter is not specified, the pool name defaults to the name of the {{product-name}} user who started the operation.
- `weight`(1.0) — weight of the started operation. It determines how much of the pool's compute quota is allocated to the operation. For more information, see [Scheduler and pools]({{ docs_root }}/core/user-guide/explanation/data-processing/scheduler/scheduler-and-pools).
- `pool_trees` — list of [pool trees]({{ docs_root }}/core/user-guide/explanation/data-processing/scheduler/scheduler-and-pools) where the operation's jobs are run. The default pool tree is configured by the cluster administrator.
...
- `resource_limits` — fair-share settings that will apply to all jobs of the operation in aggregate. For more information, see [Operation resources configuration]({{ docs_root }}/core/user-guide/how-to-guides/data-processing/scheduler/pool-settings#operations).
...
- `core_table_path` — path to an existing table for writing core dump. The table format is identical to that described for `stderr_table_path`. {{ core-user-guide-reference-data-processing-operations-operations-options-md-audience-1 }}
- `input_query` — for the [Map]({{ docs_root }}/core/user-guide/reference/data-processing/operations/map) and [Merge]({{ docs_root }}/core/user-guide/reference/data-processing/operations/merge) operations, you can pre-filter each input row using the [query language]({{ docs_root }}/core/user-guide/reference/dynamic-tables/dyn-query-language). Because the input table is already known, and it is assumed that each row will be individually filtered, the query syntax changes and gets simplified to `* | <select-expr-1> [AS <select-alias-1>]... [WHERE <predicate-expr>]`. Speeifically, you cannot use `GROUP BY` and `ORDER BY`. Queries need strictly schematized data, so input tables must have a strict schema. In the [Merge]({{ docs_root }}/core/user-guide/reference/data-processing/operations/merge) operation, the schema of the final data in the query is incorporated when outputting the schema for the output table.
...
- `annotations` — a map where the user can write any structured information related to the operation. The `annotations` contents are saved to the operation archive as [YSON]({{ docs_root }}/core/user-guide/reference/storage/formats#yson) and can then be used to look up and identify operations within the archive.
...
- `format`, `input_format`, `output_format` (yson) — [format]({{ docs_root }}/core/user-guide/reference/storage/formats) in which input table data is fed to the operation. The `input_format` and `output_format` options take priority over `format`.
...
- `user_job_memory_digest_default_value` (0.5) — initial estimate for [memory reserve]({{ docs_root }}/core/user-guide/how-to-guides/data-processing/scheduler/memory-digest) selection.
- `user_job_memory_digest_lower_bound` (0.05) — minimum threshold for the [memory reserve]({{ docs_root }}/core/user-guide/how-to-guides/data-processing/scheduler/memory-digest).{{ core-user-guide-reference-data-processing-operations-operations-options-md-audience-2 }}
...
- `copy_files` (false) — by default, user files go to the job's `sandbox` via a [symbolic link]({{ docs_root }}/core/user-guide/explanation/storage/links). If this option is enabled, they are copied to the `sandbox`. It can be useful in tandem with `tmpfs_path=.`.
...
- `disk_request` — list of disks required by the job. By default, a job is assigned a local disk on the cluster node where it was initiated. Disk space usage is not limited, but the job can be interrupted if the disk space runs out. Each disk is characterized by the following parameters:
  ...
  - `medium_name` (default) — name of the [medium]({{ docs_root }}/core/user-guide/reference/storage/media) that a disk is being requested from.
...
{{ core-user-guide-reference-data-processing-operations-operations-options-md-audience-3 }}
...
For a detailed description of this mechanism, see [Selecting memory for jobs]({{ docs_root }}/core/user-guide/how-to-guides/data-processing/scheduler/memory-digest).
...
{{ core-user-guide-reference-data-processing-operations-operations-options-md-audience-5 }}
...
- Standard [modifiers for selecting rows and columns]({{ docs_root }}/core/user-guide/reference/storage/ypath#rich_ypath_suffix).
- `transaction_id` — specifies the transaction within which to access the input table.
- `timestamp` — allows using the state of a dynamic table [at a specific point in time]({{ docs_root }}/core/user-guide/how-to-guides/dynamic-tables/mapreduce#dyntable_mr_timestamp).
