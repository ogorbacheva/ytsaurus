...
- **Disable redirection**: The `&` token can be used to suppress redirections via a [symbolic link]({{ docs_root }}/core/user-guide/explanation/storage/links).
   Examples:
...
The path prefix and suffix enable you to encode attributes in a more compact form. Both of these parts use [YSON]({{ docs_root }}/core/user-guide/reference/storage/yson). Learn more about them:

### Prefix { #rich_ypath_prefix }

The `<prefix>` part is either empty or sets attributes as a map-fragment in [YSON]({{ docs_root }}/core/user-guide/reference/storage/yson) format.
...
The `parse_ypath` driver [command]({{ docs_root }}/core/user-guide/reference/api/commands#parse_ypath) canonicalizes a rich YPath.
