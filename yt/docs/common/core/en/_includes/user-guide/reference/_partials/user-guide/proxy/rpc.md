...
When working via the RPC proxy using the C++ client, you can specify an additional user tag `user_tag` — then the user’s graph identifier will be annotated not as `<username>`, but as `<username>:<user_tag>`. This can be useful for services that have many clients, but access YT on behalf of a single user. This additional tagging lets you link the load on {{product-name}} to requests generated by external user actions.
...
{{{ core-user-guide-reference-proxy-rpc-md-audience-1 }}}
