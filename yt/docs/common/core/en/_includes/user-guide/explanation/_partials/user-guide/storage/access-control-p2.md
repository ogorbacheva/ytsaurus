...
For tables, `read` grants access to the data, but the visible set of columns and rows can be further narrowed by [columnar]({{ docs_root }}/core/user-guide/reference/storage/columnar-acl) and [row-level]({{ docs_root }}/core/user-guide/reference/storage/row-level-security) ACLs: even with `read` for the whole table, a user must be explicitly allowed to read particular columns.
...
- Using [Job Shell]({{ docs_root }}/core/user-guide/how-to-guides/problems/jobshell-and-slowjobs) requires `read` and `manage` permissions at the same time , since having access via the console enables you to read or randomly change the job state.
...
To gain access to an existing account or directory in {{product-name}}, {{ core-user-guide-explanation-storage-access-control-p2-md-audience-1 }}.
