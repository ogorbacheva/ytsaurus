...
This section describes transactions as they apply to dynamic tables. The transactional model of static tables is described in the [Transactions]({{ docs_root }}/core/user-guide/explanation/storage/transactions) section.
...
Tables have the `atomicity` attribute which defines the guarantees for the transactions to be applied. This attribute can only be changed for [unmounted]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/overview#mount_table) tables. The attribute values can be the following:
