...
This section describes transactions as applied to [Cypress]({{ docs_root }}/core/user-guide/explanation/storage/cypress), and [static tables]({{ docs_root }}/core/user-guide/explanation/storage/static-tables), [locks](#locks), and [versioning](#versioning) of Cypress objects.
The transactional model of dynamic tables is described in the [Multiversioning and transaction processing of dynamic tables]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/transactions) section.
...
Transactional processing within master servers applies to versionable [objects]({{ docs_root }}/core/user-guide/explanation/storage/objects). Examples of such objects are files, tables, and folders.
...
In addition to the attributes [inherent to all objects]({{ docs_root }}/core/user-guide/explanation/storage/objects#attributes), transactions have the following attributes:
...
- `snapshot`: The transaction can read but not modify the node. The lock is used to obtain a read-only copy of a Cypress node state in the context of the transaction and freeze the state of that node.
   ...
   To ensure access to the snapshot version of a node, use the node's `id`, which is returned by the `lock` [command]({{ docs_root }}/core/user-guide/reference/api/commands#lock).
...
When starting operations, the scheduler creates a set of transactions to provide some atomicity of data processing in the operation. To learn more about how this works, see [Transactions in data processing]({{ docs_root }}/core/user-guide/reference/data-processing/operations/overview#transactions).
