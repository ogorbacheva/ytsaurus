...
This section describes the main [object types]({{ docs_root }}/core/user-guide/explanation/storage/objects#object_types) in the {{product-name}} system. A table with the key [attributes of each object]({{ docs_root }}/core/user-guide/explanation/storage/objects#attributes) is given and the concept of an [object ID]({{ docs_root }}/core/user-guide/explanation/storage/objects#object_ids) is described.

## Object types { #object_types }

The main object types in {{product-name}} are:

* [Files]({{ docs_root }}/core/user-guide/explanation/storage/objects#files).
* [Tables]({{ docs_root }}/core/user-guide/explanation/storage/objects#tables).
* [Links]({{ docs_root }}/core/user-guide/explanation/storage/objects#links).
* [YSON documents]({{ docs_root }}/core/user-guide/explanation/storage/objects#yson_doc).
* [Primitive types]({{ docs_root }}/core/user-guide/explanation/storage/objects#primitive_types).
* [Internal objects](#internal_objects).

The description of objects takes up most of the meta-information which is stored in replicated form in the memory of the [Cypress]({{ docs_root }}/core/user-guide/explanation/storage/cypress) master servers. Cypress contains various system information as well as indications of where user data is stored.

For any Cypress node, there may be several versions, because the state of the node may look different in the context of each [transaction]({{ docs_root }}/core/user-guide/explanation/storage/transactions).

### Files { #files }

These are Cypress nodes of the **file** type designed to store large binary data in the system.
Files consist of multiple chunks and [are chunk owners]({{ docs_root }}/core/user-guide/explanation/storage/chunks#attributes).
...
For more information, see [Files]({{ docs_root }}/core/user-guide/explanation/storage/files).

### Tables { #tables }

These are nodes of the **table** type designed to store large user data. There are two types of tables: [static]({{ docs_root }}/core/user-guide/explanation/storage/static-tables) and [dynamic]({{ docs_root }}/core/user-guide/explanation/dynamic-tables/overview). Logically, a table is a sequence of rows. Each row consists of columns.
Column values are typified and can contain random structured data supported by the [YSON]({{ docs_root }}/core/user-guide/reference/storage/yson) language.
...
For more information, see [Links]({{ docs_root }}/core/user-guide/explanation/storage/links).

### YSON documents { #yson_doc }

A YSON document is a Cypress node of the **document** type designed to store random YSON structures.

Requests and modifications within the document are supported. Addressing within the document is performed using the [YPath]({{ docs_root }}/core/user-guide/reference/storage/ypath) language.

For more information, see [YSON documents]({{ docs_root }}/core/user-guide/explanation/storage/yson-docs).

For more information about the YSON format, see [YSON.]({{ docs_root }}/core/user-guide/reference/storage/yson)
...
| -------------- | ------------------------------------------------------------ |
| `transaction` | [Transaction]({{ docs_root }}/core/user-guide/explanation/storage/transactions) |
| `chunk` | Chunk |
...
| `ref_counter` | `integer` | The number of [strong links]({{ docs_root }}/core/user-guide/explanation/storage/objects#ref_counter) to the object |
| `supported_permissions` | `array<string>` | List of supported access permissions |
| `effective_acl` | `Acl` | Effective object ACL |

Some system objects have additional attributes: `inherit_acl`, `acl`, and `owner`. They affect access to this object. These attributes are called an **access control descriptor** (ACD). For more information about the ACD, see [Managing access]({{ docs_root }}/core/user-guide/explanation/storage/access-control).
