...
The system supports `soft links` in [Cypress]({{ docs_root }}/core/user-guide/explanation/storage/cypress). This link is a node of type `link` referencing another `target` object.
...
You can create a symbolic link with the `link` [command]({{ docs_root }}/core/user-guide/reference/api/commands#link). Use the path to the target node and the path to the link node in that order as arguments:
...
In addition to [attributes]({{ docs_root }}/core/user-guide/reference/storage/attributes) common to all nodes, link nodes also have the following properties:
