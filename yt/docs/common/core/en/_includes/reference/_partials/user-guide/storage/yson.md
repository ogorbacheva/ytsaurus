...
Entity is an atomic scalar value with no content of its own. There are various scenarios in which this type can be useful. For example, entity often means null. In addition, when a `get` request is made to a [Cypress]({{ docs_root }}/core/concepts/storage/cypress) subtree, [files]({{ docs_root }}/core/concepts/storage/objects#files) and [tables]({{ docs_root }}/core/concepts/storage/objects#tables) are returned as entities (actual data is stored in the [attributes](#attributes) of that node).
...
Special tokens:
`;`, `=`, `#`, `[`, `]`, `{`, `}`, `<`, `>`, `)`, `/`, `@`, `!`, `+`, `^`, `:`, `,`, `~`.
Not all of these symbols are used in YSON, some are used in [YPath]({{ docs_root }}/core/reference/storage/ypath).
