...
The data transfer rate between clusters is subject to throttling. For more information, see [Inter-cluster Network Bandwidth Throttling]({{ docs_root }}/core/reference/data-processing/operations/cluster-throttlers).

{% endnote %}

General parameters for all operation types are described in [Operation options]({{ docs_root }}/core/reference/data-processing/operations/operations-options).
...
* `schema_inference_mode` (auto) — schema definition mode. Possible values: auto, from_input, from_output. To learn more, see [Data schema]({{ docs_root }}/core/reference/storage/static-schema#schema_inference).
...
The RemoteCopy (`remote_copy`) operation is supported only for [sorted tables]({{ docs_root }}/core/concepts/dynamic-tables/sorted-dynamic-tables). To copy an ordered table, you need to convert it to a static one. It is impossible to copy an ordered table to another cluster with preserving tablets.
