...
* `enable_key_guarantee` — enables/disables the guarantee that all records with one key are given as input to one job (`true` by default). [Reduce with foreign tables]({{ docs_root }}/core/reference/data-processing/operations/reduce#foreign_tables) describes a special scenario for using this option.
