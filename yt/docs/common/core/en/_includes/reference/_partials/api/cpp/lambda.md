...
In some {{product-name}} operations, calculations are so simple that creating a class for each of them using the [C++ API]({{ docs_root }}/core/concepts/api/cpp/description) feels excessive. In this case, you can use [lambda expressions](https://en.wikipedia.org/wiki/Anonymous_function) to describe operations.
...
[Initialize {{product-name}}]({{ docs_root }}/core/concepts/api/cpp/description#init) and get the `client`. Pass the `client` or [transaction]({{ docs_root }}/core/concepts/api/cpp/description#transactions) as the first parameter to the functions described in this document.

This library's functions have good compatibility with the use of records in [protobuf]({{ docs_root }}/core/reference/api/cpp/protobuf) format.
...
A variant of ordinary operations where [Reduce]({{ docs_root }}/core/reference/data-processing/operations/reduce) has the same input and output type and is associative. In this case, you can further simplify the implementation of the Reduce transformation and use it as a combiner for free.
...
| -------- | ------------------------------------------------------------ |
| `T` | Record type (TNode or a specific [protobuf message]({{ docs_root }}/core/reference/api/cpp/protobuf) type) |
...
This function performs a classic [Map]({{ docs_root }}/core/reference/data-processing/operations/map): it reads each record from the table of one format and writes them to the table of another format. Some records aren't included in the final table.
...
The `AdditiveMapReduce` function generates a merged [MapReduce operation]({{ docs_root }}/core/reference/data-processing/operations/mapreduce), that is, it converts records of the input table (or tables), sorts the result, and performs Reduce. The reducer must be an [associative binary operation](https://en.wikipedia.org/wiki/Associative_property) on same-type records — most often, this will be an addition of numbers. As a bonus, YT can use such a reducer as a combiner if needed.
