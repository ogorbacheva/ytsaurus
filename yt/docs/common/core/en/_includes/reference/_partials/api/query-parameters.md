...
This section contains information about additional query parameters. For more information about commands, see the [Commands]({{ docs_root }}/core/reference/api/commands) section.
...
- `prerequisite_transactions` includes a list of [transactions]({{ docs_root }}/core/concepts/storage/transactions) that must exist when the query is executed.
...
In [non-mutating]({{ docs_root }}/core/reference/api/commands#concepts) queries to [Cypress]({{ docs_root }}/core/concepts/storage/cypress), you can specify a data source for reading, using the `read_from`  parameter.
