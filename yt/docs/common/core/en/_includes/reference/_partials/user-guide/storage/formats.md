...
Structured data is a node tree with attributes, where nodes correspond to [YSON types]({{ docs_root }}/core/reference/storage/yson) (map, list, primitive types) with attributes.
Structured data can be obtained using the [`get`]({{ docs_root }}/core/reference/api/commands#get) command.

Supported formats: `json`, `yson`, `yaml`.

Example: [Cypress]({{ docs_root }}/core/concepts/storage/cypress) nodes, their attributes.
...
Examples: Input and output data in operations, input data of the [`write_table`]({{ docs_root }}/core/reference/api/commands#write_table) command, output data of the [`read_table`]({{ docs_root }}/core/reference/api/commands#read_table) command.
...
Examples: Input data of the [`upload`]({{ docs_root }}/core/reference/api/commands#write_file) command and output data of the [`download`]({{ docs_root }}/core/reference/api/commands#read_file) command.
...
A format must be specified for all [`yt` utility]({{ docs_root }}/core/reference/api/cli/cli) commands that return or accept data.
...
For a detailed specification, see the [YSON]({{ docs_root }}/core/reference/storage/yson) section.
...
- **complex_type_mode** (`named`) — representation of composite types, structs, and variants.
  Possible values are `named` and `positional`. For more information, see [this section]({{ docs_root }}/core/reference/storage/data-types#yson).
- **string_keyed_dict_mode** (`positional`) — representation of dictionaries with string keys. Possible values are `named` and `positional`. For more information, see [this section]({{ docs_root }}/core/reference/storage/data-types#yson).
- **decimal_mode** (`binary`) — representation of the `decimal` type.
  Possible values are `text` and `binary`. For more information, see [this section]({{ docs_root }}/core/reference/storage/data-types#yson).
- **time_mode** (`binary`) — representation of `date`, `datetime`, and `timestamp` types. Possible values are `text` and `binary`. For more information, see [this section]({{ docs_root }}/core/reference/storage/data-types#yson).
- **uuid_mode** (`binary`) — representation of the `uuid` type. Possible values are `binary`, `text_yql`, and `text_yt`. For more information, see [this section]({{ docs_root }}/core/reference/storage/data-types#yson).
...
To convert a YSON string to a JSON string, convert each byte to a Unicode character with the corresponding number and encode it in UTF-8. This conversion occurs, for example, when reading table data using the [`read_table`]({{ docs_root }}/core/reference/api/commands#read_table) command.

When a JSON string is converted to a YSON string, the sequence of Unicode characters of the JSON string is checked to make sure each of them is in the 0–255 range. They are then converted to the appropriate bytes. This conversion is performed when writing table data using the [`write_table`]({{ docs_root }}/core/reference/api/commands#write_table) command.
...
During JSON string to YSON string conversion, UTF-8 Unicode characters are encoded into byte sequences. This conversion is performed with the [`write_table`]({{ docs_root }}/core/reference/api/commands#write_table) command.
...
- Representation of complex types depends on parameter **enable_complex_types**:
  - if `false`, complex types are represented as [arrow::binary](https://arrow.apache.org/docs/cpp/api/datatype.html#_CPPv4N5arrow4Type4type6BINARYE) with [YSON representation]({{ docs_root }}/core/reference/storage/data-types#yson) of these types.
...
Learn more about [Protobuf]({{ docs_root }}/core/reference/api/cpp/protobuf).
