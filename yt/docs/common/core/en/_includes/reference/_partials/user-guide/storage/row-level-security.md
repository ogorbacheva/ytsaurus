...
Row-Level Security (RLS) enables you to specify access rules for individual table rows based on predicates evaluated against row data. This enables you to store both public and private data in the same table and define access through arbitrary expressions in the {{product-name}} [Query language]({{ docs_root }}/core/reference/dynamic-tables/dyn-query-language).

## Operation principle { #details }

The general operating principles of the access control system in {{product-name}} are described in the [Access control]({{ docs_root }}/core/concepts/storage/access-control) section.
...
When evaluating the predicate, restrictions on access to columns (see [Managing access to table columns]({{ docs_root }}/core/reference/storage/columnar-acl)) are ignored. Since access is managed by the data owner, this is not a vulnerability per se, but may lead to a data leakage based on various metadata, so you need to be careful when designing predicates.
