...
- Key: `"data_parts_only"`; value: a flag. On media with `"data_parts_only" = %true`, [erasure coded]({{ docs_root }}/core/reference/storage/replication#erasure) object only store data parts and do not store erasure parts. For example:
...
[Cypress]({{ docs_root }}/core/concepts/storage/cypress) enables you to read individual records from a dictionary without reading the entire dictionary.
