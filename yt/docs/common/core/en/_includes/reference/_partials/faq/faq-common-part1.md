...
**A:** You cannot change the schema of a non-empty table from weak to strict since the entire table must be read to validate this action to make sure that the data match the new schema. It is easiest to create a new table and copy data from the old one via a read+write or by launching a [Merge]({{ docs_root }}/core/reference/data-processing/operations/merge) transaction.
...
**A:** They can using the [YSON]({{ docs_root }}/core/reference/storage/formats#yson) or the [JSON]({{ docs_root }}/core/reference/storage/formats#json) format.
...
**A:** One of the possible common reasons is too many (small) chunks in a table. We recommend running the [Merge]({{ docs_root }}/core/reference/data-processing/operations/merge) operation with the `--spec '{force_transform=true}'` option. When such tables come up in operation output, the console client issues a warning containing, among other things, the command you can run to increase the size of the table's chunks. You can also specify the `auto_merge_output={action=merge}` option to have a merge occur automatically.
...
Unless you specified `intermediate_data_account` (see [Operation settings]({{ docs_root }}/core/reference/data-processing/operations/operations-options), [Sort]({{ docs_root }}/core/reference/data-processing/operations/sort), [MapReduce]({{ docs_root }}/core/reference/data-processing/operations/mapreduce)), you are sharing this account with everybody else. To preempt this problem, set `intermediate_data_account`.
