...
You can control reading and writing via [table data formats]({{ docs_root }}/core/reference/storage/formats), `TableReader/TableWriter`, and `ControlAttributes`.
...
When reading input tables from other clusters (remote read), the read rate is limited by inter-cluster network bandwidth quotas. For more information, see [Inter-cluster Network Bandwidth Throttling]({{ docs_root }}/core/reference/data-processing/operations/cluster-throttlers).
...
Many formats have these indexes disabled by default; therefore, they must be enabled. Many formats have options responsible for representing `table_index` in this one. For more information on `table_index` in various formats, see [Table switching]({{ docs_root }}/core/how-to-guides/data-processing/operations/table-switch).
