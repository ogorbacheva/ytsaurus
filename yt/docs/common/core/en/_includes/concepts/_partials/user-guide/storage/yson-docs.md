...
A YSON document is a [Cypress]({{ docs_root }}/core/concepts/storage/cypress) node of the **document** type designed to store random YSON structures.
...
You can work with documents using standard [commands]({{ docs_root }}/core/reference/api/commands): `get`, `list`, `exists`, `set`, and `remove`, just like with other objects in Cypress.
...
Addressing within the document is performed using the [YPath]({{ docs_root }}/core/reference/storage/ypath) language.
...
| ----------- | ------- | ------------------------------------------------------------ |
| `value` | `any` | Full contents of the document. This attribute enables you to specify the contents of the document when it is created. The attribute is [opaque]({{ docs_root }}/core/reference/storage/attributes#system_attr), which means that when you read all attributes of a node without a filter, it will be displayed as [entity]({{ docs_root }}/core/reference/storage/yson#entity). |
