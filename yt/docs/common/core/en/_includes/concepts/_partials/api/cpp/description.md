...
See [Usage examples]({{ docs_root }}/core/how-to-guides/api/cpp/examples).
...
Logging is toggled with the `YT_LOG_LEVEL` environment variable, which can take one of the following values: `ERROR`, `WARNING` (equivalent to `ERROR` for compatibility with the [Python API]({{ docs_root }}/core/how-to-guides/api/python/userdoc)), `INFO`, `DEBUG`.
...
If you need to have clients working under different [authorization tokens]({{ docs_root }}/core/reference/storage/auth) within the same program, you can use the following options:
...
For more information on transactions, see the [Transactions]({{ docs_root }}/core/concepts/storage/transactions) section.

## TNode { #tnode }

[TNode]({{source-root}}/yt/cpp/mapreduce/interface/node.h) is the main class that provides for a dynamic DOM representation of [YSON documents]({{ docs_root }}/core/concepts/storage/yson-docs). It is used for working with Cypress, transferring additional operation specifications, as one of the ways to encode table rows, and more.
...
For more information about the commands, see [Working with the meta information tree]({{ docs_root }}/core/how-to-guides/storage/cypress-example). Implemented as the `ICypressClient` interface, from which `IClientBase` is inherited.
...
[Create]({{ docs_root }}/core/reference/api/commands#create) a node. [See the CLI equivalent]({{ docs_root }}/core/how-to-guides/storage/cypress-example#create).
...
| `type` | `ENodeType` | - | [Type]({{ docs_root }}/core/concepts/storage/objects#object_types) of node created: <br/>  `NT_STRING` — string (`string_node`);<br/> `NT_INT64` — signed integer (`int64_node`); <br/>`NT_UINT64` — unsigned integer (`uint64_node`); <br/>`NT_DOUBLE` — real number (`double_node`); <br/>`NT_BOOLEAN` — Boolean value (`boolean_node`); <br/>`NT_MAP` — map in Cypress (keys are strings, and values are other nodes, `map_node`); <br/> `NT_FILE` — [file]({{ docs_root }}/core/concepts/storage/objects#files) (`file`); <br/>`NT_TABLE` — [table]({{ docs_root }}/core/concepts/storage/objects#tables) (`table`);<br/> `NT_DOCUMENT` — [document]({{ docs_root }}/core/concepts/storage/objects#yson_doc) (`document`); <br/>`NT_REPLICATED_TABLE` — [replicated table]({{ docs_root }}/core/concepts/dynamic-tables/replicated-dynamic-tables) (`replicated_table`); <br/>`NT_TABLE_REPLICA` — (`table_replica`); |
...
| `Attributes` | `TNode` | none | [Attributes]({{ docs_root }}/core/reference/storage/attributes) of the created node. |

### Remove

[Remove]({{ docs_root }}/core/reference/api/commands#remove) the node. [See the CLI equivalent]({{ docs_root }}/core/how-to-guides/storage/cypress-example#remove).
...
[Checking the existence]({{ docs_root }}/core/reference/api/commands#exists) of the object at the specified path.
...
[Get the contents of the Cypress node]({{ docs_root }}/core/reference/api/commands#get) in `TNode` format. [See the CLI equivalent]({{ docs_root }}/core/how-to-guides/storage/cypress-example#get).
...
[Write new Cypress node content.]({{ docs_root }}/core/reference/api/commands#set) [See the CLI equivalent]({{ docs_root }}/core/how-to-guides/storage/cypress-example#set).
...
[Get a list of the node's descendants]({{ docs_root }}/core/reference/api/commands#list). [See the CLI equivalent]({{ docs_root }}/core/how-to-guides/storage/cypress-example#list).
...
[Copy the Cypress node to a new address]({{ docs_root }}/core/reference/api/commands#copy). [See the CLI equivalent]({{ docs_root }}/core/how-to-guides/storage/cypress-example#copy_move).
...
[Move the node to a new path]({{ docs_root }}/core/reference/api/commands#move). [See the CLI equivalent]({{ docs_root }}/core/how-to-guides/storage/cypress-example#copy_move).
...
[Create]({{ docs_root }}/core/reference/api/commands#link) a [symbolic link]({{ docs_root }}/core/concepts/storage/links) to the object at the new address. [See the CLI equivalent]({{ docs_root }}/core/how-to-guides/storage/cypress-example#link).
...
| `Attributes` | `TNode` | none | [Attributes]({{ docs_root }}/core/reference/storage/attributes) for `linkPath` in the form of `TNode` are written if created. |

### Concatenate

[Concatenate]({{ docs_root }}/core/reference/api/commands#concatenate) a set of files or tables (in the order in which their paths are listed). [See the CLI equivalent]({{ docs_root }}/core/how-to-guides/storage/cypress-example#concatenate).

The data merge takes place at the metadata level and exclusively on the [Cypress]({{ docs_root }}/core/concepts/storage/cypress) master server.
...
To represent a record, TNode must have the [map]({{ docs_root }}/core/concepts/storage/objects#primitive_types) type and constitute the display of column names as YSON values.

{{ core-user-guide-explanation-api-cpp-description-md-audience-3 }}

### Protobuf

For more information about Protobuf, see [Protobuf representation of tables]({{ docs_root }}/core/reference/api/cpp/protobuf).
...
- [Type conversions]({{ docs_root }}/core/reference/storage/data-types) when writing: `EnableStringToAllConversion`, `EnableAllToStringConversion`, `EnableIntegralTypeConversion%` (`uint64` <-> `int64`, enabled by default), `EnableIntegralToDoubleConversion`, `EnableTypeConversion` (all of the above simultaneously).

### Additional options

If you need even more precise read/write options, you can use the `TTableWriterOptions` and `TTableReaderOptions` structures. They contain the `Config` field, where additional settings can be passed in `TNode` format that correspond to the [table_writer]({{ docs_root }}/core/reference/storage/io-configuration#table_writer) and [table_reader]({{ docs_root }}/core/reference/storage/io-configuration#table_reader) parameters.
...
You can specify a null pointer as `mapper`, which will make the operation skip the Map stage when running, and the function's second version will additionally start the [reduce_combiner]({{ docs_root }}/core/reference/data-processing/operations/reduce#reduce_combiner) stage.
...
Get [information about an operation]({{ docs_root }}/core/reference/api/commands#get_operation).
...
Get a list of operations [that match the filters]({{ docs_root }}/core/reference/api/commands#list_operation).
...
Update the runtime parameters of the [running operation]({{ docs_root }}/core/reference/api/commands#update_operation_parameters).
...
Get [information about the job]({{ docs_root }}/core/reference/api/commands#get_job).
...
Get jobs that [match the specified filters]({{ docs_root }}/core/reference/api/commands#list_jobs).
...
Get the input of a [running or failed job]({{ docs_root }}/core/reference/api/commands#get_job_input).
...
Get fail context of a [failed job]({{ docs_root }}/core/reference/api/commands#get_job_fail_context).
...
Get stderr of a [running or failed job]({{ docs_root }}/core/reference/api/commands#get_job_stderr).
...
[Copy the file to the cache]({{ docs_root }}/core/reference/api/commands#put_file_to_cache). Returns a path to the cached file.
...
[Get a path to the cached file]({{ docs_root }}/core/reference/api/commands#get_file_from_cache). Returns the path (or `Nothing`, if there is no file with this MD5).
