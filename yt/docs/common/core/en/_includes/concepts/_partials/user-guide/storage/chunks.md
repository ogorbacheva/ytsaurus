...
Data stored in [tables]({{ docs_root }}/core/concepts/storage/static-tables), [files]({{ docs_root }}/core/concepts/storage/files), and logs is split into parts that are called chunks. A chunk stores a continuous range of data.
...
The chunk data is physically stored on the cluster nodes. {{product-name}} master servers monitor the location of the chunks and keep them in [replicated]({{ docs_root }}/core/reference/storage/replication) state.

{{product-name}} master servers store only metadata. However, metadata has a significant size, so in order to limit the memory consumption by the master servers, the number of chunks available to the account is [quoted]({{ docs_root }}/core/reference/storage/quotas).
...
You can change the block size in the [`table_writer`]({{ docs_root }}/core/reference/storage/io-configuration) settings, the `block_size` attribute.

A chunk usually occupies hundreds of megabytes or several gigabytes on a cluster node.
The size of the chunks to be created can be configured via the [`table_writer`]({{ docs_root }}/core/reference/storage/io-configuration) section, the `desired_chunk_size` attribute.
...
The row-by-row format is suitable for [dynamic tables]({{ docs_root }}/core/concepts/dynamic-tables/overview) from which data is selected by key.
...
For **dynamic tables**, this is solved by forced compaction. Before using it, make sure to read the article on [Background compaction]({{ docs_root }}/core/how-to-guides/dynamic-tables/compaction#forced_compaction).
...
In addition, for columnar **dynamic tables**, note the `group` attribute in the column schema. If your use case involves frequent lookups of table rows without column filtering, it is a good idea to set the same group for all columns. For more information about groups, see the documentation page on [table schemas]({{ docs_root }}/core/reference/storage/static-schema#schema_overview).
...
| `compression_statistics` | `CompressionStatistics` | Statistics on data sizes and the types of [compression codecs]({{ docs_root }}/core/reference/storage/compression) used. | Yes |
| `erasure_statistics` | `ErasureStatistics` | Statistics on the types of used [erasure codecs]({{ docs_root }}/core/reference/storage/replication) and data sizes. | Yes |
...
| `replication_factor` | `integer` | [Replication]({{ docs_root }}/core/reference/storage/replication) factor (equals 1 for erasure coding). | Yes |
| `compression_codec` | `string` | [Compression codec]({{ docs_root }}/core/reference/storage/compression) name. | Yes |
| `erasure_codec` | `string` | [Erasure codec]({{ docs_root }}/core/reference/storage/replication) name. | Yes |
| `vital` | `bool` | Whether the chunks of this node are [vital](#vitality) | No |
| `media` | `Media` | Replication factors and other [medium]({{ docs_root }}/core/reference/storage/media) settings. | Yes |
| `primary_medium` | `string` | [Primary medium]({{ docs_root }}/core/reference/storage/media#primary). | Yes |
