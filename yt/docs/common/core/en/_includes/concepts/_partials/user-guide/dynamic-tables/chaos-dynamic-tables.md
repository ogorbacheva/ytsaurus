...
To ensure that rows in the new replica have the same numbers as in previous replicas, you need to specify explicit [offsets]({{ docs_root }}/core/concepts/dynamic-tables/ordered-dynamic-tables#creating) and progress corresponding to these offsets when creating the replica.
...
When creating a `chaos_replicated_table`, you can specify the `replicated_table_options` attribute where you want to pass the `ReplicatedTableOptions`. For more information about the `ReplicatedTableOptions`, see the dedicated [section]({{ docs_root }}/core/concepts/dynamic-tables/replicated-dynamic-tables#attributes).
