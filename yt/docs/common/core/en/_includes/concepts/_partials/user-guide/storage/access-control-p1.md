...
Each object in the {{product-name}} system has an **Access Control List** (**ACL**). This list is stored in the `@acl` attribute of the object and consists of individual entries (**Access Control Entry** or **ACE**) where each entry contains a list of subjects, access type, and a number of other parameters listed in the table in the [Authorization]({{ docs_root }}/core/concepts/storage/access-control#authorization) section. For more information, see [How to use the ACL correctly](#acl_usage).
