...
To a user, Cypress looks like a Linux [file system](https://en.wikipedia.org/wiki/File_system) tree but with a number of significant differences. First, every tree node has an associated [collection of attributes]({{ docs_root }}/core/reference/storage/attributes), including some user-defined ones. Second, the tree is [transactional]({{ docs_root }}/core/concepts/storage/transactions). Third, files and directories, as well as [other objects]({{ docs_root }}/core/concepts/storage/objects), can serve as Cypress nodes. Like a file system, Cypress supports an [access control system]({{ docs_root }}/core/concepts/storage/access-control).

Cypress is rooted at `/` which has **map_node** type (that is, it's a directory). Cypress nodes are addressed using [YPath]({{ docs_root }}/core/reference/storage/ypath).

The number of child nodes in a directory is limited. By default, the limit is 50,000, but it depends on the cluster configuration. The depth of Cypress is theoretically unlimited, but keep in mind that there is a limit on the path resolution depth in [YPath]({{ docs_root }}/core/reference/storage/ypath).
...
You can manipulate Cypress via a [CLI]({{ docs_root }}/core/reference/api/cli/cli).
...
| `locks` | `array<Lock>` | [List of locks]({{ docs_root }}/core/concepts/storage/transactions) taken out on a node |
| `lock_mode` | `LockMode` | Current node [lock mode]({{ docs_root }}/core/concepts/storage/transactions) (transaction-dependent) |
...
| `annotation` | `string` | Human-readable [summary description]({{ docs_root }}/core/reference/storage/annotations) of an object |

Each node has its own attributes responsible for access control. Therefore, its attributes include `inherit_acl`, `acl`, and `owner`. For more information, see [Access control system]({{ docs_root }}/core/concepts/storage/access-control).
...
The `access_time` attribute stores the time of the last access to the node's content. Access to attributes is not taken into account (access to an attribute is considered to be specifying the full path to the attribute within [YPath]({{ docs_root }}/core/reference/storage/ypath)). Requests with the `suppress_access_tracking` option enabled are also not taken into account. To improve efficiency, the system does not change this attribute with each access, but accumulates access records and updates the `access_time` attribute at intervals of about a second.
...
To be able to set these attributes for a node, you need to have the [right]({{ docs_root }}/core/concepts/storage/access-control) to `write` to the node itself same as for many other attributes as well as the `remove` privilege to the node and its entire subtree because a delete is being requested in effect, albeit a deferred one. The `write` privilege is sufficient to delete these attributes.
