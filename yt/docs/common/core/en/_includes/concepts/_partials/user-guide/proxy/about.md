...
We strongly recommend using our supported SDKs for your language (for example, for [Python]({{ docs_root }}/core/how-to-guides/api/python/start) or [С++]({{ docs_root }}/core/concepts/api/cpp/description)) that already address these issues.

To work in the console, you should use the [CLI]({{ docs_root }}/core/reference/api/cli/cli) instead of sending direct HTTP requests.
...
Client SDKs typically hide role-related operations. The only thing you may need to do is to specify the role in the client configuration if you need a non-standard role for heavy queries. However, when working directly with HTTP, light and heavy queries should be sent to different hosts. If you send a heavy query via a light HTTP proxy, the server will return an error. For more information, see [List of heavy proxies]({{ docs_root }}/core/reference/proxy/http-reference#hosts).


To get examples of how to interract with {{product-name}} via the HTTP interface, check the [HTTP proxy]({{ docs_root }}/core/reference/proxy/http) section.

For reference information about the HTTP proxy, see [HTTP proxy reference]({{ docs_root }}/core/reference/proxy/http-reference).
...
- [C++]({{ docs_root }}/core/reference/proxy/rpc#c_plus_plus)
- [Java]({{ docs_root }}/core/reference/proxy/rpc#java)
- [Python]({{ docs_root }}/core/reference/proxy/rpc#python)
