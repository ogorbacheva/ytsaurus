...
{{product-name}} supports master and tablet [transactions]({{ docs_root }}/core/concepts/storage/transactions). Master transactions use a pessimistic locking model. This means that a lock on a Cypress node is acquired synchronously, and the system checks for potential conflicts.
...
To run a process under a lock, you can use the [run-command-with-lock]({{ docs_root }}/core/reference/api/cli/commands#run-command-with-lock) command. Alternatively, you can define how to acquire a lock in the application code.
