...
Technically, yes, but it's not recommended for high-load installations. If you configure clients to work without [Discovery]({{ docs_root }}/core/admin-guide/how-to-guides/cluster-access-proxy/overview#discovery) (only through Ingress), three problems arise:
...
- Transaction issues. You must configure [sticky sessions]({{ docs_root }}/core/admin-guide/how-to-guides/cluster-access-proxy/manage-traffic#sticky-sessions).
...
Configure address substitution via the `//sys/http_proxies/@balancers` attribute (see the section [Configure address substitution]({{ docs_root }}/core/admin-guide/how-to-guides/cluster-access-proxy/network-isolation#set-discovery-step)).
