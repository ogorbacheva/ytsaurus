...
Odin is a service that provides qualitative monitoring of a {{product-name}} cluster. For more details, see the [Monitoring]({{ docs_root }}/core/admin-guide/how-to-guides/monitoring#odin) section.
...
* a dedicated service user `robot-odin` with an issued token (see [Token Management]({{ docs_root }}/core/user-guide/reference/storage/auth#token-management)):
...
The UI must be installed as a Helm chart (see [installation guide]({{ docs_root }}/_includes/admin-guide/install-ytsaurus#ui)).
