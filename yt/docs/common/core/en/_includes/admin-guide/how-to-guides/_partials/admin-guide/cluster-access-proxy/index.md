...
By default, a {{product-name}} cluster deployed in Kubernetes is isolated from external networks. LoadBalancer or Ingress mechanisms are typically used to publish services. These work well for individual web services, providing a single entry point for clients, but they [cannot efficiently handle]({{ docs_root }}/_includes/admin-guide/cluster-access-proxy/*scale-traffic) large volumes of network traffic.

In this guide, you'll learn how to:

- [Solve the network isolation problem]({{ docs_root }}/core/admin-guide/how-to-guides/cluster-access-proxy/network-isolation)—configure the cluster so that external clients can directly connect to cluster nodes for efficient reading and writing of large data volumes.

- [Split load between clients]({{ docs_root }}/core/admin-guide/how-to-guides/cluster-access-proxy/manage-traffic)—isolate resources of different projects and split traffic into "light" (metadata, UI) and "heavy" (table reading and writing).

- [Configure access for SPYT]({{ docs_root }}/admin-guide/cluster-access-proxy/spyt)—set up TCP proxying for direct connections between an external Spark driver and workers inside the cluster.
...
You can [split]({{ docs_root }}/core/admin-guide/how-to-guides/cluster-access-proxy/manage-traffic) proxies into functional groups using roles. Typically, two main groups are distinguished:
...
- [How to solve the network isolation problem]({{ docs_root }}/core/admin-guide/how-to-guides/cluster-access-proxy/network-isolation)
- [How to split load between clients]({{ docs_root }}/core/admin-guide/how-to-guides/cluster-access-proxy/manage-traffic)
- [How to configure access for SPYT]({{ docs_root }}/admin-guide/cluster-access-proxy/spyt)
- [FAQ]({{ docs_root }}/core/admin-guide/how-to-guides/cluster-access-proxy/faq)
