...
For more details on installing and configuring Odin, see the [Installing Odin]({{ docs_root }}/core/admin-guide/how-to-guides/install-odin) section.
...
Instructions for setting up dashboards in the UI can be found in the ["Installing dashboards in {{product-name}} UI"]({{ docs_root }}/core/admin-guide/how-to-guides/install-monitoring#setup-ui) section.
...
Read more about setting up Grafana and integrating with {{product-name}} in the corresponding section [Setting up Grafana]({{ docs_root }}/core/admin-guide/how-to-guides/install-monitoring#setup-grafana).
...
- `lost_vital_chunks` — checks that the lost_vital_chunks counter is zero at `//sys/lost_vital_chunks/@count`. Learn more about vitality [here]({{ docs_root }}/core/user-guide/explanation/storage/chunks#vitality);
...
- `yt_chunk_server_underreplicated_chunk_count` — the number of chunks that have fewer replicas on cluster disks than specified in the replication_factor attribute. A continual growth in the number of underreplicated chunks may indicate that your [media]({{ docs_root }}/core/user-guide/reference/storage/media) settings prevent replication_factor conditions from being met in the current cluster configuration. You can view the number of underreplicated chunks at //sys/underreplicated_chunks/@count;
...
It's important to monitor [quotas]({{ docs_root }}/core/user-guide/reference/storage/quotas) of system accounts such as sys, tmp, and intermediate, as well as user quotas of product processes. Example for a sys account:
