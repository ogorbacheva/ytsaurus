...
4. Loading of master snapshots [into Cypress]({{ docs_root }}/core/admin-guide/how-to-guides/persistence-uploader#uploading-snapshots-to-cypress) and master access logs [into Cypress]({{ docs_root }}/core/admin-guide/how-to-guides/logging#structured_log_delivery) is enabled.
...
5. The [Cron Helm chart]({{ docs_root }}/core/admin-guide/how-to-guides/install-cron#process_master_snapshot) is installed, with the `process_master_snapshot` task enabled. This task must run at least once and create the necessary nodes and tables, namely: `//sys/admin/snapshots/snapshot_exports` and `//sys/admin/snapshots/user_exports`.
6. [CHYT]({{ chyt-docs-root }}/{{ lang }}/admin-guide/how-to-guides/install-chyt{{ docs-revision-query }}) is installed and [a clique is created]({{ chyt-docs-root }}/{{ lang }}/user-guide/how-to-guides/try-chyt{{ docs-revision-query }}). Save the clique name to the `CHYT_CLIQUE_NAME` environment variable. For the sake of clarity, our example uses the default clique `ch_public`.
...
Create users [according to the instructions]({{ docs_root }}/core/user-guide/reference/storage/auth):
...
- For Access Log Viewer:
  ...
  Specify a [dictionary]({{ chyt-docs-root }}/{{ lang }}/user-guide/reference/cliques/configuration{{ docs-revision-query }}#clickhouse_config) for the CHYT clique that will access Bulk ACL Checker:
