...
The solution is to split proxies into groups using [roles]({{ docs_root }}/core/admin-guide/how-to-guides/cluster-access-proxy/overview#proxy-role). This lets you distribute load between light and heavy requests and isolate resources for different projects and teams.
...
- You configure Ingress/DNS to point to the `control` group's service. Clients will automatically get Data proxy addresses through the [Discovery]({{ docs_root }}/core/admin-guide/how-to-guides/cluster-access-proxy/overview#discovery) mechanism and send heavy requests directly to them.
