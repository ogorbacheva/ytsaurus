...
Some commands require the additional dependencies (`kubernetes>=31.0.0` for `logs k8s`, `docker>=7.1.0` for `metrics replay`). These are installed together with the [extra `admin` set]({{ docs_root }}/core/user-guide/how-to-guides/api/python/start#extras):
...
This is most often needed [after updating a cluster]({{ docs_root }}/core/admin-guide/how-to-guides/update/update-ytsaurus#operator): in a new version of {{product-name}}, some config fields may become obsolete and remain in `//sys/@config` as unrecognized.
