...
| `date`         | `Date`          |
| `datetime`     | `DateTime`      |
| `any`          | `String`        | Any-колонки читаются движком ClickHouse как строковые колонки, содержащие данные в YSON. Смотрите также: [`chyt.composite.default_yson_format`]({{ docs_root }}/chyt/user-guide/reference/queries/settings.md). |
...
В системе типов {{ chyt-user-guide-reference-data-types-md-audience-1 }} любой тип `T` может быть сделан опциональным путём окружения в `optional<T>`, например, возможны типы данных `optional<optional<int64>>`, `optional<list<string>>` и `optional<tuple<int64, string>>`.
