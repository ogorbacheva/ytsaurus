...
В результате запроса помимо основных колонок таблиц всегда присутствуют виртуальные колонки `$table_index`, `$table_name` и `$table_path`. Они помогают определить, из какой таблицы взята каждая строка. Подробнее — в [разделе про виртуальные колонки]({{ docs_root }}/chyt/user-guide/reference/yt-tables.md#virtual_columns).
