...
- С помощью CLI
  ...
  1. Установите [CHYT CLI]({{ docs_root }}/chyt/user-guide/reference/cli-and-api.md) в составе пакета `ytsaurus-client`, если вы этого ещё не сделали.
  ...
  1. Создайте клику с помощью команды из [CHYT Controller CLI]({{ docs_root }}/chyt/user-guide/explanation/controller.md):
...
После создания клики её нужно настроить — задать необходимые [опции]({{ docs_root }}/chyt/user-guide/reference/cliques/configs.md#options). Основная опция для запуска клики — это `pool`, в неё нужно передать название вычислительного пула, где будет запускаться операция с инстансами для клики.

Перед установкой опции `pool` убедитесь, что у вас есть право `Use` на указываемый пул. Проверьте свои права на вкладке **ACL** на [Панели вкладок]({{ docs_root }}/chyt/user-guide/reference/cliques/ui.md#ui) в разделе **Object permissions**. Если вы не знаете, какой пул использовать, обратитесь к администратору кластера {{product-name}}.

  {{ chyt-user-guide-how-to-guides-cliques-create-start-stop-md-audience-1 }}
...
- Веб-интерфейс
  ...
  1. Нажмите ![edit speclet](../../../../_images/user-guide/how-to-guides/cliques/edit-btn.png){width=24 height=24} в правом верхнем углу, в блоке [Кнопки действий]({{ docs_root }}/chyt/user-guide/reference/cliques/ui.md#ui), или кнопку **Edit speclet** на вкладке **Speclet** на [Панели вкладок]({{ docs_root }}/chyt/user-guide/reference/cliques/ui.md#ui).
  1. В открывшемся окне настроек, в поле _Pool_ введите название вычислительного пула.
  1. Для завершения нажмите кнопку **Confirm**.

  {% note info %}

  Так как операции для клики запускаются из-под системного робота {{ chyt-user-guide-how-to-guides-cliques-create-start-stop-md-audience-2 }}, заранее
  выдайте этому роботу право `Use` на указываемый пул этому роботу. {{ chyt-user-guide-how-to-guides-cliques-create-start-stop-md-audience-3 }}
...
Теперь клика настроена и все настройки сохранены в [спеклете]({{ docs_root }}/chyt/user-guide/reference/cliques/configs.md#speclet) в Кипарисе.
...
- Веб-интерфейс
  ...
  1. Откройте интерфейс клики, как описано в разделе [Как перейти в интерфейс клики]({{ docs_root }}/chyt/user-guide/reference/cliques/ui.md#where).
  1. Нажмите ![start](../../../../_images/user-guide/how-to-guides/cliques/start-btn.png){width=24 height=24} в правом верхнем углу, в блоке [Кнопки действий]({{ docs_root }}/chyt/user-guide/reference/cliques/ui.md#action-menu).
  1. Убедитесь, что:
      ...
      Посмотреть статусы этих параметров можно в интерфейсе клики — в блоке с [характеристиками]({{ docs_root }}/chyt/user-guide/reference/cliques/ui.md#ui).
  1. Проверьте работоспособность клики. Для этого сделайте тестовый запрос в интерфейсе **Query Tracker**:

      - нажмите ![sql](../../../../_images/user-guide/how-to-guides/cliques/sql-btn.png){width=24 height=24} в правом верхнем углу, в блоке [Кнопки действий]({{ docs_root }}/chyt/user-guide/reference/cliques/ui.md#action-menu);
...
{{ chyt-user-guide-how-to-guides-cliques-create-start-stop-md-audience-4 }}

{{ chyt-user-guide-how-to-guides-cliques-create-start-stop-md-audience-5 }}
...
- Веб-интерфейс
  ...
  1. Откройте интерфейс клики, как описано в разделе [Как перейти в интерфейс клики]({{ docs_root }}/chyt/user-guide/reference/cliques/ui.md#where).
  1. Нажмите ![stop](../../../../_images/user-guide/how-to-guides/cliques/stop-btn.png){width=24 height=24} в правом верхнем углу, в блоке [Кнопки действий]({{ docs_root }}/chyt/user-guide/reference/cliques/ui.md#action-menu).
  1. Убедитесь, что:
