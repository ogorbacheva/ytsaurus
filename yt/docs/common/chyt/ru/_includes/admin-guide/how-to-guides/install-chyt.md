...
spec:
  ytsaurus:
    name:
      minisaurus
  image: ghcr.io/ytsaurus/chyt:{{chyt-release-version}}-relwithdebinfo
  makeDefault: true
```
...
После успешного выполнения всех джобов, можно запускать клику `CHYT`. Подробнее про клики можно почитать [в отдельном разделе]({{ docs_root }}/chyt/user-guide/explanation/cliques/overview.md).
...
Инструкция по обновлению CHYT подробно описана в статье [Обновление CHYT]({{ docs_root }}/chyt/admin-guide/how-to-guides/update-chyt.md).
