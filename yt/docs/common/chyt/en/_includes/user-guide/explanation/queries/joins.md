...
The table storage schema in {{product-name}} is fundamentally different from that in ClickHouse. In {{product-name}}, the basic storage primitive is a [static table]({{ core-docs-root }}/{{ lang }}/user-guide/explanation/storage/static-tables{{ docs-revision-query }}) with rows in [chunks]({{ core-docs-root }}/{{ lang }}/user-guide/explanation/storage/chunks{{ docs-revision-query }}) randomly scattered across the cluster. Static tables are extremely inefficient for tasks that require point data reads (since these point reads usually result in Random IO on the hard disk). For quick point reads, {{product-name}} has a more efficient and complex primitive called [dynamic tables]({{ core-docs-root }}/{{ lang }}/user-guide/explanation/dynamic-tables/overview{{ docs-revision-query }}).
...
The CHYT structure is described in detail In the [Query anatomy]({{ docs_root }}/chyt/user-guide/explanation/queries/anatomy.md) section. Any SELECT query is split by the coordinator into portions on the main table and then each portion is processed independently on its own instance.
