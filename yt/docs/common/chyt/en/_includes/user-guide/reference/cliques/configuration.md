...
A speclet is a set of [options]({{ docs_root }}/chyt/user-guide/reference/cliques/configs.md#options) that are interpreted by the controller at clique startup. In particular, the controller generates the final configuration of CHYT instances based on these options.
...
2. [Options]({{ docs_root }}/chyt/user-guide/reference/cliques/configs.md#options) described in the speclet.
...
- `subquery`: The configuration of the main system part which coordinates the execution of ClickHouse queries on top of {{product-name}} tables. Before configuring this part, read the article about [query execution within a clique]({{ docs_root }}/chyt/user-guide/explanation/queries/anatomy.md#query-execution).
...
- The CHYT [configuration example]({{ docs_root }}/chyt/user-guide/reference/cliques/configuration.md#configuration_example) that connects this table as an external ClickHouse dict.
