...
The web interface is a convenient way to manage [cliques]({{ docs_root }}/chyt/user-guide/explanation/general.md). It is suitable for:
...
1. In the **CHYT cliques** section, select the required clique from the list or [create a new one]({{ docs_root }}/chyt/user-guide/how-to-guides/cliques/create-start-stop.md#create).
...
![UI](../../../../_images/user-guide/reference/cliques/clique-ui-os.png){ .center }
...
- Buttons:
  - ![add to favourites](../../../../_images/user-guide/reference/cliques/add-to-favourites-btn.png){width=24 height=24} _Add to favourites_ — add the clique to your favourites.
  - ![view favourites](../../../../_images/user-guide/reference/cliques/view-favourites-btn.png){width=24 height=24} _View favourites_ — view your favourite cliques.
- The clique name.
- The **Create clique** button for [creating a new clique]({{ docs_root }}/chyt/user-guide/how-to-guides/cliques/create-start-stop.md#create).
...
- ![edit speclet](../../../../_images/user-guide/reference/cliques/edit-btn.png){width=24 height=24} _Edit speclet_ — edit the [speclet]({{ docs_root }}/chyt/user-guide/reference/cliques/configs.md#speclet) — the clique’s configuration file.
...
- `Health` — shows the operational status of the clique. The parameter can have the following values:
  ...
  When the `Health` parameter is set to `Failed`, the [Strawberry Controller]({{ docs_root }}/chyt/user-guide/explanation/controller.md) restarts the Vanilla operation. If the issue is resolved, `Health` will change to `Pending` and then to `Good`, or back to `Failed`, in which case the controller will try to restart the Vanilla operation again.
...
- **Speclet** — shows the contents of the YSON document with the clique’s settings ([speclet]({{ docs_root }}/chyt/user-guide/reference/cliques/configs.md#speclet)).
- **ACL** — shows which access permissions to the clique have been granted and for which user groups. For more details, see the [Access permissions]({{ docs_root }}/chyt/user-guide/reference/cliques/access.md) section.
- **Query Logs** — opens the query logs table. For more details, see [Getting Query Logs]({{ docs_root }}/chyt/user-guide/how-to-guides/queries/get-query-logs.md).

## Useful links

[Clique settings]({{ docs_root }}/chyt/user-guide/reference/cliques/configs.md)
