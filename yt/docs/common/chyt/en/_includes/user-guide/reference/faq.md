...
**A:** There is a dedicated [article]({{ docs_root }}/chyt/user-guide/explanation/general.md) about this.
...
**A:** This normally means that the CHYT process inside the Vanilla transaction crashed. You can view the aborted/failed job [counters]({{ docs_root }}/chyt/user-guide/reference/cliques/ui.md) in the operation UI. If there are recent jobs aborted due to preemption, it means that the clique is short on resources. If there are recent failed jobs, please contact your system administrator.
...
**A:** There are **INSERT INTO** and **CREATE TABLE** functions. Learn more in the section on [Differences from ClickHouse.]({{ docs_root }}/chyt/user-guide/reference/yt-tables.md#save)
...
**A:** You can use the `$table_name` and `$table_path` virtual columns. For more information about the virtual columns, see [Working with {{product-name}} tables]({{ docs_root }}/chyt/user-guide/reference/yt-tables.md#virtual_columns).
