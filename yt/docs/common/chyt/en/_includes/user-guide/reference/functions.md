...
In addition to the main table columns, the query output always contains the virtual columns `$table_index`, `$table_name`, and `$table_path`. Use them to determine which table each row is taken from. To learn more about virtual columns, see [the corresponding section]({{ docs_root }}/chyt/user-guide/reference/yt-tables.md#virtual_columns).
