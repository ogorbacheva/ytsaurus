...
The simplest way is to run a query via the {{ chyt-user-guide-how-to-guides-try-chyt-md-audience-1 }} web interface. You can use a public clique with the `ch_public` alias. This is the main public clique found on every cluster of CHYT's presence.
...
CHYT has the official Python API and the official command line utility. For more information, see [CLI and Python API]({{ docs_root }}/chyt/user-guide/reference/cli-and-api.md).
