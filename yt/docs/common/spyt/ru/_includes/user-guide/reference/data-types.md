...
1. **Указания типов**: некоторые сопоставления могут быть изменены [указаниями типов]({{ docs_root }}/spyt/user-guide/reference/read-options.md#schema_hint) в метаданных полей, позволяя более точный контроль над преобразованием типов.
...
6. **«Arrow не поддерживается»**: для типов с такой пометкой при включённой опции [`spark.yt.read.arrow.enabled`]({{ docs_root }}/spyt/user-guide/how-to-guides/cluster/configure.md) (по умолчанию `true`) SPYT автоматически переключается на wire-протокол.
...
Подробнее про некоторые специальные методы для работы с типами данных можно почитать в статьях [Опции чтения]({{ docs_root }}/spyt/user-guide/reference/read-options.md) и [Опции записи]({{ docs_root }}/spyt/user-guide/reference/write-options.md).
...
Если стандартное сопоставление не подходит, можно явно указать целевой {{product-name}}-тип через [schema_hint]({{ docs_root }}/spyt/user-guide/reference/read-options.md#schema_hint).
...
Подробнее про работу с `YsonType` см. в разделах [Python API]({{ docs_root }}/spyt/user-guide/how-to-guides/develop/python.md) и [Scala API]({{ docs_root }}/spyt/user-guide/how-to-guides/develop/scala.md).
