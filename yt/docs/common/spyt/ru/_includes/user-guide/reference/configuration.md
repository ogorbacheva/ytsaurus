...
| `spark.yt.write.dynBatchSize` | `50000` | Максимальное количество строк в одной операции записи в динамическую таблицу. Используется в [Structured Streaming]({{ docs_root }}/spyt/user-guide/explanation/structured-streaming.md) | 2.6.5 |
...
| `spark.yt.globalTransaction.enabled` | `false` | Использовать [глобальную транзакцию]({{ docs_root }}/spyt/user-guide/how-to-guides/read-write-in-transaction.md) | - |
...
| `spark.yt.streaming.transactional` | `false` | Включить [транзакционный режим стриминга]({{ docs_root }}/spyt/user-guide/how-to-guides/structured-streaming/transactional-mode.md) для гарантии `exactly-once`. Требует также `spark.ytsaurus.rpc.job.proxy.enabled = false` | 2.10 |
...
| `spark.ytsaurus.shuffle.enabled` | false | Использовать [{{product-name}} Shuffle сервис]({{ docs_root }}/spyt/user-guide/explanation/ytsaurus-shuffle-service.md) | 2.7.2 |
