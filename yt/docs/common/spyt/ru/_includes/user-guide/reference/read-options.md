...
Чтение под [транзакцией]({{ core-docs-root }}/{{ lang }}/user-guide/explanation/storage/transactions{{ docs-revision-query }}). Подробнее можно узнать в разделе [Чтение и запись под транзакцией]({{ docs_root }}/spyt/user-guide/how-to-guides/read-write-in-transaction.md).
...
Чтение таблиц со схемой в формате [type_v3]({{ core-docs-root }}/{{ lang }}/user-guide/reference/storage/data-types{{ docs-revision-query }}) вместо `type_v1`. Настраивается в [Spark конфигурации]({{ docs_root }}/spyt/user-guide/how-to-guides/cluster/configure.md) или опции записи.
