...
## Event log  { #event-log }
*Event log* – это специальный формат лога в Spark. Лог пишет [драйвер]({{ docs_root }}/spyt/user-guide/explanation/cluster-architecture.md#spark-app), а читает специальный сервис *Spark History Server (SHS)*. SHS запускается вместе с мастером и воркерами при старте кластера SPYT. SHS по логу рисует UI, аналогичный Spark UI.
...
При запуске кластера можно настроить отправку логов с [воркеров]({{ docs_root }}/spyt/user-guide/explanation/cluster-architecture.md#spark-standalone) в {{product-name}} таблицы. Логи будут записываться по пути `{cluster_path}/logs/worker_log`, под каждую дату будет создаваться отдельная таблица. Для включения отправки логов при запуске кластера необходимо использовать следующие опции:
