...
В транзакционном режиме запись данных и фиксация смещения консьюмера объединены в одну таблетную транзакцию. Благодаря этому дубликаты при повторном выполнении микробатча исключены — и в отличие от [идемпотентного приёмника]({{ docs_root }}/spyt/user-guide/how-to-guides/structured-streaming/idempotent-receiver.md) подход работает с любыми трансформациями, включая shuffle (`join`, агрегации).
...
- [Идемпотентный приёмник]({{ docs_root }}/spyt/user-guide/how-to-guides/structured-streaming/idempotent-receiver.md) — альтернатива для stateless 1:1 трансформаций
- [Опции стриминга]({{ docs_root }}/spyt/user-guide/reference/streaming-options.md) — справочник опций
- [Конфигурационные параметры]({{ docs_root }}/spyt/user-guide/reference/configuration.md) — параметры Spark-сессии
