...
Совместимость SPYT с различными версиями Spark приведена [в этой таблице]({{ docs_root }}/spyt/user-guide/explanation/overview.md#spyt-compatibility).
...
```
При использовании [локальных файлов]({{ docs_root }}/spyt/user-guide/explanation/direct-submit.md#submit-local):
```bash
...
Конфигурационные параметры, доступные при запуске задач напрямую, можно найти [здесь]({{ docs_root }}/spyt/user-guide/reference/configuration.md#direct-submit)
...
3. Запустите кластер:
   ...
   Опции:
    ...
    - `--worker-cores` — количество ядер у [воркера]({{ docs_root }}/spyt/user-guide/explanation/cluster-architecture.md#spark-standalone-в-yt--spark-standalone);
    - `--worker-num` — количество воркеров;
    - `--worker-memory` — количество памяти у каждого воркера;
    - `--spark-cluster-version` — [версия]({{ docs_root }}/spyt/user-guide/reference/versioning.md) кластера (опционально).
...
4. Запустите тестовый джоб на кластере:
    ...
    - `--deploy-mode` (`cluster` или `client`) — [режим]({{ docs_root }}/spyt/user-guide/explanation/cluster-architecture.md#cluster-mode--cluster-mode) запуска кластера;
...
О дополнительных параметрах при запуске кластера можно узнать в разделе [Запуск кластера Spark]({{ docs_root }}/spyt/user-guide/how-to-guides/cluster/start.md).
