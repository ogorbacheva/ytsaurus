...
В режиме [Standalone]({{ docs_root }}/spyt/user-guide/explanation/cluster-architecture.md#spark-standalone) не работают настройки `num-executors` и `spark.executor.instances`, количество экзекьюторов определяется настройкой `spark.cores.max`.
...
Дополнительные примеры содержатся в разделе [SPYT в Jupyter]({{ docs_root }}/spyt/user-guide/how-to-guides/develop/jupyter.md).
