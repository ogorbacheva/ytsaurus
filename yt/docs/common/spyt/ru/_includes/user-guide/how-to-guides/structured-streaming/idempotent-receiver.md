...
Идемпотентный приёмник работает медленнее, чем нетранзакционный стриминг и [транзакционный режим]({{ docs_root }}/spyt/user-guide/how-to-guides/structured-streaming/transactional-mode.md): это связано с тем, что данные записываются в сортированную динамическую таблицу, которая требует обновления индекса при каждой записи. Если скорость записи критична, рассмотрите транзакционный режим: он быстрее и поддерживает любые трансформации.

{% endnote %}

## Как работает { #how-it-works }

Идемпотентный приёмник достигает `exactly-once` иначе, чем [транзакционный режим]({{ docs_root }}/spyt/user-guide/how-to-guides/structured-streaming/transactional-mode.md): не через атомарность, а через идемпотентность записи. Если при повторном выполнении батча в таблицу попадают те же самые строки, дубликатов не возникает.
...
1. При чтении очереди установите опцию [include_service_columns]({{ docs_root }}/spyt/user-guide/reference/streaming-options.md#service-columns) в `true`. Тогда стриминговый датафрейм будет содержать столбцы `__spyt_streaming_src_tablet_index` и `__spyt_streaming_src_row_index`, соответствующие `$tablet_index` и `$row_index` строки в читаемой очереди.
...
- [Гарантия exactly-once]({{ docs_root }}/spyt/user-guide/explanation/structured-streaming/exactly-once.md) — выбор подхода к гарантиям записи
- [Транзакционный режим]({{ docs_root }}/spyt/user-guide/how-to-guides/structured-streaming/transactional-mode.md) — рекомендуемый подход для новых задач
- [Опции стриминга]({{ docs_root }}/spyt/user-guide/reference/streaming-options.md) — справочник опций
