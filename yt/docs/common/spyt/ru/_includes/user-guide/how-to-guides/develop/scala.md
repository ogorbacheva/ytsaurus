...
Все допустимые значения для `spytVersion` можно найти [здесь](https://github.com/ytsaurus/ytsaurus/tags) под тегами spyt/* или spyt-spark/*. Совместимые версии Apache Spark указаны в [таблице совместимости]({{ docs_root }}/spyt/user-guide/explanation/overview.md#spyt-compatibility).
...
В случае использования [запуска задач с использованием планировщика {{product-name}}]({{ docs_root }}/spyt/user-guide/how-to-guides/run-spark-jobs.md#submit), для создания `SparkSession` нужно воспользоваться стандартными рекомендациями Spark:
