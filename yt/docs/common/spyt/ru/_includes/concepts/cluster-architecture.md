...
Запуск кластера описан в отдельном [разделе]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/cluster/start{{ docs-revision-query }}).
...
В текущей реализации есть два варианта запуска Spark задач: [с использованием планировщика {{product-name}}]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/run-spark-jobs{{ docs-revision-query }}#submit), либо с использованием внутреннего Spark Standalone кластера, который разворачивается внутри Vanilla операции.
