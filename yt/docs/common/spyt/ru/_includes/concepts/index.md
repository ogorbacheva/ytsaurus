...
- Отдельные запуски расчётов напрямую в {{product-name}}, используя команду `spark-submit` [Подробнее]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/run-spark-jobs{{ docs-revision-query }}#submit).
- Создание Standalone Spark кластера как постоянного ресурса внутри {{product-name}} при помощи Vanilla операции [Подробнее]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/run-spark-jobs{{ docs-revision-query }}#standalone).

## На каких языках можно писать { #lang }

Spark поддерживает следующие языки и среды разработки:

* [Jupyter]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/development/jupyter{{ docs-revision-query }}).
* [Python]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/development/python{{ docs-revision-query }}).
* [Java]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/development/java{{ docs-revision-query }}).
* [Scala]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/development/scala{{ docs-revision-query }}).
