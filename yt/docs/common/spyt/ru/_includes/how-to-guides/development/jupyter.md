...
Перед тем как использовать Spark в Jupyter, необходимо создать [кластер]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/cluster/start{{ docs-revision-query }}). Работа со Spark из Jupyter ноутбуков в настоящий момент возможна только с использованием standalone кластера.
