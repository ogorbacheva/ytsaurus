...
Все допустимые значения для `spytVersion` можно найти [здесь](https://github.com/ytsaurus/ytsaurus/tags) под тегами spyt/* или spyt-spark/*. Совместимые версии Apache Spark указаны в [таблице совместимости]({{ spyt-docs-root }}/{{ lang }}/concepts/index{{ docs-revision-query }}#spyt-compatibility).
...
В случае использования [запуска задач с использованием планировщика {{product-name}}]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/run-spark-jobs{{ docs-revision-query }}#submit), для создания `SparkSession` нужно воспользоваться стандартными рекомендациями Spark:
