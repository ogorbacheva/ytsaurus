...
При [старте кластера]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/run-spark-jobs{{ docs-revision-query }}#standalone) каждому воркеру будет выделено целое количество GPU, указанное в опции `--worker-gpu-limit`:
