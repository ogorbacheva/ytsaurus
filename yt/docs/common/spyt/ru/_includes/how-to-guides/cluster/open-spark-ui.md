...
Чтобы вы могли попасть в интерфейс, используется [task-прокси]({{ core-docs-root }}/{{ lang }}/reference/proxy/task{{ docs-revision-query }}). Он создает стабильную ссылку, которая работает даже при перемещении джобов между узлами, и проверяет права доступа при входе.

{% note info %}

Чтобы можно было работать c task-прокси, администратор кластера должен предварительно [развернуть]({{ core-docs-root }}/{{ lang }}/how-to-guides/install-task-proxy{{ docs-revision-query }}) этот компонент.
