...
Чтобы вы могли попасть в интерфейс, используется [task-прокси]({{ core-docs-root }}/{{ lang }}/user-guide/reference/proxy/task{{ docs-revision-query }}). Он создает стабильную ссылку, которая работает даже при перемещении джобов между узлами, и проверяет права доступа при входе.

{% note info %}

Чтобы можно было работать c task-прокси, администратор кластера должен предварительно [развернуть]({{ core-docs-root }}/{{ lang }}/admin-guide/how-to-guides/install-task-proxy{{ docs-revision-query }}) этот компонент.
...
2.  **Найдите адрес сервиса**.
    ...
    *   **Строка 1 (`master`)**: UI мастер-ноды [standalone]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/cluster/start{{ docs-revision-query }}) кластера.
    *   **Строка 2 (`history`)**: UI сервера истории (History Server).
    *   **Строка 3 (`driver`)**: UI драйвера при запуске через [direct submit]({{ spyt-docs-root }}/{{ lang }}/concepts/direct-submit{{ docs-revision-query }}).
