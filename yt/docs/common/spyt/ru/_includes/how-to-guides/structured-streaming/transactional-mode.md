...
В транзакционном режиме запись данных и фиксация смещения консьюмера объединены в одну таблетную транзакцию. Благодаря этому дубликаты при повторном выполнении микробатча исключены — и в отличие от [идемпотентного приёмника]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/structured-streaming/idempotent-receiver{{ docs-revision-query }}) подход работает с любыми трансформациями, включая shuffle (`join`, агрегации).
...
В {{product-name}} есть внутреннее [ограничение]({{ core-docs-root }}/{{ lang }}/user-guide/explanation/dynamic-tables/transactions{{ docs-revision-query }}#restrictions): в рамках одной транзакции в один таблет нельзя записать больше определённого числа строк (по умолчанию 100 000). Поскольку транзакционный режим атомарно записывает весь микробатч одной транзакцией, все его строки распределяются по таблетам выходной таблицы. Если таблетов мало, а микробатч большой, на каждый таблет придётся слишком много данных — лимит будет превышен, и транзакция завершится с ошибкой `Transaction affects too many rows in tablet`.
...
Как задать число таблетов при создании таблицы или изменить его позже — в разделе [Шардирование]({{ core-docs-root }}/{{ lang }}/user-guide/how-to-guides/dynamic-tables/resharding{{ docs-revision-query }}).
...
- [Идемпотентный приёмник]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/structured-streaming/idempotent-receiver{{ docs-revision-query }}) — альтернатива для stateless 1:1 трансформаций
- [Опции стриминга]({{ spyt-docs-root }}/{{ lang }}/reference/streaming-options{{ docs-revision-query }}) — справочник опций
- [Конфигурационные параметры]({{ spyt-docs-root }}/{{ lang }}/reference/configuration{{ docs-revision-query }}) — параметры Spark-сессии
