...
В {{product-name}} есть внутреннее [ограничение]({{ core-docs-root }}/{{ lang }}/concepts/dynamic-tables/transactions{{ docs-revision-query }}#restrictions): в рамках одной транзакции в один таблет нельзя записать больше определённого числа строк (по умолчанию 100 000). Поскольку транзакционный режим атомарно записывает весь микробатч одной транзакцией, все его строки распределяются по таблетам выходной таблицы. Если таблетов мало, а микробатч большой, на каждый таблет придётся слишком много данных — лимит будет превышен, и транзакция завершится с ошибкой `Transaction affects too many rows in tablet`.
...
Как задать число таблетов при создании таблицы или изменить его позже — в разделе [Шардирование]({{ core-docs-root }}/{{ lang }}/how-to-guides/dynamic-tables/resharding{{ docs-revision-query }}).
