...
Идемпотентный приёмник работает медленнее, чем нетранзакционный стриминг и [транзакционный режим]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/structured-streaming/transactional-mode{{ docs-revision-query }}): это связано с тем, что данные записываются в сортированную динамическую таблицу, которая требует обновления индекса при каждой записи. Если скорость записи критична, рассмотрите транзакционный режим: он быстрее и поддерживает любые трансформации.

{% endnote %}

## Как работает { #how-it-works }

Идемпотентный приёмник достигает `exactly-once` иначе, чем [транзакционный режим]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/structured-streaming/transactional-mode{{ docs-revision-query }}): не через атомарность, а через идемпотентность записи. Если при повторном выполнении батча в таблицу попадают те же самые строки, дубликатов не возникает.
...
- [Гарантия exactly-once]({{ spyt-docs-root }}/{{ lang }}/concepts/structured-streaming/exactly-once{{ docs-revision-query }}) — выбор подхода к гарантиям записи
- [Транзакционный режим]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/structured-streaming/transactional-mode{{ docs-revision-query }}) — рекомендуемый подход для новых задач
- [Опции стриминга]({{ spyt-docs-root }}/{{ lang }}/reference/streaming-options{{ docs-revision-query }}) — справочник опций
