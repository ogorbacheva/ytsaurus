...
Механизм пришёл на смену [Livy]({{ spyt-docs-root }}/{{ lang }}/reference/livy{{ docs-revision-query }}) начиная со SPYT 2.10.0 и Query Tracker 0.4.
...
- После простоя.
   Чтобы не тратить ресурсы впустую, драйвер автоматически останавливается после 10 минут бездействия. Следующий запрос снова инициирует его запуск. Таймаут простоя настраивается через параметр `spark.ytsaurus.connect.idle.timeout` в [конфигурационных параметрах SPYT]({{ spyt-docs-root }}/{{ lang }}/reference/configuration{{ docs-revision-query }}).
...
|| [Прямой сабмит]({{ spyt-docs-root }}/{{ lang }}/concepts/direct-submit{{ docs-revision-query }}) | Нет выделенного кластера; Spark-приложение запускается по требованию под каждый запрос ||
|| [Внутренний кластер]({{ spyt-docs-root }}/{{ lang }}/concepts/cluster-architecture{{ docs-revision-query }}) | Кластер уже запущен; SPYT Connect подключается к нему ||
|#
...
| `spark_conf` | Конфигурационные параметры для запуска Spark-приложения. Можно указывать как [стандартные параметры Spark](https://spark.apache.org/docs/latest/configuration.html), так и [параметры SPYT]({{ spyt-docs-root }}/{{ lang }}/reference/configuration{{ docs-revision-query }}) | — |
...
- [Прямой сабмит]({{ spyt-docs-root }}/{{ lang }}/concepts/direct-submit{{ docs-revision-query }})
- [Внутренний SPYT кластер]({{ spyt-docs-root }}/{{ lang }}/concepts/cluster-architecture{{ docs-revision-query }})
- [Конфигурационные параметры SPYT]({{ spyt-docs-root }}/{{ lang }}/reference/configuration{{ docs-revision-query }})
