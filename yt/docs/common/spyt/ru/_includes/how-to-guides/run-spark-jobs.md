...
Совместимость SPYT с различными версиями Spark приведена [в этой таблице]({{ spyt-docs-root }}/{{ lang }}/concepts/index{{ docs-revision-query }}#spyt-compatibility).
...
```
При использовании [локальных файлов]({{ spyt-docs-root }}/{{ lang }}/concepts/direct-submit{{ docs-revision-query }}#submit-local):
```bash
...
Конфигурационные параметры, доступные при запуске задач напрямую, можно найти [здесь]({{ spyt-docs-root }}/{{ lang }}/reference/configuration{{ docs-revision-query }}#direct-submit)
...
3. Запустите кластер:
   ...
   Опции:
    ...
    - `--worker-cores` — количество ядер у [воркера]({{ spyt-docs-root }}/{{ lang }}/concepts/cluster-architecture{{ docs-revision-query }}#spark-standalone-в-yt--spark-standalone);
    - `--worker-num` — количество воркеров;
    - `--worker-memory` — количество памяти у каждого воркера;
    - `--spark-cluster-version` — [версия]({{ spyt-docs-root }}/{{ lang }}/reference/versioning{{ docs-revision-query }}) кластера (опционально).
...
4. Запустите тестовый джоб на кластере:
    ...
    - `--deploy-mode` (`cluster` или `client`) — [режим]({{ spyt-docs-root }}/{{ lang }}/concepts/cluster-architecture{{ docs-revision-query }}#cluster-mode--cluster-mode) запуска кластера;
...
О дополнительных параметрах при запуске кластера можно узнать в разделе [Запуск кластера Spark]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/cluster/start{{ docs-revision-query }}).
