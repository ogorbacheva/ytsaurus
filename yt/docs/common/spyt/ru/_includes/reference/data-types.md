...
6. **«Arrow не поддерживается»**: для типов с такой пометкой при включённой опции [`spark.yt.read.arrow.enabled`]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/cluster/configure{{ docs-revision-query }}) (по умолчанию `true`) SPYT автоматически переключается на wire-протокол.
...
Подробнее про некоторые специальные методы для работы с типами данных можно почитать в статьях [Опции чтения]({{ spyt-docs-root }}/{{ lang }}/reference/read-options{{ docs-revision-query }}) и [Опции записи]({{ spyt-docs-root }}/{{ lang }}/reference/write-options{{ docs-revision-query }}).
...
Если стандартное сопоставление не подходит, можно явно указать целевой {{product-name}}-тип через [schema_hint]({{ spyt-docs-root }}/{{ lang }}/reference/read-options{{ docs-revision-query }}#schema_hint).
...
Подробнее про работу с `YsonType` см. в разделах [Python API]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/development/python{{ docs-revision-query }}) и [Scala API]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/development/scala{{ docs-revision-query }}).
