...
| `spark.yt.globalTransaction.enabled` | `false` | Использовать [глобальную транзакцию]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/read-write-in-transaction{{ docs-revision-query }}) | - |
...
| `spark.yt.streaming.transactional` | `false` | Включить [транзакционный режим стриминга]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/structured-streaming/transactional-mode{{ docs-revision-query }}) для гарантии `exactly-once`. Требует также `spark.ytsaurus.rpc.job.proxy.enabled = false` | 2.10 |
...
| `spark.ytsaurus.shuffle.enabled` | false | Использовать [{{product-name}} Shuffle сервис]({{ spyt-docs-root }}/{{ lang }}/concepts/ytsaurus-shuffle-service{{ docs-revision-query }}) | 2.7.2 |
