...
В разделе перечислены опции SPYT для Structured Streaming, служебные колонки и матрица совместимости версий. Параметры Spark-сессии (включая `spark.yt.streaming.transactional`) — на странице [Конфигурационные параметры]({{ spyt-docs-root }}/{{ lang }}/reference/configuration{{ docs-revision-query }}).
...
- [Structured Streaming]({{ spyt-docs-root }}/{{ lang }}/concepts/structured-streaming/index{{ docs-revision-query }}) — обзор и основные сценарии
- [Гарантия exactly-once]({{ spyt-docs-root }}/{{ lang }}/concepts/structured-streaming/exactly-once{{ docs-revision-query }}) — выбор подхода к гарантиям
- [Транзакционный режим]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/structured-streaming/transactional-mode{{ docs-revision-query }}) — инструкция по включению exactly-once
- [Конфигурационные параметры]({{ spyt-docs-root }}/{{ lang }}/reference/configuration{{ docs-revision-query }}) — параметры Spark-сессии, включая `spark.yt.streaming.transactional` и `spark.yt.write.dynBatchSize`
