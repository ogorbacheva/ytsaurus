...
The idempotent receiver is slower than non‑transactional streaming and the [transactional mode]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/structured-streaming/transactional-mode{{ docs-revision-query }}). This is because data is written to a sorted dynamic table, which requires index updates on every write. If write speed is critical, consider the transactional mode: it’s faster and supports any transformations.

{% endnote %}

## How it works { #how-it-works }

The idempotent receiver achieves `exactly‑once` semantics differently from the [transactional mode]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/structured-streaming/transactional-mode{{ docs-revision-query }}): not through atomicity, but through idempotent writes. If the same rows are written to the table when a batch is re‑executed, no duplicates are created.
...
- [Transactional mode]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/structured-streaming/transactional-mode{{ docs-revision-query }}) — recommended approach for new tasks
