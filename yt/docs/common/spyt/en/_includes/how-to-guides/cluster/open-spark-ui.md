...
2.  **Find the service address.**
    ...
    - Row 1 (`master`): UI of the master node of a [standalone]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/cluster/start{{ docs-revision-query }}) cluster.
    - Row 2 (`history`): UI of the history server.
    - Row 3 (`driver`): UI of the driver when launched via [direct submit]({{ spyt-docs-root }}/{{ lang }}/concepts/direct-submit{{ docs-revision-query }}).
