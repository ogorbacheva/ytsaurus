...
This section contains expanded instructions for starting a Spark cluster. Basic start operations are described in the [Quick start]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/run-spark-jobs{{ docs-revision-query }}#standalone) section.

{% note warning "Attention!" %}

A started Spark cluster statically occupies the resources allocated to it. So, it is recommended that the cluster be started in a separate computational pool with guaranteed resources.  It makes sense to use one cluster for the command, and recycle the resources among several users.  We recommend [launching tasks directly]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/run-spark-jobs{{ docs-revision-query }}#submit) if you do not plan to launch Spark tasks at a high intensity (more than once per hour).
...
| `--params` | no | - | Additional {{product-name}} operation parameters specified as a YSON string. Learn more on the [configuration]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/cluster/configure{{ docs-revision-query }}#add) page. | - |
...
| `--enable-ytsaurus-shuffle`, `--disable-ytsaurus-shuffle` | no | `--disable-ytsaurus-shuffle` | Use the [{{product-name}} Shuffle service]({{ spyt-docs-root }}/{{ lang }}/concepts/ytsaurus-shuffle-service{{ docs-revision-query }}) | 2.7.2 |
| `--enable-spark-shuffle`, `--disable-spark-shuffle` | no | `--enable-spark-shuffle` | Run the native external Spark shuffle service on the cluster (see [Shuffle service]({{ spyt-docs-root }}/{{ lang }}/concepts/ytsaurus-shuffle-service{{ docs-revision-query }}#enabling-standalone)) | 2.9.3 |
...
| `--worker-disk-name` | no | default | Name of the disk requested in the operation specification. For more information, see [Disk requests for jobs]({{ core-docs-root }}/{{ lang }}/user-guide/reference/data-processing/operations/operations-options{{ docs-revision-query }}#disk_request). | - |
| `--worker-disk-limit` | no | - | Maximum disk capacity requested in the operation specification. For more information, see [Disk requests for jobs]({{ core-docs-root }}/{{ lang }}/user-guide/reference/data-processing/operations/operations-options{{ docs-revision-query }}#disk_request). | - |
| `--worker-disk-account` | no | - | [Account]({{ core-docs-root }}/{{ lang }}/user-guide/reference/storage/accounts{{ docs-revision-query }}) for requesting a disk in the operation specification. For more information, see [Disk requests for jobs]({{ core-docs-root }}/{{ lang }}/user-guide/reference/data-processing/operations/operations-options{{ docs-revision-query }}#disk_request). | - |
...
| `--ssd-account` | no | - | {{product-name}} [account]({{ core-docs-root }}/{{ lang }}/user-guide/reference/storage/accounts{{ docs-revision-query }}) used for SSD allocation. | - |
...
| `--autoscaler-period`, `--autoscaler-metrics-port`, `--autoscaler-sliding-window`, `--autoscaler-max-free-workers`, `--autoscaler-slot-increment-step` | no | - | Parameters used for working with an [auto-scaler]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/cluster/start{{ docs-revision-query }}#auto-scale). | - |
| `--enable-livy` | no | false | Run the Livy server as part of the cluster. You can learn more about the Livy server [here]({{ spyt-docs-root }}/{{ lang }}/reference/livy{{ docs-revision-query }}). | 1.74.0 |
