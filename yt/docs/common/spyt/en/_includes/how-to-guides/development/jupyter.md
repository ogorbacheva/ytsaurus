...
Before you can use Spark in Jupyter, you need to create a [cluster]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/cluster/start{{ docs-revision-query }}). Currently working with Spark using Jupyter notebooks is possible only using inner standalone cluster.
