...
- The code has been built using the `mvn clean compile assembly:assembly` command and posted to {{product-name}}: `//sys/spark/examples/spark-over-yt-examples-jar-with-dependencies.jar`. You must always upload your code to {{product-name}} as Spark does not distribute user files to [workers]({{ spyt-docs-root }}/{{ lang }}/concepts/cluster-architecture{{ docs-revision-query }}#cluster-mode).


## Running examples { #run-examples }

Each of the examples includes a single class inherited from `SparkAppJava`.`SparkJavaApp` initializes Sparks and exits without errors. This method is suitable for working with [inner Spark standalone cluster]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/run-spark-jobs{{ docs-revision-query }}#standalone). For launching Spark applications using[{{product-name}} scheduler]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/run-spark-jobs{{ docs-revision-query }}#submit) you should refer to standard Spark recommendations for creating `SparkSession`:
