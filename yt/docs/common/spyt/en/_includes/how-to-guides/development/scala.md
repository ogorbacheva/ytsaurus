...
All possible values for `spytVersion` can be found [here](https://github.com/ytsaurus/ytsaurus/tags) under the spyt/* or spyt-spark/* tag. Apache Spark compatible versions are described in [this table]({{ spyt-docs-root }}/{{ lang }}/concepts/index{{ docs-revision-query }}#spyt-compatibility).
...
For submitting Spark tasks [directly to {{product-name}}]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/run-spark-jobs{{ docs-revision-query }}#submit) a `SparkSession` object should be created according to Spark recommendations:
