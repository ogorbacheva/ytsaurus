...
In [Standalone]({{ spyt-docs-root }}/{{ lang }}/concepts/cluster-architecture{{ docs-revision-query }}#spark-standalone) mode, the `num-executors` and the `spark.executor.instances` settings are non-functional while the number of executors depends on the `spark.cores.max` parameter.
...
You can find additional examples in [SPYT in Jupyter]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/development/jupyter{{ docs-revision-query }}).
