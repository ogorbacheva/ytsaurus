...
6. **Arrow is not supported**: For types marked as such, when the [`spark.yt.read.arrow.enabled`]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/cluster/configure{{ docs-revision-query }}) option is enabled (default `true`), SPYT automatically switches to the wire protocol.
...
For special techniques related to data types, see [Read options]({{ spyt-docs-root }}/{{ lang }}/reference/read-options{{ docs-revision-query }}) and [Write options]({{ spyt-docs-root }}/{{ lang }}/reference/write-options{{ docs-revision-query }}).
...
If the default mapping is not suitable, you can explicitly specify the target {{product-name}} type via [schema_hint]({{ spyt-docs-root }}/{{ lang }}/reference/read-options{{ docs-revision-query }}#schema_hint).
...
Learn more about working with `YsonType` in the [Python API]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/development/python{{ docs-revision-query }}) and [Scala API]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/development/scala{{ docs-revision-query }}) sections.
