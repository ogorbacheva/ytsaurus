...
| `spark.yt.globalTransaction.enabled` | `false` | Use a [global transaction]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/read-write-in-transaction{{ docs-revision-query }}). | - |
...
| `spark.yt.streaming.transactional` | `false` | Enable [streaming transactional mode]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/structured-streaming/transactional-mode{{ docs-revision-query }}) to guarantee `exactly‑once`. Requires `spark.ytsaurus.rpc.job.proxy.enabled = false` as well. | 2.10 |
...
| `spark.ytsaurus.shuffle.enabled` | false | Use the [{{product-name}} Shuffle service]({{ spyt-docs-root }}/{{ lang }}/concepts/ytsaurus-shuffle-service{{ docs-revision-query }}). | 2.7.2 |
