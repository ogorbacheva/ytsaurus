...
Starting with SPYT 2.10.0 and Query Tracker 0.4, Livy integration is no longer supported. If you’re using Livy, migrate to [SPYT Connect]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/use-spyt-connect{{ docs-revision-query }}#migration).

{% endnote %}

Starting with version 1.74.0, SPYT comes with [Livy](https://livy.apache.org/), a service that allows communication between the client and a Spark cluster over a REST interface. The [Query tracker]({{ core-docs-root }}/{{ lang }}/user-guide/explanation/query-tracker/about{{ docs-revision-query }}) module uses this functionality to execute Spark SQL queries in {{product-name}}.
...
1. When Query Tracker starts executing a query, it issues the user a temporary [token]({{ core-docs-root }}/{{ lang }}/user-guide/reference/storage/auth{{ docs-revision-query }}) with a TTL of tens of minutes. The received token is used to authenticate the user in {{product-name}} when executing the query on a SPYT cluster. This ensures that the user has the necessary [permissions]({{ core-docs-root }}/{{ lang }}/user-guide/explanation/storage/access-control{{ docs-revision-query }}#authorization) when reading or writing data. If query execution takes a long time, the temporary token's TTL will be periodically extended.
