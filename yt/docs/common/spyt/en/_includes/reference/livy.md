...
Starting with version 1.74.0, SPYT comes with [Livy](https://livy.apache.org/), a service that allows communication between the client and a Spark cluster over a REST interface. The [Query tracker]({{ core-docs-root }}/{{ lang }}/concepts/query-tracker/about{{ docs-revision-query }}) module uses this functionality to execute Spark SQL queries in {{product-name}}.
...
1. When Query Tracker starts executing a query, it issues the user a temporary [token]({{ core-docs-root }}/{{ lang }}/reference/storage/auth{{ docs-revision-query }}) with a TTL of tens of minutes. The received token is used to authenticate the user in {{product-name}} when executing the query on a SPYT cluster. This ensures that the user has the necessary [permissions]({{ core-docs-root }}/{{ lang }}/concepts/storage/access-control{{ docs-revision-query }}#authorization) when reading or writing data. If query execution takes a long time, the temporary token's TTL will be periodically extended.
