...
1. **Type hints**: Some mappings can be overridden by [type hints]({{ docs_root }}/spyt/user-guide/reference/read-options.md#schema_hint) in field metadata. This enables precise control of type conversion.
...
6. **Arrow is not supported**: For types marked as such, when the [`spark.yt.read.arrow.enabled`]({{ docs_root }}/spyt/user-guide/how-to-guides/cluster/configure.md) option is enabled (default `true`), SPYT automatically switches to the wire protocol.
...
For special techniques related to data types, see [Read options]({{ docs_root }}/spyt/user-guide/reference/read-options.md) and [Write options]({{ docs_root }}/spyt/user-guide/reference/write-options.md).
...
If the default mapping is not suitable, you can explicitly specify the target {{product-name}} type via [schema_hint]({{ docs_root }}/spyt/user-guide/reference/read-options.md#schema_hint).
...
Learn more about working with `YsonType` in the [Python API]({{ docs_root }}/spyt/user-guide/how-to-guides/develop/python.md) and [Scala API]({{ docs_root }}/spyt/user-guide/how-to-guides/develop/scala.md) sections.
