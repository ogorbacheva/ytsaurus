...
| `spark.yt.write.dynBatchSize` | `50000` | Maximum number of rows in a single write operation to a dynamic table. Used in [Structured Streaming]({{ docs_root }}/spyt/user-guide/explanation/structured-streaming.md) | 2.6.5 |
...
| `spark.yt.globalTransaction.enabled` | `false` | Use a [global transaction]({{ docs_root }}/spyt/user-guide/how-to-guides/read-write-in-transaction.md). | - |
...
| `spark.yt.streaming.transactional` | `false` | Enable [streaming transactional mode]({{ docs_root }}/spyt/user-guide/how-to-guides/structured-streaming/transactional-mode.md) to guarantee `exactly‑once`. Requires `spark.ytsaurus.rpc.job.proxy.enabled = false` as well. | 2.10 |
...
| `spark.ytsaurus.shuffle.enabled` | false | Use the [{{product-name}} Shuffle service]({{ docs_root }}/spyt/user-guide/explanation/ytsaurus-shuffle-service.md). | 2.7.2 |
