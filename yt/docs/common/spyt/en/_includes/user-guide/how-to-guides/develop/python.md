...
In [Standalone]({{ docs_root }}/spyt/user-guide/explanation/cluster-architecture.md#spark-standalone) mode, the `num-executors` and the `spark.executor.instances` settings are non-functional while the number of executors depends on the `spark.cores.max` parameter.
...
You can find additional examples in [SPYT in Jupyter]({{ docs_root }}/spyt/user-guide/how-to-guides/develop/jupyter.md).
