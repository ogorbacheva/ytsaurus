...
The idempotent receiver is slower than non‑transactional streaming and the [transactional mode]({{ docs_root }}/spyt/user-guide/how-to-guides/structured-streaming/transactional-mode.md). This is because data is written to a sorted dynamic table, which requires index updates on every write. If write speed is critical, consider the transactional mode: it’s faster and supports any transformations.

{% endnote %}

## How it works { #how-it-works }

The idempotent receiver achieves `exactly‑once` semantics differently from the [transactional mode]({{ docs_root }}/spyt/user-guide/how-to-guides/structured-streaming/transactional-mode.md): not through atomicity, but through idempotent writes. If the same rows are written to the table when a batch is re‑executed, no duplicates are created.
...
1. When reading the queue, set the [include_service_columns]({{ docs_root }}/spyt/user-guide/reference/streaming-options.md#service-columns) option to `true`. Then the streaming dataframe will contain the columns `__spyt_streaming_src_tablet_index` and `__spyt_streaming_src_row_index`, which correspond to `$tablet_index` and `$row_index` of the row in the queue being read.
...
- [Exactly‑once guarantee]({{ docs_root }}/spyt/user-guide/explanation/structured-streaming/exactly-once.md) — choosing an approach to write guarantees
- [Transactional mode]({{ docs_root }}/spyt/user-guide/how-to-guides/structured-streaming/transactional-mode.md) — recommended approach for new tasks
- [Streaming options]({{ docs_root }}/spyt/user-guide/reference/streaming-options.md) — options reference
