...
## Event log  { #event-log }
*Event log* is a special log format in Spark. Logs are written by a [driver]({{ docs_root }}/spyt/user-guide/explanation/cluster-architecture.md#spark-app) and read by a special service called *Spark History Server (SHS)*. SHS launches alongside the master and the workers when a SPYT cluster is started. SHS uses the log to draw a UI similar to Spark UI.
...
When you start a cluster, you can configure [worker]({{ docs_root }}/spyt/user-guide/explanation/cluster-architecture.md#spark-standalone) logs to upload to {{product-name}} tables. Logs will be written to `{cluster_path}/logs/worker_log` with a separate table created for each date. To enable sending of logs when starting a cluster, you must use one of these options:
