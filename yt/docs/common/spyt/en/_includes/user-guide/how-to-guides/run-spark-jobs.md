...
3. Start your cluster:
   ...
   - `--worker-cores` — Number of [worker]({{ docs_root }}/spyt/user-guide/explanation/cluster-architecture.md#spark-standalone-в-yt--spark-standalone) cores.
   - `--worker-num` — Number of workers.
   - `--worker-memory` — Amount of each worker's memory.
   - `--spark-cluster-version` — Cluster [version]({{ docs_root }}/spyt/user-guide/reference/versioning.md) (optional).
...
4. Start a test job on your cluster:
   ...
   - `--deploy-mode` (`cluster` or `client`) — Cluster startup [mode]({{ docs_root }}/spyt/user-guide/explanation/cluster-architecture.md#cluster-mode--cluster-mode).
...
For additional cluster startup parameters, see [Starting a Spark cluster]({{ docs_root }}/spyt/user-guide/how-to-guides/cluster/start.md).
