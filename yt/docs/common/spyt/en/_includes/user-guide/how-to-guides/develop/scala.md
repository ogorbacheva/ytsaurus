...
All possible values for `spytVersion` can be found [here](https://github.com/ytsaurus/ytsaurus/tags) under the spyt/* or spyt-spark/* tag. Apache Spark compatible versions are described in [this table]({{ docs_root }}/spyt/user-guide/explanation/overview.md#spyt-compatibility).
...
For submitting Spark tasks [directly to {{product-name}}]({{ docs_root }}/spyt/user-guide/how-to-guides/run-spark-jobs.md#submit) a `SparkSession` object should be created according to Spark recommendations:
