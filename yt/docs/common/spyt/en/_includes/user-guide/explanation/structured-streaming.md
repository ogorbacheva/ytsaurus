...
![](../../../_images/user-guide/explanation/spyt-streaming-general-scheme.png){ .center }
...
1. In [checkpoint files]({{ docs_root }}/spyt/user-guide/explanation/structured-streaming.md#checkpoint-location) automatically created by Spark in a directory.
...
| [Transactional mode]({{ docs_root }}/spyt/user-guide/how-to-guides/structured-streaming/transactional-mode.md) (SPYT 2.10+) | `exactly‑once` for any transformations | When data accuracy is critical: financial analytics, ML features, incremental data mart construction |
| [Idempotent receiver]({{ docs_root }}/spyt/user-guide/how-to-guides/structured-streaming/idempotent-receiver.md) | `exactly‑once` for stateless 1:1 transformations | When you want to avoid the additional load on the RPC proxy from transactional mode, or when maintaining legacy code |


For a comparison of modes, operation schemes, and instructions on enabling them, see the article [Exactly‑once guarantee]({{ docs_root }}/spyt/user-guide/explanation/structured-streaming/exactly-once.md).
...
For the complete reference of streaming options (DataFrameReader/Writer options, service columns, version compatibility matrix), see the [Streaming options]({{ docs_root }}/spyt/user-guide/reference/streaming-options.md) page. For Spark session parameters (including `spark.yt.streaming.transactional` and `spark.yt.write.dynBatchSize`), see the [Configuration parameters]({{ docs_root }}/spyt/user-guide/reference/configuration.md) page.
...
![](../../../_images/user-guide/explanation/spyt-streaming-monitoring-executors.png){ .center }
...
![](../../../_images/user-guide/explanation/spyt-streaming-monitoring-streaming.png){ .center }


By clicking Run ID, you can view more detailed statistics for a specific Streaming Query.


![](../../../_images/user-guide/explanation/spyt-streaming-monitoring-streaming-query-stats.png){ .center }
