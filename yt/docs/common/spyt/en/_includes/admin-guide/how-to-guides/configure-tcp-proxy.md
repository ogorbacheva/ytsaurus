...
For SPYT to work in [Standalone]({{ docs_root }}/spyt/user-guide/explanation/cluster-architecture.md#spark-standalone) mode, the external Spark driver must connect directly to the Spark workers. These workers run not as separate Kubernetes pods, but as processes inside [Vanilla jobs]({{ core-docs-root }}/{{ lang }}/user-guide/reference/data-processing/operations/vanilla{{ docs-revision-query }}) in {{product-name}}. Standard Kubernetes network abstractions (Services) can't route traffic to such processes.
