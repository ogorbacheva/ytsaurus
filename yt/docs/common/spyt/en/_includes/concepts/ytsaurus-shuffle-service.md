...
When [launching tasks directly in {{product-name}}]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/run-spark-jobs{{ docs-revision-query }}#submit), set the conf parameter `spark.ytsaurus.shuffle.enabled` to `true`. The application will use the {{product-name}} Shuffle service instead of the native Spark shuffle.
...
When launching an [internal standalone cluster]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/run-spark-jobs{{ docs-revision-query }}#standalone) using the `spark-launch-yt` command, two independent shuffle services are available, each controlled by its own option:
...
- When [launching tasks directly in {{product-name}}]({{ spyt-docs-root }}/{{ lang }}/how-to-guides/run-spark-jobs{{ docs-revision-query }}#submit). The Shuffle service writes intermediate data to {{product-name}} chunks, ensuring its survival in case of Spark executor failures or preemptions. This approach prevents re‑computation of stages that have already been computed but were stored on lost executors.

## Configuration parameters { #configuration }

All parameters related to configuring the {{product-name}} Shuffle service are described on the [configuration parameters page]({{ spyt-docs-root }}/{{ lang }}/reference/configuration{{ docs-revision-query }}#shuffle).