...
Позволяет задать произвольную структурированную информацию, связанную с операцией. Это пригодится для поиска и идентификации операций в архиве (поиск в нём доступен через [API]({{yt-docs-root}}/api/python/userdoc#operation_and_job_info_commands)). Подробнее читайте в разделе [Настройки операций]({{yt-docs-root}}/user-guide/data-processing/operations/operations-options).

| Возможные значения | Значение по умолчанию | Вид |
| --- | --- | --- |
| Строковое представление [YSON-словаря]({{yt-docs-root}}/user-guide/storage/formats#yson) | — | Динамическая |
...
Используется для указания внешней [транзакции]({{yt-docs-root}}/user-guide/storage/transactions), в контексте которой будет выполняться запрос (например, если транзакция была создана через [CLI]({{yt-docs-root}}/api/cli/commands#start-tx)). Это может понадобиться, чтобы выполнить несколько действий атомарно — например, 2 YQL-запроса или YQL-запрос и изменение таблиц. 
...
Позволяет выдавать доступ к управлению [MapReduce-операциями]({{yt-docs-root}}/user-guide/data-processing/operations/mapreduce) в {{product-name}} (отмена, пауза, run-job-shell и т. п.) другим пользователям — помимо пользователя, запустившего запрос.
