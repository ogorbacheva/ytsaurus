...
Поддержка вычислений на Go во Flow реализуется через механизм [компаньонов]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#companion). Go-код выполняется в отдельном gRPC-процессе, который взаимодействует с C++ [воркером]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#worker).
...
Любой [пайплайн]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) Flow состоит из трёх составных частей:

- `Runner` — запускает пайплайн и устанавливает новую версию [спеки]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec).
...
Go SDK предоставляет единый подход для настройки компаньона — тип `flow.Pipeline`. Он позволяет зарегистрировать [компьютейшены]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) и запустить gRPC-сервер компаньона:
...
Конструкторы вида `flow.NewRowComputation(computationID, fn)` связывают функцию обработки с компьютейшеном с указанным идентификатором, а `pipeline.Add(computations ...*flow.Computation)` регистрирует их в пайплайне. Идентификатор должен совпадать с `computation_id` в [спеке]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec) пайплайна.
...
Типизированные YSON-стримы регистрируются через `pipeline.AddStreams(flow.NewYSONStream[T](id))`. Структура `T` встраивает `flow.YSONMessage`, а колонки схемы выводятся из её `yson`-тегов. При запуске с хоста `Run` добавляет эти схемы в `spec.streams`; вручную дублировать их в `pipeline.yson` не нужно. Подробнее — в разделе [Типизированные YSON-сообщения]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/computation{{ docs-revision-query }}#typed-yson-messages).

## Computation и SourceComputation {#computation-and-source}

Для создания [компьютейшена]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) на Go необходимо выбрать подходящий конструктор, соответствующий [типу Computation-а в C++]({{ flow-docs-root }}/{{ lang }}/concepts/companion{{ docs-revision-query }}#vidy-computation-ov-dlya-raboty-s-kompanonami):
...
- **id** — по нему происходит маппинг запросов между [воркером]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#worker) и компаньоном.
- **fn** — значение с логикой обработки [сообщений]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#message). Реализует интерфейс `flow.RowFunction` или `flow.BatchFunction`; обычную функцию можно передать через адаптеры `flow.RowFunc` и `flow.BatchFunc`.
...
Фильтрация сообщений в source-компьютейшенах выполняется через флаг [distribute]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/distribute{{ docs-revision-query }}) при эмите сообщения из Process Function.

## Process Function {#process-function}

Есть два вида ProcessFunction:

- `flow.RowFunction` — получает [сообщения]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#message) по одному, метод `OnMessage`. [Таймеры]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer) и визиты обрабатываются реализацией интерфейсов `flow.RowTimerFunction` (`OnTimer`) и `flow.RowVisitFunction` (`OnVisit`) на том же типе.
...
- `rt` — `flow.Runtime`, через него доступны [стейты]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/state{{ docs-revision-query }}), параметры компьютейшена, [вотермарки]({{ flow-docs-root }}/{{ lang }}/concepts/watermarks{{ docs-revision-query }}) и схемы стримов.
...
Подробнее — в разделе [Computation (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/computation{{ docs-revision-query }}).
...
Подробнее — в разделе [Флаг distribute (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/distribute{{ docs-revision-query }}).

## Node companion {#node-companion}

Точка входа в Go-компаньон — функция `main`. В ней необходимо сконфигурировать компьютейшены через `flow.Pipeline` и вызвать `pipeline.Run()`. Функция `main` из [WordCount]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/wordcount{{ docs-revision-query }}):
...
Если обработчик сам запускает дочерние горутины, используйте [`flow.Go`]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/computation{{ docs-revision-query }}#goroutines), чтобы их CPU и память учитывались за ту же джобу.
...
Обязательные параметры: `pool` и `worker.count`. Для остальных полей (`cpu_limit`, `memory_limit`, число контроллеров и т. д.) есть разумные значения по умолчанию — полный список полей и их описание см. в [TVanillaConfig]({{ flow-docs-root }}/{{ lang }}/reference/configuration/all_yson_structs{{ docs-revision-query }}#NYT_NFlow_TVanillaConfig) и [TVanillaTaskConfig]({{ flow-docs-root }}/{{ lang }}/reference/configuration/all_yson_structs{{ docs-revision-query }}#NYT_NFlow_TVanillaTaskConfig).
...
- [Тестирование (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/testing{{ docs-revision-query }})
- [Примеры: Word Count (Go)]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/wordcount{{ docs-revision-query }})
