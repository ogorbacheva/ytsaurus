...
Владение и чтение живут в разных пространствах имён: стейт, объявленный в `external_state_managers`, недоступен через `flow.OpenJoinedExternalState`, и наоборот. Подробнее про аксессоры и общие принципы работы со стейтом: [State Accessor (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/state-accessor{{ docs-revision-query }}).
...
Подробнее о внутреннем стейте см. [Internal State (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/internal-state{{ docs-revision-query }}).
...
- `name` — имя стейта из секции `external_state_managers` [статической спеки]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec) (в примере — `"/shuffle-state"`). Имя обязательно начинается с `/` и совпадает с ключом в спеке.
...
- `external_state_manager_class_name` — имя зарегистрированного класса external state manager. Для типового сценария — `"NYT::NFlow::TSimpleExternalStateManager"`; для профилей BigRT — `"NYT::NFlow::NBigRTExtensions::TProfileManager<TUserProfile>"`. Подробнее см. в [C++ документации]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/state{{ docs-revision-query }}#external-state).
...
Для создания таблицы рекомендуется использовать [YtSync]({{yt-sync-docs}}/). Описание таблицы стейта из [Shuffle]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/shuffle{{ docs-revision-query }}), чей компьютейшен `reducer` группирует сообщения по `farm_hash(value), value`:
...
Если внешний стейт нужен **только для чтения** — например, чтобы обогатить события справочником, который наполняет другой компьютейшен, — используется приджойненный стейт. На стороне фреймворка его обслуживает [External State Joiner]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/state{{ docs-revision-query }}#external-state-joiner), читающий таблицу с кэшированием по TTL; в спеке он объявляется в секции `external_state_joiners` (на одном уровне с `external_state_managers`).

В Go такой стейт представлен отдельным типом `flow.JoinedExternalStateAccessor`, у которого нет ни `Set`, ни `Clear`: джойнер никогда не пишет обратно, и ответ, заявивший обратное, был бы отвергнут [воркером]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#worker).
...
- [Примеры: Shuffle (Go)]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/shuffle{{ docs-revision-query }})
