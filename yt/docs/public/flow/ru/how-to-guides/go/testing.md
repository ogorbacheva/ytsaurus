...
Данная страница описывает **юнит-тестирование** [компьютейшенов]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) Go-[пайплайна]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) через харнесс `flowtest`, а также **интеграционное тестирование** полного пайплайна через `FlowTestGoBase`.

{% endnote %}

## Общая архитектура тестирования {#architecture}

В продакшене C++ [воркер]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#worker) отправляет gRPC-запросы [компаньону]({{ flow-docs-root }}/{{ lang }}/concepts/companion{{ docs-revision-query }}), передавая [сообщения]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#message), [таймеры]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer), визиты, [стейты]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/state{{ docs-revision-query }}) и [вотермарки]({{ flow-docs-root }}/{{ lang }}/concepts/watermarks{{ docs-revision-query }}). Компаньон разбирает запрос, собирает по нему `flow.Job` и `flow.Runtime` и вызывает Process Function зарегистрированного компьютейшена.
...
| `InternalStates` | Имена [внутренних стейтов]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/internal-state{{ docs-revision-query }}), которые объявляет компьютейшен, — они доезжают до него как `parameters.internal_states`. |
| `ExternalStates` | Схемы [внешних стейтов]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/external-state{{ docs-revision-query }}), которыми компьютейшен владеет, по имени стейта. Имена — абсолютные пути, как того требует воркер. |
| `JoinedExternalStates` | Схемы внешних стейтов, которые компьютейшен читает, не владея ими. |
| `Parameters` | Карта `parameters` статической [спеки]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec) — то, что компьютейшен читает через `rt.Parameters()`. |
...
Юнит-тесты маппера из [WordCount]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/wordcount{{ docs-revision-query }}) — харнесс, батч сообщений и проверка внутреннего стейта:
...
Стейт, с которым компьютейшен начинает прогон, кладётся в харнесс до вызова `Process`, а результат читается из `Response`. Подробнее о самих аксессорах — в разделе [State Accessor]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/state-accessor{{ docs-revision-query }}).
...
Юнит-тесты редьюсера из [Shuffle]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/shuffle{{ docs-revision-query }}), который считает события во внешнем стейте:
...
| `Distribute()` | Флаг [distribute]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/distribute{{ docs-revision-query }}) каждого сообщения, выровненный с `Messages()`. |
...
Запускает пайплайн в таком тесте раннер, а не сам тест: Go-бинарь стартует как `./word_count --config pipeline.yson --flow-bin flow_server`, обогащает [спеку]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec) и передаёт управление `flow_server`, который её устанавливает. Компаньон в джобе поднимает воркер — ровно как в проде.

### Зависимости {#integration-dependencies}

Интеграционному тесту нужны рецепт кластера, `DEPENDS` на бинарь пайплайна и `flow_server`, а также `DATA` со спекой. Полный `ya.make` теста из [WordCount]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/wordcount{{ docs-revision-query }}):
...
- Если дорабатываете сам Flow — [Фреймворк для тестирования пайплайнов]({{ flow-docs-root }}/{{ lang }}/how-to-guides/contributing/testing-framework{{ docs-revision-query }}).
