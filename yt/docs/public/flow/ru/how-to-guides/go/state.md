...
Данная страница описывает Go API для работы со стейтами. Общие концепции стейтов описаны в разделе [Stateful-вычисления]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }}).

{% endnote %}

Аксессор [стейта]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state) открывается свободной функцией `flow.OpenXxxState(rt, name, input)` с тремя аргументами:

- `rt` — `flow.Runtime`, второй аргумент любого обработчика (см. [Process Function]({{ flow-docs-root }}/{{ lang }}/tutorials/go/quick-start{{ docs-revision-query }}#process-function)).
- `name` — имя стейта, объявленное в [спеке]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec) компьютейшена.
- `input` — вход, к [ключу]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#key) которого привязывается аксессор: `flow.ExtendedMessage`, `flow.Timer` или `flow.Visit`. Все три реализуют интерфейс `flow.Input`, поэтому работа со стейтом в обработчике сообщения, таймера и визита выглядит одинаково.
...
Пример из [Shuffle]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/shuffle{{ docs-revision-query }}):
...
Подробнее — в разделе [External State (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/external-state{{ docs-revision-query }}).
...
API работы со стейтом в обработчике [таймеров]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer) идентичен — вместо сообщения передаётся таймер:

```go
state, err := flow.OpenExternalState(rt, "/join-state", timer)
```

Пример из [WaitClickJoin]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/wait_click_join{{ docs-revision-query }}) — окно закрывается по таймеру, и стейт очищается сразу после публикации результата:
...
В `TTransformCompanionComputation` стейт привязывается к [ключу]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#key) сообщения, определяемому через `group_by_schema` в спеке компьютейшена. Все сообщения с одинаковым ключом разделяют один стейт. Ключ, к которому привязан аксессор, берётся из входа — поэтому один обработчик не может случайно записать стейт чужого ключа.

В `TTransformOrderedSourceCompanionComputation` поле `group_by_schema` не поддерживается. Ключом внутреннего стейта служит ключ партиции источника, поэтому все сообщения одной партиции разделяют стейт. Подробнее о выборе класса для SourceComputation см. в разделе [Computation (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/computation{{ docs-revision-query }}#sourcecomputation).
...
Стейт, не объявленный в спеке, открыть нельзя: функция открытия вернёт `flow.ErrUnknownState` со списком объявленных имён. Объявляются стейты в трёх разных местах описания [компьютейшена]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation):
...
Таблицы внутренних стейтов создаёт и обслуживает Flow, таблицу внешнего стейта пользователь создаёт сам. Подробнее — в разделах [Internal State (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/internal-state{{ docs-revision-query }}) и [External State (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/external-state{{ docs-revision-query }}).
