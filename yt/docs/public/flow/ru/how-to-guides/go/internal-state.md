...
Internal State — механизм работы с внутренним [состоянием (стейтом)]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state), хранящимся во внутренних таблицах Flow. В отличие от [External State]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/external-state{{ docs-revision-query }}), пользователю не нужно самостоятельно создавать таблицы — Flow управляет ими автоматически.

Подробнее про аксессоры и общие принципы работы со стейтом: [State Accessor (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/state-accessor{{ docs-revision-query }}).
...
Go SDK предоставляет три вида аксессоров для работы с Internal State, различающихся форматом сериализации. Для работы с внешним стейтом используются отдельные аксессоры — [External State (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/external-state{{ docs-revision-query }}).
...
- `name` — имя стейта. Имена внутренних стейтов не начинаются с `/` и должны быть объявлены в `parameters.internal_states` [спеки]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec) [компьютейшена]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation), см. [Конфигурация в статической спеке](#static-spec).
- `input` — вход, к [ключу]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#key) которого привязывается стейт. Подходит любое значение, реализующее `flow.Input`: `flow.ExtendedMessage`, `flow.Timer`, `flow.Visit`.
...
Здесь стейт привязан к ключу [сообщения]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#message). Для нового ключа `Empty()` возвращает `true`, а `Value()` создаёт пустой `wordCountState`. Присваивания полям сохраняются без отдельного `Set`.

Тот же стейт открывается и в обработчике [таймера]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer). Так устроен [URL Downloader]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/url_downloader{{ docs-revision-query }}): `OnMessage` накапливает батч, а `OnTimer` читает его и очищает через `Clear`.
...
Имена внутренних стейтов должны быть объявлены в секции `internal_states` параметров [компьютейшена]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) в статической спеке:
...
- [Работа со стейтами (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/state{{ docs-revision-query }}) — краткий обзор
