## NYT::NFlow::TDeleteStatesResponse {#NYT_NFlow_TDeleteStatesResponse}
<!-- This file is generated by yt/yt/flow/yandex/tools/generate_yson_struct_doc/generate.sh script -->
<!-- Before using doc generation tool check readme: yt/yt/flow/yandex/tools/generate_yson_struct_doc/README.md -->
Источник: [yt/yt/flow/library/cpp/controller/state_access.h]({{source-root}}/yt/yt/flow/library/cpp/controller/state_access.h)

Ответ команды `delete-states`.

#|
|| **Параметр** | **Описание** ||
|| `committed` | **Тип**: `bool`
**Обязательный параметр**
Отражает значение `commit` из аргумента запроса. Если в `errors` есть записи, часть совпавших строк могла не удалиться &mdash; точные счётчики не предоставляются, на практике стейт необходимо перечитать через `read-states`. ||
|| `matched_states` | **Тип**: [NYT::NFlow::TMatchedStates](./all_yson_structs#NYT_NFlow_TMatchedStates)
**Обязательный параметр**
Счётчики совпавших строк по категориям. Заполняется всегда: и в dry-run-режиме (показывает, что было бы удалено), и при `commit=true` (показывает, что было удалено или попало под попытку удаления). См. [TMatchedStates]({{ flow-docs-root }}/{{ lang }}/reference/configuration/all_yson_structs{{ docs-revision-query }}#NYT_NFlow_TMatchedStates). ||
|| `errors` | **Тип**: `std::vector<std::string>`
**Значение по умолчанию**: `[]`
Ошибки, возникшие при формировании ответа на сам запрос (например, упал `Sync` манагера). Не относятся к ошибкам в работе пайплайна со стейтом. Если такая ошибка случилась для одного из external-стейтов, остальные категории всё равно могут быть обработаны успешно. ||
|#

