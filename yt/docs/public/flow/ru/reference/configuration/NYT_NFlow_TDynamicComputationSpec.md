...
Id троттлера для ограничения скорости обработки сообщений. Если задан, перед каждой итерацией `Computation` ждёт квоту, равную числу сообщений во входном батче. Id должен присутствовать в `dynamic_spec/throttlers`. Подробнее в разделе [Distributed Throttler]({{ flow-docs-root }}/{{ lang }}/concepts/distributed_throttler{{ docs-revision-query }}). ||
