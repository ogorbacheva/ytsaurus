...
External State is the mechanism for working with external [state]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state) stored in an external {{product-name}} dynamic table. You create the table for storing the state yourself, on the same cluster the [pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) is deployed on.
...
External State in the Go SDK is represented by two accessors: `flow.ExternalStateAccessor` for the state the computation **owns**, and `flow.JoinedExternalStateAccessor` for the state the computation only **reads**. Both convert a row of an external dynamic table into a Go structure with `yson` tags. The state is bound to the [key]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#key) of the message (`group_by_schema`).
...
Ownership and reading live in different namespaces: a state declared in `external_state_managers` isn’t available through `flow.OpenJoinedExternalState`, and vice versa. For more on accessors and the general principles of working with state, see [State Accessor (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/state-accessor{{ docs-revision-query }}).
...
For more on internal state, see [Internal State (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/internal-state{{ docs-revision-query }}).
...
- `name` — the state name from the `external_state_managers` section of the [static spec]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec) (`"/shuffle-state"` in the example). The name must start with `/` and match the key in the spec.
...
To create the table, we recommend using [YtSync]({{yt-sync-docs}}/). Here is the description of the state table from [Shuffle]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/shuffle{{ docs-revision-query }}), whose `reducer` computation groups messages by `farm_hash(value), value`:

{% code '/yt/yt/flow/examples/go/shuffle/test/yt_sync.py' lang='python' lines='[BEGIN yt_sync_tables]-[END yt_sync_tables]' %}

{% else %}

You create the table with standard commands, for example `yt create table ... --attributes '{dynamic=true; schema=...}'` and `yt mount-table` — see the [Create command]({{ core-docs-root }}/{{ lang }}/how-to-guides/storage/cypress-example{{ docs-revision-query }}#create) section. The [Shuffle]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/shuffle{{ docs-revision-query }}) example's `reducer` computation groups messages by `farm_hash(value), value`; create the state table with matching key columns `hash` (`uint64`) and `value` (`string`), plus a `count` (`int64`) value column.
...
If an external state is needed **for reading only** — for example, to enrich events with a reference dataset that another computation populates — joined state is used. On the framework side it is served by the [External State Joiner]({{ flow-docs-root }}/{{ lang }}/how-to-guides/cpp/state{{ docs-revision-query }}#external-state-joiner), which reads the table with TTL-based caching; in the spec it is declared in the `external_state_joiners` section (at the same level as `external_state_managers`).

In Go, such a state is represented by the separate `flow.JoinedExternalStateAccessor` type, which has neither `Set` nor `Clear`: the joiner never writes back, and a response claiming otherwise would be rejected by the [worker]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#worker).
...
- [Working with states (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/state{{ docs-revision-query }}) — a brief overview
- [Stateful processing (concept)]({{ flow-docs-root }}/{{ lang }}/concepts/stateful{{ docs-revision-query }})
- [Examples: Shuffle (Go)]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/shuffle{{ docs-revision-query }})
