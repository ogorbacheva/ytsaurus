...
This page describes **unit testing** of the [computations]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) of a Go [pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) through the `flowtest` harness, as well as **integration testing** of the full pipeline through `FlowTestGoBase`.

{% endnote %}

## General testing architecture {#architecture}

In production, the C++ [worker]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#worker) sends gRPC requests to the [companion]({{ flow-docs-root }}/{{ lang }}/concepts/companion{{ docs-revision-query }}), passing [messages]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#message), [timers]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer), visits, [states]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/state{{ docs-revision-query }}), and [watermarks]({{ flow-docs-root }}/{{ lang }}/concepts/watermarks{{ docs-revision-query }}). The companion parses the request, builds a `flow.Job` and a `flow.Runtime` from it, and calls the Process Function of the registered computation.
...
| `InternalStates` | The names of the [internal states]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/internal-state{{ docs-revision-query }}) the computation declares — they reach it as `parameters.internal_states`. |
| `ExternalStates` | The schemas of the [external states]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/external-state{{ docs-revision-query }}) the computation owns, by state name. The names are absolute paths, as the worker requires. |
| `JoinedExternalStates` | The schemas of the external states the computation reads without owning them. |
| `Parameters` | The `parameters` map of the static [spec]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec) — what the computation reads through `rt.Parameters()`. |
...
The unit tests of the mapper from [WordCount]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/wordcount{{ docs-revision-query }}) — the harness, a batch of messages, and a check of the internal state:
...
The state a computation starts a run with is put into the harness before the `Process` call, and the result is read from `Response`. For details on the accessors themselves, see the [State Accessor]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/state-accessor{{ docs-revision-query }}) section.
...
The unit tests of the reducer from [Shuffle]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/shuffle{{ docs-revision-query }}), which counts events in an external state:
...
| `Distribute()` | The [distribute]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/distribute{{ docs-revision-query }}) flag of every message, aligned with `Messages()`. |
...
In such a test, the pipeline is started by the runner, not by the test itself: the Go binary starts as `./word_count --config pipeline.yson --flow-bin flow_server`, enriches the [spec]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec), and hands control over to `flow_server`, which installs it. The companion in the job is brought up by the worker — exactly as in production.

### Dependencies {#integration-dependencies}

An integration test needs a cluster recipe, `DEPENDS` on the pipeline binary and `flow_server`, and `DATA` with the spec. The full `ya.make` of the test from [WordCount]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/wordcount{{ docs-revision-query }}):
...
{% include notitle [_](../../common/_includes/_partials/testing-integration-body.md) %}

{% include notitle [_](../../common/_includes/_partials/testing-test-param-body.md) %}
...
- [State Accessor (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/state-accessor{{ docs-revision-query }})
- [Examples: Word Count (Go)]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/wordcount{{ docs-revision-query }})
- If you are working on Flow itself — [Pipeline testing framework]({{ flow-docs-root }}/{{ lang }}/how-to-guides/contributing/testing-framework{{ docs-revision-query }}).
