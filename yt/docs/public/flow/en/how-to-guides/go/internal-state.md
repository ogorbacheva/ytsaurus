...
Internal State is the mechanism for working with internal [state]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state) stored in the internal tables of Flow. Unlike [External State]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/external-state{{ docs-revision-query }}), you don’t need to create the tables yourself — Flow manages them automatically.

For more on accessors and the general principles of working with state, see [State Accessor (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/state-accessor{{ docs-revision-query }}).
...
The Go SDK provides three kinds of accessors for working with Internal State, differing in the serialization format. Separate accessors are used for external state — see [External State (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/external-state{{ docs-revision-query }}).
...
- `name` — the state name. Internal state names don’t start with `/` and must be declared in `parameters.internal_states` of the [computation]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) [spec]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec), see [Configuration in the static spec](#static-spec).
- `input` — the input to whose [key]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#key) the state is bound. Any value implementing `flow.Input` will do: `flow.ExtendedMessage`, `flow.Timer`, `flow.Visit`.
...
Here the state is bound to the key of the [message]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#message). For a new key, `Empty()` returns `true`, and `Value()` creates an empty `wordCountState`. Assignments to the fields are saved without a separate `Set`.

The same state is also opened in the [timer]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer) handler. This is how the [URL Downloader]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/url_downloader{{ docs-revision-query }}) works: `OnMessage` accumulates a batch, and `OnTimer` reads it and clears it through `Clear`.
...
The names of internal states must be declared in the `internal_states` section of the [computation]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) parameters in the static spec:
