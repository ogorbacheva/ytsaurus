...
The `distribute` flag is a per-message flag that you set when you add an output [message]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#message) in a [source computation]({{ flow-docs-root }}/{{ lang }}/tutorials/go/quick-start{{ docs-revision-query }}#computation-and-source). It controls whether the message is published further along the processing graph.

The `distribute` flag ensures:

- Correct [watermark]({{ flow-docs-root }}/{{ lang }}/concepts/watermarks{{ docs-revision-query }}) evaluation: messages with `distribute=false` are still accounted for by the watermark generator (unlike simply skipping a message in the handler, which breaks watermark evaluation).
...
The flag is set separately for every message, so within a single handler call some messages can be published and others not. The order of messages is preserved: a dropped message stays in the output group and takes part in the [lineage]({{ flow-docs-root }}/{{ lang }}/concepts/lineage{{ docs-revision-query }}), it just doesn’t go further along the graph.

## The flag is read only on the source path {#source-path-only}

The [worker]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#worker) reads the `distribute` flag only on the source path. That is why the Go SDK rejects `AddUndistributedMessage` in a transform with the `flow.ErrDistributeOnTransform` error.
...
For details, see [Testing (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/testing{{ docs-revision-query }}).
...
- [The distribute flag (Python)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/python/distribute{{ docs-revision-query }})
