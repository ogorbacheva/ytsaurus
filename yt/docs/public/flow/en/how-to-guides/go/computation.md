...
This page describes Go-specific details of working with computations. For general concepts, see the [Computation]({{ flow-docs-root }}/{{ lang }}/concepts/computation{{ docs-revision-query }}) section.

{% endnote %}

## Computation types {#computation-types}

In Flow, there are two kinds of `Computation`: [`Swift`]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#swift) and `Transform`. Your choice determines how exactly-once guarantees are provided and what transformations you can implement with them.
...
On the Go side, the choice of constructor doesn’t select `Swift` versus `Transform`: it selects what the computation is declared as to the [worker]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#worker), a source or a transform. `Swift` and `Transform` computations are created by the same constructors, and it is `computation_class_name` in the spec that tells them apart.
...
For a source, `TSwiftOrderedSourceCompanionComputation` is suitable only for deterministic processing without user state. If a SourceComputation uses [internal state]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/state{{ docs-revision-query }}) or non-deterministic logic, specify `TTransformOrderedSourceCompanionComputation` in the spec: the worker materializes the output and commits it together with the state and the source offset. The internal state key in such a computation is the source partition key.

## Creating a Computation {#computation}

A computation is created by a constructor and registered in `flow.Pipeline` through `pipeline.Add`. Here is an example from [Shuffle]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/shuffle{{ docs-revision-query }}), where the companion serves both ends of the pipeline — the source and the transform:
...
`SourceComputation` is the node in the [pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) graph that reads data from external sources. On the worker side it corresponds to [TSwiftOrderedSourceComputation]({{ flow-docs-root }}/{{ lang }}/concepts/computation{{ docs-revision-query }}#tswiftorderedsourcecomputation) or [TTransformOrderedSourceComputation]({{ flow-docs-root }}/{{ lang }}/concepts/computation{{ docs-revision-query }}#ttransformorderedsourcecomputation).
...
For a passthrough source, don’t use Go. Instead, specify `NYT::NFlow::TSwiftPassthroughOrderedSourceComputation` in `computation_class_name` in the spec, and leave the computation unregistered in the Go companion. For details, see [Passthrough Computation]({{ flow-docs-root }}/{{ lang }}/concepts/computation{{ docs-revision-query }}#passthrough).

### Interaction with Worker {#companion-info}

When initializing, the [worker]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#worker) requests from the Go companion the list of registered computations together with their type (`Source` or `Transform`). The list is fixed when the gRPC server starts: a computation added to the `Pipeline` after the start is no longer visible to the worker.
...
| `rt flow.Runtime` | Access to [states]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/state{{ docs-revision-query }}), computation parameters, [watermarks]({{ flow-docs-root }}/{{ lang }}/concepts/watermarks{{ docs-revision-query }}), and stream schemas |
| `msg` / `timer` / `visit` | The input being processed, together with the [key]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#key) it is grouped by |
| `out flow.OutputCollector` | Collecting output messages and setting [timers]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer) |

An error returned by a handler stops the processing of the rest of the batch: the worker retries the whole request, so a partial response would cause already processed inputs to be counted twice. The companion returns the `INTERNAL` gRPC error to the worker with the original text and by default writes it to `stderr`; the logger can be replaced with the `flow.WithLogger` option (see [Node companion]({{ flow-docs-root }}/{{ lang }}/tutorials/go/quick-start{{ docs-revision-query }}#node-companion)).

### RowFunction {#row-function}

`flow.RowFunction` receives [messages]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#message) one at a time. The interface declares a single method, `OnMessage`.
...
The function from [WordCount]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/wordcount{{ docs-revision-query }}) counts the occurrences of every word in the key’s [state]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state):
...
[Timers]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer) and visits from [key visitor streams]({{ flow-docs-root }}/{{ lang }}/concepts/key_visitor{{ docs-revision-query }}) are handled by separate interfaces declared on the same type:
...
For complete examples, see [URL Downloader]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/url_downloader{{ docs-revision-query }}) and [Wait Click Join]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/wait_click_join{{ docs-revision-query }}).

### BatchFunction {#batch-function}

`flow.BatchFunction` receives the whole batch of messages that arrived from the [worker]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#worker) in a single call of the `OnMessages` method. Timers and visits are handled by the `flow.BatchTimerFunction` (`OnTimers`) and `flow.BatchVisitFunction` (`OnVisits`) interfaces.
...
Unlike in `RowFunction`, the output of a batch function relates to the batch as a whole: the [lineage]({{ flow-docs-root }}/{{ lang }}/concepts/lineage{{ docs-revision-query }}) of the output messages consists of the identifiers of all the batch inputs, not of one. A row function is called per input, and its output relates to exactly that input.
...
Messages in source computations are filtered with the per-message [distribute]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/distribute{{ docs-revision-query }}) flag: the message is emitted from the Process Function by calling `out.AddUndistributedMessage(msg)` and isn’t published further along the graph, but is accounted for in [watermark]({{ flow-docs-root }}/{{ lang }}/concepts/watermarks{{ docs-revision-query }}) evaluation.
...
| Method | Description |
| --- | --- |
| `rt.MessageBuilder(streamID)` | Create a `MessageBuilder` for the given output [stream]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) |
...
| `rt.KeySchema()` | The schema of the [key]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#key) the batch is grouped by |
| `rt.StreamSpecs()` | The computation streams and their schemas |
| `rt.MinWatermark()` | The minimum [watermark]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timestamps-and-watermarks) across all input streams |
| `rt.Watermark(streamID)` | The [watermark]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timestamps-and-watermarks) of a specific stream |
| `rt.InternalState(name)` | The holder of an internal [state]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#state) |
...
Holders are a low-level interface: in user code, the key’s state is opened by the `flow.OpenYSONState`, `flow.OpenProtoState`, `flow.OpenRawState`, and `flow.OpenExternalState` accessors. For details, see the [Working with states (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/state{{ docs-revision-query }}) and [State Accessor (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/state-accessor{{ docs-revision-query }}) sections.
...
The `Finish()` method returns a ready `flow.Message` without modifying the builder. The `stream_id` identifier must be present in the `output_stream_ids` list of the computation’s static [spec]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec); otherwise `rt.MessageBuilder` returns `flow.ErrUnknownStream`.
...
A whole row is written with a single `builder.SetStruct(v)` call: the columns are taken from the yson tags of the `v` structure — the same tags [states]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/state{{ docs-revision-query }}) are serialized by. A column that isn’t in the stream schema is rejected as an error. The reverse operation is `payload.ConvertTo(&v)`: it fills the structure fields from the columns of the same name, leaving as is those that aren’t in the row.
...
| `out.AddTimer(timer)` | Set a [timer]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer) on the key being processed |
| `out.WithParentIDs(parentIDs...)` | Return a collector that writes into a separate group with the given [lineage]({{ flow-docs-root }}/{{ lang }}/concepts/lineage{{ docs-revision-query }}) |
...
An incoming [message]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#message) (`flow.ExtendedMessage`) contains:

- `msg.ConvertTo(&value)` — converts the message payload into a structure with an embedded `flow.YSONMessage`.
- `msg.Key` — the message [key]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#key) from `group_by_schema`; the key structure is filled through `msg.Key.ConvertTo(&key)`.
- `msg.StreamID` — the identifier of the input [stream]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) (`string`).
...
A [timer]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer) value (`flow.Timer`) contains:

- `timer.Key` — the timer [key]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#key): `timer.Key.String("host")`.
...
A visit, `flow.Visit`, is arranged the same way but without a trigger time: it carries `Key`, `StreamID`, and the timestamps. For details, see [Key Visitor Streams]({{ flow-docs-root }}/{{ lang }}/concepts/key_visitor{{ docs-revision-query }}).
...
- [Getting started (Go)]({{ flow-docs-root }}/{{ lang }}/tutorials/go/quick-start{{ docs-revision-query }})
- [Working with states (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/state{{ docs-revision-query }})
- [The distribute flag (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/distribute{{ docs-revision-query }})
