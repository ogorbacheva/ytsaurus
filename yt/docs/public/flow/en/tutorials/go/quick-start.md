...
Go computation support in Flow is built on the [companion]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#companion) mechanism. Go code runs in a separate gRPC process that interacts with the C++ [worker]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#worker).
...
Every Flow [pipeline]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#pipeline) consists of three parts:

- `Runner` — starts the pipeline and installs a new version of the [spec]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec).
...
The Go SDK provides a single way to configure the companion — the `flow.Pipeline` type. It lets you register [computations]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#stream-and-computation) and start the companion gRPC server:
...
Constructors such as `flow.NewRowComputation(computationID, fn)` bind a processing function to the computation with the given identifier, and `pipeline.Add(computations ...*flow.Computation)` registers them in the pipeline. The identifier must match `computation_id` in the pipeline [spec]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#spec-and-dynamic-spec).
...
- **id** — used to map requests between the [worker]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#worker) and the companion.
- **fn** — the value that holds the [message]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#message) processing logic. It implements the `flow.RowFunction` or `flow.BatchFunction` interface; a plain function can be passed through the `flow.RowFunc` and `flow.BatchFunc` adapters.
...
Messages in source computations are filtered through the [distribute]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/distribute{{ docs-revision-query }}) flag when a message is emitted from the Process Function.

## Process Function {#process-function}

There are two kinds of ProcessFunction:

- `flow.RowFunction` — receives [messages]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#message) one at a time, through the `OnMessage` method. [Timers]({{ flow-docs-root }}/{{ lang }}/concepts/glossary{{ docs-revision-query }}#timer) and visits are handled by implementing the `flow.RowTimerFunction` (`OnTimer`) and `flow.RowVisitFunction` (`OnVisit`) interfaces on the same type.
...
- `rt` — `flow.Runtime`, which gives access to [states]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/state{{ docs-revision-query }}), computation parameters, [watermarks]({{ flow-docs-root }}/{{ lang }}/concepts/watermarks{{ docs-revision-query }}), and stream schemas.
...
For details, see [Computation (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/computation{{ docs-revision-query }}).
...
For details, see [The distribute flag (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/distribute{{ docs-revision-query }}).

## Node companion {#node-companion}

The entry point of a Go companion is the `main` function. In it, configure the computations through `flow.Pipeline` and call `pipeline.Run()`. The `main` function from [WordCount]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/wordcount{{ docs-revision-query }}):
...
If a handler starts child goroutines itself, use [`flow.Go`]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/computation{{ docs-revision-query }}#goroutines) so that their CPU and memory are accounted to the same job.
...
The required parameters are `pool` and `worker.count`. The remaining fields (`cpu_limit`, `memory_limit`, the number of controllers, and so on) have reasonable defaults — for the full list of fields and their descriptions, see [TVanillaConfig]({{ flow-docs-root }}/{{ lang }}/reference/configuration/all_yson_structs{{ docs-revision-query }}#NYT_NFlow_TVanillaConfig) and [TVanillaTaskConfig]({{ flow-docs-root }}/{{ lang }}/reference/configuration/all_yson_structs{{ docs-revision-query }}#NYT_NFlow_TVanillaTaskConfig).
...
- [Testing (Go)]({{ flow-docs-root }}/{{ lang }}/how-to-guides/go/testing{{ docs-revision-query }})
- [Examples: Word Count (Go)]({{ flow-docs-root }}/{{ lang }}/tutorials/go/examples/wordcount{{ docs-revision-query }})
