---
metadata:
    - property: 'og:title'
      content: 'Получение версий CHYT и {{clickHouse}}'
    - property: 'og:description'
      content: 'Статья содержит информацию, где в интерфейсе посмотреть версии CHYT и ClickHouse'
    - property: 'og:type'
      content: 'article'
    - property: 'og:url'
      content: 'https://ytsaurus.tech/docs/chyt/ru/user-guide/how-to-guides/check-versions'
    - property: 'article:tag'
      content: 'clique-instruction'
    - property: 'article:modified_time'
      content: '2026-05-27T20:00:00+03:00'
    - property: 'article:author'
      content: 'Татьяна Андриянова'
---

{% include [Получение версий CHYT и {{clickHouse}}](../../common/_includes/user-guide/how-to-guides/check-versions.md) %}
