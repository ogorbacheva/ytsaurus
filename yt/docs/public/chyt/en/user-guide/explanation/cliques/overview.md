---
metadata:
    - property: 'og:title'
      content: 'Clique lifecycle'
    - property: 'og:description'
      content: 'The article describes the full clique lifecycle and management tools.'
    - property: 'og:type'
      content: 'article'
    - property: 'og:url'
      content: 'https://ytsaurus.tech/docs/chyt/en/user-guide/explanation/cliques/overview'
    - property: 'article:tag'
      content: 'clique'
    - property: 'article:modified_time'
      content: '2026-05-27T20:00:00+03:00'
    - property: 'article:author'
      content: 'Tatyana Andriyanova'
---

{% include [Overview](../../../common/_includes/user-guide/explanation/cliques/overview.md) %}
